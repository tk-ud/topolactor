...
- Test hardening scopeとして production runtime read-source 本体は変更せず、DBなし fast lane のテスト面と seed fixture を補強した。PR537 auditで user-facing render 断線を test が露出したため、許容された最小修正として `ProjectionView` / `emissionSummary` に `projectionDefinition` 消費と user-facing summary 表示を含めた。
