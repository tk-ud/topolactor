...
Evidence-artifact generator for the `react_schema_topology_seed_translator_ssot` exchange contract (`docs/design/react-schema-topology-seed-translator-ssot.yaml`). Unlike the read-only observation tools above, `--output` is an authorized write of a tool-owned generated JSON document (e.g. `.agent/tools/generate/schema/*.json`, a tool-owned generated-evidence surface); it never writes to a database and never starts backend/frontend/nginx.

- `generate-react-schema --input <envelope.json> [--output <path>]`: implemented. `inputText` -> markup parse -> `topolactor.react_schema.v1` candidate.
- `generate-topology-seed --input <envelope.json> [--output <path>]`: implemented. `inputText` is a JSON string of a `topolactor.react_schema.v1` candidate -> `topolactor.topology_ui_seed.v1` candidate (draft/intake only, never active topology).
...
- Executable proof: `.agent/scripts/check_react_schema_topology_seed_translator.py` (CI entrypoint: `.agent/tests/check-react-schema-topology-seed-translator.sh`). The gate core's own shape-detection/fail-close-condition matrix is proven separately by `.agent/scripts/check_schema_seed_translator_entry_gate.py` (CI entrypoint: `.agent/tests/check-schema-seed-translator-entry-gate.sh`).
