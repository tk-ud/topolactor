...
Evidence-artifact generator for the `react_schema_topology_seed_translator_ssot` exchange contract (`docs/design/react-schema-topology-seed-translator-ssot.yaml`). Unlike the read-only observation tools above, `--output` is an authorized write of a tool-owned generated JSON document (e.g. `tools/generate/schema/*.json`); it never writes to a database and never starts backend/frontend/nginx.
