...
- `start --task-name <name> --worktype <worktype>`: resolves the worktype route and emits tool-generated `uuid`/`datetime` plus a short prompt excerpt, `protocol_trigger_hints` (each with `path`, `trigger_condition` — `"always"` for `required_protocols` or the triggering condition name for `triggered_protocols` — and a bounded `excerpt` of that protocol file), and `workflow_procedure` (the Execution Order fenced block read directly from `.agent/skills/agent-workflow.md`, worktype-independent). This means routing, the applicable protocol text, and the workflow step order are all available from one `start` call, without separately opening `.agent/protocols/*` or `.agent/skills/agent-workflow.md`. Excerpts are the same bounded short-read mechanism as the prompt excerpt, not full-file dumps; whether a non-`"always"` `trigger_condition` actually applies to the current task remains an agent judgment call, not something this output resolves. The AI must reuse the tool-generated `uuid`/`datetime` values verbatim in later steps rather than hand-authoring them.
