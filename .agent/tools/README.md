...
- `start --task-name <name> --worktype <worktype>`: resolves the worktype route and emits tool-generated `uuid`/`datetime` plus `prompt_content` (the full text of the worktype's routed prompt router), `protocol_trigger_hints` (each with `path`, `trigger_condition` — `"always"` for `required_protocols` or the triggering condition name for `triggered_protocols` — `content`: that protocol file's full text, and `truncated`: false unless the file exceeds the tool's fail-explicit safety cap), and `workflow_procedure` (the Execution Order fenced block read directly from `.agent/skills/agent-workflow.md`, worktype-independent). This means routing, the full applicable prompt/protocol text, and the workflow step order are all available from one `start` call, without separately opening `.agent/prompt/*`, `.agent/protocols/*`, or `.agent/skills/agent-workflow.md`. This is a targeted full read of the files a worktype/trigger has already narrowed things down to — not the indiscriminate "read every prompt/protocol/skill" breadth this tool exists to avoid; other worktypes' and untriggered conditions' files are still not read. Whether a non-`"always"` `trigger_condition` actually applies to the current task remains an agent judgment call, not something this output resolves. The AI must reuse the tool-generated `uuid`/`datetime` values verbatim in later steps rather than hand-authoring them.
