...
- `translator-entry-gate --input <json> [--expected-mode <mode>]`: an unrelated capability bolted onto this same CLI, not part of the SSOT-seed-discussion question-space flow above. It calls the shared schema<->seed translator entry gate core (`.agent/scripts/agent_tools/schema_seed_translator_entry_gate.py`, `validate_translator_entry()`) — the same core that `.agent/tools/react-schema-topology-seed-translator`'s entry (`generate-react-schema` / `generate-topology-seed`) and `.agent/scripts/check_react_schema_topology_seed_translator.py` also call, so a human/agent can inspect a translator input envelope / `topolactor.react_schema.v1` candidate / `topolactor.topology_ui_seed.v1` candidate / translator output / existing fixture JSON externally without re-authoring a separate gate-only document. `--input` reads the exact same JSON the translator entry itself reads. `--expected-mode` optionally names the translator subcommand context (`generate_react_schema` / `generate_topology_ui_seed` / `round_trip_check`) to additionally check envelope `mode` consistency. Output wraps the gate core's `gateStatus` (`pass` / `blocking` / `unsupported_input_shape`) result under `gate_result`; this command stays read-only (no DB/API/file-write) and, like the rest of this tool, is discussion/observation output only — never SSOT authority, proof completion, or seed adoption evidence. Every `gate_result` also carries a top-level `authoringReferences` array (present regardless of `gateStatus`) pointing at `docs/reference/seed-data-authoring-guide.md` — a non-SSOT authoring/comparison reference (canonical carrier classification, canonical-conformance-vs-runtime-reachability axis split, seed authoring checklist). Applicable SSOT remains design authority; the reference is comparison/procedure guidance only, never a substitute for reading the SSOT.
...
.agent/tools/topology-seed-discussion translator-entry-gate --input .agent/tests/fixtures/react-schema-topology-seed-translator/credential-management-0092.input.json
```

### `react-schema-topology-seed-translator`

**Seed authoring starting order:** (1) read the applicable SSOT for the surface being seeded (e.g. the owning `docs/design/*-ssot.yaml` and `docs/design/react-schema-topology-seed-translator-ssot.yaml`) -- SSOT is the design authority; (2) use `docs/reference/seed-data-authoring-guide.md` as comparison/procedure guidance -- carrier classification, the canonical-conformance-vs-runtime-reachability axis split, and the authoring checklist -- never as a substitute for the SSOT, and never treat translator validation alone (or "the seed SQL executed") as active-topology or runtime-completion proof; (3) author and run the translator below. On any conflict between the two documents, the SSOT wins; the guide's own Document Status header declares `Authority: non-SSOT`.

Evidence-artifact generator for the `react_schema_topology_seed_translator_ssot` exchange contract (`docs/design/react-schema-topology-seed-translator-ssot.yaml`). Unlike the read-only observation tools above, `--output` is an authorized write of a generated JSON document under `.agent/tools/generated/*` -- a gitignored local/tmp regeneration surface, never tracked evidence and never SSOT authority. `*seed.sql` files and the SSOT docs above remain the production storage authority for anything this output might inform. It never writes to a database and never starts backend/frontend/nginx.

- `generate-react-schema --input <envelope.json> [--output <path>] [--generate-log <path>] [--nametag <name>] [--task-ref <ref>] [--pr-ref <ref>] [--source-seed-sql <label>]`: implemented. `inputText` -> markup parse -> `topolactor.react_schema.v1` candidate.
- `generate-topology-seed --input <envelope.json> [--output <path>] [--generate-log <path>] [--nametag <name>] [--task-ref <ref>] [--pr-ref <ref>] [--source-seed-sql <label>]`: implemented. `inputText` is a JSON string of a `topolactor.react_schema.v1` candidate -> `topolactor.topology_ui_seed.v1` candidate (draft/intake only, never active topology).
- `round-trip-check --input <envelope.json>`: `not_implemented_out_of_scope` (fails closed with a `MODE_NOT_IMPLEMENTED` document; not a silently-accepted no-op).
- `--generate-log <path>` is opt-in (e.g. `--generate-log .agent/tools/logs/generate.log`); when omitted, nothing is appended anywhere, so ordinary test/CI runs never touch the tracked log. When given, one JSON Lines record is appended after generation: `datetime`, `nametag` (defaults to the `--input` file stem), `mode`, `source`, `sourceSeedSql` (a caller-supplied passthrough label, never opened by the translator), `seedKey`, `manifestId`, `command`, `outputKind` (always `translator_output_document` -- `sha256` hashes the full `output_format_contract` document `--output` writes/emits, never a bare candidate by itself), `outputSchemaId` (`topolactor.translator_output.v1`), `embeddedCandidateKind` (`react_schema_candidate` or `topology_ui_seed_candidate` -- which candidate the hashed document embeds), `outputPath`, `sha256`, `gateStatus`, `validationErrorCount`, `unresolvedGapCount`, `taskRef`, `prRef`. `.agent/tools/logs/generate.log` is tracked trace evidence -- a regeneration index a proof can replay and hash-check -- never seed adoption authority or proof completion by itself. Generated JSON is not SSOT.
- Translator entry gate: before `generate-react-schema` / `generate-topology-seed` run their conversion pipeline, the translator entry passes the same `--input` envelope JSON (never a separate gate-only document) through the same schema<->seed translator entry gate core described under `topology-seed-discussion translator-entry-gate` above. `gateStatus != pass` fails the entry closed (non-zero exit, no conversion, no `db/*.sql` read); `gateStatus == pass` lets the existing, unmodified conversion pipeline proceed. The gate core is entry-gate observation only — it is not SSOT authority, proof completion, or seed adoption evidence, and the translator body itself still never reads `db/*.sql` (see `translator_input_authority` in the SSOT). Both `generate-react-schema` and `generate-topology-seed` copy the gate's `authoringReferences` into their own output document verbatim, on every exit path the gate itself reaches (`gateStatus` pass or blocking) — the same `docs/reference/seed-data-authoring-guide.md` pointer described above.
- Executable proof: `.agent/scripts/check_react_schema_topology_seed_translator.py` (CI entrypoint: `.agent/tests/check-react-schema-topology-seed-translator.sh`), which also verifies `.agent/tools/logs/generate.log`'s JSON-Lines shape and re-runs a recorded command to hash-check regeneration. The gate core's own shape-detection/fail-close-condition matrix is proven separately by `.agent/scripts/check_schema_seed_translator_entry_gate.py` (CI entrypoint: `.agent/tests/check-schema-seed-translator-entry-gate.sh`).

## Agent UI protocol tools

`agent-ui-initial-contract` and `agent-ui-local-test` implement the Agent UI protocol's `initial_contract` and `local_test` steps from `docs/governance/agent-ui-protocol-ssot.yaml` (the third phase, `implementation`, is the ordinary coding work between them). They are governance/tool-side tools, not ordinary observation tools: the SSOT and this task scope explicitly permit them to write `senario-tmp.md` and append to `.agent/tools/logs/tool.log`, but they still do not mutate Topolactor application/runtime/product source, and they do not expose an `--output` mutation escape hatch.

Every subcommand in both tools emits a `next_step` field naming the exact next subcommand (or, at the two phase boundaries, the next real-world action: implement, or push/update the PR). Following `next_step` end to end — `worktypes` → `start` → `resolve-ssot` → `sections` → `end` → (implement) → `run-worktype-tests` → `read-senario-tmp` → `checklist` → `checks` → `summary` → (push/PR) — carries the full `initial_contract → implementation → local_test` pipeline without consulting `.agent/prompt/*`, `.agent/protocols/*`, or `.agent/skills/agent-workflow.md` for sequencing; `start` already surfaced the routed prompt's full text and the routed protocols' normalized obligations earlier in the chain.
...
- `start --task-name <name> --worktype <worktype>`: resolves the worktype route and emits tool-generated `uuid`/`datetime` plus `prompt_content` (the full text of the worktype's routed prompt router) and `protocol_obligations[]` (one entry per routed required/triggered protocol file: `protocol_path`, `route_mode` — `"required"` or `"triggered:<condition_key>"` — `applies` — `"always"` or `"agent_judgment_required"` — `trigger_condition`, `judgment_scope`, `foundation_ssot_read_gate`, `blocking_conditions`, `pass_conditions`, `required_fields`, `classification_vocab`, `output_boundary` (each a normalized extraction of that file's own matching `## heading` section, or `null` when the file has none), `fallback_protocol_ref` (repeats `protocol_path`), and `tool_first_instruction_note`), and `workflow_procedure` (the Execution Order fenced block read directly from `.agent/skills/agent-workflow.md`, worktype-independent). This means routing, the routed prompt's full text, the routed protocols' normalized judgment content, and the workflow step order are all available from one `start` call, without separately opening `.agent/prompt/*`, `.agent/protocols/*`, or `.agent/skills/agent-workflow.md` for sequencing. `protocol_obligations[]` is not that file's full body presented as an execution procedure — `flow_order` remains the sole execution-order authority; each entry's `fallback_protocol_ref` still points at the full file for the fallback route or deeper manual review. This is a targeted read of the files a worktype/trigger has already narrowed things down to — not the indiscriminate "read every prompt/protocol/skill" breadth this tool exists to avoid; other worktypes' and untriggered conditions' files are still not read. Whether a non-`"always"` `applies` value actually applies to the current task remains an agent judgment call, not something this output resolves. The AI must reuse the tool-generated `uuid`/`datetime` values verbatim in later steps rather than hand-authoring them.
...
- `end --task-name ... --worktype ... --uuid ... --datetime ... --target-file ... --senario-summary ... [--ng-boundary ...]`: requires a senario contract, writes `senario-tmp.md` from the reference template, and appends the compact usage record to `.agent/tools/logs/tool.log`.
