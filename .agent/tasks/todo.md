...
不足しているのはmechanismではなくDATAである: `ae200`をtargetとして名指す`hubs.hub_relations`行が、いずれのAdmin-axis source manifestからも現状存在しない。Goal 1の解決は、次段implementation_changeにおける通常の`hub_navigation:create`authoring（既存のadmin-gated dispatch action）のみで閉じる — Admin-axis source manifest（`dd010`、team_dashboard.admin.projection、既に`/admin/team-dashboard`で本番稼働中の既存route）から`ae200`への`hubs.hub_relations`行を、その sourceの次のsequence_positionへ author する。authored後は、その sourceの既存nav barから`ae200`へ1クリックで到達可能になり、他のAdmin surfaceからも既存の逐次navigationで到達可能になる — 新しいmechanism・field・table・endpoint・UIコンポーネントは一切不要。
...
- [ ] Goal 1: 次段implementation_changeで`hub_navigation:create`を用い、Admin-axis source manifestから`ae200`への`hubs.hub_relations`行を author する。その sourceを表示した際、既存ProjectionShellのnav barが`ae200`への実クリック可能な`<a href>`を表示することを実DOM操作で証明する。新しいUIコンポーネント・backend endpoint・DTO変更が加わっていないことを確認する。
