...
3. HubRelation fixed-navigationの提示責務をProjectionShellからapp-shell navigation Islandへ収束し、current topologyをそのnavigation Islandへ`Context`経由で共有したうえで、current topologyが出現する各`hubs.hub_relations`sequence（owner自身をindex 0、`sequence_position`順のtarget行をindex 1以降とする論理sequence。target側の一意性は制約されないため順列とは呼ばない）の各index occurrenceについて、current topologyの位置に隣接するimmediate prev/nextのみを、失わず提示する（sequenceの他memberを列挙しない。複数のindex occurrenceを持つ場合は各occurrenceのprev/nextを独立に失わず扱う）。
...
- sequence semantics: `db/topology_tables.sql`が正本（`db/legacy_utils/hub_relations_legacy_to_manifest_scoped.sql`がlegacy global graph shapeからこのmanifest-scoped shapeへ移行した経緯も同根拠）。sequenceは1つのmanifest（`topology_manifest_id`、全行NOT NULLの実列）が自分自身で持つ`hubs.hub_relations`行集合であり、そのmanifest自身が論理sequenceの先頭（index 0）を実列そのものとして占める（隠しnode/仮想slotの追加ではない）。続く各行が`sequence_position`昇順でindex 1以降を占める。`(topology_manifest_id, sequence_position)`のみがUNIQUE制約対象であり、target側（`target_topology_manifest_id`/`related_hub_id`）の一意性は制約されていないため、同一manifestが同一sequence内の複数indexへ重複出現すること、および`topology_manifest_id == target_topology_manifest_id`という合法な自己参照によりownerが自分自身のsequence内のtarget indexにも同時出現することを排除しない（index occurrence単位でprev/nextを扱う、下記参照）。「順列（permutation）」とは呼ばない — target側に重複が許容されるため全単射ではない。1つのmanifestが他のmanifestのsequenceを所有し他manifestの行を多段（multi-hop）に辿ることはない（global graphではない）。
- **fixed-navigation candidateの訂正（Round29以降のsemantic driftを訂正、Round31/32で確定）**: 以前のRound29は「current topologyが所属する上位relation/sequence」の復活を、candidate = そのrowの反対側の端点（opposite endpoint）を無条件に列挙する設計として実装した。これは誤り（Owner確定設計からの逸脱）であり、撤回済み。正しいcanonical semanticは: HubRelation fixed-navigationは、owner自身をindex 0とし`sequence_position`昇順のtarget行がindex 1以降に続く論理sequence（`[owner, target(p1), ..., target(pn)]`。ownerがindex 0を占めるのは`topology_manifest_id`という実在の非NULL列そのものであり、隠し要素の追加ではない）に対し、current topologyの位置からimmediate `prev`/`next`のみを提示する決定論的navigationである。current topologyは(a)自分がownerとしてindex 0を占める場合と、(b)いずれかのmanifest（自分自身の場合を含む — `topology_manifest_id == target_topology_manifest_id`は合法、例: manifest 092の自己参照）が所有するsequence内のtarget行としてindex k>=1に出現する場合の2通りで位置（index occurrence）を持ち得る。target側の一意性は制約されていないため、同一sequence内に同一manifestが複数indexへ出現することも排除しない — identityではなくindex occurrence単位でprev/nextを独立に扱う（マージ・重複排除・破棄しない）。導出は単一のadjacencyルール: current topologyがindex iに出現するなら、next = index i+1の要素（無ければnextなし）、prev = index i-1の要素（無ければprevなし。index 0は常にprevなし — ownerが実sequenceの先頭という直接構造上の事実であり、owner専用の特別ルールではない）。Ownerの例（A=[1,5,7,9,15]、B=[4,2,3,8,9,6]、current=9）は、「A」「B」を配列外の隠れowner manifest identityとして追加解釈せず、記載された値そのものを対象とする: 9はAの中で7と15に、Bの中で8と6にそれぞれ隣接する（配列内の要素として、Aおよびbの各ownerの識別子を特定・追加する必要は無い — 9はAの先頭(index0扱いのowner)には位置しないため、owner自身の識別が候補計算に関与しない）。それぞれのindex occurrenceが独立にprev/nextの対を提供する。本Roundのprev/next両方提示要件のもとでは、standing on 9のcandidateは{7, 15, 8, 6}となる（元のOwner例はnext方向のみを示す簡略化であり、本Roundのprev/next両方提示要件のほうを優先しfixtureの数値解釈を更新する）。新しいaxis/dimension/role flag列は不要 — `(topology_manifest_id, sequence_position)`の既存UNIQUE制約と`topology_manifest_id`のNOT NULL制約のみで導出できる。
- read projection: 既存`ContentBundleRepository.LoadHubNavigationSequenceAsync(topologyManifestId)`（source-scoped）が、current topology自身のsequence（(a)の場合の候補元）を過不足なく返す。ownerとして立っている場合はこの結果の最小`sequence_position`行のみをnextとして使う。これに加えて、target方向のreverse read（owner manifestは問わず — 自己参照行を含む — current topologyをtargetとして名指すrowを全て見つけ、各行ごとにその`sequence_position`と同じowner内の隣接行から current topologyのprev/nextを独立に導出する）を**追加のadditive query**として導入する — 新しいtable/column/parallel authorityではなく、同一`hubs.hub_relations`永続化に対するもう一つの読み方（新設backend read methodは必要、ただし新しいtable/columnは不要）。この読みの目的はopposite endpointの列挙ではなく、prev/next導出である。この読みが、`axis_navigation_membership`のtarget`ae200`（`dd010`のsequence内の最小position）が自分自身のoutbound行を持たなくても`dd010`をprevとして持てる仕組みそのものである（下記no_return_edge項目参照）。
- **Admin bridge（実体、名目だけでなくdd010へ収束、Round30の配置を維持しつつ意味を訂正）**: `dd010`（team_dashboard.admin.projection）は上記(1)で確定した通り、それ自体がAdmin axisのcanonical navigation source兼Admin bridgeである。標準例（訂正版）: `dd010`の3行はsequence_position昇順で`ae200`(1)/`5c100`(2)/`092`(3)。standing on `dd010`（owner）のnext = `ae200`のみ（残り2行はae200からさらにnextを辿って到達する、dd010から直接candidate化しない）。standing on `ae200`（`dd010`のsequenceの最小position）はprev = `dd010`自身、next = `5c100`。standing on `5c100`はprev = `ae200`、next = `092`。standing on `092`はprev = `5c100`、next = none。`dd010`が無条件に3 targetのどこからでも"way back"候補として現れるわけではない — `ae200`からのみprevとして直接現れ、`5c100`/`092`からはprev連鎖（`5c100`→`ae200`→`dd010`）で到達する。これによりAdmin権限内の4 manifest（`dd010`/`ae200`/`5c100`/`092`）はいずれも相互にfixed-navigationで到達可能（各hopはimmediate prev/nextのみの決定論的navigation）。新しいbridge manifest・新しいdispatch axis・新しいsequence-position規約などの新architectureは一切作らない — 既存`dd010`と既存`dd010 -> {ae200,5c100,092}`行をそのまま再利用する。
...
- frontend→backend transport wiring: navigation Island（frontend）は`HubNavigationResolver`というbackend C#クラスを直接呼べない — 到達には既存HTTP boundaryが要る。`runtime-orchestration-ssot.yaml`の`navigation_island_transport_boundary`で確定: 既存`GET /hub-navigation/relations`（`backend/Program.cs`、現状は`ResolveFallbackNavigationLinksAsync`によるcanonical-default-entry fallback専用）へ、任意の明示`topologyManifestId` query paramを追加する最小extensionとして再利用する。paramが有る場合はforward方向（`HubNavigationResolver.ResolveAsync`、ownerとして立つ場合は最小`sequence_position`行のみをnextとして採用）とreverse方向（追加のadditive read method、隣接`sequence_position`からprev/nextを導出）の両方を解決し、current topologyのimmediate prev/next対を返す。無い場合は既存fallback挙動を無変更で維持し既存callerを回帰させない。auth境界は既存JWT/session（admin限定ではない）のまま変更しない。`/dispatch`の`hub_navigation:get_hub_relations`（admin限定・`ListHubRelationsByManifestAsync`ベースでinactive行を含むauthoring用read）は不採用 — semanticも認可境界も一致しない。新しいbackend endpoint/dispatcherは追加しない（既存`GET /hub-navigation/relations`の拡張のみ）。
- **未解決点（Owner判断待ち、本Roundで新規発見）**: forward方向のindex-0-as-owner候補（next-only）は既存`HubNavigationSequenceItemDto`（`TopologyManifestId`/`RelatedHubId`/`RelatedHubLabel`/`TargetManifestId`等）でそのまま表現可能だが、reverse方向のprev候補（click destinationがowner manifest自身、例: `dd010`が`ae200`のprev）は、この既存DTOのいずれのfieldの意味も偽装・反転せずには表現できないことをrepo-wide調査で確認した（`TargetManifestId`は自身のdoc commentで「`RelatedHubId`の下で解決されるnavigable manifest」と定義されており、`ae200`向けの実行`dd010 -> ae200`行では`RelatedHubId`が`ae200`のhubを指すため、この行から`TargetManifestId=dd010`を導出するとこの派生規則と矛盾する。`RelatedHubLabel`をそのままにすると"ae200"というlabelのlinkが`dd010`へ遷移するlabel/destination不一致になる。`TopologyManifestId`を`ae200`（current）へ上書きすると実行の行identityと不一致になり`selected_link_payload_required`/cardListFactory identity passthroughを壊す）。詳細な比較と最小extension案（新規の別DTO追加を推奨、既存DTOへのdirection discriminant追加は次点、pseudo-row合成は却下）は`runtime-orchestration-ssot.yaml bidirectional_read_projection_gap`に記録済み — 次段implementation_changeがreverse方向の実transport shapeを実装する前に、Owner判断でこの中から選択すること。
...
- `db/seed_empty.sql`（axis navigation registration・`/admin`エントリ、3 forward rowのみ）
- `backend/repository/NpgsqlContentBundleRepository.cs`（`CreateHubRelationAsync`/`UpdateHubRelationAsync`/`LoadHubNavigationSequenceAsync`/`DeprecateHubRelationAsync`。加えて`bidirectional_read_projection_gap`が定義するreverse方向のadditive read method新設 — `LoadHubNavigationSequenceAsync`と対をなす、current topologyをtargetとして名指すrow（owner manifestは問わない。自己参照行を含む）を全て見つけ、各行ごとに、その`sequence_position`と同じowner内の隣接行からcurrent topologyのprev（該当行がownerの最小positionならowner自身、それ以外は隣接行のtarget）/next（隣接行のtarget、無ければnone）を独立に導出して返すmethod。owner自身を無条件に返すmethodではない。同一ownerの複数行が同じtargetを指す場合はそれぞれのindexを別occurrenceとして扱いdedupeしない。具体的なmethod名はAgent判断。新しいtable/columnは追加しない。DTOについては下記の未解決点（`runtime-orchestration-ssot.yaml bidirectional_read_projection_gap`のOwner判断待ち）を参照 — 既存`HubNavigationSequenceItemDto`の意味を偽装・反転しない前提のもとでは、reverse方向のprev候補を表現する新規DTO追加が必要になる可能性がある）
...
- [ ] frontend DOM proof 3b（reverse方向、Admin bridge）: `ae200`（自分自身のoutbound行を持たず、`dd010`のsequenceの最小`sequence_position`に位置する）を表示中、reverse方向のcandidate discoveryにより既存`dd010 -> ae200`行から`dd010`が`ae200`のprevとして提示されることを証明する。同時に、`dd010`を表示中はnextが`ae200`のみであり（`5c100`/`092`が直接candidate化されないこと）、`5c100`/`092`を表示中は`dd010`が直接のprevとして現れず、隣接sibling（`5c100`のprevは`ae200`、`092`のprevは`5c100`）が現れることを証明する。新しいbridge manifest/dispatch axis/architectureを追加していないことを確認する。
