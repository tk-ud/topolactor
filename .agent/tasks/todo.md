...
- `admin-enum`: **manifest構築・structural render proof・navigation closure proof は実装済み（下記「admin-enum subBundle 実装記録」参照）。** enum dictionary/group/item authorityは既存SSOTと `enum_dictionary:*` substrateに従う。target `ui_projection` manifest（`00000000-0000-0000-0000-0000000ae200`）は構築済み。navigation binding authoring / verificationは2026-07-22確定の解決基準を満たすproof（`AdminEnumHubRelationUiProjectionLiveDbTests.cs` の `hub_navigation:create` 実authoring dispatch + resolution chain 単一テスト）で用意済み。CRUD presetのgeneric shapeは参考にしたが `content_bundle:*` refsはコピーせず、enum authority operationへbindしている。**ただし enum_dictionary:* の write系操作（create/update/delete/set_group_items）を実際に dispatch する wiring lane が現行 runtimeInteractions taxonomy（`frontend/runtime/uiEventEffectRunner.ts`/`renderEmission.ts`: `dispatchExternalPort`/`dispatchInstanceOperation`/`localStateMutation` のみ）に存在せず、この gap は本 subBundle 単独では解消できない（新規 runtime lane 追加は本 Bundle の Governance NG boundary で明示的に禁止、旧 gap-01 と同一の owner_decision_required architecture 選択）。この gap が残る限り admin-enum を implemented 扱いにしない。**
...
- `backend/runtime/ManifestDispatcher.cs`、`backend/runtime/AdminRuntimeDispatchAdapter.cs`、`backend/runtime/OperationVectorResolver.cs`（既に汎用的なtarget/layer/action dispatch transport、変更不要である可能性が高い）
