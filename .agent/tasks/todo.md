...
- [ ] `aggregate_dashboard_seed_or_descoping`: **Problem:** aggregate_dashboard remains provisional with no implementation evidence or explicit descoping. **Purpose:** close or intentionally carry the remaining parent preset surface. **Improvement direction:** either seed-first implement aggregate dashboard using existing dashboard/read-work components, or update SSOT to explicitly descoped/defer it with rationale and roadmap status alignment. **References:** `docs/design/admin-console-workflow-ssot.yaml`, `docs/system-roadmap.yaml`. **Targets:** target SSOT, optional seed migration/tests, `.agent/tasks/todo.md` status update after audit.
