...
**Status:** `implemented`（2026-08-17、scheduler-settings subBundle実装完了により5/5 subBundle全完了。2026-08-15 round 45で`not_started`から訂正、round 46で根拠の言明を訂正、2026-08-16 round 47でcredential-management subBundle実装完了〔PR #602 latest head〕を反映、2026-08-17 PR #604 mainマージによりadmin-dashboard/admin-enum/credential-management/team-dashboardの4 subBundleがmain反映済みとなった上で、本roundにより最後のsubBundle `scheduler-settings`が実装完了した——詳細根拠は下記「scheduler-settings subBundle 実装完了記録」節、およびPR #602/#604のmain反映状況は git log参照。要約: 5 subBundle全てがproduction実装済み・SSOT completion_condition充足・test/live-DB proof完備であるため、Bundle全体を`implemented`とする——これはSSOTが所有するstatus vocabulary authorityの引用ではなく、production evidenceとcanonical carry-over classificationから直接導いた判断である）
...
### scheduler-settings subBundle 実装完了記録（2026-08-17、worktype `audit`→`implementation_change`連続、baseline: main `f5106dd`＝PR #604 merge済み）

**開始時status:** `admin-surface-topology-seed-conversion` Bundleの最後の未着手subBundle。既存hardcoded `/admin/scheduler` + `SchedulerJobSettingsPanel.tsx`は list/search/filter/enable/disable + create/edit/step chain authoringの全てを1画面に持つ約20フィールド＋step配列のhardcoded formのままだった。scheduler-settings自身のscopeは2026-07-22 design_change（本Bundle該当節）で既に3分割確定済み——(1) job本体create/edit/step chainは`/admin/contents`、(2) credential/port bindingは`credential-management`（PR #602で実装済み、`configure_scheduler_job_credential_or_port_binding`確認済み）、(3) 本subBundle自身は設定済みcron一覧のlist/search(job_key)/filter(trigger_kind, schedule_policy_kind, active)/explicit enable・disableのみ。SSOT側の`seed_contract`（component_tree: search_input.alias/select.template/table.primitive/button.primitive、capability_requirements、mutation_confirmation_contract）は既に確定済みで、本roundは設計判断ではなくこの既存authorityの実装遂行だった。

**監査で確認した実態:**
- backend dispatcher_mappingは`scheduler_jobs:list_settings/create/edit/disable`のみで対称な`enable`が欠落（SSOT `new_operation_note`が明示的に要求）。
- `DataListSchedulerJobsSettingsAsync`は`authorityScope`/`credentialRequirementRef`/`externalPortRef`/`maxBatchSize`/`leaseSeconds`をprojectionに含んでおり、SSOT `forbidden_projection_fields`/`existing_schema_fields_allowed_for_projection`と不一致だった。
- search/filterが一切実装されていなかった（SSOT `capability_requirements.search`/`filter`要求）。
- scheduler-settings専用の`ui_projection` Manifestが1件も存在しない（per-action dispatcher_mapping-onlyのf0-f3のみ）。
- `docs/design/db-schema.yaml`にscheduler系5テーブル（`topology.scheduler_jobs`ほか）の登録が一件も無い——team-dashboard roundが`topology.team_dashboard_note`で発見・是正したのと同種のSSOT driftを、schedulerでも独立に発見。
- `runtime-orchestration-ssot.yaml`の`/admin/scheduler`route retirement matrix entryが`retirement_kind: hub_navigation_only, status: pending`のまま、2026-07-22の3分割design_changeに追従できていなかった。

**実装内容:**
- **backend**（`backend/runtime/AdminRuntime.SchedulerSettings.cs`）: `DataEnableSchedulerJobAsync`を`DataDisableSchedulerJobAsync`と対称に新設（`SetSchedulerJobActiveWithConfirmationAsync`共通実装、`targetActive`で分岐）。両方とも`AdminRuntime.TeamDashboard.cs`の`DataTeamDashboardUpdateAsync`と同型のmutation_confirmation_contract（`payload.dryRun`→検証のみ／未確認→`..._NOT_CONFIRMED`でfail-close／`payload.confirmed`→実write＋`AdminMasterRosterAudit.AppendAsync`による`logs.diff`記録）を実装。同一state再適用（enable-when-already-active等）はfail-close。admin role gateをin-method明示チェックで追加。`DataListSchedulerJobsSettingsAsync`へ`search`(job_key ILIKE)/`triggerKind`/`schedulePolicyKind`/`active`のoptional filterを追加し、projectionをSSOT `existing_schema_fields_allowed_for_projection`ちょうどに縮小（`authorityScope`/`credentialRequirementRef`/`externalPortRef`等を除去）。`AdminRuntime.cs`へ`scheduler_jobs:enable`を登録。
- **repository**（`backend/repository/NpgsqlSchedulerJobManifestRepository.cs`）: `LoadSettingsProjectionAsync`をパラメータ化SQL（`ILIKE`/`=`、文字列結合なし）で search/filter対応に拡張。`LoadSettingsProjectionByIdAsync`新設（enable/disableのbefore値取得・存在確認用）。
- **DB seed**（`db/seed_empty.sql` 5c1ファミリ）: `scheduler.settings.projection` manifest（`00000000-0000-0000-0000-00000005c100`）+ 所有hub（5c101、hub_relations source/targetには使わないFK owner専用、ae201/dd011と同型）+ package/components_package_design/components_layout_design/ui_wiring_registry/ui_topology_tensor（5c102-5c106）。`.agent/tools/react-schema-topology-seed-translator`の`generate-react-schema`→`generate-topology-seed`実2段pipelineを実際に通し（`.agent/tests/fixtures/react-schema-topology-seed-translator/scheduler-settings-admin.{input,topology-seed.input}.json`新設、`gateStatus: pass`・`validationErrors: []`確認）、その出力を直接採用——手書きtensorではない。component_treeはSSOT指定通りsearch_input.alias/select.template(×3、trigger_kind/schedule_policy_kind/active別)/table.primitive/enable・disable各々のbutton.primitive+confirm_modal(dryRun preview→confirmボタン)。既存f0-f3（axes: target=admin）との`MANIFEST_AMBIGUOUS`衝突を避けるため、5c100自身のdispatcher_mappingは`target=manifest`（ae200と同型の設計判断）。scheduler_jobs:enable用dispatcher_mapping manifest（`...f4`、target=admin、f0-f3と対称のaxes）も新設。`list_settings`側dispatcher_mappingへ`default_screen_read: true`を付与し初回表示から実データ表示。**本subBundleは意図的にhubs.hub_relations rowを1件もseedしていない**——admin-enum/credential-management/team-dashboardのいずれも同様にseedへは書かず、navigation_binding_authoring_and_verificationはlive-DB testでの実dispatch証明のみで充足する既存established patternに従った。
- **frontend**（`frontend/routes/admin/scheduler.tsx`）: `SchedulerJobSettingsPanel`（削除、`frontend/islands/SchedulerJobSettingsPanel.tsx`）を、`/admin/enums`・`/admin/team-dashboard`と同型のthin `ProjectionShell`ラッパーへ置換。**Manifest UUID直書きではなくmanifest_key解決**（`manifestKey="scheduler.settings.projection"`）——team-dashboard round 4で新設された既存generic `manifest_key_target_ref_resolution_contract`（`ManifestDispatcher.cs`、"exactly one active row"、fail-close）をそのまま再利用し、scheduler専用のroute resolverは一切追加していない。`frontend/components/catalog.ts`から退役コンポーネントentryを削除。`frontend/fresh.gen.ts`/`frontend/_fresh/*`再生成（33 routes/20 islands、21→20）。
- **db-schema.yaml SSOT drift是正**: `tables:`セクションへ`scheduler_jobs`/`scheduler_job_steps`/`scheduler_job_runs`/`scheduler_job_run_steps`/`scheduler_external_event_evidence`の5テーブルを新規登録（`scheduler_job_manifest`category新設、`key_columns`/`authority_rules`は`db/topology_tables.sql`の実DDL・COMMENT ON TABLEおよび`scheduler-job-manifest-ssot.yaml`の`authority_rules`から転記）。
- **SSOT同期**: `admin-normal-surface-projection-seed-ssot.yaml` `design_blocking.target_surface_manifest_readiness.subbundle_status.scheduler-settings`を`unresolved_before_seed`→`target_manifest: resolved`/`navigation_binding_authoring_and_verification: resolved`（他4 subBundleと同型の分割shape）へ更新。`admin-console-workflow-ssot.yaml` `subbundle_target_readiness.scheduler_settings`を`blocked`→`not blocked`へ更新。`runtime-orchestration-ssot.yaml`の`/admin/scheduler`route retirement matrix entryを`retirement_kind: hub_navigation_only, status: pending`→`retirement_kind: thin_projection_wrapper, status: done`（`/admin/enums`と同型、実際に行った変更の実態に合わせて再分類）へ訂正、および`production_projection_connectivity_invariant.scope`の「team-dashboard/scheduler-settings remain unresolved_before_seed」という古い記述を「5 subBundle全て resolved」へ更新。`scheduler-job-manifest-ssot.yaml` `admin_hub_relation_navigation.status`を`blocked_pending_seed_catalog`→`resolved`へ更新。`admin-master-roster-management-ssot.yaml`へ`scheduler_jobs_read`/`scheduler_jobs_write`のresource:action分類を新設（enum_dictionary等の既存分類と同型、scheduler権限の新規付与ではなく既存4 actionの分類明示）。`react-schema-topology-seed-translator-ssot.yaml`の`declared_seed_surface_catalog`へ`scheduler.settings.projection`エントリを追加。
- **.agent/tests/check-scheduler-job-manifest-ssot.sh是正**: 退役した`frontend/islands/SchedulerJobSettingsPanel.tsx`のfile存在チェックを、新しいthin wrapper（`ProjectionShell`＋`scheduler.settings.projection`文字列）の存在チェックへ置換。

**Governance NG boundary遵守（Gate0 evidence）:**
- create/edit/step-chain authoringをこのsurfaceへ再実装していない——`/admin/contents`の既存dispatcher_mapping（f1/f2）は無変更のまま残置。
- credential/port binding操作をこのsurfaceへ持ち込んでいない——`credential-management` subBundle（PR #602）の`configure_scheduler_job_credential_or_port_binding`のまま。
- 現行`SchedulerJobSettingsPanel.tsx`をdesign authorityとしてseedへ写経していない——SSOT `seed_contract`（既に確定済み）から直接構築し、translatorのintake→generate pipelineを実際に通した。
- Manifest UUIDのroute hardcodeを追加していない——team-dashboard roundが確立した既存generic `manifest_key_target_ref_resolution_contract`をそのまま再利用。
- fake manifest/hub/relationを作っていない——hubs.hub_relations rowは1件もseedせず、navigation_binding_authoring_and_verificationはlive-DB testでの実dispatch証明のみで充足（既存4 subBundle全てと同型）。
- scheduler固有dispatcher/lane/repositoryを新設していない——既存`AdminRuntime.ExecuteDataAsync`switch・既存`ManifestDispatcher`・既存`ISchedulerJobManifestRepository`をそのまま拡張。
- 既済み4 subBundle（admin-dashboard/admin-enum/credential-management/team-dashboard）の実装ファイルは一切変更していない。

**test/proof:**
- Unit: `backend/tests/Topolactor.Runtime.Tests/AdminRuntimeSchedulerSettingsMutationConfirmationTests.cs`（新設）——enable/disableのdryRun preview、未確認fail-close、confirmed write+diff_log記録、同一state再適用fail-close、search/filter、forbidden fields非露出をrecording double経由で検証。
- Live-DB（実PostgreSQL 16、`db/init.sql`全chain適用）: `backend/tests/Topolactor.Integration.Tests/SchedulerSettingsHubRelationUiProjectionLiveDbTests.cs`（新設）——`hub_navigation:create`実authoring dispatch経由でscheduler manifest自身のhubへrelationを張り、resolution chainが5c100の実package/layout/wiring/tensorへ到達しscalar Emissionを返すことを証明（admin-enum/credential-management/team-dashboardと同型のcombined-proof pattern、shared helper`HubRelationUiProjectionResolutionChainProof.cs`再利用）。加えてmanifest_key経由resolution、`default_screen_read`による実データ初期表示、forbidden_projection_fields非露出、enable/disable mutation_confirmation_contract、non-admin fail-closeを実DB上で検証。
- 実行結果: `dotnet build`（0 errors）、`Topolactor.Runtime.Tests`（1726/1726 passed）、`Topolactor.Integration.Tests`（270/271 passed——既知の無関係な既存failure`UiTopologyLayoutPatchRollbackIntegrationTests`〔`ui_layout_registry`テーブル名不一致、本diff対象外ファイル、git stashで同一failure確認・非回帰確定〕のみ）、`deno test -A frontend/tests/`（2103/2103 passed）、`deno check`（変更frontend fileでPASS）、`deno run -A frontend/dev.ts build`（33 routes/20 islands、成功）。
- `bash .agent/tests/check-db-schema.sh`/`check-bootstrap-validation.sh`（fresh DB、PASS）、`bash .agent/tests/check-scheduler-job-manifest-ssot.sh`（PASS、is是正後）、`bash .agent/tests/check-admin-normal-surface-projection-seed-ssot.sh`（PASS）、`bash .agent/tests/check-structure.sh`（PASS）、`python3 .agent/scripts/check_react_schema_topology_seed_translator.py`（既知の無関係な既存failure「7a」〔credential-management-0092 fixture、本diff対象外、git stashで同一failure確認・非回帰確定〕のみ）。`.agent/tools/agent-ui-local-test`全chain（`run-worktype-tests`→`read-senario-tmp`→`checklist`→`checks`→`summary`、`pass_or_fail: "pass"`）。

**判定:** scheduler-settings subBundleのcompletion condition（list/search/filter/enable/disable projection、seed_contract体現、mutation_confirmation_contract、navigation_binding_authoring_and_verification live-DB proof、db-schema.yaml drift是正、SSOT同期）は全て充足した。scheduler固有のhardcode・専用bypass・fake hub_relations・route UUID直書きは一切導入していない。scheduler-settings subBundleは**Implemented**として確定する。これにより`admin-surface-topology-seed-conversion` Bundle全体は5/5 subBundle（admin-dashboard/admin-enum/credential-management/team-dashboard/scheduler-settings）全てがImplementedとなり、Bundle全体も**Implemented**として確定する。後段の`test-orchestration-review`/`frontend-canonical-surface-structure-label-boundary`/`admin-console-workflow-step-wording-boundary`（本Bundle完了後にのみ開始する後段Bundle）へは着手しない——次にこれらへ着手するかはOwner判断待ちとして報告する。
