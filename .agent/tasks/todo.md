...
- [ ] `aggregation_function_ref_wiring`: **Problem:** `aggregate_dashboard.v1` wiring candidate `dashboard_run_button` has empty `targetRef` — the contents-side aggregation function wiring surface is not yet specified in a dedicated SSOT (`ssot_ambiguity`). **Purpose:** author must manually select the aggregation operation ref after preset load; seed uses `content_bundle:search` as placeholder. **Improvement direction:** when a dedicated contents-aggregation SSOT is available, resolve the wiring surface and update the seed `targetRef` and compile snapshot accordingly. **References:** `docs/design/ui-builder-preset-ecosystem-ssot.yaml` aggregate_dashboard_preset known_gaps, `db/migrations/aggregate_dashboard_preset_seed.sql` unresolved_json `aggregation_function_ref_wiring`. **Targets:** `db/migrations/aggregate_dashboard_preset_seed.sql` (update targetRef once SSOT resolves), `docs/design/ui-builder-preset-ecosystem-ssot.yaml` (update status once resolved).
