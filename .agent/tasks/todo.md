...
### round 12追記: 既存deployed DBへの追加migrationパス、およびchangedFields意味論の未解決報告

PR #600 review round 12は、本Bundleが解消した`changed_fields_json`列に関して2点を追加指摘した。

**1. 既存deployed DBへのmigrationパス欠如（解消済み）**: round 9の実装は`db/sql_attention_logs_tables.sql`の`CREATE TABLE IF NOT EXISTS logs.diff`にのみ列を追加しており、これは新規（未作成）DBにのみ効く——既にこの列より前にデプロイ済みの既存`logs.diff`テーブルには、`CREATE TABLE IF NOT EXISTS`は何の効果も持たない（テーブルが既に存在するため丸ごとno-op）。指摘は正確だったため、同ファイルの`CREATE TABLE`直後に`ALTER TABLE logs.diff ADD COLUMN IF NOT EXISTS changed_fields_json JSONB NOT NULL DEFAULT '{}'::jsonb;`を追加した——新規DBには（列が既にCREATE TABLE側で作られているため）no-op、既存デプロイ済みDBには追加的に効く。`db/init.sql`のヘッダ自身が「`db/migrations/`/`db/patches/`は廃止済み、そこにファイルを追加しない」と明記しているため、専用migrationファイルではなく対象DDLファイル自身に追記する形にした。`.agent/tests/check-db-schema.sh`へ、既存hub_relations legacy migrationの検証と同じ手法（`BEGIN`〜`ROLLBACK`によるtransaction内simulation）で、列を`DROP COLUMN`して「移行前の既存DB」を模擬したうえで同ファイルを再適用し、列の復元とpre-existing行のデフォルト値backfillを検証するtestを追加した。
- Test結果: フレッシュDBで`check-db-schema.sh`実行、`PASS`（新規simulation blockを含む全assertion成功）。

**2. changedFieldsの意味論（全物理変更列 vs. action固有の"監査対象"列）— 未解決のまま報告**: 「changedFieldsは実際に値が変わった列すべてを機械的に列挙すべきか、それともactionごとに手動で選んだ"監査対象"列のリストであるべきか」という問いに対し、既存10呼び出し元のコードを実際に読んだ結果、**単一の答えが導出できないことを確認した**——既に2つの異なる規約が共存している:
  ...
  この2つは、round 9より前から存在していた規約であり、round 9はenvelopeの永続化を実装しただけで、この意味論自体は変更していない（=round 9が生んだ新しい不整合ではなく、round 9以前から存在していたもの）。owner指示「一意に導出可能な場合のみ実装し、そうでなければ推測せず対立を報告する」に従い、**どちらか一方へ統一する実装はしていない**——上記の通り対立をそのまま報告する。統一するとすれば10箇所全ての呼び出し元を書き換える必要があり、それ自体が本round範囲を超える別の設計判断（owner decision）を要すると判断した。
- `docs/design/admin-master-roster-management-ssot.yaml`の`logs_diff_admin_projection.physical_mapping.changed_fields`ノートへ、上記2点（migrationパスの追記、意味論の対立の存在）を追記済み。
