...
| `hub-relations-deprecate-update-resolvability-guard-gap` | `DeprecateHubRelationAsync`のraw active-count guard、`UpdateHubRelationAsync`のresolvability guard欠如、`NpgsqlManifestRepository.DeprecateAsync`のincoming-reference check欠如という3つの独立した既存本番gapをresolvability-awareにするimplementation_change（`hub-relation-target-manifest-canonical-migration` Round34監査からの分離） | not_started | 1 | 未割当（既存本番条件、上記migration Bundleとは独立scope） | `docs/design/db-schema.yaml` / `docs/design/runtime-orchestration-ssot.yaml` |
...
3. HubRelation fixed-navigationの提示責務をProjectionShellからapp-shell navigation Islandへ収束し、current topologyをそのnavigation Islandへ`Context`経由で共有したうえで、current topologyについて成立し得る2つの独立した機構（owner case: current topologyが自分自身の`hubs.hub_relations`行から成る、`sequence_position`昇順のRESOLVED TARGETSのみの配列を所有する場合、next=その配列の先頭要素、prevはこのcaseからは無し／member case: current topologyのUUIDが、いずれかのowning manifestの配列内のいずれかの要素として出現する場合、prev/next=その配列上の直前直後の隣接要素のみ、owning manifest自身は候補にならない）のうち該当する各occurrenceについて、immediate prev/nextのみを、失わず提示する（配列の他memberを列挙しない。複数のoccurrenceを持つ場合は各occurrenceのprev/nextを独立に失わず扱う。片側が存在しない場合は既存home navigationへ委ねる）。
...
- sequence semantics: `db/topology_tables.sql`が正本（`db/legacy_utils/hub_relations_legacy_to_manifest_scoped.sql`がlegacy global graph shapeからこのmanifest-scoped shapeへ移行した経緯も同根拠）。sequence（array）は1つのowning manifest（`topology_manifest_id`、全行NOT NULLの実列）が自分自身で持つ`hubs.hub_relations`行を`sequence_position`昇順に並べた、RESOLVED TARGETSのみの配列である — owning manifest自身はこの配列の要素ではない（配列外の追加要素にしない。Owner fixture `A=[1,5,7,9,15]`/`B=[4,2,3,8,9,6]`をそのまま文字通り読む）。`(topology_manifest_id, sequence_position)`のみがUNIQUE制約対象であり、target側（`target_topology_manifest_id`/`related_hub_id`）の一意性は制約されていないため、同一配列内に同一manifestが複数positionへ重複出現すること、および`topology_manifest_id == target_topology_manifest_id`という合法な自己参照によりownerが自分自身の配列内のtargetとしても同時出現することを排除しない（occurrence単位でprev/nextを扱う、下記参照）。「順列（permutation）」とは呼ばない — target側に重複が許容されるため全単射ではない。1つのmanifestが他のmanifestの配列を所有し他manifestの行を多段（multi-hop）に辿ることはない（global graphではない）。
- **fixed-navigation candidateの訂正（Round29〜32のsemantic driftを訂正、Round33で最終確定、Round35で上位SSOT根拠を確認）**: **Round35監査結果**: owner case/member caseの二機構分割は本Bundle独自の特別ルールではなく、`docs/design/db-schema.yaml hub_relations.target_reference_canonical_contract.existing_fixed_navigation_pipeline_reuse_contract`が独立に根拠付けている——同契約は(a)既存の変更されないforward call chain（`HubNavigationResolver.ResolveAsync -> LoadHubNavigationSequenceAsync -> Emission.NavigationSequence`、本Bundle以前から存在し「current topology自身のsequence、先頭行」をnextとして解決してきたもの=owner case）と、(b)「同じ`hubs.hub_relations`永続化を別の列（target-scoped）で読む、追加のreverse-direction READ method」（=member case）を、runtime-orchestration-ssot.yamlより一段上のauthorityとして既に述べている。これまでの撤回履歴 — Round29は「current topologyが所属する上位relation/sequence」の復活を、candidate = そのrowの反対側の端点（opposite endpoint）を無条件に列挙する設計として実装した（誤り、撤回済み）。Round32はこれを訂正しつつも、owner自身がsequenceの「index 0」を占めるという配列拡張モデル（`[owner, target(p1), ..., target(pn)]`）を採用し、これがreverse-direction candidateを表現するための専用DTO問題を要求していた。このindex-0-as-owner解釈自体もOwner fixtureが示す配列外要素の追加にあたるため、本Roundで撤回する。**確定semantic**: 配列はRESOLVED TARGETSのみ（owner自身は要素に含まない）。2つの独立した機構: (a) owner case — current topologyが自分の配列を所有する場合、next=配列の先頭要素、prevはこのcaseからは常に無し（自分自身の配列への入口に「その前」は無いという直接規則であり、index計算ではない）。(b) member case — current topologyのUUIDが、いずれかのowning manifest M（自分自身の場合を含む — `topology_manifest_id == target_topology_manifest_id`は合法、例: manifest 092の自己参照）の配列内のいずれかの要素として出現する場合、prev/next=その配列上の直前直後の隣接要素（他のresolved target）のみ — owning manifest M自身は候補にならない。target側の一意性は制約されていないため、同一配列内に同一manifestが複数positionへ出現することも排除しない — identityではなくoccurrence単位でprev/nextを独立に扱う（マージ・重複排除・破棄しない）。マッチングは直接UUID equalityで行い、regex等の比較手段はidentityが直接比較できない場合の実装詳細に留め、SSOT semanticへ昇格させない。Ownerの例（A=[1,5,7,9,15]、B=[4,2,3,8,9,6]、current=9）は、記載された配列そのものをordered sequenceとして扱う: 9はAの中で7と15に（position 4、隣接position 3/5）、Bの中で8と6に（position 5、隣接position 4/6）それぞれ隣接する。配列外owner/nodeは一切追加しない。それぞれのoccurrenceが独立にprev/nextの対を提供する。prev/next両方提示要件のもとでは、standing on 9のcandidateは{7, 15, 8, 6}となる。新しいaxis/dimension/role flag列は不要 — `(topology_manifest_id, sequence_position)`の既存UNIQUE制約と`topology_manifest_id`のNOT NULL制約のみで導出できる。片側が存在しない場合は、既存の`frontend/islands/ProjectionShell.tsx`の無条件「ホーム」linkがround-trip navigabilityを既に担保しており、新しいfallback機構は不要。
- read projection: 既存`ContentBundleRepository.LoadHubNavigationSequenceAsync(topologyManifestId)`（source-scoped）が、current topology自身の配列（owner caseの候補元）を過不足なく返す。ownerとして立っている場合はこの結果の最小`sequence_position`行（配列の先頭要素）のみをnextとして使う。これに加えて、target方向のreverse read（owner manifestは問わず — 自己参照行を含む — current topologyをtargetとして名指すrowを全て見つけ、各行ごとにその`sequence_position`と同じowner配列内の隣接する他のresolved targetから current topologyのprev/nextを独立に導出する。owner自身は絶対に候補にしない）を**追加のadditive query**として導入する — 新しいtable/column/parallel authorityではなく、同一`hubs.hub_relations`永続化に対するもう一つの読み方（新設backend read methodは必要、ただし新しいtable/columnは不要）。この読みの目的はopposite endpointの列挙ではなく、他の配列要素からのprev/next導出である。
- **Admin bridge（実体、dd010へ収束、Round33で最終確定）**: `dd010`（team_dashboard.admin.projection）は上記(1)で確定した通り、それ自体がAdmin axisのcanonical navigation sourceである。標準例（Round33確定版）: `dd010`の配列（targetsのみ）は`sequence_position`昇順で`[ae200, 5c100, 092]`。standing on `dd010`（owner case）のnext = `ae200`のみ、prevは無し。standing on `ae200`（`dd010`配列のposition 1、member case）はprev = 無し（配列の先頭要素なので、これより前の要素が無い）、next = `5c100`。standing on `5c100`はprev = `ae200`、next = `092`。standing on `092`はprev = `5c100`、next = 無し（配列の最後の要素）。**`dd010`自身はae200/5c100/092のいずれからもfixed-navigationのprev/nextとして得られない** — owning manifestは自身の配列のmember候補になり得ないため。**Round35訂正**: 以前のRoundは「`dd010`への復帰は既存の`frontend/islands/ProjectionShell.tsx`の無条件「ホーム」linkに委ねる」と記載していたが、これは誤りであり撤回する — Homeの`href`は`/dashboard`であり、これは`dd020`（team_dashboard.normal.projection、Normal-axis）自身の route であって、`dd010`（team_dashboard.admin.projection、`/admin/team-dashboard`）とは無関係の別axis・別manifestである（`admin-console-workflow-ssot.yaml admin_hub_relation_navigation_contract.normal_navigation_membership`参照）。Home linkをclickしても`dd010`にも他のAdmin-axis manifestにも到達しない。正確な記述: `ae200`/`5c100`/`092`から`dd010`へ戻る手段は、fixed-navigation・既存Home affordanceのいずれにも存在しない（この契約が意図的に受け入れる帰結）。この点はRound30〜32が意図していた「Admin権限内の4 manifestが相互にfixed-navigationで到達可能」という性質からの後退であり、本Roundの明示的な指示（配列外owner解釈の撤回）に従った意図的な帰結として監査へ引き継ぐ。新しいbridge manifest・新しいdispatch axis・新しいsequence-position規約などの新architectureは一切作らない — 既存`dd010`と既存`dd010 -> {ae200,5c100,092}`行をそのまま再利用する。
...
- **DTO representability（Round32の未解決点をRound33で解消）**: Round32はreverse方向のprev候補（click destinationがowner manifest自身になるケース）が既存DTOのいずれのfieldの意味も偽装・反転せずには表現できないと判断し、新規DTO追加のOwner判断待ちとしていた。本Roundの配列外owner撤回により、この問題の前提自体が消滅した: member-case prev/nextの値は常に「同じ配列内の別の、ordinary resolved target」であり、owning manifest自身が値になることは無い（例: `ae200`のnextは`5c100`——両方ともdd010配列の通常のtarget）。`backend/repository/NpgsqlContentBundleRepository.cs`の`LoadHubNavigationSequenceAsync`実装を直接確認した結果、`HubNavigationSequenceItemDto.TopologyManifestId`は元々SQL行の列ではなく、クエリの入力パラメータ（current topology自身）をそのままecho する既存実装になっていることが判明した——つまりこのfieldは元々「この応答が何について答えているか」を表すものであり、「この行の実owner」ではない。この既存の意味をそのまま再利用すれば、reverse-derived prev/next entryもTopologyManifestId=current topology、HubRelationId/RelatedHubId/RelatedHubLabel/TargetManifestId=実際の隣接行自身の値、SequencePosition=その行自身の値、という形で一切のfield意味変更・反転無く表現できる。**結論: 新規DTO/新規typeは不要、既存`HubNavigationSequenceItemDto`/`HubRelationNavigationLinksResponseDto`をそのまま再利用する。** Owner判断待ちの事項はこれで解消済み。
...
- app-shell navigation Island: UI Builder非依存・ProjectionShell非依存。`frontend/routes/_app.tsx`をapp shellとして使う。current topology identityをContextから読み、上記の拡張済み`GET /hub-navigation/relations`（`?topologyManifestId=<current>`）を呼んでcurrent topologyが所属する各sequence membershipのimmediate prev/nextを取得し（自分がownerの場合の(a)方向、target memberの場合の(b)方向reverse readの双方から導出、上記参照）、それらを単一へ縮退させず提示する。target選択後の遷移は既存`?manifest=`遷移entryを再利用し、frontend側でattractor/structure-map解決を複製しない。
...
- `backend/schema/ContentBundleContracts.cs`（`HubRelationNavigationLinksResponseDto`の`fallbackReason`値追加、または明示branch用のvalidation error code追加。新規DTOは作らない）
...
- [ ] frontend DOM proof 3b（reverse方向、Admin axis）: `dd010`を表示中はowner caseによりnextが`ae200`のみであり（prevは無し、`5c100`/`092`が直接candidate化されないこと）を証明する。`ae200`（`dd010`配列のposition 1、自分自身のoutbound行を持たない）を表示中はmember caseによりprevが無く、next=`5c100`であることを証明する。`5c100`を表示中はprev=`ae200`、next=`092`であることを証明する。`092`を表示中はprev=`5c100`、nextが無いことを証明する。**`ae200`/`5c100`/`092`のいずれを表示中も`dd010`自身がprev/nextのcandidateとして一切現れないこと**（owning manifestはmember-case candidateにならないというRound33確定semanticの直接証明）を明示的にnegative-proofする。新しいbridge manifest/dispatch axis/architectureを追加していないことを確認する。
...
**Position:** `hub-relation-target-manifest-canonical-migration` Round34監査で発見・分離された、独立scopeのimplementation_change。SQL DDL変更は不要（既存guard/checkのロジック修正のみ）。backend実装は未着手。
...
### 改善方針（未確定、次段implementation_changeで具体化）
...
- `NpgsqlManifestRepository.DeprecateAsync`に、deprecate対象Manifestを参照する他manifestのhub_relations行を検索し、resolvable-cardinalityが0になるsourceが存在する場合の扱い（fail-close、警告、許容等）をOwner判断で確定する。
- 3経路とも、新しいlifecycle状態や新しいparallel authorization/guard機構は作らず、既存のguardパターンを拡張する。
...
- [ ] `NpgsqlManifestRepository.DeprecateAsync`が、deprecate対象Manifestを参照する他manifestのincoming hub_relations行を検索し、Owner確定済みの扱い（fail-close等）を実施することをtestで証明する。
