...
- [ ] Policy Judgment のテンプレート直実行誤用を検出する監査を実施する
...
- [ ] Policy Judgment FAIL / NOT EXECUTED / queued CI を PASS と誤認しないか監査する
...
- [ ] docs-only 変更でも runtime / policy behavior claim を含む場合に Policy Judgment を要求するか監査する
...
- [ ] PR Summary と実際の diff の claim drift を監査する
...
- [ ] `.agent/tmp/tmp.txt` の作成・検証・削除順序を守るか監査する
...
- [ ] `bash .agent/tests/check-structure.sh` を最後に実行する運用を監査する
...
- [ ] todo.md の `[x]` 更新が Recursive Verification Gate 通過後に限定されているか監査する
...
- [ ] context_event append が silent failure にならないか監査する
...
- [ ] transition stats / context event / TVR extension の失敗を「継続可能」にしてよい境界を監査する
...
- [ ] function_parameters 由来にすべき値を runtime 定数化しないか監査する
...
- [ ] DB CHECK / column name / seed policy が policy可変性と衝突していないか監査する
      → 理由: schema 固定と policy 変更余地の矛盾が運用停止を誘発する。
      → 対象ファイル: db/*.sql, seed policy 関連定義（監査時に確定）
      → 次の判断点: 変更可能前提の項目が DB 制約で過固定化されていないか確認する。

- [ ] append-only log と rebuildable materialized current を混同しないか監査する
...
- [ ] frontend に topology / cosine / MLP / feedback 判定を漏らさないか監査する
...
- [ ] happy path のみのテストで boundary failure matrix を省略しないか監査する
