...
- **正直な追加知見（display値とdispatch値の分離）**: scenario testの過程で判明——`renderEmission()`はcatalog_component leafのdefault propsを毎回ゼロから再構築するため、SSE refresh後の再renderではDOMのcontrolled `<input>`の表示値が emission-derived defaultへリセットされる（これは本Bundle固有ではなく、このcodebase全体のcontrolled-component再render時の既存の一般的挙動）。dispatch向けtracked値（本Bundleの対象）はこの影響を受けず、独立してrefresh後も正しく維持されることをtestで証明済み。「refresh後もDOM表示が入力済みテキストを保持し続ける」というUXは別のscope（emission-derived default propsをtracked値へbindする機能）であり、本Bundleに含まれない——SSOT `display_value_vs_dispatch_value_authority_note`に記録済み。
