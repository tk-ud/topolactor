...
**次にこの subBundle を触る Agent への引き継ぎ（2026-08-17時点）:**
- team-dashboard自体の実装完了条件（shared table・Admin/Normal manifest・hub relation proof・capability分離・canonical Normal runtime到達性）はすべて満たされた。残るのは上記「真の end-user reachability」の1点（既存画面からのnavigation link未配線、Owner判断待ち、`self_account_capability`と同種の既知gap）のみ。
- `scheduler-settings`（親Bundleの最後のsubBundle）は本roundにより一切着手していない。
