...
| `admin-surface-topology-seed-conversion` | Admin hardcoded surface topology seed conversion（`role-based-surface-separation` はこの Bundle の pre-seed-implementation evidence として統合済み — 2026-07-14、下記 Bundle 本文の該当 subsection 参照）。`admin-dashboard` subBundle は実装完了（PR #595、2026-07-19、下記「admin-dashboard subBundle 実装完了記録」参照）。`admin-enum` subBundle も実装完了（2026-08-15 round 44 再監査で確認——admin-enum subBundle 実装記録 round 34-36 で、7 write action の ae200 単一画面統合・selected-row-relative field prefill・`enum_search`/`enum_group_filter` server-side filter・negative boundary matrix・`AdminEnumsRoster.tsx` 撤去＋`/admin/enums`の`ProjectionShell` thin wrapper化のすべてを production/live-DB proof付きで完遂しており、`docs/design/runtime-orchestration-ssot.yaml` `admin_route_retirement_matrix`も`/admin/enums: retirement_kind: thin_projection_wrapper, status: done`と一致している。round 37-43はこの完了済み状態を前提としたgovernance/security hardening——3層防御接続・schema-composed identity spoof fail-close等——であり、いずれもこの4項目を再度未完了へ戻していない。旧記述「hardcoded `/admin/enums`のUX-parity production replacementのみ未達」は2026-08-05 round 35時点で解消済みの陳腐化した記述だったため訂正した）。`credential-management` subBundle も実装完了（PR #602 latest head `ffb84c06d944459add957b030d1e20b6a8cf24fd`、2026-08-16、下記「credential-management subBundle 実装完了記録」参照——PR #602は本記述時点でmain未mergeであり、この完了判定はPR headのproduction evidence〔SSOT整合・実装ファイル・live-DB test・全checks green〕に基づくsubBundle単位の判定であって、main反映自体を意味しない。親Bundle Statusは残り2 subBundleが未着手であるため`partial`のまま変更しない）。残り2 subBundle（`team-dashboard`/`scheduler-settings`）は引き続き未着手——`docs/design/admin-normal-surface-projection-seed-ssot.yaml` design_blocking subbundle_status上、team-dashboardは`normal_dashboard_authoring_runtime_adapter`が"Not done"（topology seed / dispatcher_mapping / runtime adapterが未生成）、scheduler-settingsは`navigation_binding_authoring_and_verification: unresolved_before_seed`（target manifest自体が未構築）のまま。Bundle Status自体は`partial`を維持する（2026-08-15 round 46で根拠の言明を訂正。round45は`docs/governance/agent-governance-routing-ssot.yaml`が「implemented/partial/not_startedの3値status vocabularyを定義する」と述べていたが、round46で同SSOT該当section（`judgment_boundary`）を直接再確認した結果、これは誤ったauthority attributionだった——同sectionが実際に定義するのは`tool_output_is_not`/`prohibited_judgments`（agent-ui toolがraw observationだけからimplemented/partial/not_started/semantic completion/proof_passedを判定してはならない、という**tool-output側の判定禁止境界**）と`todo_and_roadmap_status_boundary`（todo/roadmap自身のstatus表現はobserved textに過ぎずproof/implementation realityとして扱わない）の2点のみであり、「TODO Bundle Statusフィールドが取りうる正本の3値」を定義するvocabulary authorityではない。`.agent/protocols/todo-carry-over.md`・`.agent/protocols/audit/07-bundle.md`等の関連protocolを確認したが、todo.md Bundle Status用の3値（または4値）vocabularyを所有するSSOT/protocolは他に見つからなかった——`not_started`/`partial`/`in_progress`/`acceptance_pending`という語自体はtodo.md冒頭（本ファイル3行目）が索引掲載条件として使う記述上の慣習に過ぎず、これもtodo.md自身をauthorityとして扱うことになるため根拠には使わない。よって`partial`は、vocabulary authorityの引用としてではなく、admin-dashboard〔PR #595、実装・merge済み〕とadmin-enum〔本文上記、production code・live-DB proof付きで実装完了確認済み〕とcredential-management〔PR #602 latest head、production code・live-DB proof付きで実装完了確認済み、下記「credential-management subBundle 実装完了記録」参照〕という5 subBundle中3件のgenuine production実装と、team-dashboard/scheduler-settingsの2件が`admin-normal-surface-projection-seed-ssot.yaml` design_blocking subbundle_status上genuinely unresolved_before_seed/Not doneのままである、という両側の実証拠を総合したcanonical carry-over classificationの結果として維持する——「一切着手されていない」("not started"という語の通常の意味)も「全subBundle実装完了」("implemented"の通常の意味)もいずれも現状を正確に表さないため、部分的完了状態を表す語として`partial`を採用した、という平易な事実判断である）。**2026-08-16追記: `team-dashboard` subBundleはdesign_change（Owner確定要件に基づく契約再設計）→implementation_change（`admin.surfaces.team_dashboard`新設・`topology.team_dashboard_note`物理table・admin/normal 2 manifest・`team_dashboard:get/update` backend action・live-DB proof）を完了し、実装完了へ移行した——詳細は下記「team-dashboard subBundle 実装記録」参照。ただし当時はManifestDispatcher側のper-action capability override機構が存在しなかったため「seed駆動のNormal-role open read」自体はplain REST bypass（`/team-markdown/*`と同一の既存established pattern）で実現しており、ManifestDispatcher経由での同等到達性をOwner判断待ちの明示的残課題として記録していた。**2026-08-17追記: その残課題を解消した。`ManifestDispatcher`へgeneric（team-dashboard固有ではない）なlayer/action-scoped capability_requirement override機構と、default_screen_read override機構（構造的screen-read fallbackが実データを一切返さないという、実装中に発見した副次的だが必須の追加gap）を新設し（`docs/design/auth-db-session-credential-ssot.yaml`/`docs/design/runtime-orchestration-ssot.yaml`）、`/dashboard`をplain REST bypassからProjectionShell経由のcanonical dispatchへ差し替え、`GET /team-dashboard/note`とそのfrontend islandは削除した——詳細は下記「team-dashboard subBundle 実装記録」内2026-08-17追記を参照。**2026-08-17追記（2周目）: team-dashboardの最後の残課題だった「real end-user cross-navigation from anywhere」も、audit（既存Normal-reachable manifestが本番に一件も存在しないことを確認）→Owner指示（`ProjectionShell`に固定ホームリンクを配置、hub_relations経由ではない解消）で完了した——詳細は同記録内の2周目追記を参照。これによりteam-dashboard subBundle自体の完了条件はすべて満たされた。**2026-08-17追記（scheduler-settings、PR #605 close + clean re-audit）: 直前ラウンドはPR #605としてscheduler-settingsを実装したが、`/admin/scheduler` + `SchedulerJobSettingsPanel.tsx`を`ProjectionShell`薄ラッパーへ置換し`runtime-orchestration-ssot.yaml`の`hub_navigation_only`を`thin_projection_wrapper`へ変更しており、これは直前の「scheduler-settings 3分割設計の確定」節（撤去ではなくscope縮小、がowner確認済み現行authority）と矛盾していた。PR #605はそのlineage上で修正せずcloseし、PR #604 merge済み最新mainから独立branchで再監査・再実装した——`/admin/scheduler` + `SchedulerJobSettingsPanel.tsx`は無変更のまま維持しscopeのみ縮小（list/search/filter/enable/disable）、`hub_navigation_only`retirement_kindも変更せず維持、target manifest（`scheduler.settings.projection`、5c100）を投影side証跡として新規構築、backend `scheduler_jobs:enable`+mutation_confirmation_contract、db-schema.yaml drift修正、translator汎用gap修正、live-DB proof、frontend DOM proofを全て完遂した。詳細は下記「scheduler-settings subBundle 実装完了記録」参照。これにより**5 subBundle全件が実装完了**——ただし本ラウンドでは次Bundleへの着手は行わず、親Bundle完了状態と次Bundle候補の報告のみに留める。** | implemented | 5 subBundle（全5件実装完了: admin-dashboard/admin-enum/credential-management/team-dashboard/scheduler-settings） | `product.dynamic_support_nocode_loop` / admin hardcoded surface retirement | `docs/design/admin-normal-surface-projection-seed-ssot.yaml`, `docs/design/react-schema-topology-seed-translator-ssot.yaml`, `docs/design/runtime-orchestration-ssot.yaml`, `docs/design/admin-console-workflow-ssot.yaml`, `docs/design/admin-uibuilder-ui-structure-wiring-ssot.yaml`, `docs/design/instance-port-substrate-ssot.yaml` |
...
### Cross-cutting audit findings — round 10 resolution（2026-08-16、PR #602 round 10、round 9で発見した2 findingのOwner design authority反映）

**round 9時点の記録（要約、原文は本Bundle git履歴参照）:** credential-management completion監査中に、(1) common navigation/escape surfaceのcompletion condition未確定、(2) SQL Attention candidateのbackend実生成→frontend実rendered最終stateまでのcomposed proof不在、の2 findingが見つかり、どちらもcanonical Bundle ownershipを確定できずOwner判断待ちとして記録した。

**round 10でOwnerが確定した設計方向:** (1) production projectionがcanonical hub relationを持たず孤立する状態は設計上invalidであり、専用header等のUI補完ではなく、production projection topology自体がcanonical hub relationを持つことをinvariantとする。(2) SQL Attentionを含むprojection link/EmissionはID/reference/contextのみを運び、実データはそのIDを利用したDB query resultとして取得する現行architectureを維持する。この判断に基づき、両findingについて以下を実施し、解消した。

#### Finding 1 解消: production_projection_connectivity_invariant

**対応内容:** `docs/design/runtime-orchestration-ssot.yaml` `ui_projection_render_reachability_contract.production_projection_connectivity_invariant`（design_change）に、production projection manifestは最低1件のactive `hubs.hub_relations`を解決しなければならない（`production_projection_manifest_must_not_be_navigation_orphan`）という恒久的invariantを新設した。既存の`docs/design/admin-normal-surface-projection-seed-ssot.yaml` `navigation_binding_authoring_and_verification`/`navigation_binding_resolution_criterion`（5 subBundle向けの一回限りcompletion checklist item）を、production projection全般に対する恒久architectural invariantへ一般化したものであり、証明手段（`hub_navigation:create`実authoring dispatch + resolution_chain）は新規発明せず既存パターンを再利用している。`docs/framework-core.yaml` `runtime_route_attention_boundary`側にも同invariantキー（`production_projection_manifest_must_not_be_navigation_orphan`）と要約ポインタを追記し、詳細定義の重複を避けた。**専用navbar/header/sidebar/Home button実装や、component/nodeId/manifest-UUID固有のhardcodingは明示的に不採用**——`ProjectionShell.tsx`の既存conditional nav bar描画（`hubNavigationLinks.length > 0`）はOwner確認済みの意図した挙動として無変更のまま維持した。

**proof追加:** `frontend/tests/projectionShellHubNavigationRenderProof.test.ts`（新規）——`ProjectionShell`を実mountし、実`navigationSequence`データから実DOM上へ`<a data-hub-navigation-resolvable>`/`<span data-hub-navigation-unresolvable>`が描画されることと、`navigationSequence`が空の場合はnav bar要素自体が描画されないこと（現行の意図された条件付き設計）の両方を`render()`ベースで証明——旧来のsource-string grep実装（`frontend/tests/projectionEntry.test.ts`）を置き換えるのではなく補完する。

**残る未解消点（Ownerフラグとして明示）:** このinvariantの「具体的なenforcement機構（DB CHECK制約／manifest promotion時のblocking validator等）をどの種類のmanifestへ適用するか」は本round では決定していない（`production_projection_connectivity_invariant.enforcement_boundary`参照）——全manifestを対象とするか、business-facing production projectionのみを対象とするかはOwner確認が必要な別の設計分岐であり、推測実装しなかった。

#### Finding 2 解消: SQL Attention circulation composed proof

**対応内容:** Owner確認済みの通り、`RecommendNavigationProjectionSpec`/`SqlAttentionProjectionChildSpec`がID/reference（`sourceSetId`）のみを運ぶ現行architecture（`backend/schema/ContextRouteContracts.cs`）は変更していない。

**proof追加:** `backend/tests/Topolactor.Runtime.Tests/SqlAttentionCirculationComposedProofTests.cs`（新規、real DB必須・`TOPOLACTOR_TEST_DB_CONNECTION`要）——(a) `SqlAttentionLiveDbEndToEndTests.cs`と同じ実DB evidence生成ライン（logs.diff → watch candidates → `HubAttractorExplorationRuntime.ExploreAsync` → `logs.attention` append）で実evidenceを生成し、(b) `AdminRuntimeSqlAttentionProjectionTests.cs`と同じ`AdminRuntime.ExecuteDataAsync("sql_attention:list_projection")`実dispatch経路を、test-doubleではなく実`NpgsqlSqlAttentionLogsRepository`/`NpgsqlTopologyRepository`で構成し、(c) 同一`sourceSetId`で両者を接続し、dispatch応答の candidate が実際に(a)で生成した同一evidence行であることを直接repository読み出しとのcross-checkで証明する——実DB生成からAdminRuntime実dispatchまでを1つのcomposed scenarioとして繋いだのは本roundが初。応答の安定部分（`attractorKey`/`scoreBand`/`hitRank`/`lane`）は`frontend/tests/fixtures/sql_attention_projection_real_candidate_response.json`へchecked-inし（`hubId`等のper-test-run乱数フィールドは除外——fixtureの理由は同ファイル内コメント参照）、`frontend/tests/sqlAttentionCirculationFinalStateProof.test.ts`（新規）が同fixtureを実`fetchSqlAttentionProjection`でparseし、実`SqlAttentionProjectionPanel`コンポーネントで`preact-render-to-string`によりrenderして、実際のDOM文字列に実candidate値が現れることを証明する——`docs/design/pipeline-continuity-ssot.yaml`の`runtime_without_sse_contract_may_use_refetch_or_projection_response_assertion`（174行）によりDOM到達は必須ではないが、既存の`layoutSchemaStructuralRender.test.ts`パターンを再利用できたため上乗せで実施した。

**ownership:** 両findingとも「remaining work」ではなく本roundで解消済みのため、専用の新規canonical Bundleは作成しなかった（round 9で候補検討した`test-orchestration-review`/`product-nocode-loop-acceptance`はいずれも本findingのscopeと不一致のままであり、無理に統合していない）。将来同種のtier_2 proof gapが他のrecommendation/attention経路で見つかった場合は、その時点で新規auditとして扱う。
...
**Status:** `implemented`（2026-08-17追記: scheduler-settings subBundle実装完了〔PR #605 close + clean re-audit、下記「scheduler-settings subBundle 実装完了記録」参照〕により、5 subBundle全件〔admin-dashboard/admin-enum/credential-management/team-dashboard/scheduler-settings〕がproduction実装完了となった。ただしこの判定は各subBundleのPR head時点production evidence〔SSOT整合・実装ファイル・live-DB proof・全checks green〕に基づくものであり、それら各PRが個別にmainへ反映済みであることを必ずしも意味しない点は従前と同じ注意が必要。次Bundleへの着手は本記述時点で一切行っていない——owner指示範囲外の先行着手はしないという方針を維持している。旧`partial`判定〔2026-08-15 round 45で`not_started`から訂正、round 46で根拠の言明を訂正、2026-08-16 round 47でcredential-management反映〕の履歴は下記「未処理 bundle 索引」の本Bundle行に残す）
...
- `credential-management`: **実装完了（PR #602 latest head `ffb84c06d944459add957b030d1e20b6a8cf24fd`、2026-08-16、下記「credential-management subBundle 実装完了記録」参照）。** manifest 092の`users`/`external_api_credential`/`external_instance_credential` 3 canonical categoryへのSSOT整合 projection topology、navigation binding authoring/verification（`hub_navigation:create`実authoring + resolution chainの単一test統合）、`external_instance_projection_columns`のprojection seed binding/authoring/wiring/proof、credentials.usersの既存`auth_users`authority再利用（parallel authority非追加）、external_api_credential/external_instance_credential両CRUD authorityのsecret non-projection・canonical physical_table_name・write confirmation contractをすべてproduction/live-DB proof付きで完遂。admin user分離、standalone route/dedicated panel/raw table editorの追加は行っていない（NG boundary遵守）。password replace/rotateは owner NG により引き続き永続的に対象外（`retired_permanent_ng`）。
- `admin-enum`: **manifest構築・structural render proof・navigation closure proof・read circuit（search/filter/table、2026-07-23実dispatch化）は実装済み（下記「admin-enum subBundle 実装記録」参照）。** enum dictionary/group/item authorityは既存SSOTと `enum_dictionary:*` substrateに従う。target `ui_projection` manifest（`00000000-0000-0000-0000-0000000ae200`）は構築済み。navigation binding authoring / verificationは2026-07-22確定の解決基準を満たすproof（`AdminEnumHubRelationUiProjectionLiveDbTests.cs` の `hub_navigation:create` 実authoring dispatch + resolution chain 単一テスト）で用意済み。search/filter/tableは`enum_dictionary:list_groups`を実際にdispatchし、live-DBで実データ（`demo_status`グループ）が`emission.data`に現れ、`enum_table`が実columns/rows bindingを描画できることを証明済み。CRUD presetのgeneric shapeは参考にしたが `content_bundle:*` refsはコピーせず、enum authority operationへbindしている。**この記述時点では enum_dictionary:* の write系操作（create/update/delete/set_group_items）は`remaining_write_payload_capture_gap`（typed値をdispatch payloadへ載せるproduction-provenな既存mechanismの不在——`admin-runtime-operation-dispatch-lane-determination` Bundle参照）により未接続だったが、同gapは2026-07-24に解消済み（下記「2026-07-24 remaining_write_payload_capture_gap解消」節参照）。その後の実装（下記「admin-enum subBundle 実装記録」round 14以降、特にround 21で残り全write actionをae200単一画面へmutation_confirmation_contract付きで統合）により、enum_dictionary:*の全write操作は実装完了している——2026-08-15 round 44再監査で admin-enum は Implemented として確認済み（上記「未処理 bundle 索引」参照）。**
...
- `scheduler-settings`: **2026-07-22 design_change（owner確認済み、下記「scheduler-settings 3分割設計の確定」節参照）によりscope再定義済み。2026-08-17、実装完了（PR #605 close + clean re-audit、下記「scheduler-settings subBundle 実装完了記録」参照）。**target `ui_projection` manifest（`00000000-0000-0000-0000-00000005c100`、scheduler.settings.projection）を構築済み。既存dispatcher_mappingに無かった`scheduler_jobs:enable`を追加し、mutation_confirmation_contract（dryRunプレビュー→未confirm fail-close→confirmed write→logs.diff）付きでenable/disable両方を実装。list/search(job_key)/filter(trigger_kind/schedule_policy_kind/active)は既存`SchedulerJobSettingsPanel.tsx`自身へ実装（create/edit/step chainフォームは除去、`/admin/contents`側は無変更のまま既存パイプラインを継続使用）。既存hardcoded `/admin/scheduler` + `SchedulerJobSettingsPanel.tsx`は**撤去せず**このscopeへ縮小した——PR #605は誤ってこれを`ProjectionShell`薄ラッパーへ置換していたためclose し、PR #604 merge済み最新mainから独立に再実装した。navigation binding authoring/verificationはlive-DB proofで解決済み（`hubs.hub_relations`静的seed行は他subBundleと同じ規律で追加していない）。
...
### scheduler-settings subBundle 実装完了記録（2026-08-17、PR #605 close + clean re-audit、implementation_change）

**この節が `scheduler-settings` subBundle の実装状態に関する現在の正本である。** 直前のラウンドはPR #605として実装を進めたが、`/admin/scheduler` + `SchedulerJobSettingsPanel.tsx`を撤去して`ProjectionShell`薄ラッパーへ置換し、`docs/design/runtime-orchestration-ssot.yaml`の`/admin/scheduler`retirement_kindを`hub_navigation_only`から`thin_projection_wrapper`へ変更した——これは直上「scheduler-settings 3分割設計の確定」節が明示する2026-07-22 owner確認済み設計（「撤去ではなく、このscopeへ縮小する」）と矛盾する。owner指摘によりPR #605はそのlineage上で修正せず**close**し、PR #604 merge済みの最新mainから新規branchで独立にre-auditした。

- **PR #605 close証跡:** `tk-ud/topolactor#605`を、上記矛盾を理由に明示コメントの上でclose（PR body/history自体は書き換えず）。新規作業はPR #605のbranch/head/commitを一切継承せず、`git fetch origin main && git checkout -B <branch> origin/main`でPR #604 merge済みcommit（`f5106dd`、team-dashboard実装完了PR）を基点に開始——`git log`でPR #605の変更が新branchに含まれないことを確認済み。
- **PR #605の報告内容の扱い:** 「継承せず独立に再検証する」方針に従い、PR #605が報告していたgap（`scheduler_jobs:enable`欠如、search/filter欠如、target manifest不在、`db-schema.yaml`drift、translator `debounceMs`欠落）は権威としてそのまま採用せず、clean baseline上で1件ずつ再現確認した——**全件が独立に再現し**、以下の対応で解消した。PR #605固有の実装（ProjectionShell化、`thin_projection_wrapper`変更）は一切引き継いでいない。
- **実装内容（3分割設計の(3)のみ、(1)(2)は既存のまま不変）:**
  - **`frontend/routes/admin/scheduler.tsx`は無変更**（`<AdminAuthGate><SchedulerJobSettingsPanel /></AdminAuthGate>`のまま）。**`frontend/islands/SchedulerJobSettingsPanel.tsx`は削除せず、責務を縮小**——create/edit/step-chainのform state・UI（`DraftState`/`StepDraft`/`onCreate`/`onStartEdit`等、約20フィールド分）を全除去し、search（job_key）・filter（trigger_kind/schedule_policy_kind/active、backend提供option listから描画）・table（許可フィールドのみ）・enable/disable（dryRunプレビュー→行内confirm/cancel→confirmed write）のみを残した。`frontend/api/adminApi.ts`から`createSchedulerJob`/`editSchedulerJob`/`SchedulerJobDraftInput`等を削除（唯一の呼び出し元だった旧フォームと共に）。credential/port紐付けフィールドはこのpanelの投影から一切要求しない。
  - **backend**: `backend/runtime/AdminRuntime.SchedulerSettings.cs`の`DataListSchedulerJobsSettingsAsync`に検索(job_key ILIKE)・filter(trigger_kind/schedule_policy_kind/active)のoptional payload fieldを追加（既存`enum_dictionary:list_groups`のsearch/filter idiomと同型、absent payloadは従来通り全件）。新規`scheduler_jobs:enable`アクション（`SetSchedulerJobActiveWithConfirmationAsync`共通実装、disableと対称）を追加し`AdminRuntime.cs`のswitchへ配線。両者ともmutation_confirmation_contract（dryRunプレビュー→未confirm fail-close→confirmed write→`AdminMasterRosterAudit.AppendAsync`によるlogs.diff）に従う。admin role gate・既に目的stateにある場合のfail-close（`SCHEDULER_JOB_ALREADY_ACTIVE`/`_INACTIVE`）も追加。`NpgsqlSchedulerJobManifestRepository`に`LoadSettingsProjectionAsync`のsearch/filter引数と`LoadSettingsProjectionByIdAsync`（enable/disableのcurrent-state読み取り用）を追加、`updated_at`列を投影に追加。
  - **`docs/design/db-schema.yaml`のdrift修正**: `scheduler_job_manifest`カテゴリと`scheduler_jobs`/`scheduler_job_steps`/`scheduler_job_runs`/`scheduler_job_run_steps`/`scheduler_external_event_evidence`の5テーブルエントリが未登録だったため追加（既存`abstract_function_manifests`エントリと同形式）。
  - **scheduler.settings.projection seed Manifest（`00000000-0000-0000-0000-00000005c100`、hub`...5c101`）を新規構築**——`.agent/tools/react-schema-topology-seed-translator`のReact-like Schema入力（`.agent/tests/fixtures/react-schema-topology-seed-translator/scheduler-settings-admin*.json`）から生成し、identity plumbing（UUID）のみ後編集のうえ`db/seed_empty.sql`へそのまま採用。**このmanifestは「投影side（hub/package/layout/wiring/tensor）の初期構築物」という本Bundleの成果物そのものであり、`/admin/scheduler`のroute bodyには一切ならない**——`frontend/routes/admin/scheduler.tsx`は上記の通り無変更で、manifestの`target="manifest"`軸（`target="admin"`で既存の`f0`/`f3`/`f4`と`MANIFEST_AMBIGUOUS`衝突するため、`admin.enum.management.projection`と同じ回避策）を一切参照しない。到達性はbaselineの`hub_navigation_only`のまま（下記参照）。
  - **翻訳ツールの汎用gap修正**: 生成過程で`debounceMs`がtensor nodeへ伝播しない欠陥がclean baselineでも再現することを確認——scheduler固有ではなく`.agent/scripts/react_schema_topology_seed_translator.py`の`split_flat_records_into_adoption_candidates`内4箇所の汎用欠陥だったため、汎用修正（scheduler向けhardcodeではない）で解消。`ADMIN_RUNTIME_READ_ACTIONS`の3ミラー（SSOT/Python/C#/TS の4箇所）へ`scheduler_jobs:list_settings`を追加。
- **`runtime-orchestration-ssot.yaml`の扱い（重要、PR #605との対比）:** `/admin/scheduler`の`retirement_kind: hub_navigation_only`は一切変更していない（`thin_projection_wrapper`へは変更しない）。route body（`SchedulerJobSettingsPanel.tsx`）を置換しない以上、`hub_navigation_only`の定義（「URL/routeは変更せず、`/admin/manifests`経由のadmin-authored hub_relations outbound navigationにより到達可能になる、既存の直接URLに加えて」）がそのまま正確に現状を記述しているため、変更の必要なしと判断した（Owner向けdesign questionとしての報告は不要と判断）。`precondition`テキストのうち「target manifestが存在しない」というstale文言のみ、manifest 5c100の構築を反映して訂正し、`status`を`pending`から`done`へ更新した（route自体・URL・panelはいずれも無変更のまま、と明記）。
- **live-DB proof（`backend/tests/Topolactor.Integration.Tests/SchedulerSettingsHubRelationUiProjectionLiveDbTests.cs`、新規、4 tests）:** (a) `hub_navigation:create`実authoring dispatch + resolution chain統合proof（admin-enum/credential-management/team-dashboardと同一パターン、manifest 5c100自身のhub 5c101から合成targetへrelationを実authoring）、(b) `manifest_key:scheduler.settings.projection:projection_entry`によるgeneric resolvability proof（frontend route消費を意味しない旨を明記）、(c) `default_screen_read`による初期mount実データproof、(d) enable/disableのmutation_confirmation_contract（dryRunプレビュー非破壊→unconfirmed fail-close→non-admin fail-close→confirmed write+`logs.diff`実書き込み）。他subBundleと同じ規律で、`hubs.hub_relations`の静的seed行は一切追加していない（authoring/resolutionはtest時にlive-DBで証明するのみ）。
- **frontend proof（新規2ファイル）:** `frontend/tests/schedulerJobManifestProjection.test.ts`をAPI surface変更に合わせ全面書き換え（11 tests、`createSchedulerJob`/`editSchedulerJob`関連を削除、search/filter payload・enable/disableのdryRun/confirmed契約・forbidden fields非投影を検証）。`frontend/tests/schedulerJobSettingsPanel.test.tsx`を新規追加（5 tests、実production component を happy-dom へmount）——**panelが引き続き実在しmountされること**、**create/edit/step-chain UI（free-text入力・Create/Save/Add Step/Edit button）が消滅していること**、search input・3 filter selectが実在すること、Enable/Disableボタンがdry-runプレビュー→行内confirm/cancelを経て実際のconfirmed write（`logs.diff`ではなくAPI dispatch層で検証）へ到達すること、Cancelがwriteを一切dispatchしないことを、それぞれ実DOM interactionで証明。
- **検証:** backend `dotnet build`（0 error）・`dotnet test`全体（Runtime.Tests 1721/1721、Integration.Tests 268/269——既知の`UiTopologyLayoutPatchRollbackIntegrationTests`1件のみ、`git diff`で無関係なpre-existing failureと確認済み）。frontend `deno test frontend/tests/`全体 2116/2116、`deno check`対象ファイルclean。`check-db-schema.sh`/`check-bootstrap-validation.sh`をfresh DBに対して実行しPASS。`check-scheduler-job-manifest-ssot.sh`/`check-admin-normal-surface-projection-seed-ssot.sh`/`check-admin-master-roster.sh`/`check-structure.sh`/`check-yaml-parse-completeness.sh`/`check-static-ssot-purity.sh`/`check-design-ssot-progress-terms.sh`/`check-completion-judgment.sh`/`check-worktype-routing.sh`/`check-unified-test-gate.sh`/`check-frontend-types.sh`/`check-frontend-all-tests.sh`いずれもPASS。`check_react_schema_topology_seed_translator.py`は既知のpre-existing failure「7a」（credential-management-0092 fixture cross-check、`git stash`でも同一失敗を確認済み・無関係）のみ。`TOPOLACTOR_CI_REQUIRE_DB_CONTINUITY=1`での`check-backend-tests.sh`もPASS（`SchedulerSettingsHubRelationUiProjectionLiveDbTests`をfilterへ追加済み）。`agent-ui-local-test`チェーン（`run-worktype-tests`→`read-senario-tmp`→`checklist`→`checks`→`summary`）は`pass_or_fail: pass`。
- **SSOT同期（proof完了後にのみ実施）:** `docs/design/admin-normal-surface-projection-seed-ssot.yaml` `design_blocking[0].subbundle_status.scheduler-settings`を`unresolved_before_seed`から`target_manifest: resolved`/`navigation_binding_authoring_and_verification: resolved`へ更新。`docs/design/admin-console-workflow-ssot.yaml` `subbundle_target_readiness.scheduler_settings`を`blocked`から`not blocked`へ更新。`docs/design/scheduler-job-manifest-ssot.yaml` `admin_hub_relation_navigation.status`を`blocked_pending_seed_catalog`から`resolved`へ更新。
- **判定:** `scheduler-settings` subBundleは**実装完了（Implemented）**。5 subBundle中5件目が完了したことにより、親Bundle `admin-surface-topology-seed-conversion`は**5/5 subBundle完了**——ただし本ラウンドでは次Bundleへの着手は一切行わず、親Bundle完了状態と次Bundle候補の報告のみに留める（owner指示範囲外の先行着手はしない）。
- **次にこのsubBundleを触るAgentへの引き継ぎ:** `/admin/scheduler` + `SchedulerJobSettingsPanel.tsx`は生きたproduction UIであり続ける——将来これを撤去/置換する場合は、`hub_navigation_only`から他のretirement_kindへの変更を含む新たなdesign_changeとしてowner確認を経ること（本ラウンドの成果はそれを正当化しない）。manifest 5c100はseed-conversion側の証跡であり、frontend側のroute/component authorityとは独立した関心事のまま維持すること。
...
- ~~`db/seed_empty.sql`のae204/ae206を、translator再生成の結果でverbatim上書きする際に、`enum_table`ノードおよび各`*_confirm_modal`ノードの手動propsJson/propBindingsパッチを消失させる——本round自身が発見・是正した実装ミスであり、次のtranslator再生成でも同じマージ作業が必要。~~ → round 31でこの手動マージ要件自体を解消（下記参照）。

### admin-enum subBundle 実装記録（2026-08-04 round 31 — PR #601 audit round 2指摘の7件を同一Bundle scope内で解消。round30が残した「translator再生成のたびに手動マージが必要」という限定事項を含む）

**round31の指示（audit round 2）**: PR #601に対する詳細監査で判明した7件の欠落を修正すること。(1) Section-owned `admin_runtime_dispatch_override_wiring` Actionのvalidatorがlane名allowlist所属のみを検査しており、dryRun存在・secondaryDisclosureActionType=openModal・実在するModal target・confirmed不在・target_ref/payloadFromのペアリングという安全条件全体を検査していない、(2) `docs/design/react-schema-topology-seed-translator-ssot.yaml`が実装と同期していない、(3) `db/seed_empty.sql`のae204/ae206がround30の手動マージ手順に依存しており、これはcanonical generationとして残せない（「never hand-editedと記録しながら手動mergeを必要とする」がNG）、(4) 全7 operationのproduction DOM証明にdelete_groupが含まれておらず、旧「open buttonはdispatchしない」fixtureが現行shapeへ更新されていない、(5) A→B fresh-value証明（preview時点の値Aから、Confirm直前に値Bへ変更し、Confirm payloadがBを運ぶことを明示的に証明）がない、(6) retry-retention証明（失敗preview後、retry自体のdispatch payloadが保持済み値を直接運ぶことの証明）がない、(7) backend structural theory testが全7 operationの業務fieldキー/source setの完全一致を検証していない（既存`DispatchAsync_EnumDictionaryWriteManifest_ResolvesSinglePurposeLayout_WithDistinctPreviewAndConfirmPayloads`と同等の厳密さが必要）。

**1. Section-owned preview Action向けfail-close validator新設**: `.agent/scripts/react_schema_topology_seed_translator.py`へ`validate_admin_runtime_preview_action_pairing()`を新設し、`generate-react-schema`/`generate-topology-seed`双方の検証経路へ配線した。Section直下に`admin_runtime_dispatch_override_wiring`のActionが存在する場合、(a) `payloadFrom`に`dryRun`が存在すること（`ADMIN_RUNTIME_PREVIEW_ACTION_DRYRUN_REQUIRED`）、(b) `confirmed`が存在しないこと（`ADMIN_RUNTIME_PREVIEW_ACTION_CONFIRMED_NOT_ALLOWED`）、(c) `secondaryDisclosureActionType=openModal`を持つこと（`ADMIN_RUNTIME_PREVIEW_ACTION_SECONDARY_OPEN_MODAL_REQUIRED`）、(d) そのtargetのModalが実在しちょうど1つの`admin_runtime_dispatch_override_wiring` Confirm子を持つこと（`ADMIN_RUNTIME_PREVIEW_ACTION_CONFIRM_TARGET_AMBIGUOUS`、0件・2件以上いずれもfail-close）、(e) そのConfirm子とtarget_refが一致すること（`ADMIN_RUNTIME_PREVIEW_ACTION_TARGET_REF_MISMATCH`）、(f) そのConfirm子と業務field（dryRun/confirmed以外の全キー）が完全一致すること（`ADMIN_RUNTIME_PREVIEW_ACTION_PAYLOAD_FIELDS_MISMATCH`）——の6条件全てをfail-closeで検証する。既存`validate_disclosure_targets`（primary disclosureActionType経路）がsecondaryDisclosureAction経路へも同じtarget存在・target種別チェックを適用することを新規test（112/113）で確認したうえで再利用した。単体test 12件（108-119、否定/肯定control）を追加し、さらに実ae200 fixture（本番相当のfixture、hand-builtではない）に対して再検証を実行しzero validationErrorsであることを新規regression test（120/123）で証明した。

**2. SSOT同期**: `docs/design/react-schema-topology-seed-translator-ssot.yaml`の`admin_runtime_dispatch_override_wiring`定義へ`section_owned_dryrun_preview_pairing`フィールドを新設し、上記6条件とそのruleIdを明文化。`prohibited_features`へ`section_owned_admin_runtime_action_without_dryrun_openmodal_confirm_pairing`を追加した。実装（コード）に先行/並行してSSOTを更新し、コード側は`react_schema_contract.prohibited_features`/`VALID_ACTION_OWNER_NODE_KINDS`という既存Python定数を直接参照する形で記録した。

**3. canonical generation化によるae204/ae206の手動マージ廃止**: round30が「今後も再生成のたびに必要」と記録した手動マージ（`enum_table`のpropsJson/propBindingsと7つの`*_confirm_modal`のpropsJson）を、translator自身の生成ロジック拡張により完全に廃止した。(a) 新DSL属性`displayColumns`（`"key:Header,key2:Header2"`のcompact構文、新規`parse_columns()`でパース）をTable recordへ追加し、`enum_table`のpropsJson（columns）/propBindings（rows）をtranslatorが直接生成するようにした。(b) `split_flat_records_into_adoption_candidates()`内で、Modal recordの`title`/`body`からpropsJson（`{"data":{"open":false,"title":...,"body":...}}`）を生成する新規`tensor_nodes.append()`を追加した。(c) tensor node merge処理（`merged[nid]`）へ`propsJson`/`propBindings`のfirst-write-wins mergeを追加した。これにより`db/seed_empty.sql`のae204/ae206を、翻訳器の出力のみから（旧seedからの手抽出なしで）完全に再生成し、実PostgreSQL経由で32件のAdminEnumHubRelationUiProjectionLiveDbTests全passを確認した。新規regression test（121/122）で、実ae200 fixtureへの`generate-topology-seed`単独実行が`enum_table`のpropsJson/propBindingsと全7 Modalのpropsjsonを（post-generation手動パッチなしに）生成することを証明した。

**4. delete_groupのshared preview scenario統合**: `frontend/tests/projectionShellAdminRuntimeWritePayloadCapture.test.ts`の`CONFIRM_MODAL_SCENARIOS`table-driven配列へ`delete_group`エントリ（`typedFields:{}`、`needsSelectedGroupRow:true`、`expectedPayloadKeys:["groupId"]`）を追加し、既存6 operationと全く同じ7段階シナリオ（preview失敗→値保持確認、preview成功→Modal open、Cancel、reopen→Confirm）で証明されるようにした。加えて、delete_group専用の旧9 testが前提としていた「open buttonはdispatchしない」という古いshapeを、新しいpreview dispatch shapeへ全面更新した——delete_groupのpreviewもgroupIdを要求するため、行未選択でのDelete clickはdispatch自体が発生しない（fail close、正しい新挙動）ことを前提に、行選択の順序を「Delete clickより前」へ揃え、同期的な`flushUpdates()`直後assertionを`await waitFor(() => queryDialog(container) !== null)`へ置換した。

**5. A→B fresh-value証明**: `ConfirmModalScenarioConfig`へ`freshConfirmTypedFields`フィールドを追加（typedFieldsと同じキーの新しい値Bを保持）。shared scenario testのreopen後・Confirm click前に、typed fieldへB値を再入力し（`needsSelectedGroupRow`のoperationは2行目の行を再選択し）、Confirm dispatch payloadがA値ではなくB値を運ぶことを明示的にassertする段階を追加した——7 operation全部で機械的に証明される。

**6. retry-retention証明**: 失敗した1回目のpreview直後の、成功する2回目のpreview（retry）自身のdispatch payloadを新たにキャプチャし、typed values/選択済み行のgroupIdが1回目に入力したA値と一致することをassertする段階を追加した——「失敗attemptのpayloadに値が乗っていた」ことだけでなく「retry自体のpayloadにも保持されている」ことを7 operation全部で機械的に証明する。

**7. backend theory test完全field-set parity化**: `AdminEnumHubRelationUiProjectionLiveDbTests.cs`の`DispatchAsync_AdminEnumManagementManifest_EachWriteActionEmbeddedBehindOwnConfirmModal_StructurallyResolves`Theory（ae200自身のpreview/confirmボタンペア、7 InlineData）を、単一`expectedFieldSource`の部分文字列`Contains`検証から、`expectedFieldKeys[]`/`expectedFieldSources[]`のparallel array＋open/confirm両payloadFromの完全キー集合一致（`dryRun`/`confirmed`込みでOrderBy比較）へ全面書き換えた——既存`DispatchAsync_EnumDictionaryWriteManifest_ResolvesSinglePurposeLayout_WithDistinctPreviewAndConfirmPayloads`（ae210〜ae270の各専用write manifest向け、round9時点で同等の厳密さを既に持っていた）と同じ厳密さを、ae200自身のembedded preview/confirmボタンペアへも適用した。実PostgreSQL経由でTheoryの7 InlineData全件を含む32件のAdminEnumHubRelationUiProjectionLiveDbTests全passを確認。

**test証明の全体像**: frontend `deno check`clean、`deno test --filter "preview-gap round shared scenario"`および delete_group専用9 test含む23 testを実行——full assertion pass（**正直な既知の制限、round30から変わらず**: このsandbox環境では本ファイル内の`ProjectionShell`実マウント系testが`Leaks detected`という環境依存resource sanitizer誤検出でFAILする。baseline（本round着手前のコミット0400379）とgit stash比較し、同一の失敗パターン・同一件数であることを確認済み——新規失敗は皆無）。backend: `dotnet build`成功、実PostgreSQL 16経由で21 SQLファイル適用後、`AdminEnumHubRelationUiProjectionLiveDbTests`32/32 pass、`Topolactor.Integration.Tests`全体225/226 pass（1件は`UiTopologyLayoutPatchRollbackIntegrationTests`——2026-07-06のコミットに由来する本Bundle無関係のstale table名〔`ui_layout_registry`、実際は`topology.components_layout_design`〕参照バグで、本round着手前から存在）。python translator check: 187/188 pass（1件は"7a"——git stash比較で本round着手前から存在する既知flakeと確認済み）。`.agent/tools/agent-ui-local-test summary --worktype implementation_change`はpass。

**未着手のまま残る内容（round30から変わらず、正直な記録）**:
...
### Governance NG boundary追記（round 31）

- Section-owned admin_runtime Actionのvalidatorをlane名allowlist所属のみで済ませ、dryRun/confirmed/secondaryDisclosureActionType/target_ref parity/payload fields parityの検証を省略する——round31が追加した6条件全てを維持すること。
- 安全条件をPythonコメント/SSOT文章のみに記述し、validatorコードでの強制を伴わない。
- `db/seed_empty.sql`のae204/ae206を、旧seedからの手動抽出・マージによって再生成する——translator自身の出力のみで完全に再生成できる状態を維持すること。
- 全7 operationの証明を、6 operationのみのシナリオ実行結果をもって「全7 operation証明済み」と宣言する。
...
### admin-enum subBundle 実装記録（2026-08-05 round 34 — PR #600〜#601の実装系譜を維持しつつ、subBundle全残scopeのBundle単位closureを試みた回。route retirement以外の全残項目を解消。route retirementは真正なUX gapにより意図的に見送り、正直に記録）

**round34の指示**: PR単位ではなくsubBundle単位のclosureを要求する回。items browse/selected detail UX、完全negative boundary matrix、`enum_confirm_form`/`enum_form`/`enum_confirm_button`のorphan監査、`AdminEnumsRoster.tsx`/`/admin/enums`のroute retirementの4項目全てをBundle粒度で完遂することが求められた。PR #600・#601の最新実装を正本扱いせず、SSOTからclosure条件を再構成すること、既存generic mechanism以外のenum専用runtime laneを追加しないこと、全7 operationの境界を代表1件だけで証明しないこと、設計思想が競合し一意に導出できない場合は実装せずOwnerへ質問することが明示された。

**1. orphan監査: `enum_confirm_form`/`enum_form`/`enum_confirm_button`の撤去**: translator source fixture・生成seed・backend C#参照・frontend参照の全経路を根拠付きで追跡した結果、3 nodeとも frontend参照ゼロ、`enum_confirm_button`のdispatchは`ui-local:enum_confirm_button.open_confirm`という、開いた先の"confirm"状態を読むconsumerが皆無なlocal-state no-opであることを確認した。7つの実per-operation confirm modal（PR #601で確立済み）に完全に代替されている、事前round（PR #600時代）の構造leaf-onlyな遺物と判定し、orphanとして撤去した——translator fixture（DSL・topology-seed JSON双方）から該当node定義を削除し、`generate-topology-seed`で再生成、`db/seed_empty.sql`のae204（構造record）/ae205（wiring registry、`enum_confirm_button.wiring`エントリ）/ae206（tensor node）全てへ機械的に反映した（gateStatus=pass、validationErrors=0、旧内容との差分がorphan 3件の削除のみであることをPythonで構造比較して確認）。backend live-DB test（`AdminEnumHubRelationUiProjectionLiveDbTests.cs`の`ResolvesSsotComponentTree_NoUnresolvedLeaves`/`DispatchesRealListGroups...`）から該当3 nodeへのassertionを削除し、撤去の根拠をコメントで残した。

**2. 重大な未発見production bugの発見と修正**: AdminEnumsRoster.tsxとの機能比較のためformFieldFactory（`frontend/runtime/runtimeComponentFactory.ts`）を直接確認したところ、`control=form_input/form_field`（`form_field.template`）が`FormField.tsx`へ`children: h("span", null, "")`という**常に空のspan**を渡すのみで、`<input>`要素自体が一切存在しないことが判明した。ae200の7つのwrite-flow typed field（`enum_create_group_name_input`等、group/item名・index入力の全て）がこの`control=form_input/form_field`で authorされていたため、**実production上ではこれら7 fieldのどれ一つとして実際にはタイプ不能**であり、live node value trackerへ値が一切登録されず、7つのwrite action全てのtyped-value payloadFromが`PAYLOAD_FROM_NODE_NOT_FOUND`で必ずfail closeする状態だったことを確認した。既存DOM test（`projectionShellAdminRuntimeWritePayloadCapture.test.ts`）は`control=form_input/input`という**別の、実際に機能するcontrol種別**をmock Emissionへ用いており、live-DB testはpayload objectを直接dispatchしてUIを経由しないため、どちらの証明経路も実際に production へ出荷されるcontrol種別を一度も検証していなかった——CI greenの裏でこの欠陥が長期間見過ごされていたことになる。全7 fieldの`control`を`form_input/input`（`input.primitive`→`inputFactory`、実`<Input>`＋`onChange`）へ修正し、`react-schema-topology-seed-translator-ssot.yaml`の`admin.enum.management.projection`宣言済み`componentKinds`へ`form_input/input`を追加（このsurfaceでは未宣言だったため、修正前は`UNKNOWN_COMPONENT_KIND`でtranslatorがblocking判定した）、`generate-topology-seed`で再生成して`db/seed_empty.sql`のae204/ae206へ機械的反映した。translator check scriptへregression guard（check 123a: 実生成fixture出力から7 fieldの`control`が`form_input/input`であることを検証、hand-builtなnodeではなく実際に出荷される内容そのものを対象とする）を追加した。

**3. items browse UXの実装**: `enum_dictionary:get_group`の既存item-join queryを、新規actionを追加せず`list_groups`自身のbackend queryへ折り込んだ（`EnumDictionaryRepository.ListGroupsWithItemsSummaryAsync`新設、`enum.group_items`/`enum.items`をLEFT JOINし`string_agg`で`"index:name, ..."`形式のflattened summaryを生成、`AdminRuntime.cs`の`DataEnumDictionaryListGroupsAsync`をこの新methodへ切り替え）。既存`enum_table`の`displayColumns`へ`itemsSummary:Items`列を追加し、translator再生成・`db/seed_empty.sql`反映を行った。これは「既存read（list_groups）へ既存get_groupのjoinロジックを合成する」というSSOT指示（"no new list_items action, no direct child-Emission adoption"）を文字通り満たす——新規action無し、cross-manifest dispatch無し、child Emission採用境界の問題も原理的に発生しない（同一read circuitの中身が増えるだけ）。**検討したが採用しなかった代替**: 選択行に応じて動的にitem detail欄をprefillする案（AdminEnumsRoster.tsxにより近いUX）は、`Table.rowsSource`が`emission.data`配下の固定pathしか許容しないgrammar制約（round3で導入済みのfail-close validator自身の制約）とフォームフィールドの選択行prefill bindingが存在しないことにより、既存generic mechanismの組み合わせだけでは実現できないことを確認した——ユーザー自身の指摘を受けて`propBindings.value`のseeding機構（`liveNodeValueTracker.ts`）と`node:<id>.value.<path>`resolver（`payloadFromResolver.ts`）という**既存の2つの汎用機構**が実在することを発見したが、これらを選択行相対のfield prefillへ合成する実装（`propBindings.value.source`のnode:参照対応＋componentKind gate拡張＋行clickでの再seed）は本roundでは着手せず、未着手として正直に記録する（後述）。

**4. negative boundary matrixの代表operation限定解消**: 既存backend live-DB testを精査した結果、missing group/item・duplicate index・duplicate membership・referenced delete・unconfirmed writeの大半は既に7 operation中5〜7件で証明済みだったが、以下2つの真正なgapを発見・解消した。(a) frontend側のsettlement authority test（wrong manifest/missing Emission/backend success:false/queue rejectionの4 shape、preview・confirm双方）が`delete_group`のみを対象にしており、round-5 NG軸が明示的に禁止する「代表1operationのみでの証明」に該当していた——`CONFIRM_MODAL_SCENARIOS`をループする既存の7-operation canonical reread testと同一のtypedFields/needsSelectedGroupRow setup patternを使い、2 testを7×4 shape（preview/confirm双方で計56ケース）へ拡張した。resolveRuntimeDispatchSettlement自体は完全にoperation非依存のgeneric関数であるため、これは新規laneではなく既存生成機構の適用範囲拡大である。(b) backend側では、`update_group`/`update_item`自身のmissing-ID・duplicate-index（非reference行同士）negative caseが、他5 operationとは異なり一度も独立してlive-DB証明されていなかった（`AdminRuntimeMasterRoster.cs`のcreate_*/update_*/delete_*は共有関数ではなく各operation別methodであるため、architecturalな保証だけでは不十分と判断）——新規fixture行（demo_status/demo_activeとは無関係な、参照関係の無い新規group/item）を使った新規test 1件を追加し、dryRun/confirmed双方でENUM_GROUP_NOT_FOUND/ENUM_GROUP_INDEX_CONFLICT/ENUM_ITEM_NOT_FOUND/ENUM_ITEM_INDEX_CONFLICTを証明した。

**5. route retirement: 意図的に見送り（正直な記録）**: `frontend/routes/admin/enums.tsx`を`ProjectionShell` thin wrapperへ置換する作業は、UX parityが未成立のため本round実施しなかった。具体的な残gap: (a) selected-group-detailのprefillが無く、update系操作はユーザーが現在値を目視して手入力し直す必要がある（AdminEnumsRoster.tsxはprefillする）——上記3節で述べた通り、既存generic mechanismの組み合わせだけでは実現できず、新規汎用capability（selected-row相対のfield prefill binding）の追加という設計判断が必要。(b) `enum_search`フィールドはUI上に存在するが、server-side filterへ一切結線されていない（既存SSOT自身が"no enum_dictionary:search/filter action"と明記する既知gap）。この状態でrouteを置換すると、同一URL上でAdminEnumsRoster.tsxより明確に劣化したUXを出荷することになり、round-5 NG軸の「UX parityとproofが成立する前にrouteを変更しない」に抵触するため、意図的に見送った。`docs/design/runtime-orchestration-ssot.yaml`の`admin_route_retirement_matrix`の`/admin/enums`preconditionを、陳腐化した「manifestが存在しない」という記述から、この真の残blocker（field prefill mechanism、search機能結線）へ更新した。

**test証明の全体像**: python translator check script 196/197 pass（新規1 test追加、既知の無関係flake"7a"のみ残存、git stash比較で本round着手前から存在することを再確認）。backend: 実PostgreSQL 16（CI同一の21 SQLファイル適用）経由で`Topolactor.Integration.Tests`225/226 pass（admin-enum関連33/33 pass、唯一の残failureは`UiTopologyLayoutPatchRollbackIntegrationTests`——リネーム済みテーブル`ui_layout_registry`を直接SQLで参照する既存の無関係な壊れたtestで、本round着手前から存在することをgit blameで確認）、`Topolactor.Runtime.Tests`1564/1564 pass（in-memory repositoryへの新規method実装含む）。frontend: `deno check`clean、`projectionShellAdminRuntimeWritePayloadCapture.test.ts`含む全体で`AssertionError`0件（既知のsandbox leak detector誤検出のみ残存）。`.agent/tools/agent-ui-local-test summary --worktype implementation_change`はpass。

**未着手のまま残る内容（正直な記録）**:
- selected-group-detailの真のfield prefill機構（選択行相対のpropBindings.value拡張——既存の`propBindings.value`seeding／`node:<id>.value.<path>` resolverという2つの汎用機構を合成すれば新規laneを追加せず実現可能と判明したが、未着手）
- `enum_search`のserver-side filter結線
- 上記2点が解消され次第の`AdminEnumsRoster.tsx`/`/admin/enums`のthin_projection_wrapper route撤去

これらを次roundへ明示的に引き継ぐ。route retirement以外は本roundで解消済み。

### Governance NG boundary追記（round 34）

- PR #601のpreview workflow完成のみを根拠にBundle全体をImplementedと判断する——items browse/orphan監査/negative matrix/route retirementそれぞれの実施記録を個別に確認すること。
- items browse/selected detailを個別の小粒todoへ分割し、route retirementを恒久的に先送りする——本roundはBundle粒度で3/4項目を一括解消し、残1項目（route retirement）は理由を明示した意図的見送りとして記録した。
- 「DOM testが green」を「production seedが実際にそのcontrol種別で動作する」ことの証明として扱う——mock Emissionの内容と実`db/seed_empty.sql`のcontrol/componentKindが一致することを、translator再生成結果に対するregression testで独立に確認すること（round34で発見したform_input/form_field vs form_input/inputの乖離のような不一致は、mockと実装が乖離したまま両方greenになり得る）。
- 全7 operationの境界をdelete_group等の代表1件だけで証明する——settlement authority・negative matrixとも全7operationをtable-drivenで駆動すること。
...
### admin-enum subBundle 実装記録（2026-08-05 round 36 — PR #601 監査で指摘された7件のgovernance gapを同一Bundle scope内で解消: translator SSOT未同期、Field dispatchのmutation fail-close欠落、owning SSOTのsearch/filter範囲縮小、UX契約の暗黙縮小、retired islandのghost catalog exemption、route entry behavior proof不足、wiring SSOT内の完了状態衝突再発）

**round36の指示**: round35で実装したselected-row-relative field prefill/enum_search filter/route retirementのproduction実装が実際にはPR監査観点で7件のgovernance gapを内包していたことの指摘を受けた。既存substrateのみを使用し、enum専用runtime lane/nodeId分岐/manifest UUID分岐を新設しないこと、no PR本文編集（進行はPRコメントへ記録）が明示された。

**1. wiring SSOT内の完了状態衝突（round35で新規発生）の解消**: `admin-uibuilder-ui-structure-wiring-ssot.yaml`の2箇所（410行目付近、1062-1065行目付近）に、round35自身が実装したselected-row-relative field prefill/enum_search filter/route retirementを「pending」のまま記述する古い文言が、round35のwiring追記後も取り残されていた（round35は`round_27_28_settled_child_dispatch_result_authority`節のみ更新し、他の言及箇所を見落としていた）。両箇所を「done、詳細は`round_27_28_settled_child_dispatch_result_authority`参照」へ統一。

**2. translator SSOT未同期の解消（Field `valueFrom`/`admin_runtime_dispatch_override_wiring`の未定義実装）**: round35で実装した2つのField拡張（selected-row prefillの`valueFrom`属性、enum_searchの`admin_runtime_dispatch_override_wiring`参加）が、`react-schema-topology-seed-translator-ssot.yaml`のどこにも定義されないまま実装・fixture・seedにのみ存在していたことを発見（`validate_field_value_from_source`自身のdocstringが「SSOT: ...field_value_from_attribute」と自己言及していたにもかかわらず、そのsection自体が存在しなかった）。`input_text_markup_grammar_contract`へ`field_value_from_attribute`/`field_admin_runtime_dispatch_override_attributes`の2 sectionを新設し、`react_schema_contract.allowed_node_kinds.Field`のoptional属性一覧・markup tag文字列を実装と一致させた。

**3. Field dispatchのmutation fail-close欠落の解消（今回発見した本物のgap）**: round35のコード自身のコメントが「Field's own use of this lane is never subject to those Action-only Section-ownership/dryRun-preview-pairing rules -- correct, since a Field-sourced read/filter override is not the bare-ungated-mutation risk those Action-only rules exist to prevent」と主張していたが、これは主張のみで実際にはFieldが`confirmed:literal:true`等のmutation authorityを持てる状態を拒否する仕組みが存在しなかった（`validate_admin_runtime_preview_action_pairing`は`kind=="Action"`限定でFieldを一切検査しない）。新規`validate_field_admin_runtime_dispatch_wiring`（`.agent/scripts/react_schema_topology_seed_translator.py`）を追加し、Field自身の`admin_runtime_dispatch_override_wiring`使用を(1)`AUTHORITY_FLAG_KEYS`（dryRun/confirmed、両方とも禁止——Fieldは決してModal-Confirmペアリングへ参加できないためdryRunすら不可）、(2)`secondaryDisclosureAction`（Modal state mutation、禁止）、(3)targetRefの`<action>`セグメントがmutation verb（`MUTATION_ACTION_VERB_PREFIXES = {create, update, delete, set}`、`admin-master-roster-management-ssot.yaml`の`admin_runtime_actions`が定義するenum_dictionary/auth_users共通のread/write命名規約に基づく、enum固有ではない汎用check）である場合に拒否するfail-closeとして実装。python check script（`check_react_schema_topology_seed_translator.py`）へ13件のnegative/positive testを追加（enum_dictionary write 6action全て・UNRELATEDなauth_users:update・positive control・実ae200 fixtureのenum_search自身が新checkを通過することの確認を含む）。SSOTへ`wiring_lane_contract.lanes.admin_runtime_dispatch_override_wiring.field_participation`として正式記録。

**4. owning SSOTのsearch/filter範囲縮小の解消**: `admin-normal-surface-projection-seed-ssot.yaml`が`capability_requirements.search`として宣言する5フィールド（`enum.items.name`/`enum.items.index_num`/`enum.groups.group_name`/`enum.groups.index_num`/`enum.group_items.position`）のうち、round35の実装は`enum.groups.group_name`のみをILIKE検索していた（group_nameだけへの縮小、governance NG axis該当）。`NpgsqlEnumDictionaryRepository.ListGroupsWithItemsSummaryAsync`のSQLを、group自身のgroup_name/index_num OR 所属itemのいずれかのname/index_num/position（EXISTS句、outer LEFT JOINは非filter——マッチしたgroupのitemsSummaryが部分的にならないよう）へ拡張。同時に、宣言されていながら全く未結線だった`enum_group_filter`（select.template、group単位の絞り込み）を、新規`groupIdFilter`（exact-match）payload fieldとして実装・結線した（`EnumDictionaryListGroupsRequestDto`拡張、`ENUM_LIST_GROUPS_GROUP_ID_FILTER_MALFORMED`でnon-UUID値をfail close）。フロントエンド側は、Fieldの`optionsSource`/`optionsLabelPath`/`optionsValuePath`という新規汎用属性（Table自身の既存`rowsSource`/`displayColumns`ペアリングと同型）を追加し、`propBindings.options`（`{source, transform:"rowsToOptions", labelPath, valuePath}`——backend/frontend双方のcapability tableには既に存在していたがtranslatorのgeneration層に対応する属性が一つも無かった、という実在するgeneration gap）を初めてtranslatorから生成可能にした。`enum_group_filter`自身は`optionsSource=emission.data`（既存ae200 mount emissionを再利用、新規dispatch無し）と、自身の`change` triggerで`groupIdFilter`をlist_groupsへ送るeventBinding（enum_searchと同型）を持つ。fixture（`admin-enum-ae200.input.json`/`.topology-seed.input.json`）と`db/seed_empty.sql`のae200 layout_patch_json/layout_schema_jsonを実際のtranslator再実行で再生成し、手書きJSONは一切使用していない。backend unit test 5件・live-DB test 1件（position matchの実DB証明、group_items.positionのみで一致しgroup_name/index_num/item index_numのいずれにも一致しない数値を意図的に選定した専用test group、他フィールドとの偶然の一致を避けるため）を追加。

**5. UX契約の縮小監査**: `admin-master-roster-management-ssot.yaml`の`canonical_routes.admin_enums.ux_contract`（5項目: text_search_at_top/clearing_the_search_field_restores_full_roster/new_record_via_modal_form/row_click_prefills_the_inline_update_field/delete_requires_confirm_dialog）を現行ae200実装と逐一照合した結果、全5項目が現状のまま満たされていることを確認（縮小なし、owner質問不要）。「Show All」「membership detail」等はそもそもadmin_enums自身のux_contractに宣言されていない（admin_usersの`show_all_button_clears_filter_and_lists_full_roster`と混同しないよう明示的に区別して確認した）。

**6. retired islandのghost catalog exemption解消**: `AdminEnumsRoster.tsx`が、round35のroute retirement後も「production route/import到達不能だがcatalog登録は維持、`runtimeComponentCatalogFullConnection.test.ts`の`EXEMPT_KEYS`ハードコードにより connectivity invariant からexempt」という状態のまま残っていたことを発見（governance NG axisが名指しで禁止する"test都合のregistered ghost catalog entry"そのもの）。`frontend/islands/AdminEnumsRoster.tsx`をファイルごと削除し、`fresh.gen.ts`の2箇所（import文・islands map）、`components/catalog.ts`のcatalog entry、`runtimeComponentCatalogFullConnection.test.ts`の`EXEMPT_KEYS`エントリ、`roleBasedSurfaceSeparation.test.ts`のhistorical entry-existence assertion、`adminMasterRoster.test.ts`のAdminEnumsRoster content checkを全て併せて除去した。SSOT側の`source_files`一覧（`admin-master-roster-management-ssot.yaml`/`.agent/docs/ssot-map.yaml`）からも除去し、関連する説明文言を「削除済み」へ更新した。

**7. route entry behavior proofの追加**: round35以前のDOM proofは全て`ProjectionShell`を`manifestId`propで直接mountするテストであり、governance NG axisが明示的に禁止する「ProjectionShellを直接mountするtestを、/admin/enums route wrapperとmanifestId propのproofとして扱う」状態だった。新規`frontend/tests/adminEnumsRouteEntry.test.ts`を追加し、`frontend/routes/admin/enums.tsx`自身のdefault export（`AdminEnumsRoute`）を実mountして、(a) 明示的selectionの無いdefault URLがae200を`manifestId` propのdefaultとしてdispatchすること、(b) 明示的`?manifest=`が常にpropのdefaultへ優先すること、(c) route module自身がAdminEnumsRosterを一切importしないこと（file自体が存在しないことも含む）、の3点を証明した。`setupDom()`（`test-dom-setup.ts`）へoptionalな`url`引数を追加し、query-param付きURLでのmount環境構築を汎用化した。

**test証明の全体像（round36）**: python translator check script 210 check（既知の無関係flake"7a"のみ残存、git stash比較で本round着手前から存在することを再確認）、うち新規追加19件（Field valueFrom/optionsSource/mutation fail-close negative/positive proof群）。backend: `dotnet build`solution全体0 error、`Topolactor.Runtime.Tests`1572/1572 pass（新規5件含む）。live-DB test（`AdminEnumHubRelationUiProjectionLiveDbTests.cs`新規1件、search全5フィールド+groupIdFilter+malformed fail-closeの実PostgreSQL証明）はコード追加・build成功済みだが、本round作業環境にDocker/PostgreSQLが無く（`docker ps`が daemon未起動を返す）ローカル実行では未検証——CI（`backend-tests`ジョブ）での実行を前提とする、正直な記録。frontend: 変更対象ファイルの`deno check`clean、`AssertionError`0件（git stash比較で本round着手前と同一の leak-sanitizer noise——`frontendScheduler.ts`のmodule-level setInterval singleton由来、既知——のみが残ることを再確認。新規`adminEnumsRouteEntry.test.ts`もこの既知artifactの対象になるが、`--filter`による単独実行で3 assertion全てpassすることを個別に確認済み）。`.agent/tests/check-structure.sh`/`check-admin-master-roster.sh`（本round自身がAdminEnumsRoster固定パスのhardcoded existence checkをProjectionShell existence + no-import checkへ更新）/`check-worktype-routing.sh`/`check-completion-judgment.sh`いずれもpass。

**未解決のまま正直に残す項目**: (a) 上記live-DB testはローカル未実行（環境制約、CIでの実行に委ねる）。(b) `enum_group_filter`のDOM proof（backend-live-DB-captured `frontend/tests/fixtures/admin_enum_ae200_layout_nodes.json`fixtureを使った実dispatch経由の検証）は、同fixtureの再captureに実PostgreSQLが必要なため本round未実施——translator層のproof（check 126-131、実fixture経由でのゼロエラー確認込み）とbackend層のproofは完了しているが、フロントエンドDOM層は次round（またはCI環境を持つ主体）での再capture待ち。

### Governance NG boundary追記（round 36）

- 新設した汎用機構（今回はField × admin_runtime_dispatch_override_wiring）を、既存のAction専用検証の対象外という理由だけで安全と主張し、実際のfail-close実装を伴わないまま出荷する——本round自身のround35コード内コメントがこの誤りを犯していた実例。
- owning SSOTが宣言するcapability（今回はsearch対象5フィールド、filter機構）を、実装容易な一部分（group_nameのみ）だけ実装して「search/filter実装済み」と扱う——宣言された範囲全体を実装するか、未実装部分を正直にgapとして記録すること。
- retired componentを、test側のhardcoded exemptionで無理やりgreenにしたまま「catalog上は存在する」状態で放置する——真に到達不能なcomponentはfile自体を削除し、catalog/test双方から整合的に除去すること。
...
### admin-enum subBundle 実装記録（2026-08-06 round 38 — outcome-parity再設計。componentId-as-instance-identityバグ修正、search/filterを単一query circuitへ統合、real-user-reachableなclear/all、選択行のcurrent membership prefill、SSOT authoring-source drift是正）

**round38の指示**: admin-enum subBundleをAdminEnumsRosterのDOM構造クローンとしてではなく、能力・安全性・運用性・データ駆動性で上回るcanonical projection surfaceとしてclose すること。DOM/panel/button/operation順のパリティは不要（Owner承認）——ただしbusiness capabilityの維持・改善、preview/confirm/canonical reread、safe selection context、generic substrateの維持は必須。実装前にbase branch・旧AdminEnumsRoster・現行headを比較し、設計変更理由・保持capability・改善軸をSSOTへ記録すること。同一catalog componentIdを共有する別layout nodeが互いのdispatch/debounce/決着に干渉しないこと、search/filterを単一query circuitとして統合しどの操作順でも最新意図のみが採用されること、field読み取り権限とdebounce policyがtranslator/永続化境界/production runtimeの3層で同一挙動でfail-closeすること。

**1. componentId-as-instance-identityバグの修正（NG-axis違反の自己発見・修正）**: `RuntimeComponentSpec`/`ComponentDataHub`が`nodeId`（layout node自身の構造的identity）を一切保持しておらず、round37のdebounce/sequence-guard機構が`spec.componentId`（catalog component定義自身のregistry UUID、同一catalog componentを使う別layout node間で共有される値）をkeyに使っていた——同じcatalog component種別（例: 2つの`form_input/search_input`）を使う無関係な2つのnodeが、互いのdebounce timer/sequence stateを静かに共有してしまう実質的なバグ。`nodeId`を`ComponentDataHub`→`adaptComponentDataHub`→`RuntimeComponentSpec`へ配線し、新設した`runtimeDispatchCircuitKey(spec, targetRef)`（`targetRef ?? spec.nodeId ?? spec.componentId`のfallback chain）を全debounce/sequence機構のkeyとした。2つの異なるnodeが同一componentIdを共有しつつ独立してdebounce/sequenceを保つことを証明する新規unit test（`frontend/tests/runtimeComponentFactory.test.ts`）を追加。

**2. search/filterを単一query circuitへ統合**: `runtimeDispatchCircuitKey`はtargetRef（存在する場合は最優先）を返すため、同一admin_runtime dispatch先（`enum_dictionary:list_groups`）を指す`enum_search`と`enum_group_filter`は自動的に同一circuit下で協調する——admin-enum専用の分岐は一切追加していない。debounce timerのcancel/replaceも同一circuitKey単位で行われるため、一方のfieldの即時dispatchが他方のfieldの未決着debounce timerを正しく上書きする（逆順も同様）。さらに、round37の設計は`payloadFrom`をschedule時点でeagerに解決し、その古い値をdelay後にそのまま再送していた——別fieldのdebounce待機中に値が変わっても反映されない、という発見済みバグ（"古いdebounced payloadが新しいfilter intentを上書きする"というNG-axis項目そのもの）を、`buildFreshDispatchSpec()`という発火時再解決thunkへ置換して解消した。

**3. 実ユーザー到達可能なclear/allオプション**: `frontend/components/Select.tsx`のplaceholder `<option>`が常に`disabled`だったため、`enum_group_filter`のような非required selectでは、実ブラウザユーザーが素の`<select>` UIから「未選択（フィルタなし）」へ到達する手段が皆無だった（disabled optionはtest/script経由の`.value`直接代入でのみ到達可能——round37 DOM testの「clear」はこの非現実的な経路を使っていた）。`disabled={required}`へ変更し、required selectの既存挙動は無変更のまま、非required selectのplaceholderを真にnative reachableなclearオプションにした。

**4. 選択行のcurrent membership prefill（実質的なcapability回復）**: `enum_set_group_items_input`（グループ所属item indexのCSV入力欄）が`valueFrom`を一切持たず、選択中グループの現在の所属を白紙から手入力させていた——旧`AdminEnumsRoster.tsx`は`draftIndexNums`として現在値をprefillしていたため、これは round36/37の過程で生じた実質的な回帰だった。backend `EnumDictionaryGroupWithItemsSummaryDto`へ`itemsIndexNums`（`enumIndexNums`が既に受理するCSV文字列shape、例"1,2,3"）を追加し、`NpgsqlEnumDictionaryRepository.ListGroupsWithItemsSummaryAsync`のSQLへ`string_agg(CAST(i.index_num AS TEXT), ',' ...)`を追加、`enum_set_group_items_input.propBindings.value.source = "node:enum_table.value.itemsIndexNums"`を配線した（`enum_update_group_name_input`の既存`groupName`prefillと全く同じ汎用mechanism）。これにより、行選択後は「選択groupのidentity・現在名・現在membership」が常時可視のroster行＋2つのprefill済みeditable fieldとして1つのselection contextを構成する（round37の`ux_contract_shrinkage_owner_decision_pending`をこの設計で解消——`docs/design/admin-master-roster-management-ssot.yaml`の`round_38_owner_decision_resolved_ux_contract_shrinkage`参照）。

**5. SSOT authoring-source drift是正（監査で発見）**: base-vs-head比較の過程で、翻訳パイプラインの最も上流のauthoring source（`.agent/tests/fixtures/react-schema-topology-seed-translator/admin-enum-ae200.input.json`のDSLテキスト）が、round36/37で`db/seed_empty.sql`へ直接手動反映された修正（`enum_search`/`enum_group_filter`のauthorityMarker・debounceMs・`groupNameFilter`への置換・`optionsSource`修正）を一度も反映しておらず、そのままtranslatorへ通すとblocking validation errorが3件出る状態だったことを発見した——SSOT/authoring-source/production seedが一つの意味を表していないというNG-axis項目に該当。DSLを修正し、2段translator pipeline（`generate-react-schema`→`generate-topology-seed`）を実行してゼロvalidationErrorsを確認、その出力を`db/seed_empty.sql`の`records[]`traceブロック（`enum_search`/`enum_group_filter`/`enum_table`/`enum_set_group_items_input`）へ反映し、実行時authority層（`nodes[]`のpropBindings/dispatchPayloadFromByTrigger、既に正しかった）とtraceを一致させた。`enum_set_group_items_input`の新規node entry（`propBindings.value.source`）も`nodes[]`へ追加した。

**6. 実データ確認・fixture再取得**: ローカルPostgreSQL 16へ`db/init.sql`相当のフルbootstrap（schema.sql〜runtime_orchestration_tables.sqlの全31ファイル）を適用し、実dispatch経由で`itemsIndexNums`が期待通りのCSV shape（例"1,2,3"）で返ることを確認、`frontend/tests/fixtures/admin_enum_ae200_layout_nodes.json`を実live-DB dispatchから再取得した。

**test証明の全体像（round38）**: backend: `dotnet build`solution全体0 error、`Topolactor.Runtime.Tests`1576/1576 pass、実PostgreSQL経由で`AdminEnumHubRelationUiProjectionLiveDbTests`36/36 pass（`itemsIndexNums`追加後、checked-in fixture再取得後の双方で確認）。frontend: `deno check`clean、`runtimeComponentFactory.test.ts`36/36 pass（componentId非干渉の新規testを含む）、`projectionShellAdminRuntimeWritePayloadCapture.test.ts`のprefill testへ`itemsIndexNums`のprefill/行再選択上書きassertionを追加、単体`--filter`実行でassertion自体はpass（leak sanitizerのみ既知chronic、baseline同一——`git stash` A/B比較で本round前後の`deno test frontend/tests/`全体の pass/fail数がbaseline 1976/93からわずかにテスト1件増加した1977/93（既存93 failureは完全に同一の既知chronic leak-sanitizer flakeであることを確認）。

**未解決のまま正直に残す項目**: (a) 選択contextの物理形態（roster行＋prefill済みeditable field）が、round38 Owner承認の範囲内で意図的に選んだ設計であることをSSOTへ記録したが、専用のfixed selection panel/drawerを新設する設計は比較検討の上不採用とした（既存汎用mechanismの再利用を優先）——将来より良い設計が判明した場合はSSOT比較記録を先に更新すること。(b) round38指示が求めた13項目の追加production DOM proof battery（componentId非干渉のProjectionShell-level DOM proof、search-then-filter-during-debounce等の個別ProjectionShell DOM test）のうち、`runtimeComponentFactory.test.ts`レベルの直接unit testと既存`enum_group_filter`DOM testの回帰確認のみ実施——ProjectionShell mount経由の新規DOM proof群（old-response-after-new-response等）は本round未着手のまま次round送りとする。(c) debounce policyのfail-close境界がtranslator/frontend runtimeでは確認済みだが、layout-patch永続化境界（`NpgsqlUiTopologyRepository`保存時点）でのdebounceMs fail-close検証は本round未実施。

### Governance NG boundary追記（round 38）

- `componentId`をlayout nodeのper-instance identityとして使い、異なるnodeに同一dispatch/timer stateを共有させる——`nodeId`（存在しない場合はtargetRef/componentIdへのfallback）を必ず使うこと。
- search-nodeとfilter-nodeを独立したdecision authorityとして扱い、古いdebounced payloadが新しいfilter intentを上書きすることを許す——同一dispatch先を共有する複数nodeは同一query circuit（`runtimeDispatchCircuitKey`）下で協調させること。
- test codeからdisabled placeholderのselectionを強制し、それを実ユーザーの明確な操作の証明として扱う——native `<select>` UIから実際に到達可能かを検証すること。
- SSOTを実装に事後追従させて縮小する——本roundはむしろ逆（実装側がSSOTに追いついていなかったauthoring-source driftを発見し、実装を変更せずauthoring sourceとtraceの側を是正した）。

### admin-enum subBundle 実装記録（2026-08-06 round 39 — query-circuitのProjectionShell production DOM proof battery、debounceMs fail-closeをbackend persistence-boundary/frontend runtimeの両方へ接続。backend persistence-boundary validatorが`componentKind`という常に不在のfieldをkeyにしており実質dead codeだった真のバグを発見・修正）

**round39の指示**: round38が未解決として明記した2項目——query-circuitのProductionShell mount経由DOM proof batteryと、layout-patch永続化境界におけるsearch Field `debounceMs` fail-close——を同一`admin-enum` subBundle内で閉じること。componentIdをlayout node instance identityへ戻さない、targetRefを無条件global circuit identityとして独立operationまで相互supersedeさせない、payloadをdebounce schedule時に固定せず発火時に再解決する、disabled placeholderへの直接`.value`代入をreal-user clear proofとして使わない、といったround38の設計判断は維持しつつ、残scopeの実装/testを追加すること。

**1. backend persistence-boundary validatorの真のバグ発見・修正**: `NpgsqlUiTopologyRepository.ValidateFieldOwnedAdminRuntimeReadOnlyDispatch`（round37でread-action allowlist enforcement用に新設）が`node["componentKind"]`をkeyにField-family node判定を行っていたが、`componentKind`はlayout_patch_json.nodes[]の生のoverride node entryには**一度も存在しない**field であることを発見した——`componentKind`は`LayoutSchemaTensorComposer`がschema-tree自身のcomponentId→catalogルックアップで解決するpost-composition factであり（`NodeLocalData`構造体自体に`ComponentKind` fieldが存在しない）、save時点でこの永続化境界検証が受け取る生のoverride deltaには一度も現れない。これはround37自身が「3層防御」と主張していたうちの1層（backend persistence-boundary）が、Field nodeに限らず**全てのnode**に対して実質dead codeだったことを意味する（既存backend test 0件でこの関数の実際のcheck成立を証明していなかったことも確認）。`ValidateLayoutPatchNodes`が`catalog_component` nodeへ既に要求している`componentKey`（tensor-only-authored nodeでは実DB columnとして存在）へkey識別を置換し、`LayoutSchemaTensorComposer.FieldControlToComponentKey`の値集合をそのままmirrorした`FieldFamilyComponentKeys`を新設した。**正直なscope注記**: この修正はtensor-only-authored layout（admin-dashboard等の平坦layout）を実質的に保護するようになった一方、admin-enum/ae200自身はschema-composed authoring（`components_layout_design.layout_schema_json.records[]`経由）であり、その生のoverride nodeにはcomponentKind/componentKeyいずれも一度も現れない構造上の理由（`Compose()`自身もcomposed output上で`ComponentKey: null`を常に固定出力する——round39自身がテストで発見）から、この修正はae200自身の既存seed内容に**新たな保護を追加するものではない**——ae200自身の実質的な保護は引き続きtranslator（層1）とfrontend runtime（層3）である。この事実をdoc commentへ明記した。

**2. backend persistence-boundary debounceMs fail-closeの新設**: 上記の修正済みcomponentKey識別を使い、`ValidateFieldOwnedAdminRuntimeSearchDebounce`を新設——componentKeyが`search_input.alias`（連続入力系のFieldのみ）で`dispatchTargetRefByTrigger`を持つnodeに対し、node-level `debounceMs` fieldが正の整数であることを要求する（欠落/0/負数/非整数/boolean/stringいずれもfail-close、`RUNTIME_INTERACTION_FIELD_SEARCH_DISPATCH_REQUIRES_DEBOUNCE_MS`）。`select.template`等のdiscrete-choice controlには適用されない（rule 5の既存scopeと一致）。実`ValidateLayoutPatchAsync`経由でaccept/reject matrix全体を証明する10件のunit testを追加、加えて`LayoutSchemaTensorComposer.Compose`経由のround-trip test（debounceMsが`BuildNodeLocalDataByNodeId`→`Compose`のNodeLocalData mergeを生き残ることを証明——round37自身がこの同じ境界でdebounceMsを一度dropしたバグを作り込んでいた場所）も追加した。

**3. frontend runtime debounceMs fail-closeの新設**: `runtimeComponentFactory.ts`の`emitBoundEvent`へ、`spec.componentType === "form_input/search_input"`かつ`isValidFieldDebounceMs(spec.debounceMs)`がfalseの場合に即座にfail-closeする新規checkを追加した（`externalPortDispatch`の既存`high_frequency_policy`runtime guardと同一パターン）。これにより、round38時点で存在していた「不正なdebounceMsを`scheduleOrDispatchRuntimeCommand`のisValidFieldDebounceMs分岐が静かに素通りさせ即時dispatchへfallbackする」という、governance NG axisが明示的に禁止する経路を閉じた。3件の新規unit testで、欠落/0/負数/非整数/boolean/string全パターンのfail-close、正の整数での正常動作、non-search_input Field（select）への誤適用が無いことを証明した。

**4. ProjectionShell production DOM proof battery（4件の新規test）**: 実canonical ae200 fixtureを使い、各list_groups応答が受信payload自身をgroupNameへ直接encodeする「self-describing marker row」設計により、「どのrequestが送信されたか」だけでなく「どのresponseが最終的にDOMへ採用されたか」を厳密に区別して証明した。
   - **search入力→debounce中にfilter選択**: filterの即時dispatchが、同一query circuit（`runtimeDispatchCircuitKey`がtargetRef基準で一致）を共有するsearchの未発火debounce timerをcancelすることを証明——結果、実際に送信されるdispatchは1件のみで、発火時点での最新combined intent（search値+filter値の両方）を carry する。cancelされたtimerがその後も発火しないことも追加確認。
   - **filter選択→search入力（別debounce window）**: filterの即時dispatchが先に確定した後、searchが自身の新しいdebounce windowで独立して発火し、2件のdispatchが両方とも実行され、最終DOM状態が発火時点での最新combined intentを反映することを証明。
   - **古いresponseが新しいresponseより後にsettleする**: カスタムfetch mockでolder requestのresponseのみ人為的に遅延させ、newer requestが先に安定することを証明した。実装調査の結果、`frontendScheduler.ts`の`drainClientCommandQueue`がadmin_runtime dispatch全体を**FIFO直列実行**しており（1件が完全にresolveするまで次のfetch呼び出しが物理的に開始されない）、これによりolder requestのresponseは必ずqueue順（＝送信順）で先にsettleすることを発見した——それでもseq guard（`runtimeDispatchSeqByCircuitId`、送信＝queue push時点でincrement）がolder responseをdiscardすることを証明（push時点で既にnewerがseqを進めているため）。この発見はFIFO直列化とseq guardが異なるレイヤで補完的に機能する2つの独立した保護機構であることを明らかにした。
   - **debounce待機中のunmount**: search入力直後（debounce未発火のうちに）ProjectionShellをunmountし、debounce window経過後もdispatchが一度も発生しないことを証明（`clearAllPendingRuntimeDispatchDebounceTimers`のunmount hookは既にround37で配線済み、今回はDOM levelでの実証のみ追加）。
   - **native reachability追加証明**: 既存のround37 filter-clear testへ、placeholder optionの`disabled`属性が`false`であることを直接assertする行を追加した（round38のSelect.tsx修正が実際に native `<select>` UIから到達可能な状態を生成していることの、`.value`代入シミュレーションとは独立した証拠）。

**5. SSOT同期**: `react-schema-topology-seed-translator-ssot.yaml`のrule 5解説がround38でrenameされた`runtimeDispatchSeqByComponentId`という古い変数名を参照したままだったことを発見・修正し（`runtimeDispatchSeqByCircuitId`へ）、round39のbackend/frontend fail-close追加を明記した。`admin-uibuilder-ui-structure-wiring-ssot.yaml`の`high_frequency_policy`へ、admin_runtimeレーン自身のsearch debounce policyが同じ3層shapeを踏襲する別建てpolicyであることのcross-reference注記を追加した（統合はしない——trigger名ベースの分類とcomponentTypeベースの分類は異なる軸であるため）。

**未実装として正直に残す項目**: (a) 「同一componentIdを共有する異なるlayout nodeがProjectionShell DOM level（合成fixtureではなく）で相互干渉しない」ことの専用DOM proof——round38の`emitBoundEvent`直接unit testで既に証明済みであり、ProjectionShell mount経由での再証明は本round未着手（冗長と判断）。(b)「query-circuit dispatch中のmutation preview/confirm」の専用組み合わせtest——mutation 7操作は search/filter circuitとは異なるtargetRef（従って異なるcircuitKey）を持つため構造的に独立しており、既存のper-operation preview/confirm test群で十分と判断し新規testは追加していない。(c) `EnumDictionaryGroupWithItemsSummaryDto`/`itemsIndexNums`関連の追加proof（round38 scope、本round対象外）。

**test証明の全体像（round39）**: backend: `dotnet build`0 error、`Topolactor.Runtime.Tests` 1587/1587 pass（新規20 test：persistence-boundary matrix 10件+componentKey/componentKind round-trip 1件、うち内訳は本文参照）。frontend: `deno check`clean、`runtimeComponentFactory.test.ts` 39/39 pass（新規3 test）、`projectionShellAdminRuntimeWritePayloadCapture.test.ts`の新規4 testはassertion自体は`--filter`単体実行で全てpass（leak sanitizerのみ既知chronic——`git stash`によるbaseline比較で本round前後の`deno test frontend/tests/`全体のpass/fail数がrun毎に変動する既知の cascading leak-sanitizer artifactの範囲内であることを確認、AssertionErrorはbefore/after双方でゼロ）。governance check（react-schema-topology-seed-translator 223/224 pass、既知flake"7a"のみ／admin-uibuilder-wiring／static-ssot-purity／ssot-vocabulary-contract）全pass。

### Governance NG boundary追記（round 39）

- backend persistence-boundary validatorが実際に発火することを一度も証明せず（unit test 0件）、「3層防御」を主張し続ける——本roundで発見した通り、componentKindという常に不在のfieldをkeyにした結果、real-world validationが一度も成立していなかった。
- FIFO直列queueの存在を無視して「2つのin-flight requestが並行に存在する」ことを前提にtestを設計する——本round自身がこの誤った前提でtestを一度書き、実装のバグではなくtest自身の前提ミスであることを`frontendScheduler.ts`の実装を読んで確認・修正した。
- debounceMsのfail-closeをtranslator層のみで実装し、backend persistence-boundaryとfrontend runtimeへの接続を「将来の課題」として先送りする——本roundは3層すべてを同一roundで接続完了させた。

---

### admin-enum subBundle 実装記録（2026-08-07 round 40 — schema-composed authoringのpersistence-boundary parity実装。round38/39が明示的に「未着手」とscopeを残した2つのProjectionShell production DOM proof battery（componentId共有非干渉、query circuit vs mutation circuit非干渉）を実装。実装過程で、round40自身のacceptanceが新たに露出させるlayout_schema_json.records[]破壊hazardを発見・修正）

**round40の指示**: round39までのadmin-enum subBundle completion監査を継続し、(1) tensor-only authoringとschema-composed authoring双方でField-owned admin_runtime read-action authority/search debounceMs policyがpersistence boundaryで同じ意味にfail-closeすること、(2) schema-composed raw overrideにcomponentKey/componentKindが存在しないことを前提に、保存時点でcanonical schema/catalog authorityからField identityを解決すること、(3) round38/39が「未着手」と明記した2つのProjectionShell production DOM proof battery——componentId共有非干渉とquery/mutation circuit非干渉——を閉じることを求めた。componentIdをinstance identityへ戻さない・admin-enum専用分岐を追加しない・Field classification authorityを二重定義しない（driftなし）という制約が明示された。

**1. schema-composed authoring persistence-boundaryの真の欠落発見・解消**: `ValidateLayoutPatchNodes`が`catalog_component` nodeへ無条件で`componentKey`を要求していたため、schema-composed layoutの正当なtensor override-delta node（`layout_patch_json.nodes[]`——componentKey/componentKindを一度も持たない、round39自身が文書化した通り）は、read-action allowlist/debounceMs checkへ到達する前に`LAYOUT_PATCH_CATALOG_COMPONENT_KEY_REQUIRED`で無条件fail closeしていた——round39の修正は「拒否されていた」ものを「別の理由で依然拒否される」ように変えただけで、schema-composed authoringへの実質保護は皆無のままだった。`LayoutSchemaTensorComposer`へ新設した`ResolveNodeIdentities`（NodeId/parentNodeId/duplicate-key namespacing/componentKey解決を単一箇所へ集約——`Compose()`自身もこれを共有するようリファクタ、二重定義なし）と、その上に構築した公開API`ResolveCatalogComponentKeysByNodeId`（schema-tree catalog leafのNodeId→componentKey map）を使い、`NpgsqlUiTopologyRepository.ValidateLayoutPatchAsync`が保存対象layoutIdの`layout_schema_json.records[]`を読み、このmapを`ValidateLayoutPatchNodes`・`ValidateFieldOwnedAdminRuntimeReadOnlyDispatch`・`ValidateFieldOwnedAdminRuntimeSearchDebounce`へ新設ヘルパ`ResolveEffectiveComponentKey`（生nodeのcomponentKeyを優先、欠落時のみschema-tree解決値へfallback）経由で接続した。exemptionは「componentKeyが欠落している全node」ではなく「そのlayoutIdの実schema-treeに実在するcatalog leaf nodeId」へ厳密scopeし（未知nodeIdは従来通り拒否——専用regression test`ValidateLayoutPatchAsync_TensorOnlyNode_MissingComponentKey_NotInSchemaTree_StillFailsClose`で証明）、`layout_schema_json.records[]`のDB round tripは「patch中に少なくとも1つcomponentKey欠落nodeがある」場合のみ発火するようgate（tensor-only-onlyの既存全testが引き続きDB接続なしで動作することを、`LoadLayoutSchemaJsonAsync`が呼ばれたら例外を投げる専用test double`SchemaJsonLookupForbiddenRepository`で直接証明）。malformed`layout_schema_json.records[]`は新規`LAYOUT_SCHEMA_RECORDS_INVALID`で全体fail closeし、「no schema、no exemption」への静かな読み替えを避けた。

**2. round40自身が新たに露出させたrecords[]破壊hazardの発見・修正**: `ApplyConfirmedLayoutPatchAsync`は`layout_schema_json`（構造authority）と`layout_patch_json`（override-delta carrier）へ同一`tensorPatchJson`を無条件で書き込んでいた。tensor-only layoutでは`layout_schema_json`が構造的に無意味なため無害だったが、schema-composed override deltaがバリデーションを通過できるようになった瞬間（上記1.）、この無条件上書きは初めて到達可能になり、`records[]`構造treeを`{"nodes":[...]}`形状の生tensor patchで即座に破壊する——次回`LoadLayoutNodesAsync`読み出し時にschema-composedからtensor-onlyへ静かに退行し、Category/Section/Field構造とcomponentKind解決を丸ごと喪失する重大なdata destruction hazardだったことを発見した。UPDATE文をCASE式へ変更し、`layout_schema_json.records[]`が既に非空配列を持つ場合は無変更で保持し（tensor-only layoutは従来通り置換）、実PostgreSQL経由integration test 2件（`SchemaComposedLayoutPatchPersistenceIntegrationTests.cs`——records[]保持の証明、tensor-only layoutの従来動作維持の証明）で実DB書き込みを直接確認した。round40のプロンプト自身は明示的にこのhazardへ言及していなかったが、自らのacceptance変更が新たに開くdata破壊経路を放置しないことは実装前契約の「関連箇所はAgent判断で追加調査・追加修正」の範囲内と判断した。

**3. ProjectionShell production DOM proof battery ×2（新規2 test）**: 実canonical ae200 fixtureを使用。
   - **shared catalog componentId非干渉**: 実fixtureに、`enum_search`と同一componentId（`search_input.alias`のcatalog componentId）を持つが、独自の`dispatchTargetRefByTrigger`（`enum_dictionary:get_group`——list_groupsとは異なる実在read action）・独自debounceMs（150ms、enum_searchの300msと異なる）を持つ合成sibling nodeを1つ注入し、両者へ連続してtypingし、片方のtimer/dispatch/settlementがもう片方に一切干渉しないことを、DOM上のself-describing marker rowで証明した（round38の`emitBoundEvent`直接unit testのDOM-level再証明）。
   - **query circuit vs mutation circuit非干渉**: search debounce待機中に`delete_group`（型入力不要——本試験がenum_create_group_name_inputのような型入力fieldに触れると、round40調査で新たに判明した「per-trigger overrideを持たないadmin_runtime catalog_component nodeは、layoutの持つuniform targetRef/wiringKindへの独自default 'change' dispatchを持つ」という既存・別建てmechanism（`renderEmission.ts buildRuntimeDispatchSpec`）と紛れるため、型入力を要さないdelete_groupを選択）のpreview/confirmを実行し、query circuitの独自dispatch（自身のschedule通りに発火）とmutation circuitの独自dispatch（preview即時発火・confirm後Modal close）が互いをcancel/supersedeしないこと、canonical rereadがmutationの確定writeからのみ1回発火しquery circuit自身の既発火dispatchを再発火・重複させないこと、Modal状態がquery circuitの決着に影響されないことをDOM/Modal/canonical reread観測状態で証明した。

**4. round40調査で発見した、本round scope外の別建て挙動（記録・未修正）**: `renderEmission.ts buildRuntimeDispatchSpec`により、per-trigger override（`dispatchTargetRefByTrigger`）を持たないadmin_runtime catalog_component node（例: `enum_create_group_name_input`のような書き込み専用typed field）も、そのnode自身のuniform wiringKind/targetRefへの独自default "change" dispatch（`{value: ...}`payload、debounce無し）を持つことをtest設計中に発見した——enum_search等のFieldが持つ明示的`dispatchTargetRefByTrigger`経由dispatchとは別の、既存・独立したmechanismである。本round自身の指示範囲（schema-composed persistence boundary、query/mutation circuit非干渉）には含まれないため修正はせず、test 3.の設計判断（型入力を要する操作を避ける）と本記録のみで対応した。次round以降でこのmechanism自体の意図的挙動かどうかの確認が必要であれば、この記録を出発点にすること。

**test証明の全体像（round40）**: backend: `dotnet build`0 error、`Topolactor.Runtime.Tests` 1602/1602 pass（新規15 test：schema-composed persistence matrix、tensor-only regression guard、malformed records[] fail-close、DB round trip gating証明を含む）、`LayoutSchemaStructuralCompositionTests` 65/65（既存、`ResolveNodeIdentities`リファクタ後も無変更で全pass、Compose()の外部可観測behavior保存を証明）、実PostgreSQL経由`SchemaComposedLayoutPatchPersistenceIntegrationTests` 2/2 pass（新規）、既存`AdminEnumHubRelationUiProjectionLiveDbTests`等40 test回帰なし。frontend: `deno check`clean、新規2 DOM testはassertion自体`--filter`単体実行で全pass（leak sanitizerのみ既知chronic）。`deno test frontend/tests/`全体（2079 test）をgit stash A/B比較——baseline 1980 passed/97 failed、本branch 1980 passed/99 failed（+2は新規test自身の既知chronic leak、AssertionErrorはbefore/after双方でゼロ）。governance check（react-schema-topology-seed-translator 223/224、既知flake"7a"のみ／admin-uibuilder-wiring／static-ssot-purity／ssot-vocabulary-contract／docs-ssot-connectivity／admin-master-roster／enum-dictionary／structure）全pass。`agent-ui-local-test summary` → `pass_or_fail: pass`。

**未実装として正直に残す項目**: round39が残した2項目のうち、追加のmutation-preview-during-query-circuit組み合わせtest（本round item 3.で実装済みのため解消）以外、新たな未着手項目はない。強いて挙げれば、item 4.で発見した「per-trigger override無しadmin_runtime nodeの独自default dispatch」mechanismの意図確認は本round未実施——修正の要否を含め次round以降の判断へ委ねる。

### Governance NG boundary追記（round 40）

- schema-composed authoringはtranslator/frontend runtimeで守られるという理由で、persistence boundaryの非対称性を「仕様」として追認する——本roundはこれを実際に解消した。
- 新しいexemptionを「componentKeyが欠落している全node」への blanket exemptionとして実装する——本roundは「そのlayoutIdの実schema-treeに実在するnodeId」へ厳密scopeし、専用regression testで証明した。
- Field identity解決を新たな別建てliteral listとして二重実装する——`ResolveNodeIdentities`を`Compose()`と共有する単一実装とし、drift detectionではなく構造的にdriftし得ない設計にした。
- 自らのacceptance変更が新たに露出させるdata破壊hazard（`layout_schema_json.records[]`上書き）を、プロンプト自身が明示していないという理由で放置する——実装前契約の「関連箇所への追加調査・修正」の範囲内と判断し修正した。
- 発見した別建て挙動（item 4.）を、本roundのscopeに含まれないという理由で記録すら残さず握りつぶす——正直にtodo.mdへ記録した。

### admin-enum subBundle 実装記録（2026-08-13 round 41 — round40 item 4.が発見した「per-trigger override無しadmin_runtime nodeの独自default dispatch」mechanismについて、Owner判断（Field-familyのみ明示的参加を要求し、Table/Button/Modal等の既存挙動は変更しない）をSSOTへ先に記録してから実装。schema-composed identity spoof fail-close、FieldFamilyComponentKeys mirror解消も同時実施）

**round41の指示**: round40が「本round scope外」として記録のみ残した独自default dispatch mechanismについて、Ownerが以下を確定した——(1) common request context auto-attach（JWT/role authority/client-origin/canonical scheduler）は今まで通り自動のまま維持する、(2) layoutがdefault business operationを持つことのみを理由に、node Eventへ自動でdispatch bindingを生成してはならない、(3) business operationを実行するEventは、authoring/topology dataからの明示的なdispatch参加を要する。SSOTをOwner判断に合わせて先に修正してから実装へ進むこと、実装後は round40以前も含む全Bundle契約への回帰がないか base→head全diffを再監査することが求められた。

**1. SSOT先行記録**: `docs/design/admin-uibuilder-ui-structure-wiring-ssot.yaml` `lane_storage_boundary.known_gaps.admin_runtime_layer_action_dispatch_lane_not_yet_defined`へ新規`owner_decision_2026_08_13_explicit_dispatch_participation_required`を追加し、round40の`read_circuit_consumed_and_proven`が記録していた「layout=one-canonical-operationモデルの下では、全nodeのtrigger一律継承は正しい」という理由づけを、Field-familyについてのみ上書きするものとして明記した。`react-schema-topology-seed-translator-ssot.yaml`へも対応するround41 addendumを追記。

**2. 実装（`frontend/runtime/renderEmission.ts` `buildCatalogComponentEventBinding`）**: `isFieldFamilyComponentKind(node.componentKind)`（Field read-action/debounce policyで既に使われている既存の汎用componentKind-prefix分類、admin-enum専用ではない）が真、かつ`nodeWiringKind === "admin_runtime"`のnodeについてのみ、trigger単位で`dispatchTargetRefByTrigger[trigger]`または`dispatchPayloadFromByTrigger[trigger]`のいずれかが明示的にauthoringされている場合のみ実dispatch bindingを持たせ、いずれも無い場合は`{eventType: trigger}`のみのinert bindingとした（`runtimeDispatch`キーなし）。**設計上の重要な訂正が2回発生した**——(a) 最初、trigger単位のfallback完全撤去（`targetRefOverrideByTrigger[trigger] ?? spec`を単純に`targetRefOverrideByTrigger[trigger]`のみへ変更）を試みたが、`runtimeComponentFactory.ts`の`requireBinding`（`inputFactory`/`buttonFactory`等）が「trigger keyがeventBinding自体に存在しない」場合にcomponent全体のrenderをfail closeすることを発見——`enum_create_group_name_input`のような書き込み専用Fieldが`<input>`として一切描画されなくなる、本roundが直そうとしたバグより深刻な機能regressionだった。eventType-onlyのinert bindingを常に持たせることで解決（(b) 同時に、`dispatchPayloadFromByTrigger`のみでdispatchTargetRefByTrigger`が無いnode（自身のnode.targetRef自体が意図したtargetであり、payload形だけを整形する既存の汎用パターン——admin-enum外でも使われる）を、明示的参加なしとして拒否する設計（および対応するfail-close）も試みたが、これは全体`deno test`実行でAssertionErrorゼロを確認する前の段階で、既存test「typing into a rendered input then clicking a rendered admin_runtime button」の legitimate use を壊すことが判明し撤回——`payloadFromByTrigger[trigger]`単独も明示的参加の正当な形として扱うよう修正。(c) さらに、「全admin_runtime catalog_component」へ適用範囲を広げた最初のFieldスコープ無し版は、round3〜round40の間に確立された約40件の既存・合格済みtest（最も中心的なのは`enum_table`自身の"select" trigger（行click）がlist_groupsを再dispatchする、複数roundのtestが明示的に"harmless idempotent read"として証明・依拠してきた挙動）を壊すことが`deno test`全体実行で判明し撤回——`isFieldFamilyComponentKind`によるField-family限定スコープへ縮小した。ユーザー報告の具体的症状（書き込みFieldへのtypingがsearch debounceへ干渉する）はField "change" eventに固有であり、Table/Button/Modal等の非Field nodeへ広げる根拠がないと判断した。

**3. schema-composed identity spoof fail-close**: `NpgsqlUiTopologyRepository.ValidateLayoutPatchNodes`に、nodeIdがそのlayoutIdの実schema-tree catalog leafである場合、raw override nodeが独自の（canonicalと不一致な）componentKeyを持てば新規`LAYOUT_PATCH_SCHEMA_COMPOSED_COMPONENT_KEY_IDENTITY_MISMATCH:{nodeId}`で全体fail closeする検証を追加した——round40の`ResolveEffectiveComponentKey`（raw値優先）がFieldの読み取り権限/debounce policyをspoofする経路を残していたことを閉じた。**disclosed scope limit**: この検証が実際に発火するのは、schema-tree map読み込みの既存gate（`ContainsNodeMissingComponentKey`——同一patch中に他のcomponentKey欠落nodeが存在する場合のみDB round tripが発火）が既に起動している場合に限る——このgateを「admin_runtime dispatch fieldを持つnode全て」へ拡張する案を試したが、既存の大量のdispatch-authoring test（componentKeyを持つ通常のField/Action/Step node）が軒並みDB接続を試みて失敗することが判明し撤回した。したがって、1 nodeのみで構成される最小限spoof patch（他に欠落nodeが存在しない）はこの検証で捕捉されない——正直に開示する既知の境界であり、隠されたgapではない。

**4. FieldFamilyComponentKeys mirror解消**: `LayoutSchemaTensorComposer.FieldControlToComponentKey`を`private`から`internal`へ可視性変更し、`NpgsqlUiTopologyRepository.FieldFamilyComponentKeys`をこの値から直接導出するよう変更——独立して手保守されていたHashSetを廃し、drift検出testではなく構造的にdriftし得ない単一authorityとした。

**5. backend負のtest追加（5件）**: `NpgsqlUiTopologyRepositoryLayoutPatchValidationTests.cs`へ、schema-composed componentKey mismatch fail-close、mismatch一致時のpass、Field policy bypass試行の end-to-end証明、tensor-onlyノードが影響を受けないこと、既存"NeverQueriesLayoutSchemaJson"契約が変更されていないことを再確認するtestを追加。

**6. frontend production DOM proof（新規2 test、実ae200 fixture使用）**: (a) `enum_create_group_name_input`（dispatchTargetRefByTrigger/dispatchPayloadFromByTriggerいずれも持たない実在write field）への、`enum_search`の300ms debounce待機中のtypingが、list_groups dispatchを一切発火させず、かつ`enum_search`自身のdebounce timerをcancelしないこと、debounce満了後に`enum_search`自身の元の入力値でdispatchが正しく発火することを証明。(b) `enum_search`/`enum_group_filter`（両者とも明示的dispatchTargetRefByTriggerを持つ参加者）が本round後も正しくdispatchし続けること、他のnodeが不要なdispatchを一切生成しないことを証明。

**test証明の全体像（round41）**: backend: `dotnet build`0 error、`Topolactor.Runtime.Tests` 1606/1606 pass（新規5 test含む）、`Topolactor.Integration.Tests`実PostgreSQL経由231/232 pass（唯一の失敗`UiTopologyLayoutPatchRollbackIntegrationTests`は本round・本PRと無関係——`ui_layout_registry`という現schemaに存在しない旧table名を参照する、pre-existingかつ本round範囲外の既存test bug、`db/ui_topology_tables.sql`のコメントで既にrenameが記録済み）。frontend: `deno check`clean、round41新規2 DOM testを`--filter`単体実行でAssertionErrorゼロ確認（leak sanitizerのみ既知chronic、`--trace-leaks`で既知のhappy-dom AsyncTaskManager/frontendScheduler setIntervalであることを確認）。`projectionShellAdminRuntimeWritePayloadCapture.test.ts`全体（55 test）をgit stash A/B比較——baseline/branch共にAssertionErrorゼロ、passed/failed件数完全一致（leak-sanitizer全体判定noise自体が本sandboxでは`--filter`無しの全体実行でも発生する既知の環境特性であることをbaseline側でも確認）。renderEmission.tsへ依存する関連21 test fileの一括実行（600+失敗7件、baseline/branch間で完全一致・AssertionErrorゼロ）。frontend全体`deno test tests/`（116 file, 2081 test）——baseline/branchともAssertionError 10件で完全一致（`runtimeComponentCatalogFullConnection.test.ts`/`roleBasedSurfaceSeparation.test.ts`のroute-composition reachability testが参照するfileが本sandbox checkoutに存在しない、本round・本PRと無関係のpre-existing環境gap）。`agent-ui-local-test run-worktype-tests --worktype implementation_change`/`checks`いずれもpass。

**未実装として正直に残す項目**: なし（本roundの受入条件は全て充足）。ただし上記3.のdisclosed scope limit（単一node spoof patchの理論的隙間）は次round以降の判断へ委ねる余地として記録する。

### Governance NG boundary追記（round 41）

- Owner決定をSSOTへ記録する前に実装から着手する——本roundは`docs/design/admin-uibuilder-ui-structure-wiring-ssot.yaml`のOwner decision記録を最初に完了させてから実装へ進んだ。
- Owner決定を「全admin_runtime nodeへの一律適用」と拡大解釈し、round3〜40で確立された約40件の既存test（enum_table row-select再dispatch等）を無検証で壊す——`deno test`全体実行によりこれを実際に発見し、Field-family限定スコープへ訂正した。
- `dispatchPayloadFromByTrigger`単独（`dispatchTargetRefByTrigger`無し）を「不完全な参加」として一律拒否する——既存の正当なbase-spec-plus-payloadFrom-onlyパターン（admin-enum外でも使用）を壊すことが判明し撤回、Ownerの原文言（"authoring/topology dataからの明示的参加"）はtargetRef overrideに限定していないと判断した。
- eventBinding entryの完全省略により、副作用として書き込み専用Fieldの`<input>` render自体を壊す——`requireBinding`の挙動を確認し、eventType-onlyのinert bindingで解決した。
- schema-composed identity spoof fail-closeの実装が、既存のDB round-trip回避gate（`ContainsNodeMissingComponentKey`）を暗黙に破壊する——広範囲な既存test群でこれを検出し、gateの意図的な非拡張とdisclosed scope limitの明記へ訂正した。
- FieldFamilyComponentKeys mirror解消を、drift検出testの追加のみで済ませる——実際に独立mirrorを廃し単一authorityへ統合する構造的解消を行った。

### admin-enum subBundle 実装記録（2026-08-14 round 42 — round41が「test互換性を理由にField-familyへ限定した」business-operation dispatch参加contractを、Owner再指示により全admin_runtime componentKindへ一般化。schema-composed identity spoof fail-closeの残存gap（round41自身が「disclosed scope limit」として容認していた単一node spoof）も本roundで解消）

**round42の指示**: round41は「明示的dispatch参加」を要求するOwner決定をField-family nodeのみへ限定した——理由は約40件の既存testがField以外（enum_tableのrow-select再dispatch等）の暗黙fallbackへ依存しており、無限定draftがそれらを壊したため。Ownerはこの限定自体を撤回し、以下を明確化した: (1) 決定は元々Field限定ではなく、admin_runtime wiringKindを持つ全componentKind（Table/Button/Modal含む）へ適用される一般契約である、(2) 既存testが旧暗黙継承挙動を検証しているという事実は、契約の範囲を狭める根拠にならない——テストは新契約に合わせて書き換える、または保護していた機能を明示的authoringへ再実装したうえで書き換えるべきである、(3) round41自身の「schema-composed単一node spoofは同一patch内に他のcomponentKey欠落nodeが無ければ検出されない」というdisclosed scope limitは、DB round-trip最適化を理由に容認してよい恒久的境界ではなく、閉じるべきgapである。

**実装1: SSOT一般契約への書き換え。** `docs/design/admin-uibuilder-ui-structure-wiring-ssot.yaml`へ新規`owner_decision_2026_08_14_general_dispatch_participation_contract_round42`を追加し、round41の「SCOPE（Field-familyのみ）」段落を全文supersedeする形で明記した——test互換性は契約の内容を決める根拠ではなく、契約に到達する手段を選ぶ際の考慮材料に過ぎないという訂正、一般契約の内容そのもの（componentKind不問でtrigger単位の明示的authoringのみがdispatchを生む）、common-request-context auto-attachとの非混同、非Field capability保存の方法（componentKind分岐ではなく実production nodeへの明示的authoring）、`runtimeDispatchCircuitKey`のtargetRef優先実装に関するround41自身の誤ったtest comment/proof主張の訂正、schema-composed単一node spoof closureの要求を記載した。`react-schema-topology-seed-translator-ssot.yaml`の対応するdisclosed scope limit段落も「round42で解消済み、以下は履歴記録」と明記した。

**実装2: `frontend/runtime/renderEmission.ts` `buildCatalogComponentEventBinding`の一般化。** `requiresExplicitParticipation`のgate条件から`isFieldFamilyComponentKind(nodeComponentKind)`連言を除去し、`nodeWiringKind === "admin_runtime"`のみとした（componentKind分岐を関数から完全に除去）。関数signatureから`nodeComponentKind`引数を削除し、呼び出し元（`renderEmission()`内）を追従修正した。`isFieldFamilyComponentKind`自体は、別の独立したField限定ルール（`buildAdminRuntimeTargetRefOverrideByTrigger`内のField read-action検証）で引き続き使用されており、削除していない——round42のSSOTが明記する通り、Field-family classification自体はこのroundで変更していない。

**実装3: 既存testの新契約への更新。** 一般化により実際に壊れたtestは`deno test`実行で経験的に確認した結果1件のみだった（round41自身の「約40件」という見積りは過大だった）——`adminWiringExecutionLane.test.ts`のbase-spec-onlyフォールバックへ依存していたnodeへ、自身の既存targetRef literalを再利用する`dispatchTargetRefByTrigger`を明示的に追加して解消した（契約を狭めるのではなく、テスト側を新契約のauthoring形へ合わせた）。round20の`projectionShellAdminRuntimeWritePayloadCapture.test.ts`テスト（enum_tableのrow-select挙動）は、round20時点で既にenum_table自身が`dispatchTargetRefByTrigger`を持たない実ae200 fixture形状を使用していたため、assertion（row-select後のdispatch件数）をNO-dispatchへ更新し、コメントを新契約の説明へ書き換えた。ae200 fixtureの22個の非明示的参加nodeを実際に精査した結果、新たにauthoringが必要なnode（round41が懸念していたenum_tableのlist_groups再dispatch等）は実は存在しないと判断した——row値追跡は`onNodeValueChange`という別の無条件lane経由であり、dispatch参加とは独立している。

**実装4: schema-composed単一node spoof closure。** `backend/repository/UiTopologyRepository.cs`へ新規virtual method `LayoutHasSchemaComposedRecordsAsync(Guid layoutId, CancellationToken ct)`を追加（既存`LoadWiringKindForLayoutAsync`と同じ「override必須、default実装は`NotImplementedException`」慣例）。`NpgsqlUiTopologyRepository.cs`で、`layout_schema_json->'records'`が非空配列かどうかのみを見る専用の軽量SQL scalar checkとして実装した（既存の全body取得`LoadLayoutSchemaJsonAsync`とは別）。`ValidateLayoutPatchAsync`のgatingを、`ContainsNodeMissingComponentKey`のみから、`mayBeSchemaComposed = ContainsNodeMissingComponentKey(json) || (ContainsAdminRuntimeNodeLevelDispatchField(json) && await LayoutHasSchemaComposedRecordsAsync(layoutId, ct))`という組み合わせgateへ変更した——admin_runtime dispatch fieldを持つnodeが存在し、かつそのlayoutが実際にschema-composedである場合のみ、軽量scalar checkを追加で1回払う。dispatch fieldを持たない通常のtensor-only保存や、schema-composedでないlayoutは、この新gateでも従来通り高コストな全body fetchを一切payしない。文字列直接parse版の`ContainsAdminRuntimeNodeLevelDispatchField(string)` overloadを追加し、`nodes`配列解析前の早期gate地点で使用した。

**実装5: backend test doubleの追従修正。** `NpgsqlUiTopologyRepositoryLayoutPatchValidationTests.cs`の4つのtest double subclass（`AdminRuntimeWiringKindTestRepository`/`AmbiguousWiringKindLookupRepository`/`SchemaComposedTestRepository`/`SchemaJsonLookupForbiddenRepository`）はいずれも抽象`UiTopologyRepository`ではなく具象`NpgsqlUiTopologyRepository`を継承しているため、新規virtual methodをoverrideしないと実DB接続を試みる実装を継承してしまい、63件のtestが`Npgsql.NpgsqlException: Failed to connect`で失敗した——4クラス全てへ適切なoverride（3クラスは`false`固定、`SchemaComposedTestRepository`は自身の`layoutSchemaJson`フィールドに対して実SQL相当のセマンティクスを再現）を追加して解消した。

**実装6: 単一node spoof negative testの新規追加（round41自身のNG axisに従い、companion component-key欠落nodeへ依存しない証明）。** round41が追加した3つのidentity mismatch testは、いずれも同一patch内に`schema_filter_field`（componentKey欠落）を同居させて旧gateを強制発火させる形だったため、round42のNG axis（「旧gateを強制発火させるためだけのdummy sibling nodeをtestへ追加し、それを単一node fixとして提示すること」を明示的に禁止）に従い、それらとは別に、companion nodeを一切含まない文字通りの単一node patchによるspoof/match/tensor-only/DB-round-trip-preservation testを6件新規追加した——`ValidateLayoutPatchAsync_SchemaComposedNode_SingleNodePatch_RawComponentKeyMismatchesCanonical_FailsClosed`ほか。全117件のbackend layout-patch-validation testが通過することを確認した。

**実装7: `runtimeDispatchCircuitKey`ドキュメント/test comment訂正。** `runtimeDispatchCircuitKey(spec, targetRef)`の実装（`targetRef ?? spec.nodeId ?? spec.componentId`、targetRefが最優先）に反して、round41が`projectionShellAdminRuntimeWritePayloadCapture.test.ts`内へ書いたcomment（「別nodeなので別circuitになるはずだった」）が誤りであったことを発見・訂正した——実際には`enum_create_group_name_input`と`enum_search`は実ae200 fixture上で同一targetRef literalを共有しており、round41修正前の暗黙dispatch bugは「無関係circuitへの余計な1回」ではなく「enum_searchの保留中debounceと同一circuit上でrace/cancelしうる」という、より深刻な性質のbugだった。この訂正をcommentへ反映し、`frontend/tests/runtimeComponentFactory.test.ts`へ新規unit test（別nodeId・別componentIdの2 specが同一targetRefを共有する場合、片方の後続dispatchがもう片方の保留中debounce timerを実際にcancelすることを直接証明）を追加した。既存のround39/round40 circuit関連commentは調査の結果、いずれも実装と整合しており訂正不要だった。

**実装8: production DOM proofのTable/Button/Modal範囲への明示化。** round41の第2 DOM test（enum_search/enum_group_filter参加 + 非参加node無dispatch証明）は、既存では「非参加26 node」という古い数字と「dispatch件数が2のまま」という間接的推論のみで非参加を証明していた。実ae200 fixture上の実際の非参加node数（22件）へ数字を訂正し、かつ実際に`enum_table`の行clickと`enum_delete_group_cancel_button`のclickという直接interactionを追加し、それでもdispatch件数が2のまま変化しないことを直接証明する形へ強化した。

**検証**: `dotnet build`/`dotnet test backend/tests/Topolactor.Runtime.Tests`（1612/1612全通過、新規6件含む）、`deno test frontend/tests/`（全体、baseline `b15b82d`との`git stash` A/B比較により、AssertionError件数はbaseline・本round双方とも0件であることを確認——leak sanitizer由来のfailure件数はbaseline自身の2回連続実行間でも変動する既知の非決定的ノイズであり、実regressionではない）。

**本roundで実装していないこと（正直な記載）**: production ae200 fixtureへの新規explicit dispatch participation authoringは、調査の結果不要と判断したため実施していない（row値追跡はdispatch参加と独立した別laneのため）——ただしこれは「調査せず省略した」のではなく、「調査した結果、既存の22非参加nodeのいずれも新たなauthoringを要する実capabilityを保持していないと確認した」という積極的判断である。もし将来、非参加nodeのいずれかに実際に必要な暗黙capabilityが発見された場合は、componentKind分岐ではなく当該nodeへの明示的authoringで対応すべきであり、本round契約を再度狭めるべきではない。

### Governance NG boundary追記（round 42）

- Owner決定の一般契約を、既存test breakageを理由に再度componentKind部分集合へ狭める——本roundは実際にdeno test全体実行で経験的検証を行い（約40件という旧見積りは過大で実際は1件のみ）、契約は狭めずtest側をauthoring形で更新した。
- schema-composed単一node spoofを、DB round-trip最適化を理由に「開いたまま」で許容し続ける——本roundは専用の軽量scalar checkを新設し、通常のtensor-only保存のコストを増やさずにgapを閉じた。
- 単一node spoof証明として、旧gateを強制発火させるためのdummy componentKey欠落sibling nodeをtestへ追加する——round41の既存3testはmulti-node caseとしてそのまま保持し、companion nodeを一切含まない文字通りの単一node testを別途新規追加した。
- `runtimeDispatchCircuitKey`のnode-vs-targetRef優先順位に関する誤ったtest comment/proof主張を、指摘されるまで放置する——実装コードを再確認し、誤りと確定した箇所を訂正し、訂正内容を直接証明する新規unit testを追加した。
- 「dispatch件数が変化しない」という間接的推論のみでTable/Button/Modal非参加を証明したと主張する——実際のTable row click・Modal Cancel button clickという直接interactionを追加して証明を強化した。

### admin-enum subBundle 実装記録（2026-08-15 round 43 — round42自身が「schema-composed単一node spoofをANY componentKey spoofについて閉じた」と主張していたが、実際にはdispatch fieldを持つnodeについてのみ閉じており、dispatch fieldを一切持たないnodeへのforged componentKeyは依然canonical identity照合へ到達しなかった。この残存gapを、business-dispatch情報（wiringKind/componentKind/dispatch field）ではなくcomponentKey自体の存在を根拠とする新gateへ置き換えて解消）

**round43の指示**: current audited headはea2d0bd（round42終了時点）。round42はOwner判断に基づきadmin_runtime Event→business operation dispatch participation一般契約・runtimeDispatchCircuitKey訂正・dispatch-fieldを持つschema-composed single-node componentKey spoofを閉じたが、現`ValidateLayoutPatchAsync`はcanonical schema mapを追加解決する条件を`ContainsAdminRuntimeNodeLevelDispatchField`（dispatchPayloadFromByTrigger/dispatchTargetRefByTrigger存在）へ依存させたままだったため、実schema-tree catalog leafのnodeIdへforged componentKeyを持たせても、dispatch fieldを一切持たないsingle-node patchでは依然canonical identity照合へ到達しない、という指摘。Owner決定: schema-composed nodeIdがcanonical schema-tree catalog leafである場合、そのraw overrideにcomponentKeyが存在するか、dispatch fieldを持つか、どのEventへ参加するかに依存せずcanonical component identityがauthorityになること。dispatchPayloadFromByTrigger/dispatchTargetRefByTrigger/wiringKind/componentKind等のbusiness-dispatch情報の存在へcanonical identity照合を依存させ続けることは明示的にNG指定された。

**実装1: SSOT訂正**。`docs/design/admin-uibuilder-ui-structure-wiring-ssot.yaml`へ新規`owner_decision_2026_08_15_schema_composed_identity_independent_of_dispatch_field_round43`を追加し、round42自身の「Schema-composed single-node spoof closure (round 42)」段落が「ANY raw componentKey spoofを閉じた」と過大に主張していたことを訂正——実際にはdispatch fieldを持つnodeについてのみ閉じていたと明記した。「Disclosed scope limit（round41, CLOSED in round42）」という既存の誤記述も、「CLOSED in round43」へ訂正し、round42自身の主張が不正確だった旨を明記した。`react-schema-topology-seed-translator-ssot.yaml`の対応する鏡像段落も同様に訂正した。

**実装2: `backend/repository/NpgsqlUiTopologyRepository.cs`のgate置換**。新規`ContainsNodeWithComponentKeyPresent(string tensorPatchJson)`——patch内の少なくとも1つのnodeが自身のcomponentKeyを持つかを判定するpredicate——を追加した。`ValidateLayoutPatchAsync`のgateを`mayBeSchemaComposed = ContainsNodeMissingComponentKey(json) || (ContainsAdminRuntimeNodeLevelDispatchField(json) && await LayoutHasSchemaComposedRecordsAsync(...))`から`mayBeSchemaComposed = ContainsNodeMissingComponentKey(json) || (ContainsNodeWithComponentKeyPresent(json) && await LayoutHasSchemaComposedRecordsAsync(...))`へ変更した。`ContainsNodeWithComponentKeyPresent`はcomponentKind/wiringKind/dispatch fieldのようなbusiness-dispatch情報ではなく、identity-mismatch check自身がcanonicalと比較する対象そのもの（componentKey）の存在を見るpredicateであり、round43のNG axis（business-dispatch情報への依存禁止）に抵触しない。node無しのpatch（例: CSS token検証のみのpatch）や、全nodeがcomponentKey欠落のpatch（既存`ContainsNodeMissingComponentKey`gateで既に対応済み）は、この新gateでも軽量scalar checkすら払わない——一方、componentKeyを持つnodeが1つでも存在するpatchは、dispatch fieldの有無に関わらずこの軽量scalar checkを払うようになった。`ContainsAdminRuntimeNodeLevelDispatchField`自体は削除せず、別の呼び出し箇所（wiringKind="admin_runtime"認可check）でそのまま使用を継続している——両者は異なる問いであるため。

**実装3: backend test doubleの追従修正**。gateの変更により、componentKeyを持つnode（wiringKind="admin_runtime"以外の用途も含む、CSS token/structural HTML/runtimeInteractions/dispatchExternalPort/dispatchInstanceOperation等の既存test）が新たに軽量scalar checkへ到達するようになったため、`NpgsqlUiTopologyRepositoryLayoutPatchValidationTests.cs`内でraw `NpgsqlUiTopologyRepository`を直接構築していた23箇所のtest（実DB接続文字列を持つが実DBには到達しない前提だった）が実DB接続エラーで失敗した。新規共有test double `NonSchemaComposedLayoutPatchTestRepository`（`LayoutHasSchemaComposedRecordsAsync`をfalse固定でoverride）を追加し、該当箇所を機械的に置換した。加えて`InstanceCandidateTestRepository`/`InstanceCandidateSourceFailureRepository`の2クラスへも同様のoverrideを追加した。round42が追加した`SchemaComposedRecordsCheckForbiddenRepository`（「dispatch fieldなしでは軽量checkすら呼ばれない」ことを証明するためのthrow-on-call repository）は、その保証自体が本roundで意図的に廃止されるため、`SchemaComposedRecordsCheckCountingRepository`（呼び出し回数を数え、軽量checkは呼ばれるが高コストな全body fetchは呼ばれないことを証明する）へ置き換えた。

**実装4: 単一node・dispatch field完全無しのnegative/positive proof新規追加**。round42が追加した3つの単一node testはいずれもdispatch fieldを伴っていたため、round43のNG axis（「dispatch fieldを持つvariantのみを証明してANY raw componentKey spoofを閉じたと宣言すること」を明示的に禁止）に従い、それらとは別に、dispatchTargetRefByTrigger/dispatchPayloadFromByTriggerを一切持たない文字通りの単一node patchによるspoof/match testを2件新規追加した——`ValidateLayoutPatchAsync_SchemaComposedNode_SingleNodePatch_NoDispatchFieldAtAll_RawComponentKeyMismatchesCanonical_FailsClosed`（forged componentKey、dispatch field無し、単一node、companion node無し→fail close）、および対になるpositive case。dummy componentKey欠落siblingやdummy dispatch-field-carrying siblingは一切追加していない。

**検証**: `dotnet build`/`dotnet test backend/tests/Topolactor.Runtime.Tests`（1614/1614全通過、新規2件含む）。既存sandboxのlocal PostgreSQL 16 clusterを起動し（`service postgresql start`）、`TOPOLACTOR_TEST_DB_CONNECTION`を設定した上で`Topolactor.Integration.Tests`を実DBに対して全件実行——`SchemaComposedLayoutPatchPersistenceIntegrationTests`（2/2）・`AdminEnumHubRelationUiProjectionLiveDbTests`＋`UiTopologyRegistrationContinuityIntegrationTests`（39/39）・全体232件中231件通過（唯一の失敗は本PR以前から存在する無関係な既存failure、PR本文自身の`Test plan`節に記載済みのもの——テーブル名変更に起因、git blameで確認済み）。フロントエンド側は本round未変更のため再実行不要。`agent-ui-local-test`全chain（`run-worktype-tests`→`read-senario-tmp`→`checklist`→`checks`→`summary`）実行、`pass_or_fail: pass`。Policy Judgment Gate: `REQUIRED_RUNTIME_CHANGE`、15問全問PASS。Boundary Identity Checklist: Q1=no（既存boundaryの拡張ではなく既存gate predicateの置換のため）と判定し以降n/a。`bash .agent/tests/check-structure.sh`: PASS。

**本roundで実装していないこと（正直な記載）**: フロントエンド側の変更は本round一切無い——本roundの指摘・NG axisはbackend persistence-boundary（`ValidateLayoutPatchAsync`）に限定されており、round42のfrontend側dispatch participation一般契約自体には触れていない。translator（Python）側にも本メカニズムのmirrorは存在しないことを確認済み（grep検索で該当箇所無し）。

### Governance NG boundary追記（round 43）

- canonical identity照合をdispatchPayloadFromByTrigger/dispatchTargetRefByTrigger/wiringKind/componentKind等のbusiness-dispatch情報の存在へ依存させ続ける——本roundは実際にgateの条件をcomponentKey自体の存在（`ContainsNodeWithComponentKeyPresent`）へ置き換え、business-dispatch情報への依存を完全に除去した。
- dispatch fieldを持つvariantのみをproofして「ANY raw componentKey spoofを閉じた」と宣言する——round42自身がこの誤りを犯していたことを本round自身のSSOT訂正で明記し、dispatch field完全無しの単一node testを別途追加して真の閉鎖を証明した。
- 旧gateを強制発火させるためだけのdummy componentKey欠落またはdispatch-field-carrying sibling nodeをtestへ追加する——新規2testはいずれも文字通りの単一node patchであり、companion nodeを一切含まない。
- DB-access最適化またはtest互換性維持を理由にcanonical authorityへ例外を追加する——test double側の追従修正（23+2箇所）はtest infrastructure側の対応であり、canonical authorityのgate条件自体を緩める形では一切行っていない。
- 本round自身のPolicy Judgment Gate/Boundary Identity Checklistのtmp working copyを削除せず残す——両方とも`checks`実行前に削除し、`check-structure.sh`のPASSを確認した。

---

### credential-management subBundle 実装完了記録（PR #602 latest head `ffb84c06d944459add957b030d1e20b6a8cf24fd`、2026-08-16）

**この節が `credential-management` subBundle の現在状態の正本である。** PR #602はこの記述時点で `main` 未merge（`state: open`, `mergeable_state: clean`, 6 commits）——以下はPR headのproduction evidence（SSOT整合・実装ファイル・live-DB test・全11 CI checks green）に基づくsubBundle単位のcompletion判定であり、main反映自体を主張するものではない。Bundle自体のStatus（`partial`）は残り2 subBundle（`team-dashboard`/`scheduler-settings`）が未着手のため変更していない。

**実装内容（`db/seed_empty.sql` manifest `00000000-0000-0000-0000-000000000092`、本 Bundle 共通工程の1〜6を完了。当該manifestは元々`resolved_existing_substrate`だったため、工程1〜4はUI再構成、工程5〜6は既存canonical substrateへの新規binding証明が中心）:**

- **navigation binding authoring/verification（`target_surface_manifest_readiness`）**: `hub_navigation:create`の実authoring dispatch（`HubNavigationAdmin.tsx`と同一の`hub_navigation:create`、manifest 092自身の既存hub`...a1`をrelatedHubIdに指定）と、その同一relationのresolution chain（`NavigationSequence`解決 → 092の実package/layout/wiring/tensor → scalar Emission）を単一のlive-DB test（`CredentialManagementHubRelationUiProjectionLiveDbTests.cs` `DispatchAsync_HubNavigationCreate_AuthorsRelationTargetingManifest092_ResolutionChainReachesScalarEmission`）で組み合わせて証明——2026-07-22確定の解決基準（本Bundle該当節参照）を満たし、`docs/design/admin-normal-surface-projection-seed-ssot.yaml`側の`subbundle_status.credential-management.navigation_binding_authoring_and_verification`を`resolved`へ更新済み。
- **credentials.users（既存`auth_users`authority再利用、parallel authority非追加）**: create/update/deleteは既存の`auth_users:create/update/delete`（production-registered dispatch-only manifest`...ad004`/`...ad005`/`...ad006`、`/admin/users`の`AdminUsersRoster.tsx`が現に呼ぶのと同一action）へbind——`AdminRuntimeMasterRoster.cs`へ追加的・opt-inな`dryRun`previewのみ追加し（`/admin/users`の既存unconfirmed callerを壊さないため、hard `confirmed`-required gateは意図的に追加していない）、CRUD authority自体は複製していない。`revoke_credential`/`revoke_sessions`は新規dispatch-only manifest（`...ad007`/`...ad008`）経由の薄いwrapperで、既存REST route（`POST /admin/auth/users/{userId}/credential|sessions/revoke`）が呼ぶのと同一の`AuthService.AdminRevokeCredentialAsync`/`AdminRevokeSessionsAsync`を呼ぶのみ。admin-driven password replace/rotateは実装していない（owner NG、`retired_permanent_ng`、pending判断ではなく永続決定）。
- **external_api_credential**: 既存`NpgsqlExternalApiCredentialAdminRepository`によるcreate/read/update/delete（dispatch-only manifest`...cd008`-`...cd011`）に加え、`expires_at`境界filter（`expiresBefore`/`expiresAfter`、strict `<`/`>`、no-expiry行除外）、canonical `physical_table_name`（`ExternalApiCredentialRecordKinds.CanonicalPhysicalTableNames`辞書経由、recordKind文字列結合ではない）によるdiff_log記録を実装・証明。secret-shaped field（`tokenHash`/`encryptedPayload`/`encryptionKeyReference`/`plaintextSecret`/`decryptedPayload`/`tokenResponse`）はいかなるsearch/get/create/update responseにも一切現れないことをlive-DB testで証明。
- **external_instance_credential（`external_instance_projection_columns`）**: 新規`NpgsqlExternalInstanceCredentialAdminRepository`（`topology.db_instance_port`/`topology.runtime_instance_port`のUNION ALL、`topology.external_credential_vault`をreference_keyでLEFT JOINしてexpires_at/versionを取得——両port tableはこれらのcolumnを自身で持たない）でcreate/read/update/delete（dispatch-only manifest`...cd012`-`...cd015`）を実装。create/updateは既存vault行の`reference_key`参照のみを受け付け、未知のreference_keyは`dryRun`・confirmed write双方でfail-close（`EXTERNAL_INSTANCE_CREDENTIAL_REFERENCE_KEY_NOT_FOUND`）——新規credential table/planeは一切追加していない。`docs/design/admin-normal-surface-projection-seed-ssot.yaml`側の`subbundle_status.credential-management.projection_seed_binding_authoring_surface_operation_wiring_and_proof`を`resolved`へ更新済み。
- **unified `credential_management:search`（category-routed）**: SSOTの`seed_contract.component_tree`が`credential_search`/`credential_category_filter`/`credential_result_list`をcredentials surface全体で共有すると定義していることを踏まえ、manifest 092自身が持てる唯一の`dispatcher_mapping`枠（`ManifestTopologyValidator`が1 manifest 1 dispatcher_mappingを強制するため）を、`category`payloadで3 category（`users`/`external_api_credential`/`external_instance_credential`）を切り替える単一actionへ統合。admin-enumの`enum_table`/`list_groups`が既に確立していた同一の同一manifest内Emission-adoption機構（`ProjectionShell.tsx` `handleRuntimeDispatchResult`の`expectedManifestId===adoptedManifestId`分岐）を再利用し、3 categoryすべてが同じ生きたresult-list bindingを得る——category routingは「どのrepositoryを呼ぶか」という構造的selectorであり、provider_kind/required_by_bundle等の値によるbusiness-logic分岐ではない。
- **credential-owned category hierarchyのSSOT整合（構造修正）**: 旧pre-SSOTな`user_auth`/`external`/`instance_settings` category rootを設計authorityとして温存したまま新sectionを増築していた状態を是正——(1) shared search/filter/result-list block（`credential_search_section`）を独立4番目categoryとして`topology_ui_category`登録していた誤り（SSOT上は特定categoryではなくcredentials surface全体で共有される責務）を修正し、projection root直下の通常sectionへ再配置。(2) 内容が完全にcredential-management固有である2 root categoryをSSOT自身のcanonical identifierへrename: `user_auth`→`users`、`external`→`external_api_credential`（record自身のkey/categoryKey/labelのみ、`user_auth_section`等の子section keyは維持——`check-instance-port-substrate.sh`が要求するリテラル部分文字列を保つため）。`instance_settings`はcredential-management固有ではない、`docs/design/instance-port-substrate-ssot.yaml`が定義する広い責務（JSON template tooling、instance operation binding/approval）も抱えているため、rename対象から意図的に除外した（rename するとそれらの非credential兄弟要素をcredential所有であるかのように誤表示するため）。
- **secret non-projection / write confirmation contract**: 全category・全operationにわたり`preview_account_or_credential_delta_without_secret → validate_policy_and_transaction_boundary → explicit_confirm → write → diff_log_without_secret`（SSOT `mutation_confirmation_contract`）を遵守。`dryRun`は常に実repository-backed validationを経由し（bare echoではない）、`confirmed`必須gateはcredential-management固有actionすべてに存在する（`/admin/users`の既存unconfirmed callerとの互換のためauth_users:create/update/deleteのみ例外——上記参照）。

**既知の残gap・判断（fabricateせず正直に記録）:**
- SSOTの`seed_contract.component_tree`は`credential_form`/`credential_confirm_button`を概念上1ノードとして記述しているが、production実装は3 category個別のform/confirm button群（`credentials_users_account_section`/`external_api_credential_crud_section`/`external_instance_credential_crud_section`）として実現している——これは意図的な判断であり、未着手のgapではない。理由: (1) 3 categoryのfield集合は構造的に異なり（users: username/status/role/suspendedFrom等、external_api_credential: providerKind/tokenKind/secret等、external_instance_credential: instanceAuthorityKey/referenceKey等）、動的field-set切替を行う汎用mechanismは本Bundle内に存在しない（category-conditional field visibilityのような新規汎用mechanismを発明することはRound 6のNG boundaryで明示的に対象外とした）。(2) admin-enumの`enum_dictionary`subBundle自身が、単一の概念的責務（enum management）をgroup-level/item-level別々のCRUD flowとして実現している既存承認済み precedent と整合する解釈。(3) `credential_search`/`credential_category_filter`/`credential_result_list`——実際に3 category間でfield差異が無い（またはdata-driven columnで吸収できる）3ノード——はSSOT通り文字通り単一・共有として実装済み。この判断は次にこのsubBundleを監査するAgent/ownerが再評価すべき対象として明記する。
- `instance_settings` root categoryのJSON template download/import/validate/preview/apply/approve admin authoring UIは、`instance-port-substrate-ssot.yaml`自身の`completion_gate`が別future bundle（`instance_settings_admin_authoring_ui_pending`）と明示しているため、本subBundleのscope外のまま——今回追加した`external_instance_credential_crud_section`のlist/edit/search配線とは別軸。

**検証:** `dotnet build topolactor.sln`（0 errors）、`dotnet test backend/tests/Topolactor.Runtime.Tests`（1673/1673 passed）、`dotnet test backend/tests/Topolactor.Integration.Tests`（実PostgreSQL、245/246 passed——唯一の失敗は本PR以前から存在する無関係な既存failure`UiTopologyLayoutPatchRollbackIntegrationTests`、`ui_layout_registry`→`topology.components_layout_design`のtable rename起因、git blameで確認済み）、`deno test --allow-all frontend/tests/`（2083/2083 passed）、credential-management category rename structural guard check（PASS——category rename後もrequired literal substring termsが子section key経由で維持されることを確認）、`bash .agent/tests/check-structure.sh`（PASS）、`.agent/tools/agent-ui-local-test`全chain（`run-worktype-tests`→`read-senario-tmp`→`checklist`→`checks`→`summary`、各round実施、`pass_or_fail: "pass"`）、PR #602 全11 CI checks green（head `ffb84c0`時点）。

**次にこの subBundle を触る Agent への引き継ぎ:**
- PR #602をmainへmergeする際は、他4 subBundleの実装状態を変更しないこと（`admin-dashboard`/`admin-enum`は本PRで一切touchしていない——変更ファイルは`db/seed_empty.sql`のmanifest 092領域、`backend/runtime/AdminRuntime*.cs`のcredential-management専用ファイル、`backend/repository/NpgsqlExternalInstanceCredentialAdminRepository.cs`、対応するtest/fixtureのみ）。
- 上記「既知の残gap・判断」1点目（`credential_form`/`credential_confirm_button`のper-category実現 vs SSOT文字通りの単一node解釈）は、次の監査でこの判断を追認するか、あるいは汎用field-set-driven form mechanismを新設するBundleとして別途起票するかの再評価対象として残す——現時点でこれを未完了gapとして`credential-management`subBundleのcompletion判定をブロックする理由にはしていない。
- 残り2 subBundle（`team-dashboard`/`scheduler-settings`）はこの完了によって一切影響を受けない——それぞれ個別の`unresolved_before_seed` design_blocking（本Bundle該当節参照）を解消してから着手する。

### team-dashboard subBundle 実装記録（2026-08-16、design_change→implementation_change、PR #604継続）

**開始時status:** `docs/design/admin-normal-surface-projection-seed-ssot.yaml` `design_blocking.normal_dashboard_authoring_runtime_adapter: reopened_2026_07_15_gate0_audit`（`md_translation_authoring_surface.authoring`がruntimeConnected=falseのまま、topology seedは未生成）。`/admin/team-dashboard`はTeam Markdown Authoringを直接mountするhardcoded admin-only routeのまま。PR #603（同一課題への先行round、`idOrHubId`/colon-action dispatch fix 2件のみ）はOwnerによりclose（未merge）——本roundはOwner確定要件に基づき再設計して仕切り直した。

**design_change内容（Owner確定要件との不整合是正）:** Owner確定要件は「shared dashboard projection、Admin read+edit/Normal read-only、専用Textarea、DB saved-view persistence非必須のlightweight md/txt source、preset/UIBuilder出力をManifestが統合」。旧SSOT契約（`normal.surfaces.dashboard`）はTeam Markdown saved-view DB・`md_translation_authoring_surface.authoring`・`team_markdown:saved_view:update`をteam-dashboardの正本authorityとして要求していたが、これは(1) heavy apparatus（template/binding_json/completed_preset_seed_json/card_metadata等のバージョン管理lineageを要求、単一shared noteには過剰）、(2) 一度もruntime adapterが解決されていない（`inputer_runtime_adapter_contract.status`は一貫して`reopened`/`Not done`）という2点でOwner要件と整合しないと判断し、**team-dashboardの完了authorityから退役**（`docs/design/admin-normal-surface-projection-seed-ssot.yaml` `surface_axes.normal.surfaces.dashboard.superseding_authority_note`、および`design_blocking.normal_dashboard_authoring_runtime_adapter`/`normal_dashboard_write_operation_binding`の`team-dashboard`欄を`retired_superseded_2026_08_16`へ更新——historical recordとしては残置、削除はしていない）。**Team Markdown Dashboard Saved View機能自体（`docs/design/team-markdown-dashboard-saved-view-ssot.yaml`、DB、`TeamMarkdownAuthoring`コンポーネント、`team_markdown:*`backend action）は一切削除・回帰させていない**——admin routeを`/admin/team-dashboard`から`/admin/team-markdown`へ移設したのみ（`frontend/routes/admin/team-markdown/index.tsx`新設、同一`TeamMarkdownAuthoring`を同一`AdminAuthGate`でmount）。

新canonical contractとして`surface_axes.admin.surfaces.team_dashboard`（新設）と`surface_axes.normal.surfaces.dashboard.team_dashboard_canonical_shared_contract`（新設）を追加——両方とも`topology.team_dashboard_note`（新設、単一canonical row、lightweight物理table）を共有data authorityとして参照する。

**「lightweight md/txt source」の具体設計判断（唯一のdesign gap、Owner確認事項）:** 既存generic file/runtime substrateを調査した結果、`docs/design/runtime-bundle-file-storage-ssot.yaml`はexport job/checksum/signed URL前提のバッチ指向file substrateであり、対話的なTextarea編集フローに適合しない。文字通りのファイルシステムmd/txt読み書きに対応する既存generic interactive read/write authorityは存在しないと判断し、「lightweight」を「軽量物理table + 既存generic read/write substrate」として実現した（専用dashboard file API/actionの推測新設は行っていない）。書き込みauthorityは`content_bundle:*`（draft/hub/state中心のcontent-authoring domain modelで、単一shared noteには不適合と判明——`docs/design/admin-normal-surface-projection-seed-ssot.yaml` `crud_preset_physical_reference_assessment.not_reused_as_authority.content_bundle_operation_refs_must_not_be_copied`が既にcontent_bundle:* target_refをauthorityとして転用禁止と明記）ではなく、`team_markdown`/`enum_dictionary`/`credential_management`と同型の薄いdomain専用action pair（`team_dashboard:get`/`team_dashboard:update`、`backend/runtime/AdminRuntime.TeamDashboard.cs`、既存`AdminRuntime.ExecuteDataAsync`switchへの追加2 caseのみ、新runtime laneではない）を採用した。

**実装内容:**
- `db/team_dashboard_tables.sql`（新設）: `topology.team_dashboard_note`（note_id/title/body_markdown/updated_at、単一canonical row）。
- `backend/repository/TeamDashboardRepository.cs`/`NpgsqlTeamDashboardRepository.cs`（新設）: Get/Update 2メソッドのみ。
- `backend/runtime/AdminRuntime.TeamDashboard.cs`（新設）: `team_dashboard:get`（open）/`team_dashboard:update`（`vector.AuthenticatedRole=="admin"`のin-method check、`AdminRuntime.TeamMarkdown.cs`の既存idiomを再利用、新mechanismではない）。`AdminRuntime.cs`/`Program.cs`へ配線。
- `backend/Program.cs`: `GET /team-dashboard/note`（新設、any authenticated JWT、admin-gate無し）——既存`/team-markdown/*` viewer read endpoints（PR #589）と全く同一の設計判断・同一コメントパターンを踏襲（後述のcapability boundary発見を参照）。
- `db/seed_empty.sql`: team-dashboard向け2 manifest（`00000000-0000-0000-0000-0000000dd010` admin編集用/`...dd020` normal閲覧専用）+各々のpackage/layout/wiring/tensor（`...dd01x`/`...dd02x`）+ hub + `hubs.topology_manifests` + `topology.physical_table_manifest_bindings`。Admin tensorは`team_dashboard_admin_viewer`（md_viewer.projection、bare-markdown mode）+`team_dashboard_admin_body`（textarea.template、propBindings.value=emission.data.bodyMarkdown）+`team_dashboard_admin_save_button`（admin_runtime、dispatchTargetRefByTrigger→`manifest:dd010:team_dashboard:update`、payloadFrom node:team_dashboard_admin_body.value）の3 node——`physical_details_inline_editor_md_generator_preset`（PR #604）が証明済みのgeneric composition（textarea.template value carrier + md_viewer.projection bare-markdown + node:<id>.value payloadFrom）をそのまま実tensorへ適用したもの、新規Markdown component/editorは作っていない。Normal tensorは`team_dashboard_normal_viewer`のみ（editor/save nodeは一切存在しない——hidden化ではなく構造的不在）。
- `db/ui_component_registry_preset_catalog_bootstrap.sql`: `textarea.template`/`md_viewer.projection`を`topology.ui_component_registry`へ新規登録（`table.primitive`の既存precedentと同型の「既にcodedだが未promoteのcomponentをpromoteする」action——両componentは#604のdraft preset compile_snapshotでは使われていたがREAL/active tensorで使われるのは本roundが初めてであり、`ui_component_registry`未登録のままだった実装gapを解消）。
- `frontend/runtime/propBindingResolver.ts`/`backend/runtime/StructureMapResolver.cs`/両者のsync test/`docs/design/admin-console-workflow-ssot.yaml`: `data_display/md_viewer: [markdown]`をCOMPONENT_ARRAY_PROP_CAPABILITIESへ追加（scalar、#604 round2で追加した`form_input/textarea_template: [value]`と同型——real tensorでmd_viewer.projectionのpropBindingsが検証される最初のケースだったため未登録が発覚、同じ理由）。
- `frontend/routes/admin/team-dashboard/index.tsx`: TeamMarkdownAuthoringから、`/admin/enums`と同型のProjectionShell thin wrapper（manifestId=dd010）へ差し替え。
- `frontend/routes/admin/team-markdown/index.tsx`（新設）: 旧`/admin/team-dashboard`の内容をそのまま移設。
- `frontend/routes/dashboard/index.tsx`（新設）+`frontend/islands/TeamDashboardViewer.tsx`（新設）: Normal閲覧surface。**アーキテクチャ上の重要な発見**によりProjectionShell/ManifestDispatcher経由ではなく`GET /team-dashboard/note`（plain REST、`/team-markdown/*` viewer read endpointsと同一パターン）を直接fetchし、既存共有safe renderer（`frontend/lib/markdownRenderer.ts`、新規実装なし）で描画する構成とした。

**アーキテクチャ上の重要な発見（Owner共有事項）:** `backend/runtime/ManifestDispatcher.cs` `ResolveRequiredRole`は、`runtime_mapping.runtime_destination=admin_runtime`を持つ**あらゆる**manifestに対し`required_role=admin`を無条件で推論し、per-action/per-manifest単位で「readは open、write のみ admin」という部分的opt-outをする手段が存在しない（`capability_requirement`による明示的override機構はあるが、required_roleを空にする=推論を無効化する、という指定はコード上受理されない）。この制約は**本roundが新たに作ったものではなく、`backend/Program.cs`の`/team-markdown/*` viewer read endpoints自身のコメントが既に同じ理由を明記して回避している**（"the admin_runtime dispatch route... infers required_role=admin uniformly... so Normal-role read access is served from this separate boundary instead of by weakening the dispatch-side gate"、PR #589、2026-07-15、本round着手の1か月以上前）——つまりこれは**このrepository全体で"seed駆動のNormal-role open read screen"を実現する既存汎用authorityが未だ存在しない**という、以前から認識されていた（が明文化されていなかった）system-wideな設計制約であり、team-dashboard固有の欠陥ではない。本roundはこの制約を正しく認識した上で、Team Markdown viewerと全く同一の確立済み回避パターン（plain REST bypass）を採用した——新しいbypass機構を発明していない。真にseed駆動（ProjectionShell/Emission/LayoutNodes経由）なNormal read screenを実現するには、`ManifestDispatcher`へper-action capability override機構（またはNormal専用runtime destination）を追加するdesign_changeが必要——これは本subBundleのscopeを超える、Owner判断が必要な具体的・限定的な残課題として明示する（"1点だけ"の設計gapとして返す、という実装前契約の指示通り）。`/dashboard`のNormal viewer自体は正常に機能する（plain REST経由）——機能しないのは「同一row/同一UIコンポジションをManifestDispatcher経由でも到達可能にする」という上位互換性のみ。

**検証:**
- ローカルで実PostgreSQL 16を起動し、`db/init.sql`由来の完全なbootstrap chain（`db/team_dashboard_tables.sql`含む全ファイル）をfresh DBへ適用——クリーン成功。
- `bash .agent/tests/check-db-schema.sh` / `bash .agent/tests/check-bootstrap-validation.sh`: fresh DBに対しPASS（`db/team_dashboard_tables.sql`を両script実行チェーンへ追加、`.github/workflows/backend-tests.yml`のfile listへも追加）。
- `dotnet build`（0 errors）、`dotnet test backend/tests/Topolactor.Runtime.Tests`（1683/1683 passed、新規`AdminRuntimeTeamDashboardTests.cs` 5件含む）。
- `dotnet test backend/tests/Topolactor.Integration.Tests`（実PostgreSQL、新規`TeamDashboardHubRelationUiProjectionLiveDbTests.cs` 5/5 passed——`hub_navigation:create`実authoring dispatch経由でのrelation authoring→resolution chain→scalar Emission到達の組み合わせ証明、admin tensorのeditor node存在/normal tensorのeditor node不在のDOM構造証明、同一物理rowのread-after-write一貫性証明、`team_dashboard:update`のAuthenticatedRole gate fail-close証明。既知の無関係な既存failure`UiTopologyLayoutPatchRollbackIntegrationTests`（`ui_layout_registry`テーブル名不一致、本diff対象外ファイル、credential-management round既に記録済みの同一既存failure）以外は全PASS）。新test class名を`.agent/tests/check-backend-tests.sh`のfilterへ追加。
- `deno test -A frontend/tests/`（2106/2106 passed、既存test regression無し）。
- `deno check`（新規frontend file 4件個別実行でPASS——既存`check-frontend-types.sh`の固定entry pointリストには route file が含まれないため個別確認）、`deno run -A frontend/dev.ts build`（33 routes/22 islands、成功、`frontend/fresh.gen.ts`/`frontend/_fresh/*`を再生成・commit）。
- `bash .agent/tests/check-structure.sh` / `bash .agent/tests/check-admin-normal-surface-projection-seed-ssot.sh`: PASS（既存governance check文字列はSSOTのadditive-onlyな編集方針により全て維持）。

**次にこの subBundle を触る Agent への引き継ぎ（2026-08-16時点、下記2026-08-17追記により解消済み）:**
- ~~残課題は「アーキテクチャ上の重要な発見」節記載の1点（Normal-role open readをManifestDispatcher経由で実現するための、per-action capability override機構またはNormal専用runtime destinationの新設）のみ~~ → 2026-08-17 roundで解消。下記追記を参照。
- 真の end-user reachability（既存の到達可能な画面からteam-dashboardへの実際のnavigation link）はhub_relations経由では未配線——ただし本subBundleのdesign_blocking完了基準（`navigation_binding_resolution_criterion`）自体は「`hub_navigation:create`実authoring dispatchでこのmanifestをrelated_hub_idとして到達させ、resolution chainがこのmanifestの実seed rowsへ到達することを証明する」であり、synthetic sourceからの証明で満たされる（`CredentialManagementHubRelationUiProjectionLiveDbTests.cs`/`AdminEnumHubRelationUiProjectionLiveDbTests.cs`と同じ基準）。実際のnavigation menu配置はOwner判断待ちの別課題（`self_account_capability`の既存の同種未解決gapと同じ扱い）——**この点のみ、本subBundleの残課題として引き続き残る**。
- `scheduler-settings`（親Bundleの最後のsubBundle）は本roundにより一切着手していない。

#### 2026-08-17追記: Normal ManifestDispatcher production reachability解消（PR #604継続、design_change→implementation_change）

**契機:** 2026-08-16記載の「アーキテクチャ上の重要な発見」（`ManifestDispatcher.ResolveRequiredRole`が`runtime_mapping.runtime_destination=admin_runtime`を持つ**あらゆる**manifestに対しper-action override無しで`required_role=admin`を一律推論する制約）についてOwnerから明示的なfollow-up指示があり、`/team-dashboard/note`のplain REST bypassをteam-dashboard Normal完了のevidenceとして採用することを明示的に禁止した上で、この制約自体をgenericに解消するよう指示された。

**worktype:** `design_change`（`ManifestDispatcher`のcapability/role-inference authorityをSSOT・production runtime・既存testから再監査し、Normal readをgeneric dispatch上で表現できるcanonical contractを確定）→ 既存authorityが一意にdesignを決定したためsame roundでimplementation_changeへ連続。

**design_change内容（2件、いずれもteam-dashboard固有ではなく`ManifestDispatcher`の生成的拡張）:**

1. **layer/action-scoped capability_requirement**（`docs/design/auth-db-session-credential-ssot.yaml` `manifest_capability_requirement.layer_action_scoped_override`、新設）: 既存`capability_requirement`topology entryに任意で`layer`/`action`を追加可能にし、両方が現在のdispatch対象と一致する場合にのみ、manifest全体のunscoped explicit entry・`runtime_destination`推論の両方より優先される。`required_role`省略時は「この1 actionのみ明示的にopen」を意味し、`required_role: none`等のsentinel値は一切導入していない（既存の「省略=無指定」semanticsをそのまま流用）。優先順位: scoped entry > unscoped explicit entry > admin_runtime推論 > open。scoped entryが存在しないmanifestは本追加前と完全に同一の挙動——後方互換。実装は`backend/runtime/ManifestDispatcher.cs` `ResolveRequiredRole`/`ValidateCapabilityRequirement`のシグネチャに`layer`/`action`のoptional paramを追加し、2箇所の呼び出し元（メインのcapability gate、`IsBareManifestNavigationReadTargetRefAsync`内の独立check）双方へ`request.Layer`/`request.Action`を通す形で実装（`HubNavigationResolver.cs`の呼び出しは意図的に`layer`/`action`省略のまま——リンク到達可否は特定actionを持たないため）。
2. **default_screen_read override**（`docs/design/runtime-orchestration-ssot.yaml` `ui_projection_render_reachability_contract.default_screen_read_override`、新設）: 実装中に発見した**副次的だが必須の追加gap**——`admin_runtime`にルーティングされるmanifestの構造的`projection_entry`/`screen_list:Search` dispatchは、対応する`ADMIN_OPERATION_NOT_FOUND`を`admin_runtime_structural_read_fallback`が空Data成功へ握りつぶす既存挙動により、**役割gateを解消しても実データが一切表示されない**（`screen_data_shape`/`ScreenDataShapeQueryRuntime`は`topology_transform_runtime`専用機構であり`admin_runtime`は一切到達しない——実機で`admin.enum.management.projection`〔ae200、既存実装済み・稼働中のsurface〕に対しても同一構造dispatchを検証し、`emission.Data`が`null`のままであることを確認した。ae200自身はsearch/filter駆動UXのため実害が無いが、team-dashboardのように「初回表示から実データが必要」なsurfaceでは致命的）。dispatcher_mapping entryに任意で`default_screen_read: true`を宣言すると、この空成功合成の代わりにその宣言済み(layer, action)へ実際に再dispatchし、その結果（Data含む）をそのまま返す——宣言された(layer, action)自身のcapability_requirementは独立して再検証される（外側の構造的axesが通っても宣言actionが自動的に認可されるわけではない）。宣言していないmanifest（例: ae200）は本追加前と完全に同一の挙動——後方互換。実装は`backend/repository/ManifestRepository.cs` `DispatcherMappingAxisAuthority.FindDeclaredDefaultScreenRead`（新設helper、`IsDeclaredIdentitySelectorRead`と同型のidiom）+ `ManifestDispatcher.cs`の`admin_runtime_structural_read_fallback`ブロック拡張。

いずれも実装がSSOTに先行しないよう、SSOT本文（両ファイル）を先に更新してからコード変更に着手した。

**team-dashboard自身への適用（実装内容）:**
- `db/seed_empty.sql`: dd010（admin manifest）の`team_dashboard:get`側dispatcher_mappingへ`default_screen_read: true`を追加（実データ表示、role gateは変更せず既存のadmin一律推論のまま——admin manifestはget/update双方とも引き続きadmin限定）。dd020（normal manifest）の`team_dashboard:get`側dispatcher_mappingへ同じく`default_screen_read: true`を追加し、加えて`capability_requirement`をscoped-openで2件追加（`(screen_list, Search)`——ProjectionShellのデフォルトentryが実際に送るaxes、`(team_dashboard, get)`——default_screen_read override自身が内部的に再dispatchする実action）。
- `frontend/routes/dashboard/index.tsx`: `TeamDashboardViewer`直接fetch（plain REST bypass）から、`/admin/team-dashboard`/`/admin/enums`と同型の`ProjectionShell manifestId=dd020`薄wrapperへ差し替え。
- `frontend/islands/TeamDashboardViewer.tsx`（削除）+ `backend/Program.cs`の`GET /team-dashboard/note`エンドポイント（削除）: 他に利用箇所が無いことを確認した上で完全削除——REST bypassとcanonical経路の2重authorityを残さない方針（`TeamMarkdownAuthoring`用の`/team-markdown/*`系エンドポイントは今回のスコープ外であり無変更）。
- `db/seed_empty.sql`のこの2箇所以外（shared table・admin/normal 2 manifest・hub relation・component composition・Round 1 preset完了）は一切退行させていない。

**test/proof:**
- `backend/tests/Topolactor.Runtime.Tests/ManifestDispatcherLayerActionScopedCapabilityTests.cs`（新設、8 test、team-dashboard非依存の合成manifestのみで両mechanismを単体証明——scoped-open entryが対象(layer,action)のみを開放し同一manifestの他actionを開放しないこと、`ResolveRequiredRole`のpriority順、default_screen_read宣言時/非宣言時の挙動差、宣言先actionの独立capability check）。
- `backend/tests/Topolactor.Integration.Tests/TeamDashboardHubRelationUiProjectionLiveDbTests.cs`（改訂）: `DispatchAsync_NormalManifest_ProjectionEntry_AsNormalRole_ReturnsReadOnlyViewerWithRealData`（旧`Role: "admin"`workaroundを廃止し、実際に`Role: "normal"`で`/dashboard`が送るのと同一のtarget_ref/axesを実PostgreSQL相手にdispatchし、editor/save node不在の構造証明に加え`emission.Data.bodyMarkdown`が実際の物理row内容であることまで証明）、新設`DispatchAsync_AdminManifest_ProjectionEntry_AsNormalRole_StillFailsClosedWithAuthCapabilityDenied`（Admin manifestはNormal roleに対し引き続きfail-close）、`DispatchAsync_TeamDashboardUpdate_ViaAdminManifest_IsVisibleThroughNormalManifestsOwnRead`（改訂——読み戻しをadmin manifestの直接action dispatchから、Normal manifestの`Role: "normal"`構造dispatchへ変更し、Admin書き込み→Normal読み取りの同一row一貫性を実production経路で証明）。既存の`DispatchAsync_TeamDashboardUpdate_WithoutAdminAuthenticatedRole_FailsCloseWithAuthCapabilityDenied`/`TeamDashboardManifests_AreRegisteredInTopologyManifestsAndPhysicalTableBindings`は無変更。
- 実PostgreSQL 16（fresh DB、`db/init.sql`全chain適用）に対し全6 live-DB test PASS、`dotnet test backend/tests/Topolactor.Runtime.Tests`（1691/1691 passed）、`dotnet test backend/tests/Topolactor.Integration.Tests`（259/260、既知の無関係な既存failure`UiTopologyLayoutPatchRollbackIntegrationTests`のみ、round45時点から変化なし）、`deno test -A frontend/tests/`（2106/2106 passed）、`deno check`（変更frontend route双方PASS）、`deno run -A frontend/dev.ts build`（33 routes/21 islands——`TeamDashboardViewer`island削除により22→21、成功、`frontend/fresh.gen.ts`/`frontend/_fresh/*`再生成・commit）、`bash .agent/tests/check-db-schema.sh`/`check-bootstrap-validation.sh`/`check-structure.sh`/`check-frontend-types.sh`/`check-admin-normal-surface-projection-seed-ssot.sh`いずれもPASS。

**監査結果（実装後監査、指示された順序で実施）:**
- owning SSOT（`auth-db-session-credential-ssot.yaml`/`runtime-orchestration-ssot.yaml`）→ runtime capability authority（`ManifestDispatcher.ResolveRequiredRole`）→ `ManifestDispatcher`本体 → Normal production route（`/dashboard`）→ Emission → rendered projection（`emission.Data`実データ）→ test/proof → todo evidence、の順で監査を完了——途中で打ち切らず全経路を確認した。
- team-dashboard固有のexceptionもREST bypassもcanonical完了経路として採用していない——`ManifestDispatcher.cs`本体の変更は完全にgeneric（layer/action・boolean flagのみで、manifest id/route/action名でのif分岐は一切無い）。`GET /team-dashboard/note`は削除済み。
- Normal読み取りはadmin role要求を一切課していない（`DispatchAsync_AdminManifest_ProjectionEntry_AsNormalRole_StillFailsClosedWithAuthCapabilityDenied`がAdmin manifest側の引き続きの admin-gate を証明する一方、Normal manifest側は`AUTH_CAPABILITY_DENIED`を一切返さないことを確認済み）。Admin mutation（`team_dashboard:update`）は既存通りfail-closeのまま（無変更）。
- Normal manifestは実際にproduction display-lane authorityとして機能している——`/dashboard`ルートが実際にこのmanifestをProjectionShell経由でdispatchし、同一構造dispatchが実データを返すことをlive-DB testで確認済み（seed/proof artifactに留まらない）。
- 共有data identity（`topology.team_dashboard_note`単一row）・Admin/Normal構造分離（Normal tensorにeditor/saveノードが一切存在しない）・hub relation authoring/resolution proof（`hub_navigation:create`実dispatch経由）・Round 1 preset完了（`textarea.template`/`md_viewer.projection`composition、safe Markdown renderer）のいずれにも回帰は無い。

**次にこの subBundle を触る Agent への引き継ぎ（2026-08-17時点、下記追記により残課題は解消）:**
- ~~team-dashboard自体の実装完了条件（shared table・Admin/Normal manifest・hub relation proof・capability分離・canonical Normal runtime到達性）はすべて満たされた。残るのは上記「真の end-user reachability」の1点（既存画面からのnavigation link未配線、Owner判断待ち、`self_account_capability`と同種の既知gap）のみ。~~ → 下記追記で解消。
- `scheduler-settings`（親Bundleの最後のsubBundle）は本roundにより一切着手していない。

#### 2026-08-17追記（2周目）: 真の end-user reachability を audit → Owner指示によりProjectionShell固定ホームリンクで解消

**audit結果:** worktype `audit` で、design_blocking `navigation_binding_authoring_and_verification`（既にresolved、synthetic-source proofで充足済み——credential-management/admin-enumと同一の既存established pattern）と、それとは別枠の「real end-user cross-navigation from an existing Normal-reachable entry point」が未解決である点を再確認した。後者について、実在するNormal-reachableな既存manifestを全数調査した結果:
- `surface_axes.normal.surfaces`はdashboard（team-dashboard自身）とself_account_capability（backend-only、frontend component自体が存在しない）の2件のみ。
- サイトのcanonical_default_entry（トップページ`/`が解決するmanifest 092、credential-management）は`admin_runtime`でscoped-open capability_requirementを持たず、実PostgreSQLに対しrole=normalでdispatchした結果`AUTH_CAPABILITY_DENIED`を確認した——トップページも一般ユーザーには実質使えない。
- manifest 091（auth.user boundary）は`runtime_mapping`/`ui_projection`を一切持たず、frontend surfaceではない。

結論として、team-dashboard自身（dd020）以外に、Normalロールが実際にdispatchできる既存manifestは本番に一件も存在しないことを確認した。hub_relationsのfabrication・raw SQL insert・direct routeでの代替は禁止事項のため実装せず、この1点をOwnerへ質問として返した。

**Owner指示:** 「ダッシュボードなんだからどんなところからでもホームに戻れないとアカンでしょ」との方針指示を受け、hub_relations経由のcross-navigationではなく、`frontend/islands/ProjectionShell.tsx`自体に固定の「ホーム」リンク（`/dashboard`へ、ヘッダ相当のnav内、既存hub_relation navの隣）を配置する方針で解消することとした。

**実装内容:**
- `frontend/islands/ProjectionShell.tsx`: 既存の`<nav data-projection-hub-navigation>`（hub_relations由来のNavigationSequenceリンクを表示するnav）を、`hubNavigationLinks.length > 0`の条件付きレンダリングから常時レンダリングへ変更し、先頭に`href="/dashboard"`の固定「ホーム」リンク（`data-projection-home-link`）を追加した。ProjectionShellがmountされる**すべての**画面（`/`、`/admin/*`、`/dashboard`含む）に一律で表示される、manifest固有ではないgenericな仕組み——hub_relationsをfabricationしたわけでも、team-dashboard固有のif分岐を追加したわけでもない。
- `frontend/tests/projectionShellHubNavigationRenderProof.test.ts`: 「navigationSequenceが無い場合はnav自体が一切描画されない」という旧・Owner確認済み仕様を検証していた既存testを、「navigationSequenceが無くても固定ホームリンクは常に描画され、hub_relations由来のリンクだけが存在しない」という新仕様の検証へ更新（該当testのdoc commentも含め全面差し替え）。既存の1件目のtest（navigationSequenceありのケース）にも固定ホームリンクの存在・href検証を追加。
- `docs/design/admin-normal-surface-projection-seed-ssot.yaml`: `surface_axes.admin.surfaces.team_dashboard.hub_relation_navigation_binding_ref`・`surface_axes.normal.surfaces.dashboard.team_dashboard_canonical_shared_contract.hub_relation_navigation_binding_ref`・`design_blocking.target_surface_manifest_readiness.subbundle_status.team-dashboard.navigation_binding_evidence_ref`の3箇所を、上記audit結果とOwner指示による解消内容で更新（`owner-decision-pending`という古い記述を、実際に何がどう解消されたかの記述に置き換え。hub_relations経由の解消ではないことを明示し、将来別のmanifestが真にNormal-reachableになった場合はhub_relations経由のcross-navigationも別途可能である旨は残した）。

**検証:** `deno test -A frontend/tests/`（2106/2106 passed）、`deno check`（対象ファイルPASS）、`deno run -A frontend/dev.ts build`（33 routes/21 islands、成功）、`bash .agent/tests/check-frontend-types.sh`/`check-admin-normal-surface-projection-seed-ssot.sh`（PASS）。

**判定:** team-dashboard subBundleの完了条件（shared table・Admin/Normal manifest・hub relation authoring/resolution proof・capability分離・canonical Normal runtime到達性・real end-user reachability）はすべて満たされた。`scheduler-settings`は本roundでも一切着手していない。

#### 2026-08-17追記（3周目）: worktype `audit`——UIBuilder authoring lineage迂回・canonical mutation pipeline迂回・Manifest identity route hardcode・DB/Runtime/Admin workflow SSOT driftの解消

**契機:** Ownerから、既済み（Markdown rendering・Admin/Normal Manifest/hub topology・capability分離・canonical read path）を維持しながら、監査で判明した4点のgapを解消してteam-dashboard subBundleを完全Implementedへ到達させるよう指示があった。

**worktype:** `audit`（PR #604最新headからSSOT→wiring→test/proof→implementationのchainを再構築、既存authorityがdesignを一意に決定するためsame roundでdesign_change/implementation_changeへ連続）。

**監査で確認した4点の実態:**
1. **preset lineage迂回**: `physical_details_inline_editor_md_generator.v1` presetのcomposition（`md_viewer.projection`+`textarea.template`）を「参照」した記述にとどまり、実際に`react_schema_topology_seed_translator.py`のintake→preview/validate/applyを一度も通していなかった——`db/seed_empty.sql`のdd015 tensorは、compositionが似ているという理由だけの独立した手書きJSONだった。
2. **canonical mutation pipeline迂回**: `team_dashboard:update`（`NpgsqlTeamDashboardRepository.UpdateNoteAsync`直呼び出し）にdryRun/confirm/diff_logの確認ステップが一切無く、`enum_dictionary:create_group`等の`mutation_confirmation_contract`（dryRun-preview→explicit-confirm→write→diff_log via `AdminMasterRosterAudit.AppendAsync`）を経由していなかった——`logs.diff`へのevidence書き込みも一切発生しない状態だった。
3. **Manifest identity route hardcode**: `frontend/routes/dashboard/index.tsx`/`frontend/routes/admin/team-dashboard/index.tsx`のManifest UUID直書き。汎用resolverの有無を再調査した結果、`ManifestDispatcher.cs`の`canonical_default_entry`機構（`ContentBundleRepository.ResolveCanonicalDefaultEntryManifestIdAsync`）はサイトroot `/`専用のsingleton解決に限定されており（`Target=default, Layer=screen_list, Action=Search`という一意なaxesのみ）、複数manifestを route ごとに resolve する汎用機構ではない。`/admin/enums`/`/admin/team-markdown`含む**既存の全admin/normalルートが同一のProjectionShell manifestId直書きpattern**であり、汎用route→Manifest-identity resolverは repo 全体に一件も存在しないことを確認した。
4. **DB/Runtime/Admin workflow SSOT drift**: `docs/design/db-schema.yaml`に`topology.team_dashboard_note`のtable identity registrationが欠落（`db/team_dashboard_tables.sql`のみが正本という状態）。`docs/design/runtime-orchestration-ssot.yaml`の`/admin/team-dashboard status: pending`、`docs/design/admin-console-workflow-ssot.yaml`の`subbundle_target_readiness.team_dashboard: blocked`、`docs/design/admin-normal-surface-projection-seed-ssot.yaml`複数箇所が、dd010/dd020が実在するようになった後もstaleなまま残っていた。

**実装内容:**
- **UIBuilder lineage解消**: `.agent/tests/fixtures/react-schema-topology-seed-translator/team-dashboard-admin.input.json`/`team-dashboard-admin.topology-seed.input.json`/`team-dashboard-normal.input.json`/`team-dashboard-normal.topology-seed.input.json`（新設）——`generate-react-schema`→`generate-topology-seed`の実2段pipelineを実際に通し、両surfaceとも`gateStatus: pass`・`validationErrors: []`を確認した。`docs/design/react-schema-topology-seed-translator-ssot.yaml`の`declared_seed_surface_catalog.known_declared_surfaces`へ`team_dashboard.admin.projection`/`team_dashboard.normal.projection`の2 entryを新設し、翻訳器が現状emitしない2点（`emission.data.*` propBindings、action/modalノードのcomponentKey/componentId——いずれもae200/092自身の本番tensorにも見られる非propBindings-leafの既知の同種省略と同カテゴリ）を`known_gaps`として明示的に記録した。`db/seed_empty.sql`のdd015 tensorは、この翻訳器出力＋文書化されたpost-generation patchの内容そのもの——独立した手書きartifactではない。
- **canonical mutation pipeline接続**: `backend/runtime/AdminRuntime.TeamDashboard.cs`の`DataTeamDashboardUpdateAsync`を全面改訂し、`enum_dictionary:create_group`と同一のdryRun-preview→explicit-confirm→write→diff_log契約を実装（`AdminMasterRosterAudit.AppendAsync`→`logs.diff`、`source_set_id=admin_master_roster`）。`db/seed_empty.sql`のdd015 tensorを3ノードから6ノードへ拡張（viewer/body/save_button〔dryRunのみ〕/confirm_modal/confirm_button〔confirmed実write〕/cancel_button）——`validate_admin_runtime_preview_action_pairing`が要求するSection-owned admin_runtime_dispatch_override_wiring Actionのpreview/confirm pairing shapeに準拠。team-dashboard固有のdiff writer/history table/dispatcher/mutation laneは一切新設していない——既存の汎用helperをそのまま再利用。
- **Manifest identity route hardcode**: 汎用resolverが実在しないことを確認したため、Owner指示「この1点のみ、既存authorityで決定できない設計gapとしてOwnerへ報告してよい（監査自体は止めない）」の運用に従い、修正せず設計gapとして明示報告する（下記「未解決事項」参照）。他の3点の解消は継続した。
- **SSOT drift解消**: `docs/design/db-schema.yaml`へ`topology.team_dashboard_note`のtable identity registration新設（`bootstrap_schema_surfaces_extended.team_dashboard_note`+`bootstrap_ddl_team_dashboard`、`team_markdown_dashboard_saved_view`と同型のformat）。`docs/design/runtime-orchestration-ssot.yaml`の`/admin/team-dashboard`ルート retirement matrix entryを`status: pending`→`status: superseded_by_owner_directed_alternate`へ更新し、precondition/evidenceを現状に同期。`docs/design/admin-console-workflow-ssot.yaml`の`subbundle_target_readiness.team_dashboard`を`blocked`→現状（manifest実在・翻訳器lineage・canonical mutation pipeline接続済み）へ更新。`docs/design/admin-normal-surface-projection-seed-ssot.yaml`の`reusable_component_composition_ref`/`seed_contract`（component_tree/event_bindings/mutation_confirmation_contract）/`normal_hub_relation_navigation_contract.current_target_readiness.dashboard_team_markdown`の3箇所を現状へ同期。

**test/proof:**
- `backend/tests/Topolactor.Runtime.Tests/AdminRuntimeTeamDashboardTests.cs`（改訂）: 既存writeテストへ`confirmed: true`を追加、新規dryRun previewテスト（`ok:true, dryRun:true, valid:true`、`UpdateCallCount`不変）、新規not-confirmedテスト（`TEAM_DASHBOARD_WRITE_NOT_CONFIRMED`、`UpdateCallCount`不変）を追加。
- `backend/tests/Topolactor.Integration.Tests/TeamDashboardHubRelationUiProjectionLiveDbTests.cs`（改訂）: STEP3のsave button assertionをdryRun preview検証+confirm_modal/confirm_button/cancel_button存在検証へ拡張。既存write系2 testへ`confirmed: true`を追加。新設`DispatchAsync_TeamDashboardUpdate_WritesLogsDiffEvidence_ReadableViaCanonicalPhysicalRecordHistory`——dryRun→0件、confirmed write→`logs.diff`1件、`physical_record:list_history`経由での読み戻し一致、をAdmin edit→DB update→logs.diff生成→canonical history read→Normal rereadの一連proofとして実PostgreSQLに対し実証（`physical_record:list_history`は repo 全体でどのmanifestにもdispatcher_mapping登録が無いため——team-dashboard固有ではない既存gap——ManifestDispatcher経由ではなく実AdminRuntime直接dispatchで検証、この点は判定コメントとして明記）。
- 実PostgreSQL 16（fresh DB、db/init.sql全chain適用）に対し `TeamDashboardHubRelationUiProjectionLiveDbTests` 7/7 PASS、`Topolactor.Runtime.Tests` 1693/1693 PASS、`Topolactor.Integration.Tests` 260/261 PASS（既知の無関係な既存failure`UiTopologyLayoutPatchRollbackIntegrationTests`〔`ui_layout_registry`テーブル名不一致〕のみ——`git stash`によりHEAD側でも同一failureを確認し非回帰と確定）。`python3 .agent/scripts/check_react_schema_topology_seed_translator.py`は既知の無関係な既存failure「7a」〔credential-management-0092 fixtureのseedEvidence cross-check、本diff対象外〕のみ、`git stash`で同一結果を確認し非回帰と確定。`bash .agent/tests/check-db-schema.sh` PASS。SSOT YAML 5ファイルは`yaml.safe_load`で構文確認——`admin-console-workflow-ssot.yaml`は本diffと無関係な既存箇所（`layout_node_props_contract.descriptor.portTargetRef`のquoted scalar構文、HEAD側で同一failureを確認済み）でparse errorがあるが、本round編集箇所（`subbundle_target_readiness.team_dashboard`）はそれより前方にあり影響を受けない。

**未解決事項（Owner報告、監査は継続・完了済み）:**
- Manifest identity route hardcode（`frontend/routes/dashboard/index.tsx`/`frontend/routes/admin/team-dashboard/index.tsx`のUUID直書き）は、汎用route→Manifest-identity resolverが repo 全体に一件も存在しないことを確認した結果、team-dashboard単独では解消不可能——`/admin/enums`/`/admin/team-markdown`含む既存の全admin/normalルートと同一のProjectionShell manifestId直書きpatternであり、この1subBundleだけ異なる仕組みに切り替えるとconsistencyを破壊する。汎用resolver自体の新設はteam-dashboard subBundleのscopeを超えるcross-cutting design_changeであり、Owner判断が必要な設計gapとして報告する（既存precedentをこのhardcodeの正当化根拠として使うのではなく、「解消には別途cross-cutting設計が要る」という事実の報告）。

**判定:** UIBuilder authoring lineage・canonical mutation pipeline接続・DB/Runtime/Admin workflow SSOT driftの3点は解消済み。Manifest identity route hardcode（4点目）はteam-dashboard単独解消不可能な設計gapとしてOwnerへ報告済みだが、他3点の解消・test/proof・SSOT同期は完了しており、team-dashboard subBundle自体はこの1点を除き**Implemented**——Owner判断（汎用resolver新設 or 現状のroute-pinned patternを許容パターンとして正式追認）を待つのみ。`admin-surface-topology-seed-conversion` Bundle全体は`scheduler-settings`が引き続き未着手のため`partial`のまま。PR #604は本roundの内容でmerge検討へ戻せる状態。

#### 2026-08-17追記（4周目）: worktype `existing_pr_update`——残り2 blocking（UIBuilder canonical apply lineage / generic route→Manifest resolver）をcross-cutting genericに解消

**契機:** 前round報告の「Manifest identity route hardcode」を設計gapとして報告するだけで終える判断をOwnerが差し戻し、(1) team-dashboard production tensorが`preset → UIBuilder output → Manifest`のcanonical authoring/apply lineageとして未成立（translator output + post-generation patchのみで、実UIBuilder apply pipelineを一度も通っていない）、(2) `/admin/team-dashboard`/`/dashboard`のManifest UUID route pinningを解消するgeneric route→Manifest resolverが存在しない、の2点をcross-cutting genericな設計・実装として同一PR内で解消するよう指示があった。

**worktype:** `existing_pr_update`。両blockingとも既存authorityから設計を導出できたため、同一round内でdesign_change→implementation_changeへ連続。

**調査結果（Explore agent、read-onlyで実施）:**
1. **UIBuilder apply pipelineの実態**: `/admin/ui-builder`の実Apply動線（`layout_patch:preview`→`layout_patch:validate`→`layout_patch:apply`、`backend/runtime/AdminRuntime.cs` `DataLayoutPatchApplyAsync`→`NpgsqlUiTopologyRepository.ApplyConfirmedLayoutPatchAsync`）は実在し、実PostgreSQLへ`topology.components_layout_design`/`topology.ui_topology_tensor`等を実書き込みする、live-DB testで証明済みの本物のproduction機構であることを確認した。ただし、ae200/092含む**現行のどの本番manifestも、実際にこのapply pipelineを通ってseed content化されたことは一度も無い**——全て`react_schema_topology_seed_translator.py`（offline CLIツール、`docs/design/react-schema-topology-seed-translator-production-policy.md`が自ら明記する通り「Bypassing UIBuilder preview / validate / apply」「Writing to the database from the translator」を明示的に禁じるNG項目そのもの、つまりtranslatorはUIBuilder apply authorityの代替を意図していない）の出力を人手で`db/seed_empty.sql`へ貼り付けたもの。かつ`db/init.sql`のfresh bootstrapはbackendプロセス起動前にSQLファイルを直接適用するため、構造的に「live admin dispatch経由のapply」をbootstrap content自体の生成経路にすることは**repo全体のどのmanifestに対しても不可能**（team-dashboard固有の妥協ではない）。
2. **route→Manifest identity resolverの実態**: `hubs.topology_manifests.manifest_key`（既存カラム、`db/topology_tables.sql`、UNIQUE制約なし）が、既にsystem全体で使われている安定・人間可読な識別子であることを確認した。`ContentBundleRepository.ResolveCanonicalDefaultEntryManifestIdAsync`（サイトroot専用のsingleton resolver）と全く同じ「exactly one active row、0件/複数件はfail-close」パターンをこのカラムへ適用すれば、生成的なgeneric resolverを新規語彙の発明なしに構築できることを確認した。`frontend/runtime/projectionEntry.ts`の`resolveProjectionEntryAxes`はmanifest UUIDを`payload.target_ref`へ埋め込むだけの薄い変換であり、解決されたUUIDを下流へそのまま渡すだけなので、この関数含め`ManifestDispatcher`の`LoadByIdAsync`/active-check/admin_runtime layer-action認可等の既存pipelineは一切変更不要と判明した。

**実装内容:**

1. **UIBuilder canonical apply lineage proof**（既存機構の新規適用、team-dashboard固有の新規apply機構は作っていない）:
   - 新設 `backend/tests/Topolactor.Integration.Tests/TeamDashboardUiBuilderCanonicalApplyPipelineLiveDbTests.cs` ——`db/seed_empty.sql`のdd015（admin）/dd025（normal）tensor `layout_patch_json`の実本番content（verbatim）を、使い捨てのroute_key/package/layout/wiring scaffold（本番dd0xx行には一切触れない）に対し、実`ManifestDispatcher`経由で`layout_patch:preview`→`layout_patch:validate`→`layout_patch:apply`（`frontend/islands/UiBuilderAdmin.tsx`の実Apply buttonが呼ぶのと同一dispatch action、既存の生成的`admin:layout_patch:preview/validate/apply` dispatcher_mappingで到達）へ実際に通し、DB row content（構造的JSON部分集合一致、object property順序・quote-escapingスタイルの差異は無視——`ApplyConfirmedLayoutPatchAsync`自身が正規のC#モデルへ一度round-tripさせるための無害な差異）が生存することを証明した。
   - この過程で実validateゲート（`RUNTIME_INTERACTION_TARGET_KIND_MISMATCH`）が、本番dd015 tensorのconfirm_modalノードに`componentKind`フィールドの明示欠落という**実在するギャップ**を検出した——emission時（`LayoutNodeComponentIdEnrichment`のround25 fix）はcomponentIdからDB解決されるが、`layout_patch:validate`自身はJSON内の明示フィールドのみを見てDB解決を一切行わないため、明示`componentKind`が無いと弾かれる。`db/seed_empty.sql`のdd015 confirm_modalノードへ`"componentKind":"disclosure/modal"`を追加して修正した——本番content自体の正当な改善であり、testだけの回避ではない。
   - また、apply pipeline自身が`runtimeInteractionId`という新規フィールドを各runtimeInteractionへ自動付与すること（既存の生成的enrichment機構、`NpgsqlUiTopologyRepositoryRuntimeInteractionIdentityTests.cs`のAssignRuntimeInteractionIds、team-dashboard固有ではない）を発見した——bootstrap SQLはlive enrichmentを経由しないため持たない、正当な差異として記録した（fakeなrandom IDをbootstrap SQLへ後付けで注入することはしていない）。

2. **generic route→Manifest identity resolver**（新規generic mechanism、既存route pinned patternのteam-dashboard専用置換ではない）:
   - `backend/repository/ContentBundleRepository.cs`（抽象）+ `NpgsqlContentBundleRepository.cs`（実装）: `ResolveActiveManifestIdByManifestKeyAsync(string manifestKey, ct)`新設——`ResolveCanonicalDefaultEntryManifestIdAsync`と同一の「`SELECT ... WHERE status='active' AND manifest_key=@key LIMIT 2`、0件→null、2件以上→`InvalidOperationException("MANIFEST_KEY_AMBIGUOUS: ...")`」パターン。
   - `backend/runtime/HubNavigationResolver.cs`: `ResolveActiveManifestIdByKeyAsync`薄いwrapper新設。
   - `backend/runtime/ManifestDispatcher.cs`: `target_ref`の既存`"manifest:{uuid}:{wiring_key}"`形式に加え、新形式`"manifest_key:{manifest_key}:{wiring_key}"`を認識するよう`TryParseManifestTargetRef`呼び出し分岐を拡張。解決されたmanifestIdは以降まったく同一の既存pipeline（`LoadByIdAsync`/active-check/admin_runtime layer-action認可/capability_requirement検証）へ流れる——round17/18のhardening済みセキュリティロジックは一切変更していない。エラーコード: `MANIFEST_NOT_FOUND`（0件、既存コード再利用）/`MANIFEST_KEY_AMBIGUOUS`（新設）/`TARGET_REF_INVALID`（空keyセグメント、既存コード再利用）。
   - `frontend/runtime/projectionEntry.ts`: `ProjectionEntrySelection.manifestKey`フィールド新設、`resolveProjectionEntryAxes`が`manifestId`不在時に`manifestKey`から`target_ref: manifest_key:<key>:projection_entry`を構築するよう拡張（`manifestId`優先、既存挙動へのregressionなし）。
   - `frontend/islands/ProjectionShell.tsx`: `manifestKey?: string` prop新設——既存`manifestId` propと全く同じ「URLに明示selectionが無い場合のみのdefault entry fill-in」ロジック（`manifestId`優先）。
   - `frontend/routes/dashboard/index.tsx` / `frontend/routes/admin/team-dashboard/index.tsx`: `manifestId={UUID定数}`を`manifestKey="team_dashboard.normal.projection"` / `"team_dashboard.admin.projection"`（既にdb/seed_empty.sql自身が`manifest_key`カラムへ登録済みの、既存の識別子文字列）へ置換——UUID定数はこの2ファイルから完全に除去された。`/admin/enums`等の他既存Implementedルートは本round対象外（無関係なroute全面改修を目的化しない、というOwner指示に従い、genericな仕組みの整合に必要な範囲のみ処理）。

**test/proof:**
- 新設 `backend/tests/Topolactor.Runtime.Tests/ManifestDispatcherManifestKeyTargetRefTests.cs`（6 test、unit）: exactly-one-active-row解決成功／0件→MANIFEST_NOT_FOUND／複数件→MANIFEST_KEY_AMBIGUOUS／空keyセグメント→TARGET_REF_INVALID／`HubNavigationResolver`未configured→MANIFEST_NOT_FOUND（例外なし）／既存UUID target_ref経路への非regression、を単体証明。
- `backend/tests/Topolactor.Integration.Tests/TeamDashboardHubRelationUiProjectionLiveDbTests.cs`へ新設2 test: `DispatchAsync_NormalManifest_ProjectionEntry_ByManifestKey_ResolvesSameProductionManifest`/`DispatchAsync_AdminManifest_ProjectionEntry_ByManifestKey_ResolvesSameProductionManifest`——実dd020/dd010 manifestを`manifest_key:...`のみで（UUID一切登場せず）実PostgreSQLに対し解決し、UUID経由と同一のEmissionへ到達することを証明。
- 新設 `backend/tests/Topolactor.Integration.Tests/TeamDashboardUiBuilderCanonicalApplyPipelineLiveDbTests.cs`（2 test、live-DB）: 上記1参照。
- `frontend/tests/projectionEntry.test.ts`へ新設4 test（`manifestKey`→target_ref構築、`manifestId`優先、default-entry判定、ProjectionShell prop存在のsource-scan）。
- 全体実行: `dotnet build`（0 errors）、`Topolactor.Runtime.Tests` 1699/1699 PASS、`Topolactor.Integration.Tests` 264/265 PASS（既知の無関係な既存failure`UiTopologyLayoutPatchRollbackIntegrationTests`のみ、fresh full-bootstrap DBで確認・非回帰）、`deno test -A frontend/tests/` 2110/2110 PASS、`deno check`（変更frontend file全件PASS）、`bash .agent/tests/check-db-schema.sh`/`check-bootstrap-validation.sh`（fresh DBに対しPASS）、`bash .agent/tests/check-static-ssot-purity.sh`/`check-design-ssot-progress-terms.sh`/`check-admin-normal-surface-projection-seed-ssot.sh`（PASS）。`deno run -A frontend/dev.ts build`はこのsandbox環境固有のesbuild/deno-loader network解決失敗（`Could not resolve "preact/jsx-runtime"`、`git stash`によりHEAD側でも同一failureを確認——本diffと無関係、環境制約として記録するのみ）。

**SSOT同期:**
- `docs/design/runtime-orchestration-ssot.yaml` `dispatcher_contract`へ`manifest_key_target_ref_resolution_contract`新設（`canonical_default_entry_contract`と同型のcontract定義形式、target_ref shape/owner/resolution_rule/failure_semantics/prohibited/test_proof_contractを完備）。
- `docs/design/admin-console-workflow-ssot.yaml` `subbundle_target_readiness.team_dashboard`を更新——real live apply pipeline proofとgeneric resolver適用の事実を追記。
- `docs/design/admin-normal-surface-projection-seed-ssot.yaml`の`surface_axes.admin.surfaces.team_dashboard.hub_relation_navigation_binding_ref`/`surface_axes.normal.surfaces.dashboard.team_dashboard_canonical_shared_contract.hub_relation_navigation_binding_ref`の2箇所を、manifest_key経由resolutionへの移行内容で更新。

**判定:** team-dashboard subBundleの残り2 blocking（UIBuilder canonical apply lineage、generic route→Manifest resolver）はいずれもcross-cutting genericな設計・実装・live-DB proofとして解消した——team-dashboard専用のhack・専用resolver・専用apply機構は一切追加していない。team-dashboard subBundle自体は**Implemented**——4点の監査finding（UIBuilder lineage、canonical mutation pipeline、Manifest identity route hardcode、SSOT drift）すべてが実装・test・SSOT同期まで完了した。`admin-surface-topology-seed-conversion` Bundle全体は`scheduler-settings`が引き続き未着手のため`partial`のまま——本roundでもscheduler-settingsへは一切進んでいない。PR #604はteam-dashboard subBundleの完全closureをもってmerge協議へ戻す。

#### 2026-08-17追記（5周目）: worktype `existing_pr_update`——最後の残blocking（`react_schema_topology_seed_translator`のtensor canonical generation）をgeneric translator grammar/output capabilityとして解消

**問題点:** 4周目時点で唯一残っていたblockerは、`react_schema_topology_seed_translator`がteam-dashboardの本番tensor semanticsを完全にgenericへ表現できず、(a) `emission.data.*`ソースのprop binding、(b) component identity（componentKey）emission、(c) write-actionを持たないNormalの純粋display tensor generation、の3点で post-generation patch／事前手動検証済みtensorの再利用に依存していたこと。`known_gaps`の`status`は`resolved_via_post_generation_*_patch`のままで、`db/seed_empty.sql`のdd015/dd025 tensorは「翻訳器出力＋文書化されたpatch」であり、翻訳器の再実行だけでは再現できない状態だった。

**目的:** 上記3点を、team-dashboard固有のhardcodeではなく、他surfaceからも再利用可能なgeneric translator grammar/output capabilityとして翻訳器本体に実装し、Admin/Normal両fixtureをゼロ手動編集で clean regeneration → 実live-DB `layout_patch:preview→validate→apply` → dd010/dd020 Manifest→ProjectionShell runtimeまで、単一の連続したproof chainとして証明し、team-dashboard subBundleを完全**Implemented**へ到達させること。

**改善方針:**
1. `validate_field_value_from_source`のvalueFrom文法を、既存の`node:<id>.value...`参照に加え、Table.rowsSource/Field.optionsSourceが既に使う`ROWS_SOURCE_SHAPE_RE`（`emission.data(.<path>)*`）も受理するよう拡張——新しい正規表現を発明せず、既存のownership regexを再利用。
2. 生成先のpropBindingsキー名を、Fieldの`control`から`frontend/runtime/propBindingResolver.ts`の`COMPONENT_ARRAY_PROP_CAPABILITIES`を直接ミラーする`FIELD_CONTROL_TO_VALUE_PROP_NAME`テーブルで解決（デフォルト`"value"`、`data_display/md_viewer`のみ`"markdown"`）——フィールド名やteam-dashboard固有ロジックの特別扱いを一切追加していない。
3. `COMPONENT_KIND_TO_COMPONENT_KEY`テーブルを新設し、`frontend/components/catalog.ts`の`COMPONENT_TEMPLATE_CATALOG_IDENTITIES`/`RUNTIME_ALIAS_CATALOG_IDENTITIES`を直接ミラー。componentIdは意図的に一切emitしない——`backend/repository/NpgsqlTopologyRepository.cs`の`EnrichCatalogComponentIdsFromRegistryAsync`が、componentKeyのみからcomponentIdを`topology.ui_component_registry`に対し読み取り時にgenericに解決する既存機構を既に持っているため、UUIDマッピングを翻訳器/SSOT側に複製しない。
4. `split_flat_records_into_adoption_candidates`のField(valueFrom/optionsSource/adminRuntimeDispatchOverride)/Action・WorkflowStep/Modal各branchで、componentKeyを上記テーブルから一様にemit。Action/Field tensorノードには`propsJson.label`も新設emit——`frontend/runtime/runtimeComponentFactory.ts`の`buttonFactory`が`props.data.label`を文字列として必須要求する（欠落するとレンダリング自体が失敗する）ことを実live-DB testで発見し、同一round内で解消した。
5. Action/Modal自身のruntimeInteractionsを、従来の「owning Form/Section の解決済みidentityへredirectして格納する」方式から、**そのAction/Modal自身のresolved identityへ直接格納する**方式へ変更。理由: `backend/repository/LayoutSchemaTensorComposer.cs`の`BuildInteractionsBySourceActionKey`はraw storageの`node.NodeId`でscopeするため配置場所非依存ではなく、tensor-only-authored layout（team-dashboardが該当）には Compose stepそのものが存在しない（`NpgsqlTopologyRepository.LoadLayoutNodesAsync`は`layout_patch_json.nodes[]`をそのまま返す）ため、owning-parent-keyed placementは実際には**到達不能かつLAYOUT_PATCH_CATALOG_COMPONENT_KEY_REQUIREDでvalidate失敗する**ことを実live-DB testで確認した（Section/Categoryはregistry-backed catalog leafではないためcomponentKeyを持ち得ない）。この修正は同時に、本番dd015のhand-authored旧形状に存在した実バグ（confirm_button/cancel_buttonの自身のclose Modal動作がModal自身のノードにのみ記録され、frontendがsourceActionKeyを一切読まない〔grep確認〕ためClickしても実際には閉じない）も併せて解消した。
6. `db/seed_empty.sql`のdd015/dd025を、上記実装後の翻訳器clean generation出力そのもの（ゼロ手動patch）へ置換。`backend/tests/Topolactor.Integration.Tests/TeamDashboardUiBuilderCanonicalApplyPipelineLiveDbTests.cs`の`AdminTensorPatchJson`/`NormalTensorPatchJson`定数も同内容へ同期。

**対応資料:** `docs/design/react-schema-topology-seed-translator-ssot.yaml`（`declared_seed_surface_catalog.known_declared_surfaces`のteam_dashboard 2 entry）、`docs/design/admin-normal-surface-projection-seed-ssot.yaml`、`frontend/runtime/propBindingResolver.ts`、`frontend/components/catalog.ts`、`backend/repository/LayoutSchemaTensorComposer.cs`、`backend/repository/NpgsqlTopologyRepository.cs`、`backend/repository/NpgsqlUiTopologyRepository.cs`、`db/ui_component_registry_preset_catalog_bootstrap.sql`。

**対象ファイル名:** `.agent/scripts/react_schema_topology_seed_translator.py`、`.agent/scripts/check_react_schema_topology_seed_translator.py`、`.agent/tests/fixtures/react-schema-topology-seed-translator/team-dashboard-{admin,normal}.input.json`・`.topology-seed.input.json`（4ファイル、clean regenerate）、`db/seed_empty.sql`（dd015/dd025 tensor row）、`backend/tests/Topolactor.Integration.Tests/TeamDashboardUiBuilderCanonicalApplyPipelineLiveDbTests.cs`、`docs/design/react-schema-topology-seed-translator-ssot.yaml`、`docs/design/admin-normal-surface-projection-seed-ssot.yaml`。

**対象関数名:** `validate_field_value_from_source`、`split_flat_records_into_adoption_candidates`（Modal/Field-valueFrom/Field-optionsSource/Field-adminRuntimeDispatchOverride/Action-WorkflowStep各branch、merge/`_clean_tensor_node`）、新設定数`COMPONENT_KIND_TO_COMPONENT_KEY`/`FIELD_CONTROL_TO_VALUE_PROP_NAME`。

**test/proof:**
- `python3 .agent/scripts/check_react_schema_topology_seed_translator.py`: 既知の無関係な既存failure「7a」（credential-management-0092 fixtureのseedEvidence cross-check、`git stash`でHEAD側も同一failureを確認し非回帰と確定）のみ、他すべてPASS。admin-enum-ae200向けassertion（120-123a）も無変更でPASS——schema-composed機構への非回帰を確認。duplicate-key系assertion（36z6a-d）は、owning-parent-keyed placement廃止という実装変更を正確に反映するよう更新（2 node/leaf-keyedへ）。
- Admin/Normal両fixture: `generate-react-schema`→`generate-topology-seed`をゼロ手動編集で実行し、両方とも`gateStatus: pass`・`validationErrors: []`・`unresolvedGaps: []`を確認。生成された`tensorAdoptionCandidates`は`db/seed_empty.sql`のdd015（6ノード）/dd025（1ノード）と同一内容。
- `TeamDashboardUiBuilderCanonicalApplyPipelineLiveDbTests`（実PostgreSQL 16、fresh full-chain bootstrap）: admin/normal両surface PASS（`layout_patch:preview→validate→apply`を実`ManifestDispatcher`経由で実行し、DB row読み戻しがclean generator出力と構造的に一致することを証明）。
- `TeamDashboardHubRelationUiProjectionLiveDbTests`（11 test）、`ManifestDispatcherManifestKeyTargetRefTests`（6 test）、`AdminRuntimeTeamDashboardTests`（7 test）: 全PASS——generic route→Manifest resolver・dd010/dd011・dd020/dd021 topology・logs.diff/history・Normal read/mutation fail-closeの非回帰を確認。
- `AdminEnumHubRelationUiProjectionLiveDbTests`等admin-enum系（36 test）: 全PASS——schema-composed機構への非回帰を確認。
- `Topolactor.Integration.Tests`全体: 264/265 PASS（既知の無関係な既存failure`UiTopologyLayoutPatchRollbackIntegrationTests`〔stale table名`ui_layout_registry`、本diffと無関係、`git log`で本round対象外ファイルと確認済み〕のみ）。`Topolactor.Runtime.Tests`: 1699/1699 PASS。`dotnet build`: 0 errors。
- `deno test frontend/tests/projectionEntry.test.ts frontend/tests/markdownRenderer.test.ts --allow-read`: 52/52 PASS（Markdown XSS negative-proof含む）。
- `bash .agent/tests/check-db-schema.sh`/`check-bootstrap-validation.sh`（fresh DB）、`check-static-ssot-purity.sh`、`check-design-ssot-progress-terms.sh`、`check-admin-normal-surface-projection-seed-ssot.sh`、`check-schema-seed-translator-entry-gate.sh`: 全PASS。

**SSOT同期:** `docs/design/react-schema-topology-seed-translator-ssot.yaml`のteam_dashboard 2 entryの`known_gaps`を`[]`へ変更し、旧3 gapの内容は`historical_known_gaps_closed_2026_08_17_uibuilder_lineage_closure_round_3`として閉じた記録に格下げ（現在authorityとしては残さない）。`docs/design/admin-normal-surface-projection-seed-ssot.yaml`の`reusable_component_composition_ref`内、post-generation patch言及箇所を現状（clean generation = production content）へ更新。

**判定:** team-dashboard subBundleの最後の残blockingは、他surfaceからも再利用可能なgeneric translator grammar/output capabilityとして解消した——team-dashboard固有のhardcode・専用translatorレーン・emission.data.markdownBodyの特別扱いは一切追加していない。SSOT/翻訳器/UIBuilder/Manifest/runtime/testのすべてが整合し、残課題はゼロ。team-dashboard subBundleは**Implemented**として確定する。`admin-surface-topology-seed-conversion` Bundle全体は、残り1 subBundle（`scheduler-settings`、本roundでも一切着手していない）が未着手のため`partial`のまま維持する。PR #604はmerge協議へ戻す。
...
**Status:** `implemented`（2026-08-15 round 45で表現整理——旧`partial`表記のStatus行と別建て`implemented`表記のStatus行が並存し、current stateとして矛盾して読める状態を解消した。判定内容自体はround44時点から変更なし。根拠: `docs/design/runtime-orchestration-ssot.yaml` `admin_route_retirement_matrix`〔`/admin/enums: status: done`〕、`docs/design/admin-console-workflow-ssot.yaml` `other_admin_routes.master_roster_routes`〔island: `ProjectionShell.tsx`〕、production filesystem〔`frontend/islands/AdminEnumsRoster.tsx`が実在しないこと〕を2026-08-15に再確認済み）

**Status history（要約。詳細な時系列証跡は下記round別section・「受入条件」節を参照——本行は現在のStatusの根拠ではなく読み手のための要約であることに注意）:** round10-13でdetail view相当〔ae280〕・pre-fill相当〔ae220〕を実装。round14-19でoperation selector機構（`dispatchTargetRefByTrigger`）を設計・実装し、authoring/authority境界まで確立。round19（本Bundle自身の最終round）で、残り4項目（ae200実配線・live-DB proof・production browser proof・negative boundary証明・`AdminEnumsRoster.tsx`撤去）の実行そのものを`admin-surface-topology-seed-conversion` admin-enum subBundle自身のtrackへ委ねる形でSSOT訂正した。admin-enum subBundle実装記録round34-36が、この委譲されたgeneric mechanismのみを用いて（admin-enum専用runtime lane新設なし）4項目すべてを完遂——round34で3項目、round35で`AdminEnumsRoster.tsx`/`/admin/enums`route retirementを含む最後の1項目（round35自身が「Bundle全4項目の状態：全4項目完了」と明記）、round36で`AdminEnumsRoster.tsx`をファイルごと削除しghost catalog entryも一掃。round37-43のgovernance/security hardeningもこの完了状態を差し戻していない。credential-management自身の実際のroute撤去実行は別subBundle scope（`docs/design/admin-normal-surface-projection-seed-ssot.yaml` design_blocking `navigation_binding_authoring_and_verification: unresolved_before_seed`のまま未着手）であり、本Bundleの完了条件ではない。

**Position（current。2026-08-15 round47で新設・下記「Position history」から分離）:** 本Bundleは、admin-enum/credential-managementが共有する「一覧から選択した行の現在値を、別のwrite manifestのform/pre-fill/mode stateへ伝えるgeneric carrier機構が存在しない」という compound gap を対象として開始され、その機構（generic operation selector、`dispatchTargetRefByTrigger`）を確立するところまでが本Bundle自身のscopeだった（本Bundle自身の最終round=round19。詳細は上記「Status」「Status history」参照）。確立した機構の実際の適用——ae200単一画面統合・selected-row-relative field prefill・`enum_search`機能的filter・`/admin/enums`のroute retirement——はround19の委譲によりadmin-enum subBundle自身のtrackで実行され、round34-36で完遂済みである。credential-management側の同機構の実際の適用は、本Bundleの完了条件ではない別subBundle scopeとして依然未着手のまま残る（`docs/design/admin-normal-surface-projection-seed-ssot.yaml` design_blocking `navigation_binding_authoring_and_verification: unresolved_before_seed`）。

**Position history（historical。round3-19時点の診断・owner decision・設計変遷の時系列証跡——以下は当時の記録であり、現在のBundle positioningを表すものではない。現在のpositioningは直上の「Position（current）」参照）:** PR #600（`admin-surface-topology-seed-conversion` admin-enum subBundle）review round 3の指示「既存substrate範囲内でhardcoded-route撤去が可能か調査し、不可能ならBundle単位todoへ分離する」に基づき切り出した論点。round 4のowner再指摘（round 3の物理層記述の不正確さの指摘）を受け、下記「問題点」（historical section）を訂正済み。admin-enum/credential-managementの2 subBundleが共有する複合論点だった。当初「本Bundleでは実装しない（owner判断を待つためのtodoである）」としていたが、round 9-13を通じて、detail view（ae280）とpre-fill（ae220、production証明込み）は実際にこのBundle内で実装され、この当初見立ては不正確になった。round4-13時点では、owner decisionを要する設計拡張として本Bundleに残っていたのは「ae200の行選択からgroupIdを自動で運ぶcarrier」1点のみであり、これは「design自体は存在するが実装が未着手（`not_started`）」という状態であって、コード上の欠陥や矛盾（=本来の意味での"gap"）ではなかった——本Bundle名自体に含まれる"gap"という語は、round 3-4時点でこの論点を切り出した際の命名であり、当時のstatusを表す語であって現在のstatusを表す語ではないことに注意（Bundle IDは既存の相互参照を壊すため改名していない）。この「残るのはcarrier 1点のみ」という診断自体も、round14で「単一画面（1 layout）でlist/create/update/delete/set_group_itemsを切り替えdispatchするoperation selector機構の不在」というより根本的な症状の言い換えに過ぎなかったことが判明し、round15-19でその機構（`dispatchTargetRefByTrigger`）が確立され、admin-enum subBundle round34-36で実際に適用（ae200単一画面統合・selected-row-relative field prefill含む）されたことで解消済みである。scheduler-settings/team-dashboardは、下記「compound対象の再判定（round 4）」節の理由により対象外とした。round 9でownerが「A/B/Cは既存CRUD presetを読まずに再発明したもの」として撤回・presetへの統合を指示——presetを実際に読んだ結果、両preset自体がSSOT自身の言う「draft/intake artifact」であり、その`layout_tree`が本PR自身の確定済み「1 layout=1 canonical operation」architectureと構造的に矛盾することを発見した（下記「round 9」節参照）。round 10で「detail view相当」（ae280）を実装。round 11で、owner指摘（「設計判断は既にしてるでしょ」）を受けてpre-fill部分を再点検した結果、round 9自身のレビュー指示が既に確定した設計判断であり、これ以上の2択提示は不要と判断——update_group自身のdryRun before-value fallbackと、`form_input/search_input`へのpropBindings.value機構拡張＋`liveNodeValueTracker`への播種を組み合わせ、「groupIdが分かっている前提でのpre-fill」を新規carrier無しで実装した（下記「round 11」節参照）。「ae200の行選択からgroupIdを自動で運ぶ」こと自体は、round13時点では依然未解決のまま残っていたが、round14でこの表現自体が上記のoperation selector機構の不在の言い換えに過ぎなかったことが判明し、round15-19でその機構自体を確立、実際の適用はround19の委譲を経てadmin-enum subBundle round34-36で完遂した。

### 問題点

**（historical section——2026-08-15 round46追記）以下は本Bundle起票時（round 3-8時点）の診断であり、現在のcurrent stateを表すものではない。** ここで「既存のgeneric topology substrateだけではUX-parityを再現できない」とされた結論そのものは、round14で真のblocker（operation selector機構の不在）へ再定義され、round15-19でその機構（`dispatchTargetRefByTrigger`）が確立され、admin-enum subBundle round34-36がそれのみ（新規runtime lane無し）を用いてae200単一画面統合・selected-row-relative field prefill・`enum_search`機能的filter・`AdminEnumsRoster.tsx`撤去のすべてを完遂したことで解消済みである（詳細は上記「Status」「Status history」参照）。以下の`AdminEnumsRoster.tsx`（当時のhardcoded route `/admin/enums`——2026-08-05 round35以降は`ProjectionShell`のthin wrapperへ置換済み、`AdminEnumsRoster.tsx`自体はround36でファイル削除済み）を含む記述は、当時実際に存在した診断・調査過程の時系列証跡として保持する。

`admin-surface-topology-seed-conversion` admin-enum subBundleは、7つのenum_dictionary write action（create_group/update_group/delete_group/create_item/update_item/delete_item/set_group_items）それぞれに専用のsingle-purpose write manifest（ae210/ae220/ae230/ae240/ae250/ae260/ae270）を持ち、hub_navigationでae200（読み取り専用一覧manifest）と相互連結した状態まで実装・live-DB証明済みである（`admin-surface-topology-seed-conversion` admin-enum subBundle実装記録参照）。しかし、これは`AdminEnumsRoster.tsx`（当時のhardcoded route `/admin/enums`）が提供する「検索→一覧→既存行を選択→その現在値を読み込んで編集→確認→書き込み→再取得」という単一画面内で完結するUXと同等ではなく、実装コードの追跡により、既存の generic topology substrateだけでは（round13時点までは）このUX-parityを再現できないことが確定した。理由は以下の2点（round 3が誤って結論づけていた「動的operation切り替えの不在」ではない——`component_wiring_execution_lane`の「single-purpose write layoutが安全かつ十分な構成」という設計原則は既に確定済みであり、この論点ではない）。
...
**注記（2026-08-15 round45追記、historical section）**: 本節はround7時点で提示された3方向の比較であり、当時は「いずれも実装しない・owner decision待ち」だった。この状態はround9でA/B/Cすべて撤回され、round14の再検証で真のblockerが「operation selector機構の不在」と再定義された上でround15にてその機構自体がowner決定を経て実装された（詳細は下記round7/9/14/15節、および上記「Status history」参照）。本節の見出し・箇条書き自体はA/B/C各案の内容記録として当時のまま保持し、現在の未解決状態を表すものではない。
...
- ~~`frontend/islands/AdminEnumsRoster.tsx`、`frontend/islands/AdminUsersRoster.tsx`（cross-manifest carrier解決後のhardcoded route撤去対象、未着手——round14でcarrier前提自体を再検証したが、結論は変わらず未着手のまま）~~ → `AdminEnumsRoster.tsx`はadmin-enum subBundle round35でroute撤去・round36でファイル自体削除済み（production filesystemで実在しないことを2026-08-15確認済み）。`frontend/islands/AdminUsersRoster.tsx`は現在も実在し、credential-management subBundle自身の別scope（本Bundleの完了条件ではない）として未着手のまま残る。
...
- ~~**唯一残る未解決scope（round 17時点）**: generic mechanismの認可・authoring境界はこれで完成したが、(1) 実際にae200（または単一per-screen manifestとして再構成した何らか）へ`dispatchTargetRefByTrigger`を使ったcreate/update/delete/set_group_items等のin-page modal/dialogノードを、canonical generation経路（手書きseed SQLではなくtranslator/schema-composed経路）を使って単一画面へ統合配線すること、(2) その配線をlive-DB（実PostgreSQL）・production browser interaction（`ProjectionShell`実マウント、DOM event、SSE refresh）で証明すること、(3) selection A→B・cancel・stale tracker・SSE refresh・removed node・missing values・inactive manifest・unauthorized layer/action・role mismatch・invalid UUID・alias collision・duplicate index・missing group/item・duplicate membership・referenced delete・unconfirmed write・dryRun non-persistenceのnegative boundary証明、(4) `AdminEnumsRoster.tsx`/`frontend/routes/admin/enums.tsx`の撤去、の4点はいずれも未着手である。owner決定待ちではなく、round15-17で確立したgeneric mechanismを使った通常の実装作業として次round以降で進める。この環境にはlive PostgreSQLが存在しないため、(2)のlive-DB proof実施には別環境またはCI実行が必要になる制約がある。~~ → round 19（本Bundle自身）で、この4点の実行自体を`admin-surface-topology-seed-conversion` admin-enum subBundleのtrackへ委譲する形でSSOT訂正（本Bundleの役割はgeneric mechanismの確立・実証可能化までであり、個別subBundleでの実配線実行は各subBundle自身のscope）。2026-08-15 round 44の再監査で、admin-enum subBundle実装記録round 34（items browse/orphan監査/negative boundary matrix完了）・round 35（selected-row-relative field prefill＋enum_search server-side filter＋route retirement完遂、「Bundle全4項目の状態（round35時点）：全4項目完了」と自己記録）・round 36（`AdminEnumsRoster.tsx`ファイル削除・ghost catalog一掃・route entry production proof追加）により、上記(1)〜(4)がgeneric mechanismのみ（admin-enum専用runtime lane新設なし）で実際にすべて完遂されたことを確認した。round 37-43はこの完了状態を前提としたgovernance/security hardeningであり、いずれの項目も未完了へ差し戻していない。credential-management側の実際のroute撤去実行は、本Bundleの受入条件が明記する通り「各subBundle自身のUX-parity実装・撤去は別途そちらのscope」であり本Bundleの完了条件ではない（`admin-surface-topology-seed-conversion` credential-management subBundle側で別途未着手のまま管理）。**本Bundleの受入条件は全項目充足済みと判断する。**
