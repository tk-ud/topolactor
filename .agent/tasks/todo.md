...
- proof manifest追従: `docs/design/test-proof-manifest-ssot.yaml` に `aggregate_trigger_substrate_live_db_executable_proof` と `aggregate_trigger_substrate_step3_dynamic_authoring_proof`(いずれも executable/integration proof)を追加し、`aggregate_trigger_substrate_executable_proof_gap`(known_gap)を下記の残 scope のみへ縮小。`.agent/docs/test-bundles.yaml` 追従済み。

残 scope(`aggregate_trigger_substrate_executable_proof_gap` known_gap 参照、未閉鎖 — 上記以外はすべて閉鎖済み):
- [ ] admin/contents Step3 の `processing_function_scope.function_id` / `operation_definition_id` は現状 safe-identifier 検証済みの自由入力であり、`docs/design/admin-console-workflow-ssot.yaml` の `select_processing_function_or_operation_definition` が文字どおり要求する「登録済み processing function からの選択」ではない。理由: このリポジトリには processing function / operation definition を列挙する registry(テーブル・admin 画面)がまだ存在しない。存在しない registry を本 selector のためだけに捏造することは authority データの偽装になるため、実施していない。
  - 目的/判断事項: (a) 現行の「safe-identifier 検証済み自由入力」を正式な authority 表現として SSOT 文言を整合させるか、(b) processing function registry を新設し、そこから選択させる設計にするか — いずれを取るかは product/design 判断であり、本 Bundle の implementation_change scope 単独では決定しない。
  - 対応資料: `docs/design/admin-console-workflow-ssot.yaml`(`aggregate_trigger_step3_extension_contract`)
  - 対象ファイル: `frontend/components/AggregateTriggerAuthoringPanel.tsx`, `frontend/lib/aggregateTriggerAuthoring.ts`
  - OK軸: registry 実装を追加する場合は raw SQL/CASE/WHERE/任意 table 名を保存しない構造化 selector とし、backend が引き続き validation authority を持つこと。SSOT 文言を整合させる場合は、実装の後付け正当化ではなく、なぜ registry 化が不要かの理由を明示すること。
  - NG軸: 存在しない registry をでっち上げて selector の見た目だけ整える。この known_gap を証拠なく削除する。
