...
- [ ] legacy row migration proof: exactly-one-active-target行が正しくbackfillされること、zero/multiple-active-target行が`status='deprecated'`へ遷移すること、deprecate化がsource側を新規orphanにするケースがmigrationをfail closeし手動対応へ回すことをtestで確認する。
