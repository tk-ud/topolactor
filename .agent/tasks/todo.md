...
- PR574 retry-safe dispatch idempotency is implemented evidence and must not be reclassified as missing: frontend runner plus backend ledger protect `dispatchExternalPort` / `dispatchInstanceOperation` retries, concurrent duplicates, reload/reconnect runner recreation, and failed-claim reclaim. This remains untouched execution-gate authority.
- **Implemented (PR577 follow-up)**: `docs/design/admin-uibuilder-ui-structure-wiring-ssot.yaml` `lifecycle_policy.projection_authority_runtime_interaction_identity` (`status: implemented`) — backend `NpgsqlUiTopologyRepository.AssignRuntimeInteractionIds` assigns a stable `runtimeInteractionId` to id-less `runtimeInteractions[]` entries at the `layout_patch:apply` persistence boundary (never client-generated); `computeDispatchIdempotencyKey`'s identity portion uses `runtimeInteractionId` when present, falling back to the original `nodeId + interactionIndex` composition unchanged when absent (backward compatible, no ledger contract change); `cloneVisualNode` strips `runtimeInteractionId` on node/interaction duplication so a copy is never mistaken for its source; both the lifecycle path (`uiEventEffectRunner.ts`) and the event-triggered path (`renderEmission.ts` binding builder) forward the assigned id verbatim.
- Backend idempotency ledger (`topology.runtime_dispatch_idempotency_ledger`, PR574) remains execution gate and is distinct from `runtime_event_log`; do not turn event-log evidence into execution gate authority. This implementation only changed the structural-identity component the ledger key is built from — the ledger contract itself was not touched.
- **field_shape hardening (PR577 follow-up)**: `runtimeInteractionId` is declared as a UUID string; `AssignRuntimeInteractionIds` now enforces this — a present-but-blank/non-string/non-UUID-format value is treated as absent and replaced with a freshly assigned valid UUID (never preserved, never forwarded to frontend dispatch identity), and the rewrite never emits a duplicate `runtimeInteractionId` JSON key when replacing an invalid value.
