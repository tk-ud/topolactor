...
### admin-enum subBundle 実装記録（2026-07-27 追記: mutation_confirmation_contractのconfirmed/dryRun gate実装 + seed-wiring architecture制約の発見）

`admin-runtime-operation-dispatch-lane-determination` Bundleが2026-07-24に`remaining_write_payload_capture_gap`を解消し、PR #599 review round 8まででLane 2 (`component_wiring_execution_lane`, `wiringKind=admin_runtime`, node-level `dispatchPayloadFromByTrigger`)が本番proven状態になったことを受け、本ラウンドは admin-enum 自身の write 実装に着手した。**admin-enum はこのラウンドでも `implemented` 扱いにしない** — 以下の通り、backend側のmutation_confirmation_contractは実装・test証明したが、seed側のUI wiring（7 write actionをae200のUIノードから実際にdispatchさせる部分）は新たに発見した実アーキテクチャ制約により未着手のまま持ち越す。

**実装したもの（backend、`backend/runtime/AdminRuntimeMasterRoster.cs`）:**
- `enum_dictionary:create_group`/`update_group`/`delete_group`/`create_item`/`update_item`/`delete_item`/`set_group_items` の7 actionすべてに、`backend/runtime/AdminRuntime.TeamMarkdown.cs` `DataTeamMarkdownSavedViewUpdateAsync` が既に確立している `payload.dryRun`（preview/validate、非mutating）→ `payload.confirmed`（explicit_confirm gate、無ければ`ENUM_GROUP_WRITE_NOT_CONFIRMED`/`ENUM_ITEM_WRITE_NOT_CONFIRMED`でfail-close）→ write という既存genericパターンをそのまま拡張した——enum専用の新しい確認機構は追加していない。write step は dryRun の有無に関わらず同一のvalidation gate（malformed UUID、group/item existence、manifest参照チェック等、既存のまま変更なし）を毎回re-runする（team_markdownのコメント「dryRun is not trusted as prior proof」と同じ規律）。diff_log は既存 `AdminMasterRosterAudit.AppendAsync` を無変更のまま再利用——新しいaudit機構は追加していない。cancel は「確認しない」ことそのもの（confirmedを送らなければrepositoryへ到達しない）で表現され、新しいcancel APIは不要だった。
- 新規helper `IsTruthyPayloadFlag` を追加——JSON boolean `true` に加え、JSON文字列 `"true"` も受け付ける。理由: `docs/design/ui-builder-preset-ecosystem-ssot.yaml` `payloadFrom_resolver_contract` の `literal:` sourceは常にJS文字列を解決するため（`frontend/runtime/payloadFromResolver.ts` `resolvePayloadFromSource` の `literal` caseがstring固定）、`confirmed`/`dryRun`をseed-authored `literal:true` payloadFromで送る経路がJSON boolean専用チェックのままだと必ず失敗する。これは新しいpayload resolverではなく、既存の2 boolean flagの読み取り方法を汎用ヘルパーとして1箇所にまとめただけ。
- 新規error code 2件を`docs/design/enum-dictionary-ssot.yaml` `fail_close.error_codes`へ追加: `ENUM_GROUP_WRITE_NOT_CONFIRMED`（create_group/update_group/delete_group/set_group_items共通）, `ENUM_ITEM_WRITE_NOT_CONFIRMED`（create_item/update_item/delete_item共通）——既存の`RUNTIME_INTERACTION_*`/`ENUM_*`という既存語彙prefixファミリーを踏襲。

**Test証明:**
- `backend/tests/Topolactor.Runtime.Tests/AdminRuntimeMasterRosterTests.cs`（既存ファイル拡張、新規ファイルなし）: 既存`EnumDictionaryCreateGroup_ThenList_IncludesGroup`に`confirmed:true`を追加（新contractへ追随）、新規4 test（confirmed無しのfail-close+非persist、dryRunのpreview+非persist、delete_itemのconfirmed無しfail-close、`confirmed:"true"`という文字列literal値でのset_group_items成功）。`dotnet test Topolactor.Runtime.Tests` 1483/1483 pass（既存1479 + 新規4、regressionなし）。
- `backend/tests/Topolactor.Integration.Tests/AdminEnumHubRelationUiProjectionLiveDbTests.cs`（既存ファイル拡張、新規ファイルなし）: 既存`CreateGroupThenDeleteGroup`テストに、confirmed無しのcreate_group fail-close（+非persist再list確認）とdryRun=trueのcreate_group preview（+非persist再list確認）の2 negative caseを追加し、実write呼び出し（create_group/delete_group）へ`confirmed:true`を付与。実PostgreSQL（ローカルbootstrap、`db/schema.sql`〜`db/auth_tables.sql`までCI同順序で適用）で`AdminEnumHubRelationUiProjectionLiveDbTests`5/5 pass。`Topolactor.Integration.Tests`全体は193件中190 pass——3件のfailureは全て本ラウンドの変更対象外ファイル（`UiTopologyLayoutPatchRollbackIntegrationTests`は既存記録済みのstale `ui_layout_registry`参照、`BackendErrorNotifyBridge*LiveDbTests`2件はローカル手動DB bootstrapの環境差起因、CI相当の完全なseed適用順序では発生しないと推定——admin-enum/enum_dictionaryとは無関係）。

**新たに発見したseed-wiring architecture制約（write実装が未着手の理由、fabricateせず正直に記録）:**
`db/seed_empty.sql` ae200のUIノードから7 write actionを実際にdispatchさせる作業に着手したところ、`admin-runtime-operation-dispatch-lane-determination` Bundleが確立した Lane 2 (`component_wiring_execution_lane`) の `wiringKind`/`target_ref` は **wiring行単位（レイアウト全体で1つ）** の値であり、node単位のoverrideを持たないことを実装前調査で確認した（`backend/tests/Topolactor.Integration.Tests/AdminEnumHubRelationUiProjectionLiveDbTests.cs` `DispatchesRealListGroups...`テストが`enum_search`/`enum_group_filter`/`enum_table`いずれも同一の`target_ref=...:list_groups`を継承することをすでに証明済み——「レイアウト全体が正確に1つのoperationである場合のみ、レイアウト全体を1つのadmin_runtime actionへ一律bindしてよい」という2026-07-23確定の設計方針通り）。node単位でoverrideできるのは Lane 3 (`runtimeInteractions`、`localStateMutation`等) のみで、admin_runtime dispatch自体のtargetをnode単位で変える経路は存在しない。
これにより、既存5ノード（`enum_search`/`enum_group_filter`/`enum_table`/`enum_form`/`enum_confirm_button`）で構成される単一layoutから7つの異なるadmin_runtime write targetを配線するには、(a) 新しいper-node target_ref override機構を発明する（新規mechanism、本Bundleおよび`admin-runtime-operation-dispatch-lane-determination`のGovernance NG boundaryが要求するowner decisionなしに実装すべきでない）か、(b) 既存の`/admin/manifests` hub relation authoring + hub_navigation:createによるnavigation機構（`AdminEnumHubRelationUiProjectionLiveDbTests.cs`の`HubNavigationCreate_RealAuthoringPath...`テストで既に証明済みの既存経路）を再利用し、write operationごとに専用の小さな子manifest（1 layout = 1 operationという既に確定した安全な構成）を追加してae200から遷移させる、のいずれかが必要になる。(b)は新しいruntime mechanismではなく既存パターンの反復適用だが、7 manifest分のhub/manifest/topology_manifests/package/layout/wiring/tensor行と対応するlive-DB proofの追加が必要な、比較的大きな追加実装であり、本ラウンドの残り予算では未着手のまま次のAgentへ引き継ぐ。

**次にこの subBundle を触る Agent への引き継ぎ:**
1. 上記(b)の子manifestアプローチが最有力候補——ただし着手前にオーナー確認を推奨（(a)の新機構発明との比較を含め、他のsubBundle（team-dashboard/scheduler-settings）にも同じ制約が及ぶcross-cutting決定のため）。
2. 子manifestアプローチを採る場合、`delete_group`/`delete_item`（既存行のid のみ必要、`event.row.<field>`経由でtyped値capture不要）を最初の完全end-to-end proof候補にすること——本ラウンドで実装したbackendのconfirmed gateと組み合わせれば、typed値capture面の複雑さを避けて最初の1件を完成させられる。
3. backendのdryRun/confirmed gateは7 action全て実装・test済みなので、seed-wiring側はこのcontractへそのまま乗せればよい（backend側の追加実装は不要）。
4. `db/seed_empty.sql` ae200のheader comment（"write dispatch blocking gap"節）は本ラウンドの発見を反映してsync予定だが未実施——次のAgentは同comment・`enum_write_dispatch_gap` Validation seed record・`docs/design/react-schema-topology-seed-translator-ssot.yaml` `declared_seed_surface_catalog.known_gaps`を、実装前に本節の内容と突き合わせて更新すること。
