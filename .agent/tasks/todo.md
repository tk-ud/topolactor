...
- read projection: 既存`ContentBundleRepository.LoadHubNavigationSequenceAsync(topologyManifestId)`（source-scoped）が、current topology自身のsequence（forward方向）を過不足なく返す。これに加えて、target方向のreverse read（他manifestがcurrent topologyをtargetとして名指すrowを見つけ、そのrowのsource manifestを候補として返す）を**追加のadditive query**として導入する — 新しいtable/column/parallel authorityではなく、同一`hubs.hub_relations`永続化に対するもう一つの読み方（新設backend read methodは必要、ただし新しいtable/columnは不要）。この双方向読みが、`axis_navigation_membership`の3 target（`ae200`/`5c100`/`092`）が自分自身のoutbound行を持たなくても`dd010`へ戻れる仕組みそのものである（下記no_return_edge項目参照）。
- **Admin bridge（実体、名目だけでなくdd010へ収束）**: `dd010`（team_dashboard.admin.projection）は上記(1)で確定した通り、それ自体がAdmin axisのcanonical navigation source兼Admin bridgeである — 単なるreverse readの例示ではない。標準例: `ae200`に立っているとき、reverse方向の読みが既存`dd010 -> ae200`行を見つけ、`dd010`を候補として返す。新しいbridge manifest・新しいdispatch axis・新しいsequence-position規約などの新architectureは一切作らない — 既存`dd010`と既存`dd010 -> ae200`行をそのまま再利用する。
