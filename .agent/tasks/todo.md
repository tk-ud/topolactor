...
- [ ] `aggregate_dashboard_seed`: **Problem:** aggregate_dashboard remains provisional even though its UX scope is intentionally small. **Purpose:** provide a reusable dashboard preset that lets the author choose which aggregation elements to place and which period width/window to use. **Improvement direction:** seed-first implement a dashboard preset composed from existing dashboard/read-work/catalog display components, with configuration for aggregation element list, period width/window, data binding source, and preview/validate/apply boundary; do not create a one-off dashboard screen. **References:** `docs/design/admin-console-workflow-ssot.yaml`, `docs/system-roadmap.yaml`, `docs/design/ui-ux-primitive-catalog-ssot.yaml`. **Targets:** target preset SSOT block, optional seed migration/tests, `frontend/tests/mockPresetIntake.test.ts`, `.agent/tasks/todo.md` status update after audit.
