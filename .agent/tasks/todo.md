...
#### admin-dashboard round 5 監査（PR #594 owner review comment、round 4 差分の2件の未達を同一PR内で解消）

owner から round 4 の有効差分（`select_hub_relation_link` 削除・generic hub navigation substrate・`table.primitive` 接続・CRLF修正・CI green）を SETTLED FACTS として再調査対象から除外した上で、残る2件の未達（gap taxonomyの下位SSOT family同期、admin static content の実sourcing未達）を同一PR内で解消するよう指示があった。両件を最後まで調査・実装した。

1. **`authoring_schema_vocabulary_gap` の下位translator SSOT family同期 — 確定：真正の同期漏れ、design_change + 実装で解消。**
   `docs/design/react-schema-topology-seed-translator-ssot.yaml` `text_decomposition_contract.units.known_gap_ref.meaning` を確認した結果、親SSOT `canonical_gap_types` の5種（`component_catalog_gap`/`prop_binding_gap`/`event_payload_resolver_gap`/`runtime_dispatch_or_projection_gap`/`ssot_ambiguity_gap`）を**未追跡のprose文字列として複製**しており、round 4 で親SSOTへ追加した `authoring_schema_vocabulary_gap` を含んでいなかった（真正の同期漏れ）。さらに `.agent/scripts/react_schema_topology_seed_translator.py`（translator本体）・`.agent/scripts/check_react_schema_topology_seed_translator.py`（検証系）のいずれにも、`knownGapRefs` の classification 語彙を実際に検証するコードは**一切存在しなかった**（`validate_input_envelope` は `knownGapRefs` の非空性のみを確認し、値の形式・語彙は無検証。既存 test「7c」も無分類の合成文字列 `synthetic_probe_gap_for_propagation_check` をそのまま受理して伝播確認のみ行う設計）。owner指摘「下位translator SSOTや検証系が未知文字列を黙って通す状態」は事実として確認された。
   **実装内容:**
   - `docs/design/react-schema-topology-seed-translator-ssot.yaml`（version 1.1.0 → 1.2.0）の `known_gap_ref` に構造化フィールド `allowed_classifications`（6種、親SSOTと同一集合）を追加し、`meaning` を「親SSOTと同期する、fork しない」という文言へ訂正。
   - `.agent/scripts/react_schema_topology_seed_translator.py` の `validate_input_envelope` に実 runtime 検証を追加: 封筒レベル `knownGapRefs` の各エントリが `"<classification>:<description>"` 規約（コロンあり）を使っている場合のみ、`classification` が自SSOTの `allowed_classifications`（親SSOTを直接読まない——同ファイル冒頭の「reads only the SSOT YAML above」という既存アーキテクチャ原則を尊重し、親からのcross-readではなく自SSOT内の同期済みコピーを参照）に含まれることを検証し、含まれなければ `KNOWN_GAP_REF_CLASSIFICATION_UNKNOWN`（blocking）を返す。コロン無しの**既存レガシー形式**（例: `physical-search-crud-aggregate` golden fixture の `enum_status_select_options_from_content_bundle_list_states`）は意図的に対象外とし、既存の合格テストを破壊しない設計とした（この判断は "未知のgap type/誤分類" という owner の指摘範囲——分類を試みて間違えたケース——に正確に一致し、"分類自体が存在しない" という別パターンを遡及的に破壊しないための線引きである）。
   - `.agent/tests/fixtures/react-schema-topology-seed-translator/admin-dashboard-hub-relation-navigation.{input,topology-seed.input}.json` の `knownGapRefs` を、既存の翻訳者生成コード・checker既存fixtureの実際の慣習（`<classification>:<description>`、分類が先）に合わせて反転した（round 2–4 は逆順 `<description>:<classification>` を使っており、これは本コードベース全体の実際の慣習と不整合だったことを今回発見・訂正した）。再生成し `gateStatus: pass` 再確認。
   - `.agent/scripts/check_react_schema_topology_seed_translator.py` に新規 check 7d0/7d/7e/7f を追加: 7d0/7d は親SSOT `canonical_gap_types` と自SSOT `allowed_classifications` の raw-text独立抽出による集合一致（drift検出、既存 36r1-36r3 と同じ規律）、7e は未知分類が blocking エラーになることの負例証明、7f は新分類 `authoring_schema_vocabulary_gap` 自体が正しく受理されることの証明。
   - 検証: `python3 .agent/scripts/check_react_schema_topology_seed_translator.py` 164 checks 中 163 OK、失敗は既存の pre-existing 「7a」のみ（`git stash` で本PR変更を除いた状態でも同一 failure が再現することを再確認——round1の記録と同じ、本PR由来ではない）。`.agent/tests/check-schema-seed-translator-entry-gate.sh` 40/40 pass。`dotnet test backend/tests/Topolactor.Runtime.Tests` 1445/1445 pass（無変更、regression無し）。
2. **`ADMIN_ROUTE_CARDS`/`ADMIN_INDEX_GUIDE` の実sourcing — 確定：round 4のtest-onlyな検証を実装のsourcingへ格上げ。**
   round 4 の `static_navigation_sourcing_contract` は「production consumer (`adminGuides.ts`) は独立したTypeScript literalを読み続け、testがそれをSSOTと突き合わせて検証するだけ」という設計であり、owner指摘の通り実consumerの sourcing ではなかった。build-time generation + checked-in generated artifact パターン（owner提示の許容パターンの一つ、runtime read pathは前提としない）を採用し、以下を実装:
   - `.agent/scripts/generate_admin_route_identity.py`（新規）: `docs/design/admin-console-workflow-ssot.yaml` の `authority.canonical_routes` / `other_admin_routes.master_roster_routes` / `canonical_authoring_order.{contents_pipeline,canvas_workspace_entry,post_contents_entry}` を raw-text 正規表現抽出し、`frontend/content/adminRouteIdentity.generated.ts`（新規、checked-in generated artifact）へ route identity（href + category、順序込み）のみを出力する。wording/copy は一切出力しない。`--check` mode で drift 検出（無書き込み、diff のみ）。
   - 導出順序: main_flow（contents→ui-builder→manifests、`canonical_authoring_order` 由来）→ master_roster（enums→users、`other_admin_routes.master_roster_routes` 由来）→ 残る `canonical_routes`（`/admin` 自身を除く、team-dashboard→scheduler）。この導出は既存の6カード実際の表示順と完全一致することを確認済み（team-dashboard は wording 不在によりカード側でフィルタされ、結果的に scheduler が最後に来る現行順序と一致）。
   - `frontend/content/adminGuides.ts`: `ADMIN_ROUTE_CARDS` を `ADMIN_CANONICAL_ROUTE_IDENTITY`（生成ファイル）から導出するよう変更。hand-authored wording は `ADMIN_ROUTE_CARD_WORDING`（href keyed record）へ再構成し、identity にあって wording に無い href は自動的に `ADMIN_ROUTE_IDENTITY_WITHOUT_WORDING`（新規export）へ現れる——silent omission ではなく明示的・追跡可能な状態にした。`/admin/team-dashboard` は現時点でこのリストの唯一のメンバーであり（既存日本語UX文言が repo 内のどこにも存在しないため）、文言執筆は `frontend-canonical-surface-structure-label-boundary` Bundle の責務として todo.md に記録し、本Bundleでは発明しない。
   - `ADMIN_MAIN_FLOW_STEPS` も同様に `ADMIN_MAIN_FLOW_ROUTE_ORDER`（生成ファイル、3ルート）から href/step/順序を導出するよう変更。main flow の3ルートは常に wording を持つべき前提のため、対応する wording が欠けている場合は**モジュール読み込み時に例外を投げる**（fail-close、ADMIN_ROUTE_CARDS側の「明示的unresolved状態」とは異なり、main flow stepper には許容されるsilent gapが無いという設計判断）。
   - `frontend/tests/adminMainFlow.test.ts`: round4の subset-onlyテストを置き換え、(a) 生成ファイルをSSOT raw-textから独立再導出したものと突き合わせる drift 証明（generatorスクリプトを呼び出すのではなく、TypeScript側で同じアルゴリズムを独立実装——2つの独立実装が一致することの方が「スクリプトが自分自身と一致する」より強い証拠になる）、(b) `ADMIN_ROUTE_CARDS` の順序・メンバーシップが `ADMIN_CANONICAL_ROUTE_IDENTITY` から構造的に導出されていることの証明、(c) `ADMIN_ROUTE_IDENTITY_WITHOUT_WORDING` が現在の追跡済み期待値（`["/admin/team-dashboard"]`）と厳密一致することの証明（新しい canonical route が無文言のまま追加された場合に fail する）、を追加。
   - `ADMIN_INDEX_GUIDE` 監査: 全フィールド（title/purpose/howToSteps/prerequisites/inputs/actions/outputs/boundaryNotes）を確認した結果、独自の href/order 配列を持たず全て wording prose であることを確認した。唯一の構造的（非wording）性質は、purpose/howToSteps 内で main-flow route の UX label が canonical_authoring_order の順序通りに言及されている点であり、これを wording 本文自体には触れずに順序のみ検証するテスト2件を追加した。
   - `docs/design/admin-console-workflow-ssot.yaml`（version 0.10.0 → 0.11.0）の `static_navigation_sourcing_contract` を、round4のtest-only設計から round5 の実装（generation + checked-in artifact + 複数fail-close証明）を反映する内容へ全面改訂。
   - 検証: `tsc --noEmit`（プロジェクトの実 `tsconfig.json` 設定を再現したフラグで、`frontend/content/adminRouteIdentity.generated.ts` / `adminGuides.ts` / `adminUxTerms.ts` / `adminMainFlow.test.ts` / `frontend/routes/admin/index.tsx` / `AdminMainFlowStepper.tsx` を対象）で **0件の実質的な型エラー**（Deno globalおよび `deno.land`/`preact` npm解決に起因する既知の環境ノイズのみ、既存ファイルでも同様に発生することを `adminUxGuard.test.ts` で比較確認済み）。ローカル `deno` 不在のため実行テストは `REQUIRED_NOT_EXECUTED`、remote CI green を要求する。

**round 5 実施結果:** gap taxonomy family 同期（design_change + 実装 + 4新規 checker test、164中163 pass、pre-existing 1件除く）、`ADMIN_ROUTE_CARDS`/`ADMIN_MAIN_FLOW_STEPS`/`ADMIN_INDEX_GUIDE` の実sourcing化（新規 generator + checked-in artifact + adminGuides.ts 再構成 + 6新規/更新 test）を同一PR内で完了。`dotnet test` 1445/1445 pass、`check_react_schema_topology_seed_translator.py` 163/164（既知の無関係な1件を除き pass）、`tsc --noEmit` 実質0エラー。**これは round 5 の checkpoint clear の記録であり、Bundle `admin-surface-topology-seed-conversion` 全体の "Implemented" 宣言や merge 承認ではない。** Bundle Status は引き続き `not_started`。残存: (a) 他4 subBundle 未着手、(b) `declared_seed_surface_catalog` 正式登録未実施、(c) topology UI seed の物理 manifest 到達点未解決、(d) `/admin/team-dashboard` の文言執筆（`frontend-canonical-surface-structure-label-boundary` Bundle 引き継ぎ、今回 identity としては sourcing 済みだが wording は明示的unresolved状態のまま）。

#### admin-dashboard round 6 監査（PR #594 owner review comment、round 5 差分の2件の未達を同一PR内で解消）

owner から round 5 の有効差分（`authoring_schema_vocabulary_gap` 同期・`ssot_ambiguity_gap`/`authoring_schema_vocabulary_gap` の2つの knownGapRefs・generated route identity の現行値）を SETTLED FACTS として再調査対象から除外した上で、残る2件の未達（`targetSurface` 未宣言時のSSOT rule完全実装、generated identityの親authorityとのcross-proof）を同一PR内で解消するよう指示があった。

1. **`input_format_contract.required_fields.targetSurface.rule` の完全実装 — 確定：真正の未達、実装+negative/positive proofで解消。**
   `docs/design/react-schema-topology-seed-translator-ssot.yaml` `input_format_contract.required_fields.targetSurface.rule` を逐語確認: 「must equal an existing declared_seed_surface_catalog[].seed_surface_key, **or be a new surface key explicitly paired with at least one known_gap_ref of classification ssot_ambiguity_gap or component_catalog_gap** in knownGapRefs」。round 5 実装の `validate_input_envelope` は `target_surface not in declared_keys and not known_gap_refs`（knownGapRefsが空でないことのみ）しか検証しておらず、SSOTが要求する「**特定の2分類のいずれか**」という条件を実装していなかった（真正の未達、owner指摘通り）。
   **実装内容:**
   - `.agent/scripts/react_schema_topology_seed_translator.py` に共有ヘルパー `gap_ref_classification()`（colon の有無で legacy/classified を判定）と定数 `UNDECLARED_SURFACE_QUALIFYING_CLASSIFICATIONS = {"ssot_ambiguity_gap", "component_catalog_gap"}` を追加し、`validate_input_envelope` の targetSurface検証を「未宣言 surface の場合、`knownGapRefs` のいずれかがこの2分類のいずれかであることを要求する」形へ書き換えた（旧: 非空性のみ）。colon-free legacy ref・未知分類・許可対象外分類（`authoring_schema_vocabulary_gap`/`runtime_dispatch_or_projection_gap` 等、実在するが「このsurface自体が未宣言である」ことを説明しない分類）はいずれも要件を満たさない。既存の宣言済みsurface（`auth.external.credential_management.projection`・`physical_search_crud_aggregate.v1`）は本チェックの対象外であり、それらの既存 colon-free legacy ref・任意分類は無変更・無影響（`declared_keys` に含まれるため target_surface 未宣言分岐に到達しない）。
   - `.agent/scripts/check_react_schema_topology_seed_translator.py` に新規 check 7g/7h/7i/7j/7k/7l を追加: 7g（knownGapRefsなし）・7h（未知分類のみ）・7i（許可対象内だが非該当分類——`authoring_schema_vocabulary_gap`——のみ）・7j（colon-free任意文字列のみ）はいずれも未宣言surfaceでは blocking `TARGET_SURFACE_UNRESOLVED` になることを確認する negative proof。7k は適格分類（`ssot_ambiguity_gap`）を持てば未宣言surfaceでも `gateStatus: pass` になることを確認する positive proof。7l は**チェックイン済みの admin-dashboard fixture 自体**（未宣言surface `admin.dashboard.hub_relation_navigation`、既存の `ssot_ambiguity_gap:...` エントリを維持——round5からの再分類なし）が本チェックを実際に通ることを確認する regression proof（この fixture が本 checker script の自動テストとして実行されるのは今回が初めて——round 2–5 は CLI 手動実行での `gateStatus: pass` 確認のみだった）。
   - 検証: `python3 .agent/scripts/check_react_schema_topology_seed_translator.py` 171 checks 中 170 OK、失敗は既知の pre-existing 「7a」のみ。`.agent/tests/check-schema-seed-translator-entry-gate.sh` 40/40 pass。`dotnet test backend/tests/Topolactor.Runtime.Tests` 1445/1445 pass（無変更）。既存の宣言済みsurface向けgolden fixture（credential-management-0092、physical-search-crud-aggregate）は無変更で pass を維持。
2. **generated route identity の親authorityとのcross-proof — 確定：round5の drift testは subordinate SSOTとの一致のみ証明しており、親authorityとの一致は未証明だった。cross-checkを追加して解消。**
   round 5 で追加した「adminRouteIdentity.generated.ts matches independent re-derivation from admin-console-workflow-ssot.yaml」テストは、generated artifact が **subordinate SSOT**（`admin-console-workflow-ssot.yaml`）と一致することのみを証明しており、subordinate SSOT 自身が拠り所とする **親authority**（`docs/design/runtime-orchestration-ssot.yaml` `frontend_routes.admin`——`admin-console-workflow-ssot.yaml` 自身の `authority.canonical_admin_route_registry` がこれを指定し、"MUST NOT add canonical admin routes outside that registry" と明記）との一致を独立に検証していなかった。親のみの追加・削除、subordinate-onlyの追加・削除、generated-onlyのdriftのいずれも round5 のテストだけでは検出できない構造的な穴だった（owner指摘の通り）。
   **実装内容:**
   - `.agent/scripts/generate_admin_route_identity.py` に `ADMIN_INDEX_ROUTE = "/admin"` という named constant（旧: inline の bare string比較）、`extract_parent_admin_routes()`、`check_parent()`、`--check-parent` CLI mode を追加。親の `frontend_routes.admin` から `/admin` を除いた集合と、生成済み identity の href集合を比較し、parent-only/identity-only/重複を個別に報告する。
   - `frontend/tests/adminMainFlow.test.ts` に新規 test「ADMIN_CANONICAL_ROUTE_IDENTITY route set matches parent runtime-orchestration-ssot.yaml frontend_routes.admin (excluding /admin itself)」を追加: 親SSOT `frontend_routes.admin` を raw-text抽出し、`/admin` を除いた集合と `ADMIN_CANONICAL_ROUTE_IDENTITY` の href集合をソート済み配列で比較（重複検出も別途assert）。既存の「Fresh /admin route registry matches runtime-orchestration SSOT exactly」（親SSOT ↔ 物理Freshルートツリー）・round5の drift test（subordinate SSOT ↔ generated artifact）と合わせて、4つのauthority（親SSOT・subordinate SSOT・generated artifact・物理Freshルートツリー）がペアワイズで相互チェックされる構成にした。
   - `docs/design/admin-console-workflow-ssot.yaml`（version 0.11.0 → 0.12.0）の `static_navigation_sourcing_contract` に、`/admin` 除外規則を明示契約として記載（`generation.admin_index_exclusion_rule`）し、`source_authority`/`missing_source_fail_close`/`proof_surface` へ親authorityとのcross-checkを追記した。
   - 検証: `python3 .agent/scripts/generate_admin_route_identity.py --check` / `--check-parent` いずれも `OK`。`tsc --noEmit`（実プロジェクト設定を再現したフラグ）で `frontend/content/adminRouteIdentity.generated.ts` / `adminGuides.ts` / `adminMainFlow.test.ts` に実質0エラー。

**round 6 実施結果:** `targetSurface` 未宣言時の分類要件を完全実装（実装 + 6新規 checker test、171中170 pass、pre-existing 1件除く）、generated route identityと親authorityの直接cross-proofを追加（generator拡張 + 新規 frontend test + SSOT明文化）を同一PR内で完了。`dotnet test` 1445/1445 pass（無変更）、`check_react_schema_topology_seed_translator.py` 170/171、`check-schema-seed-translator-entry-gate.sh` 40/40、`tsc --noEmit` 実質0エラー。**これは round 6 の checkpoint clear の記録であり、Bundle `admin-surface-topology-seed-conversion` 全体の "Implemented" 宣言や merge 承認ではない。** Bundle Status は引き続き `not_started`。残存（round4/5から変更なし）: (a) 他4 subBundle 未着手、(b) `declared_seed_surface_catalog` 正式登録未実施、(c) topology UI seed の物理 manifest 到達点未解決、(d) `/admin/team-dashboard` の文言執筆（`frontend-canonical-surface-structure-label-boundary` Bundle 引き継ぎ）。

#### admin-dashboard round 7 監査（PR #594 owner review comment、`/admin`存在proof・qualifying classification構造化・physical adoption境界の大規模調査）

owner から round 6 の有効差分（targetSurface classification rule実装・6件のproof・parent route集合cross-check・`table.primitive`・generated identity consumer・generic navigation・CI green）を SETTLED FACTS として維持した上で、4件の未達（`/admin`除外proofの欠落経路、qualifying classificationのSSOT構造化、physical adoption境界の誤認、Bundle継続）を同一PR内で解消するよう指示があった。前3件は実装で解消し、4件目（他4 subBundleとphysical adoptionそのものの実装可否）は徹底調査の上で明確な理由とともに報告する（owner自身が明示的に許容した対応: 「adoption不能と判定する場合は…調査した…adoption pathと不適合理由を一対一で報告する」）。

1. **`/admin`除外proofの欠落経路 — 確定：真正の欠落、実装で解消。**
   `.agent/scripts/generate_admin_route_identity.py` の `derive_identity()` は `canonical_routes` から `/admin` を除外するループを持つが、そもそも `/admin` が `canonical_routes` から削除された場合、除外ループは何もスキップしなくなるだけで**出力に一切変化がない**——`--check`（subordinate SSOTとの一致）も `--check-parent`（親route集合との一致、`/admin`を除いた集合同士を比較）も、この削除を検出できないことを実際に再現して確認した（`canonical_routes` から `/admin` の行を一時的に削除し `--check` を実行 → 元は `exit 0` だったところ、修正前のコードでは検出されないことを確認してから、修正後に同じ削除で `exit 1` になることを確認）。
   **実装:** `extract_canonical_routes()` に「`ADMIN_INDEX_ROUTE`（`/admin`）が抽出結果に存在しなければ `SystemExit`」という明示assertionを追加（`--check`・`--check-parent` 両方がこのSystemExitを通じて即座に fail するようになった、共通の抽出関数を経由するため）。`frontend/tests/adminMainFlow.test.ts` の独立再導出関数 `deriveExpectedAdminRouteIdentity()` にも同じassertionを追加（CIで実際に実行されるのはこちらであり、Python generatorはローカル開発ツールに過ぎないため、CI側にも同じ保証を持たせる必要がある）。実際に `/admin` を一時削除して両方が `exit 1`/throw することを確認後、元に戻して green を再確認した。
2. **qualifying classificationのSSOT構造化 — 確定：真正の未達、design_change + 実装で解消。**
   round 6 で実装した `UNDECLARED_SURFACE_QUALIFYING_CLASSIFICATIONS = {"ssot_ambiguity_gap", "component_catalog_gap"}` は Python内のみのhardcoded定数であり、SSOT側（`input_format_contract.required_fields.targetSurface.rule`）には「`ssot_ambiguity_gap` または `component_catalog_gap`」という2つの名前がprose内にinlineで書かれているだけで、構造化フィールドとしては存在しなかった（owner指摘通り）。
   **実装:** `docs/design/react-schema-topology-seed-translator-ssot.yaml`（version 1.2.0 → 1.3.0）の `input_format_contract.required_fields.targetSurface` に構造化フィールド `undeclared_surface_qualifying_classifications`（list、`ssot_ambiguity_gap`/`component_catalog_gap`）を追加し、`rule` テキストをこのフィールドを参照する形へ書き換えた。`.agent/scripts/react_schema_topology_seed_translator.py` の `validate_input_envelope` は、Python定数の代わりに新規ヘルパー `undeclared_surface_qualifying_classifications(ssot_root)`（`allowed_gap_classifications()` と同じ `dig()` パターン）経由でこのSSOTフィールドを読むよう変更。`.agent/scripts/check_react_schema_topology_seed_translator.py` に新規 check 7d1 を追加: このqualifying subsetが親 `ui-builder-seed-first-gap-discovery-ssot.yaml canonical_gap_types` の**真部分集合**であることをraw-text独立抽出で証明（7d0/7dと同じ discipline）。round6で追加した6件のtargetSurface test（7g–7l）は全て維持され、SSOTフィールド経由の値に対して自動的に再接続された（実装がSSOTを読む経路を変えただけで、テスト側のアサーション対象は変わらない）。
   検証: `check_react_schema_topology_seed_translator.py` 172 checks 中 171 OK（既知の pre-existing 「7a」のみ）。`check-schema-seed-translator-entry-gate.sh` 40/40。
3. **physical adoption境界 — 徹底調査完了。結論: 実装可能な具体的経路を1つ特定したが、共有ランタイムコード（`ProjectionShell.tsx`）と稼働中の実manifest（092）へ影響する変更のため、本ラウンドでは実装せず、確定した経路を報告する。他4 subBundleへの継続は、Bundle自身の既存進行ゲートにより現時点で不可能であることを確認した。**

   **3-1. `surface_axes.admin.surfaces.dashboard` の性質の再確認（設定済み事実の追加確認、再判定ではない）:** `axis_kind: projection_surface_axis`, `not_role_or_rbac: true` であり、`data_authority_ref: hubs.hub_relations`（`topology_manifest_id` で明示的にスコープされる）——これは特定のmanifestに紐付かない**再利用可能な汎用UIパターン**（「あるmanifestが自分自身の outbound hub_relations を検索・一覧・遷移表示する」ウィジェット）であり、`/admin` 固有のコンテンツではない。この理解はround1–6の実装方向（React Schema/translator/composer proof）と整合しており、re-litigationではなく、physical adoption境界を判定するための前提確認である。

   **3-2. 決定的な発見: `capability_requirements.search`/`filter` は既存ランタイムに一切実装されていない。** `frontend/runtime/projectionEntry.ts` の `resolveHubNavigationLinks()` と `frontend/islands/ProjectionShell.tsx` の `<nav data-projection-hub-navigation>` レンダリングを実際に読んだ結果、既存の hub navigation 表示は「バックエンドが既に解決済みの `navigationSequence` 配列を、フラットな未フィルタのリンク一覧としてそのまま描画するだけ」であることを確認した——search input、table、フィルタリングロジックは一切存在しない。round 2–6で確定した「hub relation navigationは既存汎用substrateで完全に充足される」という結論は**遷移機構（クリック→正しいmanifestへ移動）についてのみ正しく**、`surface_axes.admin.surfaces.dashboard` が要求する search/filter capability については充足されていない、真正で具体的なgapである（既存汎用substrateを理由に必要な機能を省略しているのではなく、その機能自体が存在しないことを実際にコードを読んで確認した）。

   **3-3. `admin-normal-surface-projection-seed-ssot.yaml seed_physical_hierarchy_and_definition` の精読による重要な整合:** `seed_definition.rule` は「"seed" とは...ある surface について、hub_relation/topology/hub/package にわたる物理DB行を意味し、/admin/contents + /admin/ui-builder + /admin/manifests を実際に使った場合に得られるのと同じ形の、事前構築済みの初期サンプルデータである」と明記している——つまり物理 seed の期待される到達形態は「実際にauthoring toolを使ったのと同じ形の物理行」である。一方、`.agent/tasks/todo.md` 自身の `admin-dashboard` SubBundle scope（「含めないもの」）は「route registry authority の新規定義」を明示的に禁止している。この2つの制約は、**admin-dashboard自身の新規専用manifest/routeを作ることは禁止されるが、既に実在する（ui_projectionを持つ）manifestへ、この汎用ウィジェットパターンを追加することは、新規routeを作らず、かつ物理行としての形も満たすため、両制約と矛盾しない**、という形で整合する。

   **3-4. 検討した具体的候補と判定:**
   - **候補A: manifest 092（credential-management）のui_projectionへ、search_input.alias/table.primitive/panel.aliasの構造（round1–6で既にbyte-exact証明済みの7-record候補と同じ形）を追加する。** manifest 092は既に実在するui_projectionと自分自身のhub_relations（self-referencing、round1–3で確定済み）を持ち、`subbundle_target_readiness.credential-management`（`docs/design/admin-console-workflow-ssot.yaml`）は「manifest 092は既にreal ui_projectionを持つ——target側readiness blockerなし」と明記している。**構造的には合法な候補と判定する**（新規route不要、fake manifestではない、既存の実manifestへの追加）。ただし3-2の search/filter gap が未解消のため、この候補単独では機能的に完全な物理採用にならない——検索できないsearch_input、フィルタできないtableを実manifestの実ページに追加することは、動作しないUIを実際の管理画面に表示することになり、これまでの全ラウンドで一貫して禁止されてきた「構造だけの証明を機能完成の証拠として扱う」ことの逆パターン（構造を機能未完成のまま実環境に混入させる）に相当するため、単独では実施しない。
   - **候補B: search_input.alias/table.primitiveのために新しいbackend dispatch actionを追加し、hubs.hub_relationsに対する実検索/フィルタクエリを構築する。** 技術的に可能だが、新しいdispatch action・wiring lane・backend query実装を要する、本ラウンド発見時点での即時実装には大きすぎる新規機能追加であり、専用のdesign_changeとして別途設計判断（action key命名、wiring lane選定、既存`content_bundle:search`パターンとの関係）を要する。
   - **候補C（本ラウンドで新たに特定した、より小さく低リスクな代替案）: `frontend/islands/ProjectionShell.tsx` の `hubNavigationLinks`（既にemissionとして読み込み済みの、全manifest共通の配列）に対して、新しいbackend actionを追加せず、クライアントサイドのフィルタ（search_inputの入力値で `label` を絞り込むだけ）を追加する。** `capability_requirements.search: [relation_label_or_manifest_metadata_when_available]` は「label」による検索であり、既にemissionに含まれる小さな配列に対するクライアントサイドフィルタで意味的に満たせる可能性が高く、新しいbackend authority・dispatch action・DB queryを一切必要としない。汎用コンポーネント（全manifestが共有する `ProjectionShell.tsx`）への追加であり、admin-dashboard固有のhackにはならない（reusable abstraction、one-off実装ではない）。
   - **結論:** 候補Cが最も低リスクで、SSOT根拠（capability_requirements）に直接裏付けられた、実装可能な具体的経路として特定できた。ただし、これは全manifestが共有する中核ランタイムコード（`ProjectionShell.tsx`）への変更と、稼働中の実manifest（092、credential-managementの実データ）への追加変更を伴う。これまでの6ラウンドで行ってきた変更（このBundle専用のtranslatorスクリプト・fixture・frontend content file・自作generatorスクリプト）と異なり、システム全体で共有される実行時コードと、既に稼働中の実manifestのライブコンテンツの両方に影響する変更であるため、本ラウンドでは実装を見送り、この具体的な経路を報告する形に留めた（"設計変更が必要と判断した場合は、実装を先行させず、根拠となるSSOT間の衝突箇所を報告する" という本エンゲージメント全体の一貫した指示、および全体のsystem-level instructionにある「共有システムに影響する、元に戻しにくい変更は透明に伝え、確認を得てから進める」という原則に従う）。次ラウンドでの実装権限確認を求める。

   **3-5. `declared_seed_surface_catalog` 登録 — 引き続き未登録が正しい。** owner指示「登録はphysical adoption evidenceと同期して行う」に従い、3-4で adoption evidence がまだ存在しない（候補Cが未実装）以上、登録は行わない。round2–6からの継続的な正しい判断。

   **3-6. `hub_relation_navigation_binding` の `authoring_schema_vocabulary_gap` — 本ラウンドでの解消は不要と再確認。** `topology_seed_schema_contract.minimum_required_fields.hub_relation_navigation_binding` は最終的な物理seedが表現すべき概念の形を定義するが、`topology_manifest_identity` 直下の記述「this SSOT does not allocate UUIDs or SQL rows」および `seed_definition.translator_role`「it never owns seed adoption authority」から、この概念自体は**既存の実manifest（092等）が既に持つ実 `hubs.hub_relations` row + 汎用dispatcher解決機構**によって、新しい seed record を追加しなくても既に体現されている（round2–3で確定済み）。よって、search_input/table.primitive/panel.aliasの構造3ノードの物理採用（候補A/C）に対して、`authoring_schema_vocabulary_gap` の解消は前提条件ではない——同gapは新規manifest（既存hub_relationsを持たないもの）に対してのみ前提条件になり得るが、候補A/Cはいずれも既存manifest（092）を対象とするため、このgapは引き続き未解決のまま維持してよい。round4–6の判断を維持する。

   **3-7. 他4 subBundleへの継続 — 既存の進行ゲートにより、本ラウンドでは不可能と確認した（新たな制約ではない）。** `.agent/tasks/todo.md` 自身の「Owner pause lifted」節（2026-07-18記録）に、以下が**既に明文化されている**（本ラウンドで新規に発見した制約ではなく、既存の記録を正確に再確認した）: 「進行ゲート（subBundle単位、必須）: seed generation / seed registration は...現時点でこの条件を満たすのは `admin-dashboard`（5 id すべて `subBundle_not_applicable`）のみ。他の4 subBundle（credential-management / admin-enum / team-dashboard / scheduler-settings）はそれぞれの残 `unresolved_before_seed` を個別に解消（またはSSOT裏付けのある `subBundle_not_applicable` 証明）してから着手する。」——具体的には: `admin-enum`/`team-dashboard`/`scheduler-settings` は各画面固有の `ui_projection` target manifestそのものが未作成（`unresolved_before_seed`）、`credential-management` は target manifest(092)自体はresolvedだが「explicit navigation binding authoring / verification」が `unresolved_before_seed` のまま残っている。これらはいずれも本ラウンドの調査で新しく解決されたものではなく、それぞれ専用のdesign_change/実装（3画面分のtarget manifest新規作成、credential-managementのnavigation binding authoring/verification）を要する、admin-dashboardとは独立した作業である。これを本ラウンドで同時に着手することは、Bundle自身が明示的に定めた進行ゲートに違反するため行わない。checkpoint clear・CI green・admin-dashboard単独のproofをこれらのsubBundleのgate解除や、Bundle全体のcompletionとして扱うことはしない。

**round 7 実施結果:** `/admin`存在fail-close実装（generator+frontend test）、qualifying classificationのSSOT構造化実装（design_change + 実装 + 新規checker test 7d1）を完了。physical adoption境界は徹底調査の上、実装可能な具体的経路（候補C: manifest 092 + client-side label filter）を特定したが、共有ランタイムコード・稼働中manifestへの影響を理由に本ラウンドでは実装せず報告に留めた。他4 subBundleへの継続は、Bundle自身の既存進行ゲート（2026-07-18記録）により現時点で不可能であることを確認した（新たな制約の発見ではない）。`dotnet test` 1445/1445 pass（無変更）、`check_react_schema_topology_seed_translator.py` 171/172、`check-schema-seed-translator-entry-gate.sh` 40/40、`tsc --noEmit` 実質0エラー。誤解を招く可能性のあった2箇所のコメント（`LayoutSchemaStructuralCompositionTests.cs`・`layoutSchemaStructuralRender.test.ts`、いずれも「/adminにmanifestがないから恒久的にtest-only」という誤った一般化）を訂正した。**これは round 7 の checkpoint clear の記録であり、Bundle `admin-surface-topology-seed-conversion` 全体の "Implemented" 宣言や merge 承認ではない。** Bundle Status は引き続き `not_started`。残存: (a) 他4 subBundle未着手（既存進行ゲートにより本ラウンドでは着手不可）、(b) admin-dashboard自身のphysical adoption（候補C、owner確認待ち）、(c) `declared_seed_surface_catalog` 正式登録（adoption evidence待ち）、(d) `/admin/team-dashboard` の文言執筆（別Bundle引き継ぎ）。

#### admin-dashboard round 8 監査（PR #594 owner review comment、round 7 physical adoption判定の誤認訂正、SQL Attention Projection発見）

owner から round 7 の3-2（「search/filter は既存ランタイムに一切実装されていない」）と3-4候補A/C（manifest 092への構造追加をphysical adoption候補、client-side label filterをProjectionShell.tsxへ追加する必要ありとした結論）が、GPT監査側の誤認に基づくものであるとの指摘があった。実際にコードを読み直して独立に検証した結果、指摘は正しかった。以下、訂正と再判定を記録する。

1. **SETTLED FACTS として確定（owner指示通り、再訂正の対象ではない）:** hub relation projection axesは `admin` と `normal` の2軸である。`credentials` と `dashboard` はいずれも `admin` axis配下のsurfaceである（`docs/design/admin-normal-surface-projection-seed-ssot.yaml surface_axes.admin.surfaces`）。manifest 092は credential-management自身の `hubs.topology_manifests` row であり、hub_relationそのものではなく、admin-dashboardが汎用的に流用できるcarrierでもない。`hubs.hub_relations` はmanifestスコープのnavigation edgeであり、manifest 092というrow自体とは別概念である。SQL Attentionには既存の `hubs.hub_relations` に対するsearch/filter/ranking/projection経路（active conditions, score, sequence, topK, neighbor score, recent window）が実装済みである。frontendにはSQL Attention projection blockと、backend notify契機の再取得・再描画経路が既に存在する。

2. **訂正1: manifest 092は admin-dashboard candidateのphysical adoption targetではない（round 7の3-4候補A・候補Cを撤回）。** round 7の3-4は「manifest 092は既に実在するui_projectionを持つため、そこへsearch_input/table.primitive/panel.aliasの構造を追加することは構造的に合法」と判定したが、これはmanifest 092がcredential-management自身の専有topologyのrowであり、他subBundle（admin-dashboard）のcontentを追加してよい汎用carrierではないという事実を見落としていた。この判定は撤回する——manifest 092の実データへ admin-dashboard 由来の構造を追加することは、そもそも候補として成立しない。

3. **訂正2: 「search/filter capabilityは既存ランタイムに一切実装されていない」という3-2の結論を撤回。** 実際には `docs/design/sql-attention-logs-ssot.yaml` を実体とする、既存の相当規模な「SQL Attention」システムが、manifestスコープの `hubs.hub_relations` に対する探索・スコアリング・ランキング・projectionを既に提供している: `neighbor_score_policy`、`topK`選定、`active_condition`、`recent_window`、EMAスコア、fail-close（`NoHubRelations`）。frontend側は `frontend/components/SqlAttentionProjectionBlock.tsx`（fetch経路: `frontend/api/sqlAttentionProjection.ts`）と `frontend/components/SqlAttentionProjectionPanel.tsx`（ランク付き `TopologyProjectionCandidate[]` を `attentionScore`/`scoreBand`/evidence内訳とともにレンダリング）が、`frontend/components/RecommendNavigationIsland.tsx` 経由で `frontend/islands/ProjectionShell.tsx`（568-573行目、`recommendProjection = emission?.recommendNavigationProjection`）にマウントされている——**round 7で読んだのと同じファイルであり、スクロールが足りずSqlAttention関連セクションを見落としていた。** backend側は `backend/schema/SqlAttentionContracts.cs`、`backend/runtime/SqlAttentionTopologyProjectionRuntime.cs`（`policy.TopK`/`policy.MinNeighborScore`/`policy.RecentWindowDays`）、`backend/runtime/RuntimeExecutor.cs` 228-266行目（Step 9: `RecommendNavigationProjectionSpec.FromRecommendation` を毎dispatchで無条件に `workingShape.RecommendNavigationProjection` へ代入）で構成される。この誤りは、owner指摘にある通り「GPT監査側の誤認」が発端だが、それを鵜呑みにせず自らコードを読んで独立に検証した。3-2で「実際にコードを読んで確認した」と書いた行為自体は事実だが、読んだ範囲（`resolveHubNavigationLinks`と`<nav>`部分）が不十分であり、`ProjectionShell.tsx`の別セクション（SQL Attention関連マウント部）を見落としていた、という調査範囲の不足が誤りの直接原因である。

4. **owner指摘の重要な留保: 共有ランタイム・稼働中manifestへ手を付けずに止まった判断自体は正しく、保護的だった。** round 7は3-2/3-4の誤った前提の上でも、`ProjectionShell.tsx`や稼働中manifest 092への実装を本ラウンドでは行わず、報告に留めるという判断をした。この「実装せず止まる」という決定は、たとえ根拠の一部が誤っていたとしても、結果として不適切な設計を本番へ混入させることを防いだ、correctかつprotectiveな判断だった。この停止判断そのものは失敗ではなく、訂正の対象外である。

5. **round 7の他の実装（`/admin`存在fail-close、qualifying classification構造化）は本訂正と独立に有効であり、revertしない。** これらはphysical adoption判定の誤りとは無関係な、別個の確定済み実装であり、そのまま維持する。

6. **admin-dashboard candidateの残存必要性の再判定（manifest 092 adoptionを前提とせず）:** `capability_requirements.search`/`filter` が要求する機能的意図を、既存ランタイムの実際の提供内容と比較した。SQL Attention Projectionはmanifestスコープの `hubs.hub_relations` に対する探索・スコアリング・ランキングを提供するが、自身のコードコメント（`SqlAttentionProjectionPanel.tsx`: "Candidates are read-only recommendation guidance; users may navigate or act externally"）および `docs/design/runtime-orchestration-ssot.yaml` の `sql_attention_relation_recommend_candidate_only`/`sql_attention_or_recommendation_must_not_overwrite_route_state_automatically` が明示する通り、read-onlyのrecommendation guidanceであり、authoritativeなsearch-and-navigate機構ではない——`surface_axes.admin.surfaces.dashboard.seed_contract` が定義するsearch_input.alias/table.primitive/panel.aliasという具体的widget形状の、文字通りの代替物ではない。一方で、(a) round 2–3で確定済みの汎用hub-navigation `<nav>` クリック遷移経路、(b) SQL Attention Projectionによるmanifestスコープの探索・スコアリング・ランキング・projection、(c) admin-dashboard subBundle自身の明示的scope境界（`.agent/tasks/todo.md` Bundle定義: 「含めないもの」に「business projectionとしての/admin扱い」「route registry authorityの新規定義」を明記）、の3点を総合すると、`capability_requirements.search`/`filter` が意図する機能的な狙い（hub_relationsに対する何らかの探索・絞り込み・遷移手段の提供）は、admin-dashboard candidateがどこにも物理採用されない状態でも、既に稼働中のランタイムによって実質的にカバーされている、という評価に至る。**ただし、これは「seed_contractの特定widget形状が不要になった」という主張ではなく、「その形状で新たに物理採用すべき対象・gapが、re-judgeの結果として見当たらなくなった」という限定的な結論である。** 候補A（manifest 092）は3-3の性質理解（汎用UIパターンという理解自体）はround1-6の実装方向と整合するため撤回しないが、adoption先としての092は誤りであり、他に代替candidateとなる実manifestを本ラウンドで新たに特定してはいない。候補C（ProjectionShell.tsxへのclient-side label filter追加）は、SQL Attention Projectionという既存の同種機能（探索・スコアリング・projection）がある以上、新たに並行する検索レーンを追加する動機が失われたため、提案として撤回する。
7. **結論: admin-dashboard candidateのphysical adoptionは、本ラウンド時点で具体的な実装対象・実装経路を持たない。** round 7が識別した候補A（manifest 092）・候補C（client-side filter）はいずれも撤回される。新たな候補も本ラウンドでは特定しない——他manifestへの物理採用可否は、対象となる具体的なmanifestが提示されない限り判定不能であり、それを本ラウンドで想像・仮定して判定することは「unobservable design intentの断定」に該当するため行わない。この状態は「blocked（owner確認待ち）」ではなく、「本ラウンド時点で識別可能な採用対象がない」という状態として記録する——continued blockingという誤ったフレーミングも避ける。

**round 8 実施結果:** owner指摘のSQL Attention Projection発見を独立に検証し（`docs/design/sql-attention-logs-ssot.yaml`、`SqlAttentionProjectionBlock.tsx`/`Panel.tsx`、`RecommendNavigationIsland.tsx`、`ProjectionShell.tsx` 568-573行目、`RuntimeExecutor.cs` Step 9、`SqlAttentionContracts.cs`）、round 7の3-2（「search/filter実装なし」）・3-4候補A/C（manifest 092 adoption・client-side filter）を誤認として撤回した。round 7の停止判断（実装しなかったこと）自体はcorrect/protectiveだったと確認した。round 7の他の実装（`/admin` fail-close、qualifying classification構造化）は無変更・有効のまま維持。誤解を招く2箇所のtest comment（`LayoutSchemaStructuralCompositionTests.cs`・`layoutSchemaStructuralRender.test.ts`）を最小差分で訂正した（production code・SSOT構造・test assertionには一切変更なし、コメントのみ）。**これは round 8 の checkpoint clear の記録であり、Bundle `admin-surface-topology-seed-conversion` 全体の "Implemented" 宣言や merge 承認ではない。** Bundle Status は引き続き `not_started`。残存: (a) 他4 subBundle未着手（既存進行ゲートにより本ラウンドでは着手不可）、(b) admin-dashboard自身のphysical adoption対象は本ラウンド時点で未特定（将来、具体的な既存manifestが提示された場合に再判定）、(c) `declared_seed_surface_catalog` 登録は引き続き見送り（adoption evidenceなし）、(d) `/admin/team-dashboard` の文言執筆（別Bundle引き継ぎ）。

#### admin-dashboard round 9 監査（PR #594 owner review comment、gate0再監査: 「実装可能」を「実装必要」へ変換しない、SSOT上のadmin-dashboard seed_contract必要性の精密な確定/未確定分離）

owner から、round 8はaudit lineageの訂正であり誤った実装は入っていないことを確認した上で、round 8だけではSSOT上の責務境界・seed_contractの必要性・既存SQL Attention/UI Builderとの充足関係・physical adoption/catalog registrationの要否は未確定のままだという指摘があった。「UI Builderで設定可能」を「実装が必要」へ変換しないこと、SSOTから一義に確定できない場合は実装せずOwnerへ質問することが明示的に要求された。本ラウンドは実装を伴わない、フィールドレベルの精密比較監査である。

1. **本ラウンドで新規に読んだSSOT:** `docs/design/admin-uibuilder-ui-structure-wiring-ssot.yaml`（UI Builder layout/wiring mode境界、runtimeInteractions authority、trigger vocabulary、idempotency）、`docs/design/ui-builder-preset-ecosystem-ssot.yaml`（preset seed contract群、`hub_search.readonly.v1`を含む）。既読の `admin-console-workflow-ssot.yaml`・`admin-normal-surface-projection-seed-ssot.yaml`・`sql-attention-logs-ssot.yaml`・`runtime-orchestration-ssot.yaml` は該当section再読で再確認（re-litigationではなく、フィールドレベル比較のための再確認）。

2. **admin-dashboard seed_contractの確定事項（SSOT逐語）:** `admin-normal-surface-projection-seed-ssot.yaml surface_axes.admin.surfaces.dashboard` は次を定義する — `data_authority_ref: hubs.hub_relations`；`operations: [read, search, filter, navigate]`；`capability_requirements: {read: required, search: [relation_label_or_manifest_metadata_when_available], filter: [status, source_manifest, sequence_position], mutation: none_in_this_surface, navigation: required}`；`hub_relation_navigation_binding.selected_link_payload_required: [hub_relation_id, topology_manifest_id, related_hub_id]`；`seed_contract.component_tree`: `hub_relation_search`(search_input.alias) / `hub_relation_link_list`(table.primitive) / `target_projection_shell`(panel.alias)。この具体的component_tree・フィールド要件は、現在いかなる本番経路にも物理的に存在しない（candidate/fixtureのみ、round1-6で確定済み・re-litigation対象外）。

3. **既存経路のフィールドレベル精密比較（本ラウンドの中心作業、round 8では未実施）:**
   - **汎用hub-navigation `<nav>`（`ProjectionShell.tsx` / `resolveHubNavigationLinks`）:** 使用DTOは `HubRelationNavigationLinksResponseDto` → `HubNavigationSequenceItemDto`（`backend/schema/ContentBundleContracts.cs` 198-203行目）で、フィールドは `relatedHubId` / `relatedHubLabel` / `sequencePosition` / `targetManifestId` のみ。`hub_relation_id` と `status` を一切持たない。navigate=充足（クリック→`?manifest=<id>` href、round2-3で確定済み）。read=部分充足（一覧は取得できるがrelation自身のidとstatusが無い）。search/filter=UI実装なし（round7-8で確定済み、re-litigation対象外）。**加えて本ラウンドで新規確認: このDTOの形状そのものが `hub_relation_navigation_binding.selected_link_payload_required`（`hub_relation_id`・`topology_manifest_id`・`related_hub_id`）を満たしていない — search/filter UIを仮に追加しても、現行DTOのままでは `select_hub_relation_link` イベントの必須payload要件を満たせない。**
   - **SQL Attention Projection（`SqlAttentionProjectionBlock.tsx`/`Panel.tsx`）:** 使用DTOは `TopologyProjectionCandidate`（`backend/schema/SqlAttentionContracts.cs` 355-367行目）で、フィールドは `HubId` / `AttractorKey` / `RelationRegistryId` / `AttentionScore` / `ScoreBand` / 各evidence JSON / `HitRank`。**`HubRelationId` を持たない**（`HubId`はhub、`RelationRegistryId`はregistry空間の別id—`sql-attention-logs-ssot.yaml sql_attention_target.not_target: [topology, registry]` と整合する別concept）。`SqlAttentionProjectionPanel.tsx` を実際に読んだ結果、`onClick`/`href`/navigate相当のwiringは一切存在しない——round8で「read-only recommendation guidance」と記録した通り、文字通りクリック遷移を提供しない。ranking/scoringはtopK・EMA score・active_condition・recent_windowによる「次に見るべき候補の推薦」であり、`capability_requirements.filter: [status, source_manifest, sequence_position]` とは異なる軸（score帯・鮮度）でのフィルタである。**結論: SQL Attention Projectionは read はあるが navigate=なし、filter軸が異なる、payload形状もhub_relation_navigation_bindingを満たさない——round8の「機能的な狙いは実質的にカバーされている」という評価は方向性としては誤りではないが、フィールドレベルでは navigate と正確な filter 軸の両方に実際のギャップがある、というより精密な評価に訂正する。**
   - **admin authoring経路（`hub_navigation:get_hub_relations`、`/admin/manifests` HubNavigationAdmin.tsx）:** 使用DTOは `HubNavigationHubRelationItemDto`（`backend/schema/ContentBundleContracts.cs` 153-161行目）で、フィールドは `hubRelationId` / `topologyManifestId` / `relatedHubId` / `relatedHubLabel` / `sequencePosition` / `relationConfig` / `status` — **`hub_relation_navigation_binding.selected_link_payload_required` と `capability_requirements.filter: [status, ...]` を完全に満たすフルフィールドは、実は既にこのadmin authoring経路のDTOとして本番に存在する。** ただしこの経路は `admin_hub_relation_navigation_contract.authoring` の一部として設計されたCRUD authoring用action（create/update/deprecate/reorderと同列）であり、read-only browse/search/filter UIとして一般化・転用された実績はない。

4. **UI Builder既存能力（能力確認と必要性判定の分離）:** `admin-uibuilder-ui-structure-wiring-ssot.yaml` / `ui-builder-preset-ecosystem-ssot.yaml` を読んだ結果 — UI Builderは component palette・layout canvas・wiring canvas・propBindings・package wiring（`PackageWiringEditor` → `manifest:list_screen_read_query_wiring` → `screenReadQueryWiring` candidates）・preset intakeを提供するが、**`PackageWiringEditor` の manifest targetは `screenReadQueryWiring`（content読み取りクエリ）のみを候補とし、`hubs.hub_relations` に対する検索/フィルタをwiringする経路は存在しない。** 既存の `hub_search.readonly.v1` preset（`ui-builder-preset-ecosystem-ssot.yaml hub_search_preset`、`status: active_seed`）は `search_input.alias` + `button.primitive` + `panel.alias` + `card_list.primitive` という、seed_contractのcomponent_treeに近い形状を持つが、**canonical dispatch は `content_bundle:search`（`hub:search` はSSOT非認可と明記）であり、data_authority は `hubs.hub_relations` ではない。widget形状の一致は、data_authority/機能契約の一致を意味しない。** これは owner のNG軸「`hub_search.readonly.v1`が存在することを、admin-dashboard用preset追加の根拠とする」を裏付ける具体的証拠として確認した——このpresetをadmin-dashboardへ転用することはしない。UI Builderで新しいwiring/preset/lane設定が「可能」であることは、本ラウンドで新規実装の根拠として使用しない。

5. **manifest 092 ownership判定（SSOT ownership契約から、row存在ではなく）:** `admin-console-workflow-ssot.yaml admin_hub_relation_navigation_contract.subbundle_target_readiness.credential-management`: 「manifest 092 already exists with a real ui_projection — no target-side readiness blocker」——これはmanifest 092がcredential-management subBundle自身のtarget manifestとして明示的に位置付けられている記述であり、他surfaceが汎用carrierとして流用してよいという記述はどこにも存在しない。**この点は一義的に確定できる: manifest 092はcredential-managementの専有target manifestであり、admin-dashboardのadoption先ではない。** round8の訂正を維持する。

6. **SSOT再発防止の不足（本ラウンドで新規確認、design_change候補として整理・Owner判断待ち、SSOT本文は書き換えない）:**
   - `admin-normal-surface-projection-seed-ssot.yaml` 全文に `sql-attention-logs-ssot.yaml` への参照が一切ない（grep確認済み、0件）。この surface の owning SSOT から SQL Attention authority を発見する経路が存在しないため、将来の監査者がround7と同じ見落としを再現する構造的リスクが残っている。
   - 「あるmanifestのui_projectionを他surfaceのcontentのcarrierとして流用してはならない」という一般境界を明文化したSSOT記述がどこにも存在しない（grep確認済み）。現状は `subbundle_target_readiness.credential-management` の個別記述からの黙示的推論でのみ防止されている。
   - `HubRelationNavigationLinksResponseDto`（汎用nav用の意図的最小DTO）と `HubNavigationHubRelationItemDto`（admin authoring用のフルDTO）の分離はコードコメントでのみ説明されており、「最小DTOにフィールドが無いことからrepository全体にその機能が存在しないと結論しない」という一般原則がSSOT側に明文化されていない。
   - これら3点は具体的なdesign_change候補として整理したが、正確な文言・配置先セクションはOwner判断を要するため、本ラウンドではSSOT本文を変更しない。

7. **admin-dashboard seed_contractの新規実装必要性判定（design-opaqueとして扱う、実装しない）:** `capability_requirements` は `read`/`navigation` に明示的に文字列 `"required"` を持つ一方、`search`/`filter` はフィールド配列のみで同じ `"required"` マーカーを持たない——SSOT記法上、search/filterが「navigate/readと同格の必須要件」なのか「もし実装するならこのフィールドを使う、という仕様記述」なのかが構文上一義に確定できない。加えて、admin-dashboard subBundle自身の宣言scope（`.agent/tasks/todo.md` Bundle定義「含めないもの」: business projectionとしての/admin扱い、route registry authorityの新規定義の除外）と、`hub_relation_is: manifest_scoped_outbound_navigation_sequence_only`（business projectionではなくnavigation側に分類）は、seed_contractの詳細な検索/フィルタ機能が「landing/navigation/guide entry」という宣言scopeに収まる規模なのか、それを超える「事実上の業務プロジェクション」になるのかについて、SSOT文言だけからは一義に判定できない、実質的なテンションを残す。**したがって「未実装」（3節の通り、component_treeが物理的に存在しない事実そのものは確定）と「今回のBundleでの実装必須性」は分離して評価する: 前者は確定事実、後者は設計不透明として扱い、本ラウンドでは実装しない。** candidateの維持・削除・再分類・adoptionのいずれも行わない。

8. **physical adoption / catalog registration判定:** 3節の通り、既存経路のいずれもseed_contractの完全な機能契約（navigate + hub_relation_id保持payload + status/source_manifest/sequence_position filter軸）を単独で満たさない。admin authoring経路（4節）はフィールド形状こそ完全一致するが、CRUD authoring用actionをread-only browse UIへ転用することの当否はdesign_change判断であり、本ラウンドでは判定しない。したがって `declared_seed_surface_catalog` への登録は本ラウンドでも見送る（adoption evidence不在、round2-8からの継続的な正しい判断）。

9. **他4 subBundle状態（owning SSOTと再照合、変化なし）:** `admin-console-workflow-ssot.yaml subbundle_target_readiness` を再照合した結果、round7/8の記録から変化なし——`admin-enum`/`team-dashboard`/`scheduler-settings` は各画面固有 `ui_projection` target manifest自体が `unresolved_before_seed`（未作成）、`credential-management` はtarget manifest(092)は `resolved_existing_substrate` だが navigation binding authoring/verificationが `unresolved_before_seed` のまま。設計未確定（本ラウンドのadmin-dashboard seed_contract必要性の不透明さ）と実装未達（他4 subBundleのtarget manifest未作成）は異なる性質の残scopeであり、混同しない。進行ゲート（2026-07-18記録）により、他4 subBundleへの着手は本ラウンドでも不可能なまま。

**round 9 実施結果:** 実装は行わない（gate0停止、未実装ではなくSSOT上の必要性が設計不透明であることによる意図的停止）。round8の訂正（manifest 092非採用、SQL Attention Projection発見）が誤った実装を含んでいなかったことを確認した。本ラウンドはフィールドレベルの精密比較により、round8の「機能的にカバーされている」という評価を「navigate能力とfilter軸の両方に実際のギャップが残る」というより正確な評価へ精緻化し、admin authoring経路（`hub_navigation:get_hub_relations`）がフィールド形状としては完全一致することを新規発見した。SSOT再発防止の不足3点をdesign_change候補として整理した（SSOT本文は未変更）。`hub_search.readonly.v1` preset を admin-dashboard へ転用しない根拠を具体的コードで確認した。round7の `/admin` fail-close・qualifying classification構造化実装は本ラウンドでも無変更・有効。他4 subBundleの状態は変化なし。**これは round 9 の checkpoint clear（audit記録）であり、Bundle `admin-surface-topology-seed-conversion` 全体の "Implemented" 宣言や merge 承認ではない。** Bundle Status は引き続き `not_started`。Ownerへの質問は本ラウンドのPRコメント参照。

#### admin-dashboard round 10 監査（PR #594 owner review comment、Round 8のImplemented判定をowning SSOTへ正本化するdesign_change）

owner から、round 8で確認されたdashboard capability（navigation / search / filter / ranking）の既存substrateによる充足を、owning SSOT（`docs/design/admin-normal-surface-projection-seed-ssot.yaml`）へstructured contractとして正本化し、TODOをそれに整合させ、同一scopeの再展開を禁止するよう明示的な決定があった。本ラウンドは実装要否の再判定ではなく、owner自身の確定判断をSSOT/TODOへ記録する最小差分のdesign_changeである。

1. **owner決定の性質:** `capability_requirements.search`/`filter` のリテラルなフィールド列（`relation_label_or_manifest_metadata_when_available` / `status`,`source_manifest`,`sequence_position`）と、SQL Attention Projectionの実際の機構（label検索ではなくscore/topK/recency ベースのexploration・ranking）は、round 9で確認した通り文字通り同一の実装ではない。owner は、この意味的ギャップを、SQL Attention Projection自身の探索・スコアリング・ランキング機構が `capability_requirements.search`/`filter` の**意図**を満たすものとして扱う、という製品権限内の判断を行った——これはAgentが独自技術判断で「一致する」と再結論したものではなく、owner自身の明示的な受入判定として記録する。SSOT追記の`fulfillment`フィールドは、round 9で確認したSQL Attention Projectionの実際の挙動（exploration/filtering(active_condition)/scoring/ranking(AttentionScore, ScoreBand, HitRank)/topK/projection）をそのまま正確に記述しており、「label検索が実装された」等の事実と異なる主張は行っていない。navigationについては、round 2–3で確定済みの汎用hub-navigation `<nav>` が文字通り navigate operationを満たすことに曖昧さはなく、owner決定と技術的事実の間に緊張はない。

2. **実装内容（`docs/design/admin-normal-surface-projection-seed-ssot.yaml` version 0.1.0 → 0.2.0）:** `surface_axes.admin.surfaces.dashboard` の既存構造（`data_authority_ref`/`operations`/`capability_requirements`/`hub_relation_navigation_binding`/`seed_contract`）を削除・変更せず、新規sibling key `existing_substrate_fulfillment` を追加した。内容: `status: implemented_by_existing_substrate`；`navigation` エントリ（capability/status/existing_authority/existing_lane/fulfillment/bundle_scope_effect、汎用hub navigationを根拠として引用）；`search_filter_ranking` エントリ（同形式、SQL Attention Projectionを根拠として引用、fulfillmentはround9で確認した実際の機構を正確に記述）；`prohibited_scope_reopening`（6項目、owner NG軸をそのままSSOT契約化）；`seed_contract_component_shape_boundary`（component_treeが物理採用対象ではなく、パターン参照であることを明記）。`operations`/`capability_requirements`/`seed_contract.component_tree` は一切削除・変更していない——要求capabilityの記述と充足判定を併記する形を維持した（owner OK軸の明示指示通り）。

3. **検証:** `bash .agent/tests/check-admin-normal-surface-projection-seed-ssot.sh` を実行し、既存の全pinned assertion（component key存在、DB table存在、除外文言等）が本追記後も全てPASSすることを確認した——既存構造を壊していない。production code、test assertion、fixture、manifest row、package/layout/wiring/tensorは一切変更していない。

4. **`.agent/tasks/todo.md` の整合:** round 6–9の記録本文（履歴）はこのファイルの既存パターンに従い書き換えない——上書きではなく、本round 10セクションが現在の正本状態を上書き的に更新する。今後の「残存」記述は以下に統一する:
   - dashboard search/filter/ranking: **implemented by SQL Attention Projection**（`docs/design/admin-normal-surface-projection-seed-ssot.yaml surface_axes.admin.surfaces.dashboard.existing_substrate_fulfillment.search_filter_ranking` 参照）。
   - dashboard navigation: **implemented by manifest-scoped hub navigation**（同 `.navigation` 参照）。
   - admin-dashboard candidate（`seed_contract.component_tree` / React-like Schema / translator output / composer fixture / frontend render fixture）に対する追加実装（新preset/新package/新physical seed/manifest adoption/新wiring/新runtime lane）: **not required**。
   - 上記3点のscope reopening: owning SSOTの`existing_substrate_fulfillment`が先に変更されない限り**prohibited**（owner決定）。
   - round 7–9で使っていた「candidate」「physical adoption未特定」「owner確認待ち」という表現は、本round時点では上記のimplemented判定に置き換わる。ただしround 6–9の本文自体は監査証跡として残し、書き換えない。

5. **他4 subBundleへの影響:** なし。`credential-management`/`admin-enum`/`team-dashboard`/`scheduler-settings` はそれぞれの`unresolved_before_seed`（target manifest未作成、またはcredential-managementのnavigation binding authoring/verification）が本ラウンドの対象外であり、進行ゲート（2026-07-18記録）は無変更。admin-dashboard capability のimplemented判定は、他4 subBundleのgate解除や、Bundle全体の"Implemented"化を意味しない——本ラウンドはadmin-dashboard capability一件のSSOT正本化に限定されたcheckpointである。

6. **round 7の有効差分:** `/admin`存在fail-close、qualifying classification構造化は本ラウンドで一切変更していない。

**round 10 実施結果:** owner決定に基づき、admin-dashboard candidateのnavigation/search/filter/rankingがexisting substrate（汎用hub navigation / SQL Attention Projection）によってimplementedであることを、owning SSOT（`admin-normal-surface-projection-seed-ssot.yaml`）へstructured contract（`existing_substrate_fulfillment`）として正本化し、scope reopeningを明示的に禁止した。SSOT追記はround9で確認した実際の機構を正確に記述しており、事実と異なる主張は含まない。TODOの今後の残存記述をこの判定へ整合させた（履歴本文は書き換えず、本セクションが現在の正本）。`check-admin-normal-surface-projection-seed-ssot.sh` で既存pinned assertion全PASSを確認、production code/test/fixture/manifest/package/layout/wiring/tensorは無変更。round7の有効差分は無変更。**これは round 10 の checkpoint clear（design_change記録）であり、Bundle `admin-surface-topology-seed-conversion` 全体の "Implemented" 宣言や merge 承認ではない。** Bundle Status は引き続き `not_started`。残存: (a) 他4 subBundle未着手（既存進行ゲートにより本ラウンドでは着手不可、無変更）、(b) `declared_seed_surface_catalog` 正式登録は引き続き見送り——本ラウンドの判定により、admin-dashboard candidateはそもそも物理採用を要しない（catalog登録はadoption evidenceの有無ではなく、adoption自体が不要と確定したことによる非対象）、(c) `/admin/team-dashboard` の文言執筆（別Bundle引き継ぎ、無変更）。
