...
閉鎖済み範囲(実コード・live PostgreSQL executable proof で確認済み、proof manifest `aggregate_trigger_substrate_live_db_executable_proof` 参照):
...
- AdminRuntime definition persistence route: `AdminRuntime.AssignScreenDataShapeAsync` が検証成功した aggregate trigger definition を `runtime_orchestration.aggregate_trigger_definitions`(正本永続化先)へ `AggregateTriggerRepository.SaveDefinitionAsync` 経由で保存する。`screen_data_shape` 側の格納は Step3 フォーム再表示用の authoring draft projection として残置(実行 authority ではない)。live DB proof: `backend/tests/Topolactor.Integration.Tests/AdminRuntimeAggregateTriggerDefinitionPersistenceLiveDbTests.cs`。
- cron/hook/client scheduler-to-manifest DB route: `RuntimeTimelineScheduler`(cron/hook/client 全trigger種別が同一経路を通る既存基盤)-> `ManifestDispatcher` -> `AggregateTriggerRuntime` -> `NpgsqlAggregateTriggerRepository` の代表経路を live DB で証明(`backend/tests/Topolactor.Integration.Tests/AggregateTriggerSubstrateRouteLiveDbTests.cs`)。
...
- DB SSOT追従: `docs/design/db-schema.yaml` の `aggregate_trigger_table_contracts.status` を `implemented` に更新し、`canonical_bootstrap_files` / `source_files` に `db/runtime_orchestration_tables.sql` を追記。
- proof manifest追従: `docs/design/test-proof-manifest-ssot.yaml` に `aggregate_trigger_substrate_live_db_executable_proof`(executable proof)を追加し、`aggregate_trigger_substrate_executable_proof_gap`(known_gap)を残 scope のみへ縮小。`.agent/docs/test-bundles.yaml` 追従済み。
- Step3 UI 部分修正: `AggregateTriggerAuthoringPanel.tsx` が `execution_scope` / `transaction_boundary` / `approval_policy` を固定値ではなく構造化 selector として公開するよう修正(transaction_boundary の runtime gating 追加に伴う機能断絶の是正)。

残 scope(`aggregate_trigger_substrate_executable_proof_gap` known_gap 参照、未閉鎖):
- [ ] admin/contents Step3 の残り構造化 authoring: `processing_function_scope`(function_id / operation_definition_id の選択)、`conflict_key_fields`(対象フィールドからの選択)、`delta_map`(宣言済み event/target フィールドからの構成)、`threshold_policy`(minimum_trial_count / ratio フィールド / 比較演算子 / target_ratio の構成)、`materialization_payload_map`(許可済み source からの構成)が `frontend/components/AggregateTriggerAuthoringPanel.tsx` / `frontend/lib/aggregateTriggerAuthoring.ts` でまだ固定値 hardcode のまま。`docs/design/admin-console-workflow-ssot.yaml` の `aggregate_trigger_step3_extension_contract.ui_authoring_boundary.allowed` に列挙された authoring action のうち、上記5項目のみ未実装。
  - 目的: Step3 が Step2/2.5 定義済み対象のみを選ばせる構造化 UI として、上記5項目も selector/フィールドピッカーとして公開する。frontend は依然として structured authoring projection only であり、threshold/approval/materialization の final judgment は持たない。
  - 改善方針: 既存の selector パターン(canonicalTriggerKind 等と同様の `<select>` + state)を conflict_key_fields / threshold_policy の数値・演算子入力、delta_map のフィールド選択、materialization_payload_map のエントリ配列編集へ拡張する。processing_function_scope は registered processing function 一覧(未定義の場合は SSOT へ先に登録経路を明示)から選択させる。
  - 対応資料: `docs/design/admin-console-workflow-ssot.yaml`(`aggregate_trigger_step3_extension_contract`), `docs/design/runtime-orchestration-ssot.yaml`(`aggregate_trigger_substrate_contract`)
  - 対象ファイル: `frontend/components/AggregateTriggerAuthoringPanel.tsx`, `frontend/lib/aggregateTriggerAuthoring.ts`, `frontend/tests/aggregateTriggerAuthoring.test.ts`, `frontend/tests/uiAssuranceSubstrate.test.tsx`
  ...
  - NG軸: 特定アプリ専用の hardcode 実装、特定アプリケーション名・特定ユースケース名を substrate 要件へ混入、UI の raw SQL/CASE/WHERE 保存、任意 table 名入力、event 側を scope owner とする設計、Step2/2.5 未定義対象への登録、frontend persistence 判断、app-side read -> count++ -> update race、閾値超過時の二重 materialization、approval policy 未定義、proof manifest の過剰主張。
