...
- **DTO representability（Round32の未解決点をRound33で解消、Round37はこの解消を根拠にdd010を配列へ復帰）**: Round32はreverse方向のprev候補（click destinationがowner manifest自身になるケース）が既存DTOのいずれのfieldの意味も偽装・反転せずには表現できないと判断し、新規DTO追加のOwner判断待ちとしていた。`backend/repository/NpgsqlContentBundleRepository.cs`の`LoadHubNavigationSequenceAsync`実装を直接確認した結果、`HubNavigationSequenceItemDto.TopologyManifestId`は元々SQL行の列ではなく、クエリの入力パラメータ（current topology自身）をそのままecho する既存実装になっていることが判明した——つまりこのfieldは元々「この応答が何について答えているか」を表すものであり、「この行の実owner」ではない。**Round37の帰結**: この既存の意味により、隣接occurrenceが位置0（S自身）である場合もfield意味の反転無く表現できる——S自身が隣接値になる場合はRelatedHubId/RelatedHubLabel/TargetManifestIdをSの既存識別列（hubs.hub/hubs.topology_manifests）から直接埋め、HubRelationIdは隣接する実際の行自身のidを再利用する（S自身用の新規idを発明しない）。位置k>=1（ordinary target）が隣接値である場合は既存のtarget行由来の値をそのまま使う——両者は同一fieldセットへの単なるdata-sourcingの違いであり、新しいderivationルールではない。TopologyManifestIdはどちらの場合もcurrent topology（=応答が何について答えているか）のままで変わらない。**結論: 新規DTO/新規typeは不要、既存`HubNavigationSequenceItemDto`/`HubRelationNavigationLinksResponseDto`をそのまま再利用する。** Round32のOwner判断待ちの事項はこれで解消済みであり、Round33〜36がこの解消の上でなお配列外へS(owner)を追い出したのは不要な後退だった——Round37はこの解消結果をそのまま活かし配列内へ復帰させる。
