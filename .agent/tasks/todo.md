...
### admin-dashboard subBundle 記録（2026-07-19、最終scopeへの圧縮。旧・詳細round履歴は本節により置き換え）

**この節が admin-dashboard subBundle の現在の正本である。** 2026-07-18〜19にかけて、PR #594上で React-like Schema → translator → topology UI seed candidate → composer/render構造証明という広いパイプラインが試作されたが、2026-07-19時点のOwner明示判断により、admin-dashboard capabilityは既存substrate（汎用hub navigation / SQL Attention Projection）で充足済みと確定し、上記パイプラインおよびそれに付随するPython/JSON/generated file群は不要と判定され、本PRから撤去された。過去のround 2–10監査の詳細本文（GPT監査プロンプト経由で段階的にscopeが拡張された長い調査記録）は、現在の正本ではなくなったため、この短い最終記録に置き換える——「有効差分」として個別に認定されたことは、現在のOwner scopeを上書きする根拠にならない。

**最終成果物（4責務のみ）:**
1. **SQL registry fix:** `db/ui_component_registry_preset_catalog_bootstrap.sql` に `table.primitive`（component_id `00000000-0000-0000-0001-00000000001f`）を追加。これはadmin-dashboard専用の成果物ではなく、独立したregistry gapの修正である——`table.primitive` は `topology.components_bucket`（`db/seed_empty.sql`）と `frontend/components/catalog.ts` に既に登録済み・runtimeConnectedなコンポーネントだったが、`topology.ui_component_registry`（固定UUID lookup用テーブル）にはrowが無く、この経路からは解決できなかった。
2. **SQL直接test:** `backend/tests/Topolactor.Runtime.Tests/LayoutSchemaStructuralCompositionTests.cs` の `UiComponentRegistryBootstrapSql_TablePrimitiveRow_HasExpectedFieldsAndNoDuplicateIdentifiers` が、上記SQLファイルを直接読み込み、`table.primitive` rowの存在と、component_id/component_keyの重複が無いことを検証する（既存の `EmailPortConsumerContractTests.cs` の `RepoRoot()`+`File.ReadAllText` パターンを再利用、新規fixture/translator/composerへの依存なし）。
3. **SSOT scope closure:** `docs/design/admin-normal-surface-projection-seed-ssot.yaml`（0.2.0）`surface_axes.admin.surfaces.dashboard.existing_substrate_fulfillment` — navigation は汎用manifest-scoped hub navigationにより、search/filter/rankingは既存SQL Attention Projectionにより、いずれも `implemented_by_existing_substrate` である旨と、`prohibited_scope_reopening`（本Bundleでの重複実装禁止、manifest 092の他surfaceでの流用禁止等）を構造化契約として記録済み（round 10、無変更で維持）。
4. **TODO current lineage:** 本節。

**撤去済み（過去roundで試作されたが、最終的に不要と判定されたもの）:**
- `.agent/scripts/generate_admin_route_identity.py`（新規generatorファイル、削除）、`frontend/content/adminRouteIdentity.generated.ts`（生成物、削除）、および `docs/design/admin-console-workflow-ssot.yaml` の `static_navigation_sourcing_contract` 追記（route identity生成契約、revert）。
- `.agent/scripts/react_schema_topology_seed_translator.py` / `.agent/scripts/check_react_schema_topology_seed_translator.py` へのqualifying classification関連追記（revert）、および `docs/design/react-schema-topology-seed-translator-ssot.yaml` / `docs/design/ui-builder-seed-first-gap-discovery-ssot.yaml` への対応するSSOT追記（revert）。
- admin-dashboard candidate専用のReact-like Schema fixture・topology seed候補JSON・generated output（`.agent/tests/fixtures/react-schema-topology-seed-translator/admin-dashboard-*.json` 等、削除）。
- `backend/repository/LayoutSchemaTensorComposer.cs` への resolver追記（`search_input.alias`/`panel.alias`/`table`→`table.primitive`、revert——これらのresolver追記は react-schema→composer変換pipeline専用であり、admin-dashboard含め他のどのpresetの実行経路もこの特定resolverを経由していなかったため、pipeline撤去に伴い不要と確認した）。
- `frontend/content/adminGuides.ts` のADMIN_ROUTE_CARDS/GUIDE data-driven sourcing化（revert）、および対応する `frontend/tests/adminMainFlow.test.ts` の追記（revert）。
- `frontend/tests/layoutSchemaStructuralRender.test.ts` へのadmin-dashboard candidate render証明追記（revert）。
- いずれも `git diff <merge-base> HEAD` で該当ファイルがPR追加分のみであることを確認した上でmerge-base状態へ戻した（他subBundle/他featureと混在した差分は無かった）。

**他4 subBundleへの影響:** なし。`credential-management`/`admin-enum`/`team-dashboard`/`scheduler-settings` の残 `unresolved_before_seed` は本記録の対象外であり、無変更。

**Bundle Status:** 引き続き `not_started`。admin-dashboard capabilityのimplemented判定は、他4 subBundleのgate解除やBundle全体の"Implemented"化を意味しない。scope reopening（navigation/search/filter/rankingを未実装/partialへ戻す、manifest 092の流用、重複するclient-side filter/backend search action/parallel projection laneの追加等）は、owning SSOT（`admin-normal-surface-projection-seed-ssot.yaml`）の変更なしには禁止。
