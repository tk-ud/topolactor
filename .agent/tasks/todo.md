...
**次にこの subBundle を触る Agent への引き継ぎ:**
- 残課題は「アーキテクチャ上の重要な発見」節記載の1点（Normal-role open readをManifestDispatcher経由で実現するための、per-action capability override機構またはNormal専用runtime destinationの新設）のみ——これはOwner判断が必要な具体的design_changeであり、本subBundle自体は`/admin/team-dashboard`（admin edit）と`/dashboard`（normal read-only、plain REST経由）の両方が実際に機能する状態で完了している。
