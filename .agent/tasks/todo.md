...
- `team-dashboard`: team Markdown saved view / rendered Markdown / completed preset seed summary authorityは既存SSOTに従う。target `ui_projection` manifest は未作成（着手条件として`unresolved_before_seed`）。normal.dashboard viewer/inputer責務分離、`md_translation_authoring_surface.authoring` runtime adapter/route-composition binding、preview/validate/write/diff operation bindingは全て解決済み（role-based-surface-impl bundle, 2026-07-14）。ただし`normal_dashboard_authoring_runtime_adapter`はGate0監査で2026-07-15にreopenされたまま（`/dashboard/team`撤去・`/admin/team-dashboard`復元、下記節参照）——manifest/dispatcher_mapping/runtime adapter自体は未生成。manifest構築後、navigation binding authoring / verificationも同基準で用意する。**2026-08-16 round 1（下記「team-dashboard subBundle 実装記録」参照）: target manifest自体はまだ未構築のまま（`unresolved_before_seed`は解消していない、Bundle/subBundle Statusは変更なし）だが、着手前提として存在した汎用dispatch substrateの2件の欠落（`idOrHubId`未転送、admin_runtime target_ref正規表現がaction内colonを拒否）を解消した——いずれもteam_markdown固有ではなく、次にmanifest構築へ進むAgentが再発見してブロックされないようにするための汎用substrate修正。**
...
### team-dashboard subBundle 実装記録（round 1、2026-08-16、`implementation_change`）

**この節の Status: `partial`（subBundle自体は未完了のまま）。** target `ui_projection` manifest・hub relation navigation authoring・production seed行は本roundでは一切作成していない——`design_blocking.normal_dashboard_authoring_runtime_adapter`（team-dashboard）は依然 `reopened_2026_07_15_gate0_audit` のまま、`target_surface_manifest_readiness.team-dashboard` の `unresolved_before_seed` も未解消。本roundが行ったのは、次にmanifest構築へ着手するAgentが同じ場所で無自覚にブロックされないよう、着手前提として存在した汎用dispatch substrate自体の2件の欠落を発見・解消したことのみ。

**問題点:**
1. `docs/design/admin-normal-surface-projection-seed-ssot.yaml` `surface_axes.normal.surfaces.dashboard.seed_contract`が要求するpreview/validate/explicit_confirm/write/diff_log操作（`team_markdown:saved_view:get/update/refresh/clone/rebind/archive`）は、いずれもbackend側で`vector.IdOrHubId`（`AdminRuntime.TeamMarkdown.cs`、`DataTeamMarkdownSavedViewUpdateAsync`ほか）によって対象レコードを識別する——`EndpointRequestDto.IdOrHubId`という、`payload`とは別のtop-levelフィールド。ところが既存の汎用admin_runtime dispatch配線（`dispatchTargetRefByTrigger`/`dispatchPayloadFromByTrigger`→`emitBoundEvent`→`enqueueRuntimeComponentCommand`、admin-enum/credential-managementが確立した`payloadFrom`機構）は、解決結果を常に`payload`オブジェクトへ書き込むのみで、`RuntimeDispatchSpec`/`queueAdminClientCommand`のいずれも`idOrHubId`を転送する経路を持たなかった（`frontend/runtime/frontendScheduler.ts`の`enqueueRuntimeComponentCommand`実装を直接確認）。admin-enum/credential-managementの既存write actionは全てレコード識別子を`payload`フィールド（`groupId`/`indexNum`/`recordId`）として運ぶ設計だったため、この欠落はこれまで一度も露見していなかった。
2. `frontend/runtime/renderEmission.ts`の`ADMIN_RUNTIME_TARGET_REF_RE`（および`backend/runtime/ManifestDispatcher.cs`の同一ミラー`AdminRuntimeTargetRefRe`）は`^manifest:([0-9a-fA-F-]{36}):([^:]+):([^:]+)$`——layer/action双方をcolonを含まない1segmentとして要求する。ところがteam_markdownのaction語彙は全て`<resource>:<verb>`形（`template:create`/`saved_view:update`等、`AdminRuntime.TeamMarkdown.cs`の`ExecuteTeamMarkdownAsync`switch文で確認）——action自体がcolonを含む。この正規表現のままでは`manifest:<uuid>:team_markdown:saved_view:update`のようなtarget_refは4segment超過でマッチせず、frontend側は`mapWiringKindToLayer/Action`がnullを返し（fail-close、ノードがerror componentTypeになる）、backend側は`TryHandleAsync`より前の`AdminRuntimeTargetRefRe`不一致によりauthorization checkそのものをskipしてしまう（round17/18 hardeningが依拠する`AdminRuntimeShapeMatch`が得られないため）——結果としてteam_markdownのいかなるactionも、admin-enumが確立した「node自身のtargetRef/payloadFromで完結する」汎用配線パターンでは一切dispatchできなかった。

**目的:** 上記2件を、team_markdown固有のhardcodeではなく、既存の汎用dispatch pipelineそのものへの拡張として解消し、次のmanifest構築roundが同じ場所で発見・再ブロックされないようにする。

**改善方針・対応内容:**
1. `RuntimeDispatchSpec`（`frontend/runtime/frontendScheduler.ts`）へ`idOrHubId?: string`を追加し、`enqueueRuntimeComponentCommand`がそれを`queueAdminClientCommand`（`Omit<DispatchRequest, "triggerKind"|"role">`——`DispatchRequest.idOrHubId`は元々存在した）へ転送するよう1行追加。`frontend/runtime/runtimeComponentFactory.ts`の`emitBoundEvent`（payloadFrom解決ブランチ）に`splitIdOrHubIdFromResolvedPayload`ヘルパーを追加——`payloadFrom`が予約フィールド名`idOrHubId`を解決した場合、それを`payload`から取り出し`RuntimeDispatchSpec.idOrHubId`（top-level）へ回す。`idOrHubId`キーを持たない既存の全wiring（admin-enum全7action、credential-management全操作）はこの分岐に触れず無変更——`"idOrHubId" in resolved`の条件分岐でopt-inのみ。
2. `ADMIN_RUNTIME_TARGET_REF_RE`（frontend）と`AdminRuntimeTargetRefRe`（backend、`ManifestDispatcher.cs`——両者は元々「exactly mirrors」とdoc commentで明記された対の実装）の両方で、action captureグループを`([^:]+)$`から`(.+)$`へ変更。layer側は無変更（既存layer名にcolonを含むものは無いことを確認済み）。既存の4segment・colonなしactionは`(.+)$`でも同一にマッチするため後方互換。
3. Substrate Planの`ng_axis_gate_triggers`判定: `data_driven_projection_completion_gate.dispatch_resolution`（既存汎用dispatch経路の欠落補完、新dispatcher/新laneの追加ではない）に該当。`admin_authoring_completion_gate`は本round未着手（manifest自体を作っていないため対象外）。

**対象ファイル名:** `frontend/runtime/frontendScheduler.ts`、`frontend/runtime/runtimeComponentFactory.ts`、`frontend/runtime/renderEmission.ts`、`backend/runtime/ManifestDispatcher.cs`、`frontend/tests/adminWiringExecutionLane.test.ts`、`backend/tests/Topolactor.Runtime.Tests/ManifestDispatcherTargetRefTests.cs`。

**対象関数名:** `enqueueRuntimeComponentCommand`、`emitBoundEvent`、`splitIdOrHubIdFromResolvedPayload`（新設）、`parseAdminRuntimeLayerAction`、`ValidateAdminRuntimeTargetRefLayerAction`。

**Governance NG boundary遵守（Gate0 evidence）:**
- 既存`team_markdown:*` CRUD/write authorityは一切再実装していない——`AdminRuntime.TeamMarkdown.cs`・`NpgsqlTeamMarkdownRepository.cs`・`TeamMarkdownAuthoring`/`SavedViewAdjustmentAuthoringPanel`/`SavedViewOperationPanel`は無変更。
- 専用dispatcher・専用navigation mechanism・Manifest UUID固有分岐・専用ProjectionShell forkのいずれも追加していない——変更した4本のproduction fileはいずれも既存の汎用dispatch pipeline自身（team_markdown固有のcase分岐は追加していない）。
- 既存route direct-mount（`/admin/team-dashboard`）はcompletion evidenceとして扱っていない——本roundは同routeを一切変更していない。
- fake/empty topology manifestは作成していない——manifest自体を本roundでは作っていない。
- hub relationは一切authoringしていない（target manifest不在のため、順序上ありえない——本Bundle「navigation_binding_authoring_and_verification 解決基準の確定」節の順序原則どおり）。

**検証:** `deno test -A frontend/tests/`（2009 passed / 0 failed——本round追加2件含む、pre-existing 79 failedは`git stash`によるbaseline比較で本diffと無関係と確認済み〔teamMarkdownSavedView.test.ts/roleBasedSurfaceSeparation.test.ts等、既存route-composition関連のred markerで本round着手前から存在〕）、`dotnet build`（0 errors）、`dotnet test backend/tests/Topolactor.Runtime.Tests`（1679 passed / 0 failed——本round追加1件含む）、`bash .agent/tests/check-structure.sh`。Integration.Tests（実PostgreSQL）は本セッションのコンテナでdockerデーモンが利用不可のため未実行——次にこのsubBundleを触るAgentが実manifest/hub relation live-DB proofを書く際に、本roundのdispatch substrate修正を経由したend-to-endの実dispatchも合わせて証明すること。

**次にこの subBundle を触る Agent への引き継ぎ（残scope、意味単位）:**
1. **target `ui_projection` manifest構築**: `surface_axes.normal.surfaces.dashboard.seed_contract`に従い、React-like Schema→translator→topology UI seed→seed登録（`db/seed_empty.sql`）。read側（`saved_view:search`、identity不要）とget/update側（本roundで直った`idOrHubId`機構を使用）を分けて設計すること。
2. **write vertical slice（`edit_saved_view_adjustment`＝`saved_view:update`が最小の最初の1枚、`docs/design/team-markdown-dashboard-saved-view-ssot.yaml` `dashboard_surface_contract.actions`参照）**: admin-enumの`mutation_confirmation_contract`（dryRun preview→confirm modal→confirmed write）パターンを再利用し、`saved_view:update`のみをまず配線する。refresh/clone/rebind/archive/copy_markdown/create_follow_up_todo_candidateは後続roundのscope。
3. **hub relation navigation authoring/verification**: manifest構築後、credential-management/admin-enumと同一の解決基準（`hub_navigation:create`実authoring dispatch + resolution chainを単一live-DB testで証明）で用意する。
4. **本round対象外（意図的に着手しなかった）**: `md_translation_authoring_surface.authoring`カタログエントリを`RUNTIME_COMPONENT_FACTORIES`へ登録する経路は検討したが、`credential_management.admin_operation`/`scheduler_job_settings.admin_operation`が同種の複合パネルをmanifest-driven seedとして使わず既存route-composition専用のまま残している既存precedentと整合させ、write UIはgeneric primitive（textarea/button/modal等、admin-enumが確立した既に`runtimeConnected`なcomponentKind群）で構成する方針を採る——複合パネルのラップではなく、汎用primitive分解によるteam_markdown write authorityの非重複再利用。この判断の再評価は次のAgent/ownerの対象として残す。
