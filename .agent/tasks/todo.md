...
3. HubRelation fixed-navigationの提示責務をProjectionShellからapp-shell navigation Islandへ収束し、current topologyをそのnavigation Islandへ`Context`経由で共有したうえで、current topologyが所属する各`hubs.hub_relations`sequence（`sequence_position`順の順列）について、current topologyの位置に隣接するimmediate prev/nextのみを、失わず提示する（sequenceの他memberを列挙しない。複数sequenceに所属する場合は各sequenceのprev/nextを独立に失わず扱う）。
...
- sequence semantics: `db/topology_tables.sql`が正本。sequenceは1つのmanifest（`topology_manifest_id`）が自分自身で持つ`hubs.hub_relations`行集合そのものであり、`sequence_position`がその中の順序authority（`sequence_position`順の順列）。1つのmanifestが他のmanifestのsequenceを所有し他manifestの行を多段（multi-hop）に辿ることはない（global graphではない）。
- **fixed-navigation candidateの訂正（Round29以降のsemantic driftを訂正、本Round確定）**: 以前のRound29は「current topologyが所属する上位relation/sequence」の復活を、candidate = そのrowの反対側の端点（opposite endpoint）を無条件に列挙する設計として実装した。これは誤り（Owner確定設計からの逸脱）であり、本Roundで撤回する。正しいcanonical semanticは: HubRelation fixed-navigationは`sequence_position`で定義された順列に対し、current topologyの位置からimmediate `prev`/`next`のみを提示する決定論的navigationである。current topologyは各sequenceに対し(a)自分自身がそのsequenceのOWNER（自分自身のoutbound行集合）である場合と、(b)他manifestが所有するsequence内のtarget memberとして名指されている場合の2通りで位置を持ち得る。(b)の場合、あるowner Mの行がsequence_position昇順でp1<p2<...<pnのとき、current topologyがpkに位置するなら: next = p(k+1)のtarget（k<nなら）、prev = p(k-1)のtarget（k>1なら）、k=1（Mの最小位置）ならprev = M自身（Mの行集合には自分の最初の行より前の行が存在しないため、その直前の暗黙位置をowner自身が占めるという、この設計が明示する唯一の補完規約 — 根拠と却下した代替案は`runtime-orchestration-ssot.yaml bidirectional_read_projection_gap`参照）。(a)の場合（current topology自身がowner M）、prev = none（Mの行集合の中でMの最初の行より前は無い）、next = Mの最小sequence_position行のtargetのみ（Mの残りの行はp1のtargetからさらにnext/prevを辿ることで到達する、Mから直接candidate化しない）。複数sequenceに所属する場合は各membershipのprev/nextを独立に失わず扱う（マージ・破棄しない）。Ownerの例（A=[1,5,7,9,15]、B=[4,2,3,8,9,6]、current=9）は、manifest 9が自分自身で2行を持つ設計ではなく、9が異なる2つのowner（A、B）それぞれのsequence内でtarget memberとして位置を持つ設計として再解釈する: Aのsequence内では9の前後が7/15、Bのsequence内では9の前後が8/6であり、それぞれのmembershipが独立にprev/nextの対を提供する。本Roundのprev/next両方提示要件のもとでは、standing on 9のcandidateは{7, 15, 8, 6}となる（元のOwner例はnext方向のみを示す簡略化であり、本Roundのprev/next両方提示要件のほうを優先しfixtureの数値解釈を更新する）。新しいaxis/dimension/role flag列は不要 — `(topology_manifest_id, sequence_position)`の既存UNIQUE制約のみで導出できる。
- read projection: 既存`ContentBundleRepository.LoadHubNavigationSequenceAsync(topologyManifestId)`（source-scoped）が、current topology自身のsequence（(a)の場合の候補元）を過不足なく返す。ownerとして立っている場合はこの結果の最小`sequence_position`行のみをnextとして使う。これに加えて、target方向のreverse read（他manifestがcurrent topologyをtargetとして名指すrowを見つけ、そのrowの`sequence_position`と同じowner内の隣接行から current topologyのprev/nextを導出する）を**追加のadditive query**として導入する — 新しいtable/column/parallel authorityではなく、同一`hubs.hub_relations`永続化に対するもう一つの読み方（新設backend read methodは必要、ただし新しいtable/columnは不要）。この読みの目的はopposite endpointの列挙ではなく、prev/next導出である。この読みが、`axis_navigation_membership`のtarget`ae200`（`dd010`のsequence内の最小position）が自分自身のoutbound行を持たなくても`dd010`をprevとして持てる仕組みそのものである（下記no_return_edge項目参照）。
...
- `backend/repository/NpgsqlContentBundleRepository.cs`（`CreateHubRelationAsync`/`UpdateHubRelationAsync`/`LoadHubNavigationSequenceAsync`/`DeprecateHubRelationAsync`。加えて`bidirectional_read_projection_gap`が定義するreverse方向のadditive read method新設 — `LoadHubNavigationSequenceAsync`と対をなす、current topologyをtargetとして名指す他manifestのrowを見つけ、そのrowの`sequence_position`と同じowner内の隣接行からcurrent topologyのprev（該当行がownerの最小positionならowner自身、それ以外は隣接行のtarget）/next（隣接行のtarget、無ければnone）を導出して返すmethod。owner自身を無条件に返すmethodではない。具体的なmethod名はAgent判断。新しいtable/column/DTOは追加しない）
...
- [ ] frontend DOM proof 3: current topologyが複数の異なるowner sequenceのtarget memberであるfixture（Owner例のA/B/9/15/6を再解釈した型: manifest 9が、owner Aのsequence内とowner Bのsequence内それぞれで独立にtarget memberとして位置を持つ）で、両方のmembershipから導出されるprev/next（7・15・8・6の4件）が失われずそれぞれ独立に提示され、いずれのsequenceの他member（1つのsequenceに3件以上ある場合の残り）も列挙されないことを証明する。
