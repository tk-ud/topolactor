...
check "$NPGSQL" "GenerateExportPackage"
check "$NPGSQL" "\"csv\" => ToCsvBytes"
check "$NPGSQL" "\"json\" => JsonSerializer.SerializeToUtf8Bytes"
check "$NPGSQL" "\"pdf\" => ToPdfBytes"
check "$NPGSQL" "\"zip\" => ToZipBytes"
check "$NPGSQL" "ZipArchive"
check "$NPGSQL" "SHA256.HashData\(bytes\)"
...
check "$TEST" "Npgsql_export_package_generation_is_format_distinct_and_checksummed_from_file_bytes"
check "$TEST" "checksums.Count"
check "$TEST" "SHA256.HashData\(package.Bytes\)"
...
absent "$RUNTIME" "ReadString\(payload, \"port_id\"\)|ReadString\(payload, \"portId\"\)|CLI_READER_EXPORT_PORT_ID_REQUIRED|approval_execute|ApproveExport"
absent "$NPGSQL" "pending-export-job-id|export_job_id = \(Guid\?\)null|manifestChecksum|topolactor://exports/\{command.IdempotencyKey\}/manifest.json|approval_execute|ApproveExport"
