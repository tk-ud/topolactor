...
public partial class AdminRuntime
{
        ...
        OperationVector vector, CancellationToken ct = default)
    {
        ...
        return layerAction switch
        {
            ...
            "scheduler_jobs:list_settings"              => await DataListSchedulerJobsSettingsAsync(vector, ct),
            ...
            "scheduler_jobs:enable"                     => await DataEnableSchedulerJobAsync(vector, ct),
    ...
    private async Task<(JsonElement? data, ValidationError? error)> DataListExternalPortAuthoringCandidatesAsync(CancellationToken ct)
    {
        try
        {
            var candidates = await _uiTopologyRepository.ListExternalPortAuthoringCandidatesAsync(ct);
            return (JsonSerializer.SerializeToElement(new { candidates }), null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "DataListExternalPortAuthoringCandidatesAsync failed.");
            return (null, new ValidationError("DB_UNAVAILABLE", ex.Message));
        }
    }

    private async Task<(JsonElement? data, ValidationError? error)> DataListInstanceOperationAuthoringCandidatesAsync(CancellationToken ct)
    {
        try
        {
            var candidates = await _uiTopologyRepository.ListInstanceOperationAuthoringCandidatesAsync(ct);
            return (JsonSerializer.SerializeToElement(new { candidates }), null);
        }
        catch (Exception ex)
        {
