...
public sealed class NpgsqlAbstractFunctionManifestRepository : IAbstractFunctionManifestRepository
{
    ...
    public async Task<AbstractFunctionManifest?> LoadAsync(string functionKey, CancellationToken ct = default)
    {
        ...
        {
            cmd.CommandText = """
                SELECT abstract_function_id, runtime_lane, authority_scope, projection_deny_keys, active
        ...
        {
            cmd.CommandText = """
                SELECT abstract_function_step_id, step_order, primitive_key, step_config, result_context_key, active
