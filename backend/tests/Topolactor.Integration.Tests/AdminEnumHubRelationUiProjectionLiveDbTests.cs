...
public class AdminEnumHubRelationUiProjectionLiveDbTests
{
    ...
    /// (db/seed_empty.sql enum_create_group_form node -- generated by
    ...
    /// not hand-invented). Unlike DispatchAsync_AdminEnumManagementManifest_CreateGroupThenDeleteGroup_
    ...
    public async Task DispatchAsync_AdminEnumManagementManifest_CreateGroupFormNode_SurfacesDispatchOverride_AndExecutesViaAe210Authority()
    {
        ...
        // Step 1: render ae200's own layout and confirm enum_create_group_button (the LEAF Action
        // node -- not its owning enum_create_group_form, per LayoutSchemaTensorComposer.Compose's
        // catalog-leaf-only NodeId merge for these two fields) surfaces the EXACT override the
        // seed declares -- this is the node-local data a real click would read.
        ...
        var targetRefByTriggerText = createGroupButtonNode.DispatchTargetRefByTrigger!.Value.GetRawText();
        ...
        var payloadFromByTriggerText = createGroupButtonNode.DispatchPayloadFromByTrigger!.Value.GetRawText();
