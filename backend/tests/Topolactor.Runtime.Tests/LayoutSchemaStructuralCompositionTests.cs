...
public class LayoutSchemaStructuralCompositionTests
{
    ...
    public async Task ComposeAndMapToLayoutNode_AdminDashboardHubRelationNavigation_MatchesCheckedInFrontendFixture()
    {
        ...
        // actions already use).
        const string interactionsJson = """[{"trigger":"item.click","actionType":"localStateMutation","payloadFrom":{"hub_relation_id":"node:hub_relation_link_list.value"},"sourceActionKey":"select_hub_relation_link","targetRef":"ui-local:hub_relation_link_list.selected_row"}]""";
        await AssertComposedLayoutNodesMatchFixtureAsync(
