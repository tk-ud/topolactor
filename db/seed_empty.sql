...
VALUES (
    ...
    ARRAY[
        ...
        '{"type":"runtime_mapping","runtime_destination":"admin_runtime"}'::jsonb,
        -- Round 4: manifest 092's OWN dispatcher_mapping entry for external_api_credential:search
        -- (docs/design/admin-normal-surface-projection-seed-ssot.yaml surface_axes.admin.surfaces.
        -- credentials.categories.external_api_credential.seed_contract's credential_result_list
        -- responsibility). Deliberately the search action ONLY -- ManifestTopologyValidator rejects
        -- 2+ dispatcher_mapping entries on one manifest, so create/update/delete stay on their own
        -- dispatch-only manifests (cd008/cd009/cd010) below. Making search resolve to 092's OWN
        -- manifest id (rather than a separate dispatch-only manifest, as create/update/delete use)
        -- means a search dispatch's response.emission.manifestId equals 092's adopted identity, so
        -- the EXISTING generic same-manifest response-adoption path in
        -- frontend/islands/ProjectionShell.tsx (handleRuntimeDispatchResult's
        -- expectedManifestId===adoptedManifestId branch) adopts the response into the SAME rendered
        -- screen -- the identical mechanism admin-enum's enum_dictionary:list_groups dispatch (its
        -- own manifest ae200) already uses for enum_table's propBindings.rows.source=
        -- "emission.data.groups". No new frontend response-binding mechanism is introduced; this
        -- reuses that one exactly, only the seed-authored dispatch target differs.
        '{"type":"dispatcher_mapping","role":"admin","target":"admin","layer":"external_api_credential","action":"search"}'::jsonb,
