...
-- =============================================================================
-- Scheduler job manifest substrate seed
-- scheduler-job-manifest-substrate-implementation bundle
--
-- Seeds a minimal demo scheduler job (demo_schedule) as the representative
-- route for the scheduler job manifest substrate.
-- demo_schedule is manual_only (safe for demo/test); not a production cron job.
-- Demonstrates the canonical shape: scheduler_job → scheduler_job_steps →
-- abstract_function_key chain → AbstractFunctionExecutor dispatch.
--
-- Abstract function manifest (af11): demo.scheduler_projection
-- - runtime_lane = 'scheduler_job_runtime' (scheduler-specific lane, not external_port_runtime)
-- - authority_scope = 'demo_scheduler_job'
-- - primitive: projection (read-only output of scheduler context fields)
-- - policy authority: 'demo_scheduler_job_policy'
-- - Credential plaintext: never stored here. No external port reference in demo.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- Abstract function manifest: demo.scheduler_projection (af11)
-- Proves the scheduler_job_runtime lane and scheduler_context binding source.
-- Step 1: projects job_key and run_id from scheduler_context.
-- ---------------------------------------------------------------------------
INSERT INTO topology.abstract_function_manifests
    (abstract_function_id, function_key, runtime_lane, authority_scope, output_shape, projection_deny_keys, active)
VALUES
...
-- Step: projection primitive — reads job_key and run_id from scheduler_context
-- scheduler_context binding source is the isolation boundary from request payload.
INSERT INTO topology.abstract_function_steps
    (abstract_function_step_id, abstract_function_id, step_order, primitive_key, step_config, result_context_key, active)
VALUES
...
-- Input bindings: job_key and run_id from scheduler_context (not from request payload)
INSERT INTO topology.abstract_function_input_bindings
    (input_binding_id, abstract_function_step_id, input_key, binding_source, binding_path, required, secret, active)
VALUES
...
-- Authority bindings: policy 'demo_scheduler_job_policy' (scheduler-internal policy key)
INSERT INTO topology.abstract_function_authority_bindings
    (abstract_function_id, authority_kind, authority_ref, active)
VALUES
...
-- Represents the minimal scheduler job manifest shape.
-- schedule_policy_kind = 'manual_only': not auto-triggered by cron (safe for demo).
-- manual_run_allowed = true: can be triggered via admin/test.
-- No input_table_ref: demo job has no due-row lease step.
-- authority_scope = 'demo_scheduler_job': matches abstract function manifest.
-- Credential references: none (demo job does not call external services).
-- ---------------------------------------------------------------------------
...
-- abstract_function_key links to af11 above.
-- on_error = 'fail_run': explicit failure, no silent fallback.
