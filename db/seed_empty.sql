...
VALUES (
    ...
    ARRAY[
        ...
        -- The 37 topology_ui_seed_record entries below are storage_adoption_contract-flattened
        -- (docs/design/react-schema-topology-seed-translator-ssot.yaml) output: reproducible via
        -- .agent/tools/react-schema-topology-seed-translator generate-topology-seed
        --   --input .agent/tests/fixtures/react-schema-topology-seed-translator/credential-management-0092.topology-seed.input.json
        --   --seed-sql-fragment <path>
        -- Resolves declared_seed_surface_catalog known_gap
        -- instance_settings_projection_category_not_yet_represented. Each element independently
        -- fits the manifest.topology / idx_manifest_topology GIN index byte budget (see
        -- storage_adoption_contract) -- do not re-merge these into one nested jsonb element.
        '{"type":"topology_ui_seed_record","seedKey":"auth.external.credential_management.projection","parentKey":null,"record":{"recordType":"topology_ui_projection","key":"auth_external_credential_management_projection","label":"Auth / external credential management","sourceYamlRefs":["react-schema-topology-seed-translator-ssot.yaml#declared_seed_surface_catalog","instance-port-substrate-ssot.yaml#existing_credential_management_projection_extension"],"sourceReactPath":"$.root","knownGapRefs":[],"surface":"auth.external.credential_management.projection","categoryKeys":["user_auth","external","instance_settings"]}}'::jsonb,
        '{"type":"topology_ui_seed_record","seedKey":"auth.external.credential_management.projection","parentKey":"auth_external_credential_management_projection","record":{"recordType":"topology_ui_category","key":"user_auth","label":"User auth","sourceYamlRefs":["instance-port-substrate-ssot.yaml#existing_credential_management_projection_extension.projection_categories.user_auth"],"sourceReactPath":"$.root.children[0]","knownGapRefs":[],"categoryKey":"user_auth","sectionKeys":["user_auth_section"]}}'::jsonb,
        ...
        '{"type":"topology_ui_seed_record","seedKey":"auth.external.credential_management.projection","parentKey":"external","record":{"recordType":"topology_ui_section","key":"external_section","label":"External port credential context","sourceYamlRefs":["instance-port-substrate-ssot.yaml#existing_credential_management_projection_extension.projection_categories.external"],"sourceReactPath":"$.root.children[1].children[0]","knownGapRefs":[],"sectionKey":"external_section","sectionKind":"readonly_boundary","childKeys":["credential_reference_key"]}}'::jsonb,
        '{"type":"topology_ui_seed_record","seedKey":"auth.external.credential_management.projection","parentKey":"external_section","record":{"recordType":"topology_ui_field","key":"credential_reference_key","label":"Credential reference key","sourceYamlRefs":["instance-port-substrate-ssot.yaml#existing_credential_management_projection_extension.projection_categories.external"],"sourceReactPath":"$.root.children[1].children[0].children[0]","knownGapRefs":[],"fieldKey":"credential_reference_key","control":"form_input/form_field","required":false,"validationRefs":[]}}'::jsonb,
