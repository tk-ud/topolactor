...
--       physical_tables, function_parameters,
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS topology.registrar_entries (
...
COMMENT ON TABLE topology.registrar_entries IS
...
COMMENT ON COLUMN topology.registrar_entries.table_ref IS
...
CREATE INDEX IF NOT EXISTS idx_registrar_entries_active
    ON topology.registrar_entries (active)
    WHERE active = true;
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS topology.master_registry (
...
COMMENT ON TABLE topology.master_registry IS
...
CREATE INDEX IF NOT EXISTS idx_master_registry_active
    ON topology.master_registry (active)
    WHERE active = true;
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS topology.state_registry (
...
COMMENT ON TABLE topology.state_registry IS
...
COMMENT ON COLUMN topology.state_registry.owner IS
    'Ownership tier: '
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS topology.relation_registry (
...
COMMENT ON TABLE topology.relation_registry IS
...
CREATE INDEX IF NOT EXISTS idx_relation_registry_active
    ON topology.relation_registry (active)
    WHERE active = true;

CREATE INDEX IF NOT EXISTS idx_relation_registry_master_ids
    ON topology.relation_registry USING GIN (master_ids);

CREATE INDEX IF NOT EXISTS idx_relation_registry_manifest_candidate
    ON topology.relation_registry (manifest_candidate)
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS topology.package_registry (
...
COMMENT ON TABLE topology.package_registry IS
...
COMMENT ON COLUMN topology.package_registry.package_def IS
...
CREATE INDEX IF NOT EXISTS idx_package_registry_active
    ON topology.package_registry (active)
    WHERE active = true;

CREATE INDEX IF NOT EXISTS idx_package_registry_package_def
    ON topology.package_registry USING GIN (package_def);
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS topology.schema_registry (
...
COMMENT ON TABLE topology.schema_registry IS
...
COMMENT ON COLUMN topology.schema_registry.schema_def IS
...
CREATE INDEX IF NOT EXISTS idx_schema_registry_active
    ON topology.schema_registry (active)
    WHERE active = true;

CREATE INDEX IF NOT EXISTS idx_schema_registry_schema_def
    ON topology.schema_registry USING GIN (schema_def);
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS topology.component_registry (
...
COMMENT ON TABLE topology.component_registry IS
...
COMMENT ON COLUMN topology.component_registry.component_def IS
...
CREATE INDEX IF NOT EXISTS idx_component_registry_active
    ON topology.component_registry (active)
    WHERE active = true;

CREATE INDEX IF NOT EXISTS idx_component_registry_component_type
    ON topology.component_registry (component_type);

CREATE INDEX IF NOT EXISTS idx_component_registry_component_def
    ON topology.component_registry USING GIN (component_def);
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS topology.function_parameters (
...
COMMENT ON TABLE topology.function_parameters IS
...
COMMENT ON COLUMN topology.function_parameters.function_name IS
...
COMMENT ON COLUMN topology.function_parameters.parameter_value IS
...
CREATE INDEX IF NOT EXISTS idx_function_parameters_function_name
    ON topology.function_parameters (function_name);

CREATE INDEX IF NOT EXISTS idx_function_parameters_active
    ON topology.function_parameters (active)
    WHERE active = true;


-- ---------------------------------------------------------------------------
-- topology.physical_tables
-- Canonical physical table catalog (replaces topologys.registrar_entries).
-- Authoritative record of physical tables in the topology space with their
-- import / create / select flow metadata.
-- ---------------------------------------------------------------------------
...
COMMENT ON TABLE topology.physical_tables IS
    'Canonical physical table catalog. Authoritative source for physical table '
    'identity used by logs.diff, logs.current, and topology import/select flows.';

CREATE INDEX IF NOT EXISTS idx_topology_physical_tables_active
    ON topology.physical_tables (active)
    WHERE active = true;
