...
-- Payload fields: idempotency_key, export_format, period, requested_by.
...
WITH upserted_preset AS (
    ...
    VALUES (
        ...
        'ui_builder_canvas',
        'file_storage_export_job.v1.seed.2026-06-21',
        $$
        {
          ...
          "note": "authorization_key is an opaque DB reference (not the signed URL). Actual signed URL resolution is runtime secret store concern."
        }
        $$::jsonb,
        $$
        {"nodes":[
    ...
    VALUES
        ((SELECT preset_id FROM preset), 'export_job_shell', 'export_job_shell', 'catalog_component', 'section.alias', 'disclosure_structure/section', NULL, 'root', 0, '{"x":0,"y":0,"width":1024,"height":720}'::jsonb, '{"title":"Export Job","description":"Initiate an export job through the file_storage access port. Configure portTargetRef and payload mapping before apply."}'::jsonb, '{"layoutIntent":"crud_derived_shell"}'::jsonb, 'mapped'),
        ...
        ((SELECT preset_id FROM preset), 'export_job_submit_button', 'export_job_submit_button', 'catalog_component', 'button.primitive', 'action/button', 'export_job_shell', 'actions', 5, '{"x":24,"y":232,"width":200,"height":48}'::jsonb, '{"label":"Start export job","variant":"primary"}'::jsonb, '{"layoutIntent":"external_port_dispatch"}'::jsonb, 'mapped'),
        ((SELECT preset_id FROM preset), 'export_job_result_json', 'export_job_result_json', 'catalog_component', 'json_viewer.template', 'data_display/json', 'export_job_shell', 'results', 6, '{"x":24,"y":296,"width":928,"height":392}'::jsonb, '{"title":"Export job result"}'::jsonb, '{"layoutIntent":"result_projection"}'::jsonb, 'mapped')
    ...
    VALUES
        ((SELECT preset_id FROM preset), 'export_job_submit_button', 'export_job_submit_button', 'requires_external_port_export_job', 'dispatchExternalPort', 'external_port', 'external-port:access_port:00000000-0000-0000-0000-000000000f01',
            $${"event":"click","actionType":"dispatchExternalPort","portTargetRef":"external-port:access_port:00000000-0000-0000-0000-000000000f01","payloadFrom":{"idempotency_key":"node:export_job_id_input.value","export_format":"node:export_format_input.value","period":"node:period_input.value","requested_by":"node:requested_by_input.value"},"outputProp":"exportJobResult","payloadResolverRef":"frontend/runtime/payloadFromResolver.ts","note":"Dispatches to file_storage access_port via portTargetRef. Payload fields are resolved from form input nodes. Projection response: file_artifact_id / authorization_key (opaque reference) / file_name / file_type. No signed_url or credential in response."}$$::jsonb, 'pending')
...
VALUES (
    ...
    'file-storage-export-job-seed.v1',
    $${"nodes":[
      {"nodeId":"export_job_shell","nodeKind":"catalog_component","componentKey":"section.alias","componentKind":"disclosure_structure/section","isDraftOnly":false,"slotKey":"root","orderIndex":0,"parentNodeId":null,"x":0,"y":0,"width":1024,"height":720,"propsJson":"{\"title\":\"Export Job\",\"description\":\"CRUD-derived export job surface. Configure portTargetRef payload mapping in PackageWiringEditor before apply.\"}"},
      ...
      {"nodeId":"export_job_submit_button","nodeKind":"catalog_component","componentKey":"button.primitive","componentKind":"action/button","isDraftOnly":false,"slotKey":"actions","orderIndex":5,"parentNodeId":"export_job_shell","x":24,"y":232,"width":200,"height":48,"propsJson":"{\"label\":\"Start export job\",\"variant\":\"primary\"}"},
      {"nodeId":"export_job_result_json","nodeKind":"catalog_component","componentKey":"json_viewer.template","componentKind":"data_display/json","isDraftOnly":false,"slotKey":"results","orderIndex":6,"parentNodeId":"export_job_shell","x":24,"y":296,"width":928,"height":392,"propsJson":"{\"title\":\"Export job result\"}","propBindings":{"data":{"source":"emission.data.exportJobResult"}}}
    ...
    $$[{"nodeId":"export_job_shell","reason":"file_name and file_type are not present in the form inputs; they are supplied by the caller at dispatch time and resolved through the external port pipeline — not by this preset surface","knownGapRef":"export_job_file_name_type_caller_supplied_at_dispatch","status":"pending_caller_resolution"}]$$::jsonb
);
