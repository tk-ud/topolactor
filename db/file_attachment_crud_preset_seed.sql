...
VALUES (
    ...
    (SELECT jsonb_agg(jsonb_build_object('nodeId', node_id, 'sourceObjectId', source_object_id, 'capabilityTag', capability_tag, 'wiringKind', wiring_kind, 'targetSurface', target_surface, 'targetRef', target_ref, 'status', status, 'binding', binding_json) ORDER BY node_id) FROM topology.mock_preset_wiring_candidate WHERE preset_id = (SELECT preset_id FROM preset)),
