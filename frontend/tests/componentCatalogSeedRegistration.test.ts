...
async function loadSeedAndSsot() {
...
Deno.test("catalog entries that require registration are represented in ui_component_bucket seed", async () => {
    const { seedSql } = await loadSeedAndSsot();
    const rows = extractSeedRows(seedSql);
...
Deno.test("seed classification vocabulary is subset of SSOT allowed values", async () => {
    const { seedSql, ssot } = await loadSeedAndSsot();
    const rows = extractSeedRows(seedSql);
    const allowed = {
