...
// ---------------------------------------------------------------------------
// Credential Registry — admin credential reference management
// SSOT: docs/design/runtime-bundle-secret-credential-ssot.yaml
//   authority_boundary: admin_config_and_runtime_secret_store
//   trigger_kind: admin_config_credential_registration
//
// Security boundary:
//   These functions operate on credential references only — no credential values
//   appear in requests, responses, or logs. The actual credential values are
//   managed exclusively via environment variables or secret store adapters
//   on the backend.
// ---------------------------------------------------------------------------

export type CredentialReferenceItem = {
...
async function callCredentialRegistry(
  action: string,
  payload?: Record<string, unknown>,
): Promise<Emission | null> {
  return callAdminDispatch({
    operationType: "admin",
    target: "admin",
...
export async function listCredentialReferences(): Promise<
  CredentialReferenceItem[] | null
> {
  const emission = await callCredentialRegistry("list");
  if (emission === null) return null;
  const data = emission.data as { items: CredentialReferenceItem[] } | undefined;
  return data?.items ?? [];
}

export async function registerCredentialReference(
  referenceKey: string,
  providerKind: string,
): Promise<CredentialReferenceItem | null> {
  const emission = await callCredentialRegistry("register", {
    reference_key: referenceKey,
    provider_kind: providerKind,
  });
  return emission ? (emission.data as CredentialReferenceItem) : null;
}

export async function validateCredentialReference(
  referenceKey: string,
): Promise<{ reference_key: string; is_valid: boolean } | null> {
  const emission = await callCredentialRegistry("validate", {
    reference_key: referenceKey,
  });
  return emission ? (emission.data as { reference_key: string; is_valid: boolean }) : null;
}

export async function rotateCredentialReference(
  referenceKey: string,
  rotationActorId: string,
): Promise<{ reference_key: string; rotated: boolean } | null> {
  const emission = await callCredentialRegistry("rotate", {
    reference_key: referenceKey,
    rotation_actor_id: rotationActorId,
  });
  return emission ? (emission.data as { reference_key: string; rotated: boolean }) : null;
}
