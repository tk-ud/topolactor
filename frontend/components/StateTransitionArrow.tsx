...
export type StateTransitionArrowProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function StateTransitionArrow({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: StateTransitionArrowProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "StateTransitionArrow"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">state transition arrow preview</div>
    </section>
  );
}
