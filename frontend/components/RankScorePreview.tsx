...
export type RankScorePreviewProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function RankScorePreview({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: RankScorePreviewProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "RankScorePreview"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">rank score preview</div>
    </section>
  );
}
