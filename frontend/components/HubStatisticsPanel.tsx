...
export type HubStatisticsPanelProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function HubStatisticsPanel({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: HubStatisticsPanelProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "HubStatisticsPanel"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">hub statistics preview</div>
    </section>
  );
}
