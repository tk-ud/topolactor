...
export type ComponentPlacementHandleProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function ComponentPlacementHandle({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: ComponentPlacementHandleProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "ComponentPlacementHandle"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">component placement handle surface</div>
    </section>
  );
}
