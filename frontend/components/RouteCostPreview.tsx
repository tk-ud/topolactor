...
export type RouteCostPreviewProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function RouteCostPreview({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: RouteCostPreviewProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "RouteCostPreview"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">route cost preview</div>
    </section>
  );
}
