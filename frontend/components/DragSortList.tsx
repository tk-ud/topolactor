...
export type DragSortListProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function DragSortList({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: DragSortListProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "DragSortList"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">sortable list preview</div>
    </section>
  );
}
