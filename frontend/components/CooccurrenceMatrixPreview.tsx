...
export type CooccurrenceMatrixPreviewProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function CooccurrenceMatrixPreview({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: CooccurrenceMatrixPreviewProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "CooccurrenceMatrixPreview"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">cooccurrence matrix preview</div>
    </section>
  );
}
