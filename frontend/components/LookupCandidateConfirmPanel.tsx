...
export type LookupCandidateConfirmPanelProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function LookupCandidateConfirmPanel({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: LookupCandidateConfirmPanelProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "LookupCandidateConfirmPanel"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">lookup candidate confirm panel</div>
    </section>
  );
}
