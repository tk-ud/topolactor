...
export type FormulaBuilderProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function FormulaBuilder({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: FormulaBuilderProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "FormulaBuilder"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">formula input preview builder</div>
    </section>
  );
}
