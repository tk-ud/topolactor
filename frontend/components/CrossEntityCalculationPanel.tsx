...
export type CrossEntityCalculationPanelProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function CrossEntityCalculationPanel({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: CrossEntityCalculationPanelProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "CrossEntityCalculationPanel"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">cross entity calculation preview</div>
    </section>
  );
}
