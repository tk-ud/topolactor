...
export type TelAddressCandidateLookupProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function TelAddressCandidateLookup({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: TelAddressCandidateLookupProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "TelAddressCandidateLookup"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">tel/address candidate lookup</div>
    </section>
  );
}
