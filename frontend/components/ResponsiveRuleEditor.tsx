...
export type ResponsiveRuleEditorProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function ResponsiveRuleEditor({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: ResponsiveRuleEditorProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "ResponsiveRuleEditor"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">responsive rule preview editor</div>
    </section>
  );
}
