...
export type SnapGridOverlayProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function SnapGridOverlay({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: SnapGridOverlayProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "SnapGridOverlay"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">snap grid reference overlay</div>
    </section>
  );
}
