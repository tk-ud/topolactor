...
export type KanaAssistInputProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function KanaAssistInput({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: KanaAssistInputProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "KanaAssistInput"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">kana assist candidate input</div>
    </section>
  );
}
