...
export type AttentionWeightPreviewProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function AttentionWeightPreview({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: AttentionWeightPreviewProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "AttentionWeightPreview"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">attention weight preview</div>
    </section>
  );
}
