...
export type AddressPostalLookupProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function AddressPostalLookup({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: AddressPostalLookupProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "AddressPostalLookup"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">address postal candidate lookup</div>
    </section>
  );
}
