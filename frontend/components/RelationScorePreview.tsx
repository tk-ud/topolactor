...
export type RelationScorePreviewProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function RelationScorePreview({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: RelationScorePreviewProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "RelationScorePreview"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">relation score preview</div>
    </section>
  );
}
