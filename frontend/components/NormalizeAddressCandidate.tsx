...
export type NormalizeAddressCandidateProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function NormalizeAddressCandidate({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: NormalizeAddressCandidateProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "NormalizeAddressCandidate"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">normalized address candidate preview</div>
    </section>
  );
}
