...
export type TopologyDistancePreviewProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function TopologyDistancePreview({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: TopologyDistancePreviewProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "TopologyDistancePreview"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">topology distance preview</div>
    </section>
  );
}
