...
export type RelationDropZoneProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function RelationDropZone({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: RelationDropZoneProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "RelationDropZone"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">relation drop target surface</div>
    </section>
  );
}
