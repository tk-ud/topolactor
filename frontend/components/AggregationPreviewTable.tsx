...
export type AggregationPreviewTableProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function AggregationPreviewTable({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: AggregationPreviewTableProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "AggregationPreviewTable"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">aggregation preview table</div>
    </section>
  );
}
