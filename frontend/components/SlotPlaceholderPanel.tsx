...
export type SlotPlaceholderPanelProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function SlotPlaceholderPanel({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: SlotPlaceholderPanelProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "SlotPlaceholderPanel"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">slot placeholder panel</div>
    </section>
  );
}
