...
export type LayoutDropZoneProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function LayoutDropZone({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: LayoutDropZoneProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "LayoutDropZone"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">layout drop target surface</div>
    </section>
  );
}
