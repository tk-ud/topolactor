...
export type TreeReorderDropZoneProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function TreeReorderDropZone({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: TreeReorderDropZoneProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "TreeReorderDropZone"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">tree reorder drop target surface</div>
    </section>
  );
}
