...
export type ComputedFieldPreviewProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function ComputedFieldPreview({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: ComputedFieldPreviewProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "ComputedFieldPreview"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">computed field preview</div>
    </section>
  );
}
