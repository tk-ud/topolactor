...
export type PostalAddressLookupProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function PostalAddressLookup({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: PostalAddressLookupProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "PostalAddressLookup"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">postal address candidate lookup</div>
    </section>
  );
}
