...
export type BulkImportCandidatePanelProps = ComponentIdentityProps & {
  title?: string;
  value?: string;
  items?: unknown[];
  preview?: unknown;
...
export function BulkImportCandidatePanel({ title, value, items, preview, className, design, onChange, onSelect, onConfirm }: BulkImportCandidatePanelProps): JSX.Element {
  ...
  return (
    <section className={mergedClassName} style={mergedStyle}>
      <div style="font-weight:600;margin-bottom:8px">{title ?? "BulkImportCandidatePanel"}</div>
      ...
      <div style="font-size:0.78rem;color:#666;margin-top:6px">bulk import candidate panel</div>
    </section>
  );
}
