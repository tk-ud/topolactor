...
**CI Attention（現在の実装）:** `backend.system_operation_ci_runtime`（`SystemOperationCiRuntime` / `SystemOperationCiScheduler`）の runtime system CI subpath は実装済みです。cron 実行でHub Attention 整合性・current rebuildability・registry continuity を検査し、`SystemCiStatus`（Pass / Gap / Blocking）で診断結果を structured log に出力します。診断結果は現時点では DB 永続化されておらず、自動 follow-up アクションも未実装です。`product.ci_attention_user_guidance_ux`（エンドユーザー向けの不足入力案内・有効候補提示 UX）は将来の方向性であり、現時点では未実装です。

**SQL Attention（現在の実装）:** `backend.sql_attention_observation_runtime` のスケジューラ/探索 runtime/repository subpath は partial 実装です（`SqlAttentionScheduler` / `HubAttractorExplorationRuntime` / `NpgsqlSqlAttentionLogsRepository`）。`logs.current`（L2 norm watch）・`logs.hub_current`（attractor current）・`logs.attention`（evidence 永続化）の各 DB 層は実装済みですが、`product.sql_attention_recommendation_feedback_ux`（次ユーザーの文脈候補・導線・UI projection への還流）は構築中です。`context_hub_recommendation_current` による topology 内部推薦は別途実装済みですが、SQL Attention observation の完全フィードバックサイクルは未完です。

**動的サポート付きノーコード UX:** manifest-driven dispatch・admin registry surface・component topology などの基礎インフラは partial / implemented 状態にあります。`product.dynamic_support_nocode_loop`（CI Attention/SQL Attention を通じた end-to-end feedback loop）は構築中であり、本記事で説明する完全な UX は将来の方向性です。
